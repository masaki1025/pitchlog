...
- 状況計算はクライアント側でも実行できる（**対象の列挙は NFR-018 を正とする** — 本項では繰り返さない。単一実装の達成条件は NFR-018、一致性の担保は NFR-019(a) のCI一致性テスト）
...
- **要件**: ドメイン計算（下記**対象**）は**単一の実装コード**とし、クライアント・サーバーの双方が**同一のコードに由来する実行体**を用いる。UI別・経路別のコピー実装を作らない（一致はNFR-019で担保）
- **対象**（**本項の列挙を正**とする — 達成条件(d)）: **状況判定・座標変換・捕球選手推定・成績集計の前処理・終了判定**。**実行の所在**で次のとおり区分する（**区分の用途はNFR-019(a)の一致性テストの対象判定に限る**。コア領域の境界〔開発ハーネス設計書6.3の入出力境界〕には用いない）:
  - **(α) クライアント・サーバーの双方で実行するもの**: 状況判定・座標変換・捕球選手推定・終了判定、および**断中にクライアントで実行する成績集計の前処理**（FR-021の当日成績・FR-023の球種割合）。**NFR-019(a)の一致性テストの対象**
  - **(β) サーバーでのみ実行するもの**: チーム集計（FR-042）等の後段集計。**一致性テストの対象としない**（FR-042補足の限定を維持）。担保は付録Aの集計定義に対する固定ベクタとバックエンドの単体テストに置く
- **達成条件**:
  - **(a) 実装の単一性**: 対象の計算ごとに、**人手で保守する実装は1系統**とする。他方の実行体は生成・変換・埋め込みによる**派生物**とし、人手で編集しない
  - **(b) 宣言と機械判定**: 対象ごとに**出所（正本の実装）・すべての実行入口・派生関係**を機械可読に宣言し、CIが ①**出所と派生物の乖離**（派生物が古い／手で修正された状態） ②**宣言外の実装が混入していないこと** を機械的に判定できること。**②を欠く担保は本要件を満たさない**（出力が偶然一致する間だけ通る未申告の実装を検出できないため。旧システムは共有モジュールを持ちながら「定義は共有・ロジックは別コード」の状態が解消しなかった＝旧NFR-011）
  - **(c) 逐語移植対象の例外**: 旧システムからの**逐語移植を受入条件とする箇所**は、〈対象シンボル・対応する契約値・逐語性の受入条件の終了証跡〉の3点組を**下表に列挙した範囲に限り**、契約値との一致がテストで固定されていることをもって(a)を満たすとみなす。**下表への追加は本書の改訂を要する**（実装判断で増やさない）。当該シンボルの**逐語性の受入条件を解いた時点で例外は失効**し、以後(a)を満たす（判定は逐語性を解く変更のレビューで行い、当該行を下表から削除する）

    | 対象シンボル | 対応する契約値 | 逐語性の受入条件の終了証跡 |
    |---|---|---|
    | `frontend/src/lib/courseInputView.ts` の `COURSE_COORDINATE_SIZE` | `contracts/display_geometry_263_v1.json` の表示座標系スケール（263） | 打席入力画面の逐語移植の受入判定 |

  - **(d) 列挙の一元性**: 対象の列挙は本項を正とし、**他の条項・文書は本項を参照する**（4.0-4・NFR-019(a)・改善台帳 I-4/I-8。同じ列挙を各所で繰り返さない — 列挙が分散すると片方だけが更新される）
- **測定方法**: (b)の宣言に対する機械検査（**乖離の検出**および**宣言外実装の検出**）+ NFR-019(a)の一致性テスト。(β)は付録Aの固定ベクタに対するバックエンド単体テスト
- **正解ベクタ**: (α)のうち**状態遷移・成績計上は付録Eのゴールデンケース**、**指標算出は付録Aの固定ベクタ**を正解とする（付録Eは状態遷移・成績計上フラグの規範であり、指標定義の正は付録A）
- **申し送り（設計フェーズのADR — 10章に登録）**: **実現手段**（共有ランタイム／WebAssembly／コード生成等）の選択、**(b)の判定面**（探索対象の閉域・実行入口の定義・生成物の識別・動的ロード等の禁止規則・到達性検査の方式）、**付録A固定ベクタの整備**を、実装着手前にADRで確定する（v1.8で申し送りを明文化し、v2.1で達成条件と申し送り項目を明示した）

#### NFR-019: テスト規約のCI強制
- **カテゴリ**: 保守性 ／ **優先度**: Must
- **要件**: テストランナーを標準化する（バックエンド=pytest・フロントエンド=Vitest・E2E=Playwright。v1.8で明確化 — バックエンドのテストは pytest 以外に分散させない）。PRごとにCIで自動実行、全グリーンでないとマージ不可とする。CIには (a) クライアント/サーバー状況計算の一致性テスト（同一のプレイ入力列から同一状況を導く。**対象は NFR-018 の区分(α)**＝クライアント・サーバーの双方で実行するドメイン計算とし、**列挙は NFR-018 を正とする**〔本項では繰り返さない〕。**NFR-018 の区分(β)〔サーバーでのみ実行する後段集計〕は本テストの対象としない** — FR-042 補足のとおり別建ての契約と単体テストで担保する。**正解ベクタ**は状態遷移・成績計上が**付録Eマトリクスのゴールデンケース**、指標算出が**付録Aの固定ベクタ**） (b) 越境アクセステスト — **合否は「越境が1件でもあれば fail」ではなく「FR-034 の認可行列どおりに通り、行列外はすべて404」で判定する**。網羅すべき組合せは:**付与の粒度3種（`成績`/`選手個別`/`書き出し`）の組合せ × 相互性**／**グループ終了後・離脱後・付与撤回後・テナント無効化後**／**複数グループへの同時参加**（同じ2テナントが片方では付与あり・他方では非付与のとき、要求で指定されたグループの付与のみを評価し、他グループの付与は一切影響しないこと）／**資源種別ごと**（成績・選手個別・概要・エクスポート・生記録・カルテ所見・第三者データ・身体プロフィール分布は常に404）／**制御資源の読み取り**（①参加テナントの正当な閲覧 ②`member` が `admin` 限定の情報を要求したときの拒否 ③非参加テナントからの要求が404）／**制御資源への操作**（**テナント側の権限を持たない要求元からの7操作すべてが404**になること。2層を要する3操作＝招待の発行・失効／役割変更／終了 で `member` からの要求が404になること。テナント側の権限のみの4操作＝作成／参加／付与変更／離脱 が役割によらず通ること。参加は招待の有効性検証も要すること。**最後の `admin` の離脱・降格が拒否**されること）／**再有効化後**（共有が復帰すること・自動終了したグループは復帰しないこと・無効化中に構築された除外キャッシュが残らないこと）／**既定拒否の検証**（許可リストにない資源への共有グループ経由のアクセスがすべて404になること）／**グループを指定しない単一テナント名指しの直接アクセス**／**参加上限の同時受諾**（直列化により超過しないこと）／**同時比較上限を超える集合**の要求に対する400拒否（切り捨てないこと）／**自テナントを含む集合**の要求（自テナント要素は付与によらず通り、上限には数えられること）／**選手の在籍区分の変更（FR-017）後**にキャッシュが失効しOB化した選手が共有結果に現れないこと (c) 主要分岐のE2Eテスト（タイブレーク・コールド・引き分け・終了後修正→再集計・交代フルセット・通信断→復帰同期・**記録権の通常/緊急引き継ぎと退避経路**） (d) 同期プロトコルの故障系テスト（**墓標/改訂の適用・サーバー適用の原子性（クラッシュ注入）・複数タブの単一書き手競合**）を含む
- **測定方法**: CI構成の確認と実行履歴
...
| **NFR-018 の実現手段と判定機構**（①実現手段＝共有ランタイム／WebAssembly／コード生成等の選択 ②達成条件(b)の判定面＝探索対象の閉域・実行入口の定義・生成物の識別・動的ロード等の禁止規則・到達性検査の方式 ③(α)の指標算出に用いる**付録A固定ベクタの整備**） | 暫定＝未定。**要件は達成条件までを定め、手段と判定方式はADRで確定する**（v2.1で申し送り項目を特定。①が実装可能な手段を持たない場合は本要件の再改訂を要する） | 山田さん + Claude（ADR起案） | **実装着手前**（設計フェーズ） |
