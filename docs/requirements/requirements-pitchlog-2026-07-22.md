...
> 現行システムからの改善点とその理由は別文書 [`docs/improvements-from-baseball-scoring.md`](../improvements-from-baseball-scoring.md)（改善台帳 I-1〜I-24）に記録し、本書とセットで維持する。
> 対話の決定経緯は [`requirements-draft-pitchlog.md`](requirements-draft-pitchlog.md)（D-1〜D-37）を参照。
...
stateDiagram-v2
    ...
    進行中 --> 進行中: 操作イベント（毎球入力/undo/交代/TB開始/その場登録）
    進行中 --> 進行中: 通信断（クライアント計算で継続・復帰後同期）
    進行中 --> 終了: 試合終了宣言（規定成立・コールド・サヨナラ・引き分け・途中打ち切り）
...
- **受け入れ基準**（旧FR-002継承+以下を上書き）:
  ...
  - Given 必須項目が未入力 / When プレイ確定 / Then 不足項目が明示されて確定できない
...
- **補足**: 打撃結果の語彙と各値の自動更新挙動は管理者管理語彙（4.0-3）。値集合の初期値は現行 config.py と同一
...
- **受け入れ基準**（旧FR-004継承）:
  ...
  - Given 打球が発生 / When フィールド画像をタップ / Then 打球位置座標（付録B）と捕球選手（**座標からの自動推定・修正可** — ドメインロジック単一実装 NFR-018 の対象）が記録される
...
- **受け入れ基準**（旧FR-005継承+以下を上書き）:
  ...
  - Given 試合の適用規則（FR-014）で定まる終了・コールド・サヨナラ条件が成立 / When 該当プレイを確定 / Then 試合終了が判定される
  - Given 通信断中 / When プレイを確定 / Then 上記の計算がクライアント側で実行され、画面が正しく更新される（4.0-4）
- **補足**: クライアント/サーバーの計算一致は NFR-019 のCI一致性テストで担保
...
- **補足**: 境界=DH解除・同一枠への再交代・直前交代の取り消し。連盟規定にない交代（リエントリー等）は対象外
...
- **受け入れ基準**:
  ...
  - Given 全操作イベント / When 生成 / Then クライアント生成のべき等キーと**試合スコープの単調増加の連番**が付与される
  ...
  - Given 途中の1件がサーバーに内容起因で拒否された / When 同期 / Then 後続は停止し、どこで止まったかが明示され、操作者は当該イベントを**編集して再送**または**破棄**して再開できる（破棄時は後続イベントへのサーバー補正の内容が通知される）。**記録権拒否（FR-013）は内容の編集では解決できないため別カテゴリとして扱い、退避（FR-013）へ誘導する**
...
- **受け入れ基準**:
  - Given 試合開始 / When 記録を開始 / Then 記録権がその端末に付与される（記録権には世代番号を持たせ、引き継ぎのたびに更新する）
  - Given 記録権が別端末にある試合 / When 別の端末が記録を開こうとする / Then 警告が表示され、**明示的な引き継ぎ操作**をしない限り記録できない（閲覧は可能）
  - Given **通常の引き継ぎ**（元端末がオンライン） / When 引き継ぎを実行 / Then 元端末の未同期キューの**同期完了を待ってから**記録権が新端末に移り、元端末は読み取り専用になる（同期完了前に引き継ぎは成立しない）
  - Given **緊急の引き継ぎ**（元端末が断中・故障・応答不能） / When 緊急引き継ぎを実行 / Then 「元端末の未送信記録は自動では取り込まれない」旨の明示的な警告と確認を経て、記録権の世代が更新され新端末で記録を再開できる
...
- **補足**: 試合区分のデフォルト規則はシステム管理者が管理（FR-037）。移行時は現行コードの判定ロジック相当を初期規則として割り当て
...
- **受け入れ基準**:
  ...
  - Given ゴミ箱内の試合 / When 30日以内に復元 / Then 完全に元に戻る
  - Given ゴミ箱で30日経過 / When 期限到来 / Then チームのゴミ箱UIから消える（**DB上は残存**し、システム管理者がフラグ復旧できる。物理削除は管理者のDB保守のみ）
...
- **受け入れ基準**（旧FR-024継承+以下を上書き）:
  ...
  - Given コールド・引き分け・途中終了の試合 / When 出力 / Then 適用規則（FR-014）に基づく終了の形が正しく表記される
...
- **受け入れ基準**:
  ...
  - Given 移行完了 / When 検証 / Then 件数突合・代表集計値（通算打率・投球数等）の新旧一致・**コース分布チャートの右打者のみ/左打者のみ別の突合**がすべて一致する
...
- **補足**: バックアップファイルは全チームのデータを含むため、取り扱い規律（運用者の管理端末のみ・第三者と共有しない・不要世代は削除）を運用文書の必須項目とする
...
- **要件**: パスワードはソルト付きbcryptで保存し、平文・可逆形式で保持しない。パスワードポリシー=8文字以上・英字と数字を必ず含む。認証トークンの有効期限は7日とし、利用中は自動延長（スライディング方式。放置端末は自然失効）。パスワード変更で全経路のトークンが即時失効する
...
#### NFR-019: テスト規約のCI強制
- **カテゴリ**: 保守性 ／ **優先度**: Must
- **要件**: テストはpytestに一本化し、PRごとにCIで自動実行、全グリーンでないとマージ不可とする。CIには (a) クライアント/サーバー状況計算の一致性テスト（同一のプレイ入力列から同一状況を導く） (b) 越境アクセステスト (c) 主要分岐のE2Eテスト（タイブレーク・コールド・引き分け・終了後修正→再集計・交代フルセット・通信断→復帰同期）を含む
- **測定方法**: CI構成の確認と実行履歴
...
4. **性能実測**: NFR-001〜004 の目標値を、**移行実データ量+5シーズン成長分を下回らない規模**（最低15万プレイ相当）+前提負荷（NFR-005）で実測達成
...
| R-2 | 断中の端末故障 | 受容済みリスク | 未同期キュー（最大1試合分）は端末内にのみ存在し、断中の端末故障・ブラウザデータ消去で失われ得る | 中 | **受容**（断がなければそもそもデジタルに存在しないデータ。最悪ケース=従来の通常ケース）。緩和: 未同期件数の常時表示（Must）・永続ストレージAPI・ローカル書き出し（Could） |
...
| 指標 | 定義 |
|------|------|
| 投球回 | アウト数÷3。1/3回単位で保持し「5回2/3」形式で表示 |
| 投球数 | 記録された投球イベント数 |
...
| 失点 | 在任中に本塁到達した走者数（自責点は算出しない=防御率はWon't） |
...
| エンドラン | 打者がインプレーにし、走者が進塁 | 空振り・走者アウト・併殺 |
...
5. **移行時の検証**: 現行263pxスケール座標→正規化座標の変換式を1つ定め、変換後のコース分布チャートが**右打者のみ/左打者のみに分けて**新旧一致することを移行検証（FR-038）に含める

---

## 付録C: システム設定値一覧

> システム管理者が管理コンソール（FR-037）で変更できる値。変更は以後の表示に反映され、関連キャッシュ・事前集計は無効化→再計算される。

| 設定値 | 既定値 | 用途 |
|--------|--------|------|
| FIP定数 | 3.10 | 付録A-2 |
| 球速の妥当範囲 | 60〜170 km/h | FR-002 の警告閾値 |
| ゴミ箱の自己復元期限 | 30日 | FR-019 |
| トークン有効期限 | 7日（スライディング延長） | NFR-011 |
| 所見の変更履歴世代数 | 10世代 | FR-028（Could） |
| レート制限の閾値・ロック時間 | 未決（10章） | FR-033 |
| 未同期キューの警告閾値 | 350イベント（ブロックしない・警告のみ） | FR-012 |
| 同期済みイベントの端末保持 | 24時間 | FR-012 |
...
3. **CSV出力**: 88列互換の本体は不変とし、88列に収まらない新概念（交代履歴等）は**別ファイル方式（例: `substitutions.csv`）を正**として出力する。ファイル構成・列定義は文書化し、ゴールデンフィクスチャ（正解サンプル）をテストに含める
4. **語彙の初期値**: 値集合の出所は旧リポの UI 語彙定義（`frontend/src/lib/vocab.ts` および Streamlit 入力UI。※`config.py` は列名の定義であり値集合は含まない — v1.3で出所を訂正）。pitchlog では初期値一覧を機械可読なシードデータとしてリポジトリに収載し、それを正とする。**球種の初期値（10種）**:
   - 直球系: ストレート
   - スラ系: スライダー、カット、カーブ
   - 落ち系: チェンジ、フォーク、ツーシーム、シュート、シンカー
   - 未分類: 特殊球
   ※系統の初期割り当ては旧版分析コードのグルーピングに準拠。打撃結果・作戦・打球情報の初期値一覧も同様にシードデータで固定する（値の表記は旧版と同一 — 88列互換の値レベルの互換性を保つ）
