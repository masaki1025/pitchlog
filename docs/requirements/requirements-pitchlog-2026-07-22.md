...
| 2.4 | 2026-09-01 | **TSK-233: NFR-018 (c) の逐語移植対象の例外を有効化（7.6-3 前段の実装追随・PR レビュー。版は上げない）**: 本書が改訂を要すると列挙するのは例外表への**追加・削除・失効**であり、**有効化は列挙されていない**。かつ NFR-018 (c) は検証テストの存在と契約値との一致により例外成立を自動判定するため、状態欄はその事実から導かれる従属値である。v2.1 の変更履歴も「要件の承認と実装適合の判定は別」「テスト整備は別タスク」と明記している。`frontend/src/lib/courseCoordinateContract.spec.ts` の整備により本行の例外が成立し、NFR-018 の違反状態を解消した。**卒業（逐語性の受入条件を解くこと）は本タスクの射程外**であり、別タスクで ADR-003 D-12 の4操作を同一 PR で原子的に行う | 山田 + Claude |
...
- **達成条件**:
  ...
  - **(c) 逐語移植対象の例外**: 旧システムからの**逐語移植を受入条件とする箇所**は、**下表に列挙した範囲に限り、本項の要件（単一の実装コード・コピー実装の禁止）および達成条件(a)の適用を除外する**。除外が成立する条件は、**下表の「検証テスト」が存在し、契約値との一致を固定していること**であり、**テストが無い行の例外は成立しない**（その状態は本要件の違反として扱う）。**下表への追加・削除・失効はいずれも本書の改訂を要する**（実装判断で増やさない・消さない）。当該シンボルの**逐語性の受入条件を解いた時点で例外を失効**させ、以後(a)を満たす
    ...
    |---|---|---|---|---|---|
    | `frontend/src/lib/courseInputView.ts` の `COURSE_COORDINATE_SIZE` | `contracts/display_geometry_263_v1.json` の表示座標系スケール（263） | `frontend/src/lib/courseCoordinateContract.spec.ts` | 打席入力画面の逐語移植の受入判定（判定した PR の URL を本欄に記録する） | システム管理者（本書の改訂権者） | 有効 |
