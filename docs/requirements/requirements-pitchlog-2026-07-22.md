...
- **測定方法**: 次の**受入プロファイル**を試験対象として事前に一意化し、approvedな `docs/development/onboarding.md` の手順を完走できることを2時点で8章の判定者が確認する
  ...
  - **固定版の切替規則**: 新しいUbuntu LTSへの切替は、(1) WSL向けの正式提供開始 (2) `docs/development/onboarding.md` の更新 (3) 新版での受入再実施 の3点を満たした時点で本項の版を改訂して行う（「最新」を都度判定する運用はとらない）。**本規則の判定はアーキテクチャ単位では分割しない** — (1) は**受入を実施するホストのアーキテクチャ向けの提供開始**をもって判定し、(3) も**そのアーキテクチャについて1回**で足りる（保証対象の全アーキテクチャでの再実施は求めない）。**固定版の標準サポート終了前に必ず切替を完了する**
  ...
  - **証跡**: 日時・commit SHA・Windows版・WSL版・ディストリビューション版・onboarding版・**主要ツールの版（python / uv / node / docker）**・実行コマンドと終了コード・**各合格項目の期待値と実測値**（DB接続の応答・HTTP疎通の応答コード等）・**標準出力またはログ成果物への参照**（**実施したホストおよびWSLのアーキテクチャを識別できる出力を含むこと** — v2.3。両アーキテクチャを保証対象とするため、どちらで完走したかが証跡から一意に定まる必要がある）・判定者を記録する
