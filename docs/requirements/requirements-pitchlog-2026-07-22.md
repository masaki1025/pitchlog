...
- **挙動・表示契約を参照する管理者語彙の二段階ゲート**: 管理者管理のうち、ゲートの対象は **打撃結果（打撃結果2を含む）**と**打球情報（打球タイプ・強度）**の2種に閉じる。打撃結果は付録Eの規範行と状態遷移ゴールデンベクタがIDを参照し、打球情報はFR-022の結果表示名（`{捕球選手}{打球タイプ}`等）を検証する成績集計前処理のゴールデンベクタ、および付録A-2bの被打球分布を検証する固定ベクタがIDを参照するためである。付録B-6の領域シードは管理者語彙ではなく独立した参照データ契約であり、打球情報には含めない
- **対象外の閉じた集合**: 管理者管理の**シーズン**、システム固定の全語彙、チーム拡張可の**球種・作戦・大会名・在籍ステータスラベル**は、本ゲートの対象外とする。シーズン通算の期間ラベルは付録A-1が定めるJST年度から導出され、付録Eの規範行または値ごとのゴールデンベクタを増やさない。チーム拡張可の語彙はラベルと参照解決の経路を等価分割で検査し、値ごとの状態遷移規範を持たない。とくにチーム追加の作戦は直下の既存規律により傾向集計だけに用い、成否自動判定へ入らない。チーム拡張値を付録A-4または付録Eの挙動へ昇格させる変更は、単なるチーム拡張ではなく、該当する正本と契約を改訂する別タスクとする。この対象判定は上表の変更権限を移さず、管理者管理の変更権限は引き続きシステム管理者だけにある
- **第1段（契約 PR への登録）**: 対象語彙へ新しいIDを追加する場合、または既存IDの分類・挙動・表示合成上の意味を変更する場合は、製品で有効化する前に、同一の契約 PR へ (1) 安定IDを持つ語彙シードの更新、(2) 当該IDを参照する付録Eの規範行（打撃結果のみ。打球情報には要求しない）、(3) 影響する対象計算のゴールデンベクタ、(4) schema検査・全参照の参照整合、(5) 語彙シードの版と内容 hash の宣言を揃える。これらがすべてCIに合格してPRがマージされ、登録されたID・意味・内容 hash を製品が参照できる状態になって初めて**登録済み**とする。いずれかを欠く部分登録、未知ID、参照切れ、宣言 hash と実体の不一致は登録済みとして扱わない。対象計算ベクタはNFR-018の発効条件も満たさなければならない
- **第2段（管理者による有効化）**: 第1段で登録済みとなったID・意味・内容 hash だけを、システム管理者がFR-037の管理コンソールで有効化できる。未登録、部分登録、内容 hash 不一致、または登録された意味と有効化しようとする意味が異なる値は、製品側が有効化を拒否し、入力候補へ出してはならない。契約 PR のマージは自動的な有効化を意味せず、管理者は第1段を飛ばして有効化できないため、順序を逆転できない
- **改名・無効化との境界**: IDと意味を変えない表示名だけの変更は本ゲートの対象外とする。付録Eの規範行は`resultId`だけを保持して表示名を保持せず、表示名は語彙シードを正としてID参照で解決する。実行時の改名は同じIDに対する表示名の差し替えとして、改名後の語彙スナップショットを用いる参照解決の等価分割で検査するため、付録Eの規範行を増減しない。無効化も選択可能な値を減らすだけで既存ID・規範行・履歴を残すため対象外とする。無効化した登録済みIDの再有効化には新しい契約 PR を要求しないが、第2段の登録・内容 hash 検査は省略しない。表示名変更を装った分類・挙動・表示合成上の意味変更は第1段の対象である
- **実装担当の境界**: 本タスク **TSK-236** が担当するのは、第1段の契約側（語彙シード・schema・参照整合・内容 hash・規範行・ゴールデンベクタ）と本条文までである。第2段の、FR-037で有効化するときに登録・内容 hash を検査して未登録値を拒否する製品実装は、**FR-037の実装タスク**が担当する。本条文の追加をもって製品側の拒否を実装済みとしてはならない
...
- **受け入れ基準**（旧FR-004継承）:
  ...
  - Given 打撃妨害・走塁妨害・守備妨害のいずれかを審判が裁定 / When プレイを確定 / Then 記録者は`interferenceRuling`として、打撃妨害では採用結果（`penalty-award`または`play-result`。後者は採用する`resultId`も必須）、走塁妨害ではプレイ区分（`play-on-obstructed-runner`または`no-play-on-obstructed-runner`）と審判が最終的に与えた各走者・打者走者の到達塁、守備妨害では審判が宣告したアウト対象と他走者の帰塁先を入力する。付録E-1の`INT-01`〜`INT-07`が要求する値が1つでも欠ければ確定できない
- **補足**: `interferenceRuling`は、審判および攻撃側監督の裁定後に**採用された結果だけ**を状態遷移へ渡し、採用されなかった仮定上の結果は保持しない。ただし、妨害が発生した事実と3種の別はイベントpayloadへ保持する。採用結果・授与塁・アウト対象・帰塁先はプレイ結果だけから一意に推定できないため、**自動推定せず、既定値も置かず、記録者の裁定入力を正**とする。既定値を置くと、監督の選択または審判の判断を経ていないアウト・進塁・成績を確定するため認めない
...
- **受け入れ基準**:
  ...
  - Given 4.0-3の二段階ゲート対象である打撃結果または打球情報 / When システム管理者が新規IDの追加、意味変更後のID、または無効化済みIDを有効化 / Then 契約 PR へ登録済みのID・意味・内容 hash と完全一致する場合だけ有効化し、未登録・部分登録・hash不一致・意味不一致は理由を示して拒否し、入力候補へ出さない
...
- **達成条件**:
  ...
  - **(b) 宣言と機械判定**: 対象ごとに**出所（正本の実装）・すべての実行入口・派生関係**を機械可読に宣言し、CIが ①**出所と派生物の乖離** ②**差分の検出可能性** ③**宣言された構成の完全性** を機械的に判定できること。**②を欠く担保は本要件を満たさない**（旧システムは共有モジュールを持ちながら「定義は共有・ロジックは別コード」の状態が解消せず、**片側だけが修正されて値がずれた**＝旧NFR-011）:
    ...
    - **② 差分の検出可能性**（v2.2で目的を変更 — **「再実装が存在しないことの証明」から「差分が必ず検出されること」の担保へ**）: **保証の範囲を次のとおり閉じたうえで、その範囲内では取りこぼしを許さない**:
      ...
      - **保証する入力の範囲**:
        - **入力軸の唯一の正と「全列挙」の定義**: 対象計算ごとの入力軸、その分類、値集合または境界値・等価分割、および軸どうしの組合せ規則の唯一の正は、[ADR-003](../adr/ADR-003-domain-calc-method.md) D-11の**入力軸descriptor**とし、本書に具体的な軸名を重複列挙しない。descriptorは各軸に由来条文を持ち、有限列挙に分類した軸は宣言した値集合の全値を、有限にならない軸は要件・付録から導出した境界値・等価分割・範囲の端点を、同descriptorの組合せ規則に従ってcoverage義務へ展開する。**「全列挙」とは、このcoverage義務に未消費の値・分割・必須組合せがなく、descriptorと派生schema・正解ベクタの軸集合および値域がexact-setで一致すること**をいう。終了判定についても、その入力軸と組合せ規則を同じdescriptorの保証範囲へ含め、終了判定の軸群が未宣言または未消費なら不合格とする
        - **用語の分離**: **状態軸**はdescriptorが事前状態の入力として宣言するフィールド、**イベント軸**は付録E-1の毎球入力の下位分類に加えて、同節が別層とした操作イベント規範行およびundo規範行を含む入力である。付録E-1の**成績計上フラグ**は入力軸ではなく期待出力・比較面であり、閉じた23フラグを全て比較する。状態軸の一部だけを選ぶことを示唆する呼称は用いず、入力軸の包含・除外はdescriptorの分類だけで判定する
        - **(i) 合法空間の定義による限定**: 付録E-1の適用範囲と規範行の層、10列のexact型、`eventKind`別allowlist、および`XC-01`〜`XC-08`を同じ合法入力空間の定義として適用する。型、allowlistまたは交差制約に反する組合せは、もともと存在する入力からcoverageを削るのではなく、合法入力ではないため直積へ含めない。**この限定は保証範囲の縮小ではない**。除いた組合せも検証対象とし、各型制約・allowlist外の組合せ・各`XC-*`違反に対応する負例が拒否されることを機械検査する。付録E-1の10列で表現しない操作イベントとundoは除外せず、別の規範行の層からdescriptorへ収容する
        - **(ii) 判定に影響しない入力のcoverage除外**: 要件・付録の条文から対象計算の出力に影響しないと導出できる入力だけを、descriptorの`nonCoverageFields`へ由来条文と理由を伴って明示し、直積coverageの軸から外してよい。実装量・性能・既存schemaの都合を根拠に外してはならない。`nonCoverageFields`も入力schemaでは省略せず、派生schemaとのexact-set突合の対象とする。さらに、当該入力だけをdescriptorが定める境界値・等価分割で変化させても出力が変わらないことを独立性fixtureで検査し、fixtureが無い、出力が変わる、または影響の有無を判定できない場合はcoverage除外を認めない
        - **縮小禁止とfail-closed**: 要件・付録から導出すべき入力がdescriptorのcoverage軸または`nonCoverageFields`のどちらにも分類されない場合、descriptorにない軸を派生schema・正解ベクタへ追加した場合、descriptorの軸を派生schema・正解ベクタから落とした場合、値域・分類・組合せ規則が一致しない場合、またはcoverage義務の消費を判定できない場合は、いずれも合格としない。入力の追加・削除・再分類は由来条文の改訂をdescriptorへ反映して行い、実装判断で保証範囲を変更しない
      ...
      - **保証の範囲外（明示）**: ①上記のdescriptorが定める入力範囲の外側。ただし、`nonCoverageFields`は有効な入力であり範囲外にはせず、schema保持・exact-set突合・独立性fixtureによる検証対象とする。また、合法空間の制約に反する組合せは対象計算の出力保証には含めないが、拒否されること自体を上記(i)の負例で保証する ②**製品側が正しい出力を誤って選択・抑止・改変する誤り**（対象計算そのものの誤りではない。**FRの受け入れ基準と NFR-019(c) のE2Eが主要分岐を覆うが、全対象計算・全表示経路・全選択条件を閉じるものではない — 残余リスクとして受容する**）③**正本と常に同一の値を返す再実装が存在する事実**（値が同一である限り本項の対象となる差は生じない。ただし**上位要件のコピー実装の禁止と(a)の単一性には違反する**ため、(a)とレビューで抑止する）
...
| 投球数 | 記録された投球イベント数 | 整数 |
| 奪三振 | 打撃結果が三振系のすべてを計上する: 見逃し三振・空振り三振・**振り逃げ（打者が出塁しても投手に奪三振1）**・`K3`（スリーバント失敗）・三振ゲッツー。各語彙値の状態遷移は付録E-2の`SO-01`〜`SO-05`を正とする | 整数 |
...
| `secondary-result` | `unchanged` \| `delta` | `true` \| `false` | `continue` \| `reach` \| `out` \| `not-applicable` |
...
- `secondary-result`（打撃結果2・特殊プレイ）は、PB／WP／ボーク等のように打者の行き先を決めない場合があるため`not-applicable`を許し、守備妨害で打者がアウトを宣告される`INT-06`と、打者側のピッチクロック違反が第3ストライクに当たる場合のため`out`を許す。ステップ5の導出時は打撃結果2をPB／WP／ボーク等の「打者の行き先を決めない特殊プレイ」だけで代表させ、打者がアウトになる打撃結果2を見落としていたため、本是正で許可集合を閉じ直す。
- `secondary-result`に`score`は許さない。打撃結果2の語彙には、それ自体が打者へ本塁を与える結果がないためである。打撃妨害で実プレイを採用する`INT-01`・`INT-03`において、採用した通常結果が本塁打等で`batterDestination.kind = score`となる場合、その状態効果は参照先の`eventKind = batting-result`行から適用し、`secondary-result`行へ`score`を重複定義しない。
...
各起点塁の`Adv`は、当該行の`precondition`が表す走者配置と`batterDestination`から、次の`ADV-01`〜`ADV-04`を上から評価して最初に一致するちょうど1件で決める。ここで、起点塁`b`（一塁=`1`、二塁=`2`、三塁=`3`）の**フォース連鎖**とは、`batterDestination.kind = reach`であり、かつ事前状態で一塁から`b`までの全ての塁に走者がいることをいう。打者走者が一塁を取得することで一塁走者が占有権を失い、その前位走者も後位側から連続して押し出されるが、最初の空き塁で連鎖は切れる。`forced`は最終到達塁の全区間が強制であることではなく、**起点塁を離れて少なくとも次塁へ進む義務があること**を表す。したがって、既定到達塁がさらに先でもフォース連鎖中の走者の`modality`は`forced`とする。

| 分岐ID | 判定条件と`Adv` | 正例（当該分岐に適合） | 負例（当該分岐に非適合） |
| --- | --- | --- | --- |
| `ADV-01` | `precondition`で当該起点塁に走者がいなければ`{modality: "not-applicable", destination: null}` | 一塁走者なしの行の`first`を`not-applicable`とする | 一塁走者がいる行は、進塁しなくても`hold`であり`not-applicable`ではない |
| `ADV-02` | 当該起点塁に走者がいてフォース連鎖に含まれるなら`modality = forced`とし、`destination`を起点塁別の到達可能集合に属する既定到達塁とする | 一・三塁で四球となる行では、一塁走者は`forced`で二塁へ進む | 同じ一・三塁の三塁走者は、二塁が空いて連鎖が切れているため`forced`ではない |
| `ADV-03` | 当該起点塁に走者がいてフォース連鎖に含まれず、既定として別の塁へ動くなら`modality = optional`とし、`destination`を起点塁別の到達可能集合に属する既定到達塁とする。審判の授与による非フォース進塁も本分岐とし、`optional`は授与を拒否できるという意味ではなく「フォース連鎖によらない進塁」を表す | 三塁走者が犠飛で本塁へ進む行は`optional`・到達塁4とする | 満塁の四球で三塁走者が本塁へ押し出される行はフォース連鎖に含まれるため`ADV-02` |
| `ADV-04` | 当該起点塁に走者がいてフォース連鎖に含まれず、既定では動かなければ`{modality: "hold", destination: null}` | 一・三塁の四球で、連鎖外の三塁走者を三塁に停止させる | 一塁走者がいる四球の行で一塁走者を`hold`にはできず、`ADV-02` |

4件は、各起点塁を「不在／在塁かつフォース連鎖内／在塁かつ連鎖外で進塁／在塁かつ連鎖外で停止」に相互排他的かつ網羅的に分ける閉じた分岐集合である。`ADV-01`は`XC-02`、`ADV-01`・`ADV-04`の`destination = null`は`XC-03`、`ADV-02`・`ADV-03`の到達塁は`XC-08`をそれぞれ満たさなければならない。フォース連鎖中の走者を`hold`とした行は4件のどれにも適合せずfailとする。

- `batting-result`と`secondary-result`は、`batterDestination.kind = reach`なら同じフォース連鎖を評価する。打者が一塁より先へ到達する行でも、一塁を取得して走者となるため連鎖の判定は変えない。`batterDestination.kind`が`continue`・`out`・`score`・`not-applicable`なら、この定義によるフォース連鎖は生じない。
- `runner-event`は`batterDestination.kind = not-applicable`に固定されるためフォース連鎖を生じず、在塁走者は`ADV-03`または`ADV-04`となる。盗塁・牽制・暴投／捕逸・ボーク等で進む既定は`optional`、動かない既定は`hold`とする。
- 投球だけで打撃結果2または走者イベントを伴わない行は在塁走者を`hold`とする。PB／WP／ボーク等を伴うときは、その特殊プレイの行で`optional`と到達塁を定める。手動上書き後の実到達塁はFR-003に従うが、本列の既定値の分類は上記4件から外さない。

**妨害系3種の遷移（典拠: 公認野球規則5.05(b)(3)、6.01(a)〜(c)・6.01(g)・6.01(h)、9.02(a)(1)）**:

本書でいう**打撃妨害**は捕手その他の野手が打者の打撃を妨げる守備側の妨害、**走塁妨害**は野手が走者または打者走者の走塁を妨げる行為、**守備妨害**は打者・打者走者・走者その他の攻撃側要員が守備側のプレイを妨げる行為をいう。各妨害イベントはFR-004の`interferenceRuling`を入力とし、種類ごとに次の順で評価して最初に一致するちょうど1件へ落とす。表中の「通常結果」は、採用された`resultId`の通常行が定める打者・走者・アウト・成績計上の全効果をいう。

| 分岐ID | 判定条件と遷移 | 正例（当該分岐に適合） | 負例（当該分岐に非適合） |
| --- | --- | --- | --- |
| `INT-01` | **打撃妨害・プレイ結果の自動成立**: 実プレイで打者が一塁へ到達し、全ての在塁走者も少なくとも1個進塁したなら、妨害の罰則を選択せず通常結果を採用する。打者・走者・アウト・全成績フラグは通常結果どおりとし、`打撃妨害出塁 = false` | 一塁走者ありで妨害後の打球が安打となり、打者が一塁、走者が二塁へ達した場合は安打の通常結果を採用する | 三塁走者が動かず打者だけが一塁へ達した場合は全走者が1個進んでいないため本分岐ではない |
| `INT-02` | **打撃妨害・罰則採用**: `INT-01`でなく、採用結果が`penalty-award`なら、打者へ一塁を与えて打席を終了し、アウトを増やさない。走者は`ADV-*`のフォース連鎖に含まれる者だけ進め、それ以外は`hold`とする。ただし、三塁走者がスクイズまたは本盗で得点を試みた際の妨害は公認野球規則6.01(g)を適用し、ボークとして全走者へ1個の塁を与える。`打席 = true`、`打数 = false`、`打撃妨害出塁 = true`とし、押し出し得点の打点は`RBI-07`で判定する | 満塁で罰則を採用した場合、打者一塁、各走者1個進塁、得点1、`RBI-07`により打点1 | 一・三塁の通常の打撃妨害で罰則を採用し、スクイズ・本盗でない三塁走者まで本塁へ進めるのはフォース連鎖外のため本分岐の遷移ではない |
| `INT-03` | **打撃妨害・実プレイ選択**: `INT-01`でなく、攻撃側が`play-result`を採用したなら、FR-004で指定した通常結果の全効果を採用し、妨害罰則側の一塁授与と強制進塁は適用しない。成績も通常結果だけを計上し、`打撃妨害出塁 = false` | 一死三塁で妨害後の犠飛により得点し、攻撃側が実プレイを選んだ場合は、打者アウト・三塁走者得点・犠飛の通常結果を採用する | 同じプレイで罰則を採用した場合は打者一塁・三塁走者`hold`となる`INT-02` |
| `INT-04` | **走塁妨害・妨害走者へのプレイあり**: `play-on-obstructed-runner`、または打者走者が一塁到達前に妨害された場合はボールデッドとし、審判が「妨害がなければ到達した」と裁定した塁を各走者・打者走者の到達塁とする。妨害された走者には最後に正規に触れた塁の1個先以上を与え、妨害の罰則によるアウトは0とする。打者走者に一塁以上が与えられて打席が終了したときは`打席 = true`、`打数 = false`、`走塁妨害出塁 = true`とし、在塁走者だけへの妨害なら打者の成績を変えない | 一塁走者が二塁でタッグされようとした際に妨害され、審判が二塁を与えた場合は走者を二塁へ置き、その妨害を理由とするアウトを記録しない | 妨害走者へのプレイがなく、プレイ継続後に裁定する場合は`INT-05` |
| `INT-05` | **走塁妨害・妨害走者へのプレイなし**: `no-play-on-obstructed-runner`ならプレイ可能な行為が終わるまで実プレイを進め、その後、審判が妨害を取り除くため最終裁定した到達塁とアウト対象をそのまま採用する。保護された塁を越えて走者自身の危険で進んだ結果のアウトも、この最終裁定に含める。打者に一塁以上が与えられた場合だけ`打席 = true`、`打数 = false`、`走塁妨害出塁 = true`とし、在塁走者だけへの妨害なら打者の成績を変えない | 一塁走者へのプレイがない間に妨害があり、プレイ終了後に審判が三塁を与えた場合は、入力された裁定どおり三塁へ置く | 妨害された走者へのタッグプレイ中に直ちにボールデッドとなる場合は`INT-04` |
| `INT-06` | **守備妨害・打者をアウト対象に含む**: 審判のアウト対象に打者または打者走者を含むならボールデッドとし、`batterDestination.kind = out`、`plateAppearanceEnded = true`とする。宣告された全対象だけを`outEffect.targets`へ置き、他走者は審判が指定した帰塁先へ戻す。`打席 = true`、`打数 = true`とし、各アウトを`投球回算入アウト`へ算入する。併殺阻止等で走者も同時に宣告された場合も本分岐とする | 打者走者が故意に併殺を妨げ、審判が打者走者と本塁に最も近い走者の2人をアウトとした場合は、両者を`outEffect.targets`へ置く | 走者だけがアウトを宣告され、打者がアウトでない場合は`INT-07` |
| `INT-07` | **守備妨害・打者をアウト対象に含まない**: 審判の非空のアウト対象が走者だけならボールデッドとし、その走者だけを`outEffect.targets`へ置く。打者は`batterDestination.kind = continue`、`plateAppearanceEnded = false`として打席・打数を計上せず、他走者は審判が指定した帰塁先へ戻す。宣告された各アウトを`投球回算入アウト`へ算入する | 一塁走者が盗塁中に捕手の送球を故意に妨げ、審判が一塁走者だけをアウトとして打者の打席を続行させた場合 | 打者もアウト対象に含む併殺阻止の裁定は`INT-06` |

`INT-01`〜`INT-03`は打撃妨害を「自動的に実プレイ成立／罰則採用／実プレイ選択」に、`INT-04`〜`INT-05`は走塁妨害を「妨害走者へのプレイあり（打者走者の一塁前を含む）／なし」に、`INT-06`〜`INT-07`は守備妨害の非空アウト対象を「打者を含む／含まない」に相互排他的かつ網羅的に分ける。この7件を妨害3種の閉じた分岐集合とし、必要な`interferenceRuling`を欠く行、空の守備妨害アウト対象、または複数分岐に一致する行はfailとする。

走塁妨害の授与塁、守備妨害のアウト対象・帰塁先、および打撃妨害で採用された側は審判または攻撃側の選択による観測事実であり、システムは打球・走者位置から自動推定せず、既定値も置かない。これはFR-003の`thirdOutTimingByRunner`と同じ境界である。`precondition`はFR-004の入力を実在する軸IDで参照し、同じ妨害結果でも裁定が異なれば別行とする。守備妨害時の帰塁先は、原則として妨害発生時に最後に正規に触れていた塁とし、打者走者が一塁へ達する前なら投球時に占有していた塁へ戻す。公認野球規則が別のアウトまたは進塁を明示する場合は、その審判裁定を`interferenceRuling`へ入力して優先する。

`INT-04`・`INT-05`で審判が与えた非フォース進塁は`ADV-03`の`optional`とする。打者走者への一塁授与によって生じる連続占有の押し出しだけは`ADV-02`の`forced`とする。ここで`optional`は審判の授与を任意に取り消せるという意味ではなく、フォースプレイの連鎖ではないことを表す。これにより、妨害系の全行も`XC-02`・`XC-03`・`XC-08`を満たす。
...
妨害系3種を`INT-01`〜`INT-07`へ展開しても、成績計上フラグは上表の23件で過不足がないため、**守備妨害専用フラグは追加しない**。打撃妨害による一塁授与は`打撃妨害出塁`、走塁妨害による打者走者の出塁は`走塁妨害出塁`で打数除外を表せる。守備妨害は攻撃側への罰則であって、付録A、FR-020またはFR-022が独立した「守備妨害数」の集計を要求していない。守備妨害という結果の別は`resultId`とイベントpayload、アウト対象と人数は`outEffect`、帰塁は走者状態、打者がアウトとなる場合の成績は既存の`打席`・`打数`・`投球回算入アウト`でそれぞれ表すため、同値の真偽を専用フラグへ重複保持しない。これは導出元からの漏れではなく、導出元の和集合に新しい計数項目が存在しないことによる非追加である。
...
| 打撃妨害 | `INT-01`〜`INT-03`。罰則採用なら打者一塁・原則としてフォース連鎖の走者だけ進塁・**打数に数えない**（スクイズ・本盗時は`INT-02`の例外）。実プレイ採用ならその通常結果を適用し、採用側はFR-004の裁定入力で確定する |
| 走塁妨害 | `INT-04`〜`INT-05`。妨害走者へのプレイの有無でボールデッドの時点を分け、審判が裁定した授与塁・最終アウトを適用する。打者走者への出塁授与は**打数に数えない** |
| 守備妨害 | `INT-06`〜`INT-07`。審判が宣告した打者または走者をアウトとし、他走者を裁定された塁へ戻す。打者がアウト対象なら打席終了・打数算入、走者だけなら打席続行 |

**三振系5語彙値の閉じた分類**:

三振系の語彙シード5値を、次の`SO-01`〜`SO-05`へ語彙値で相互排他的に分ける。全分岐で`eventKind = batting-result`、`plateAppearanceEnded = true`、`countEffect`はストライク・ボールとも`reset`、`打席 = true`、`打数 = true`、`奪三振 = true`、`三振 = true`とする。`投球回算入アウト`が`true`のときに投球回へ加える数量は、同じ行の`outEffect.count`から取得する。

| 分岐ID | 語彙値と状態遷移・成績計上 | 正例（当該分岐に適合） | 負例（当該分岐に非適合） |
| --- | --- | --- | --- |
| `SO-01` | **見逃し三振**: `batterDestination.kind = out`、`outEffect = {count: 1, targets: ["batter"]}`、`投球回算入アウト = true`。在塁走者の既定は`hold` | 捕手が正規に捕球した第3ストライクを打者が見逃した場合 | 第3ストライクを空振りした場合は`SO-02` |
| `SO-02` | **空振り三振**: `batterDestination.kind = out`、`outEffect = {count: 1, targets: ["batter"]}`、`投球回算入アウト = true`。在塁走者の既定は`hold` | 捕手が正規に捕球した投球を空振りして第3ストライクとなった場合 | 第3ストライクを捕手が正規に捕球せず、打者走者に一塁到達の可能性が生じた場合は`SO-03` |
| `SO-03` | **振り逃げ**: 第3ストライク不捕球後に打者走者が生きれば`batterDestination = {kind: "reach", base: 1}`、`outEffect = {count: 0, targets: []}`、`投球回算入アウト = false`。一塁到達前にアウトとなれば`batterDestination.kind = out`、`outEffect = {count: 1, targets: ["batter"]}`、`投球回算入アウト = true`。成立条件は付録E-2の既存行に従い前提条件で分ける | 二死で第3ストライクを捕手が後逸し、打者走者が一塁へ到達した場合はセーフ側 | 捕手が第3ストライクを正規に捕球して打者がアウトなら、見逃し方または空振り方により`SO-01`か`SO-02` |
| `SO-04` | **`K3`（スリーバント失敗）**: 2ストライク後のバントがファウルとなった三振を表す。`batterDestination.kind = out`、`outEffect = {count: 1, targets: ["batter"]}`、`投球回算入アウト = true`。ボールデッドとなり在塁走者の既定は`hold` | 2ストライク後のバントがファウルとなり打者アウトになった場合 | 1ストライク以下でのバントファウルは打席続行であり`K3`ではない |
| `SO-05` | **三振ゲッツー**: 第3ストライクによる打者アウトと、同じプレイ中の走者アウト1件を記録する。事前アウト数は0または1、`batterDestination.kind = out`、`outEffect = {count: 2, targets: ["batter", {runner: <アウトとなった走者の起点塁>}]}`、`投球回算入アウト = true`とする | 一死一塁で空振り三振と同時に一塁走者が二塁で盗塁死となり、3アウトで攻守交代した場合 | 走者アウトを伴わない通常の空振り三振は`SO-02`。二死からは打者の三振が第3アウトとなるため三振ゲッツー行を認めない |

この5件は語彙シードの「見逃し三振／空振り三振／振り逃げ／`K3`／三振ゲッツー」と1対1で対応し、未知の三振系値および複数分岐に一致する値はfailとする。`SO-03`だけは、1つの語彙値`振り逃げ`を前提条件と実際のアウト成否で付録E-2の既存2行「振り逃げ（セーフ）」「振り逃げ（アウト）」へ展開する。したがって、この2行は行名であって別の語彙値ではない。`K3`は`SO-04`のスリーバント失敗を表し、この2行のいずれとも重複しない。

`K3`の意味の**同定**には、継承元の値集合に`K3`が実在すること（`docs/legacy/research/data-layer.md:197`）、旧UIで`K3`が打者アウト・走者不変の三振系値として`振り逃げ`と分けられていること（`docs/legacy/research/input-screen.md:157-163`）、三振の略記として扱われていたこと（`docs/legacy/research/docs-tests-ops.md:90`）を証拠として用いる。この参照は旧から継承した語彙値`K3`をスコア記号「K₃」=スリーバント失敗と同定するためだけのものであり、状態遷移と成績計上の規範は上表および付録A-2とする。これにより、付録A-2が奪三振に含めるスリーバント失敗は既存語彙値`K3`で記録でき、語彙値を追加しない。

**三重殺の記録**:

1プレイで3アウトが成立する三重殺は、専用の語彙値を追加せず、打球と公式記録に適合する既存の`resultId`（`併殺打`・`ライナー併殺`、またはそのプレイに適合する他の打撃結果）と、`outEffect = {count: 3, targets: [Target, Target, Target]}`の組で記録する。3つの`Target`はそのプレイでアウトとなった打者・走者を表す**相互に異なる対象**とし、`XC-04`に従って`count = targets.length = 3`を必須とする。`投球回算入アウト = true`とし、投球回へ加える数量3は`outEffect.count`から取得する。専用語彙がなくても、アウト数・対象、打者の行き先、走者状態および既存の成績フラグという状態遷移の全比較面を表現できるため、状態計算のための語彙値追加は不要である。

三重殺行の`precondition`は事前アウト数0を必須とする。付録E-2の併殺行にある「第3アウト超過を許さない」は、確定後のアウト数を3で止めて4アウト以上を記録しない規則である。事前アウト数0から`outEffect.count = 3`とする三重殺はちょうど第3アウトまでであり、超過ではない。事前アウト数が1以上の行に3アウト効果を設定した場合、または`targets`が3件未満・重複対象を含む場合はfailとする。三振ゲッツーは`SO-05`のとおり最大2アウトであり、三重殺へ読み替えない。

三重殺専用の表示名は語彙シードに存在せず、表示名は語彙シードを正として`resultId`から参照解決するため、この記録から「三重殺」という結果表示名を合成してはならない。利用者向けの結果表示名は選択した既存語彙の表示名となり、三重殺であることは構造化された`outEffect.count = 3`と3件の`targets`から判定する。専用表示名を必要とする将来変更は管理者管理の語彙シード追加を要し、本ステップでは追加しない。
...
コールド条件の各段を`{点差: M, 適用開始回: N}`とする。`M`は首位側得点から劣位側得点を引いた1以上の点差であり、この値域の妥当性は`DRAW-02`で検証する。`N`はその回の開始時点からコールドを許す指定ではなく、**第N回の攻撃機会を基準に成立判定を開始する完了時点の指定**とする。すなわち、公認野球規則7.01(a)が規定試合を「後攻チームが同回の裏を必要としないか、その一部だけで足りる」場合に短縮すると定め、7.01(b)(1)が同点時の継続を「先攻チームが完了した回の終了時に上回るか、後攻チームが未完了の回で勝ち越す」までと定めるのと同じ終了タイミングで、先攻チームのリードでは後攻チームの同回の攻撃完了を待ち、後攻チームのリードでは同回表の完了後に裏を行わず成立でき、または同回裏の途中で必要な点差に達した確定プレイの直後に成立できる。
