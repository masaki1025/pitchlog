...
1. **対象のグループが `active` であること**。**終了したグループの制御資源は、読み取りも操作もすべて404**である（終了後の招待の受諾・招待の発行・役割の変更・付与の変更・離脱はいずれも成立しない。グループの終了時点で**未消費の招待はすべて無効**になる）
2. **要求元テナントの参加が `active` であること**（作成と招待の受諾は参加前の操作であるため本条件の対象外。招待の受諾は招待の有効性検証で代替する）
3. **要求元テナントが無効化されていないこと**

**拒否の応答**: 上記前提を満たさない要求、および `member` が `admin` 限定の制御資源（招待の一覧・状態、メンバーの参加日時・役割）や `admin` 限定の操作（招待の発行・失効／役割の変更／グループの終了）を要求した場合は、**いずれも404**とし**理由コードを返さない**（403 を返すと「そのグループが存在し、自分が権限を持たない操作がある」ことが漏れる）。
