...
- **「全件」の範囲は FR-038 の移行仕様で確定する**（v2.0 注記）: 旧システムは本書 v1.9 の版固定アーカイブ時点から開発が継続しており、テーブル構成が増えている。本書は「88列の毎球データと既存の試合・選手・所見を全件取り込む」ことを要件とし、**アーカイブ後に旧側で追加された領域を取り込むかどうかは FR-038 の移行仕様で確定する**（旧側の追加分を無条件に移行対象と読まない）。カットオーバー日・移行対象の線引き・除外テーブルと「全件移行」の整合は、**FR-038 の移行仕様を策定する別タスクで決定する**
