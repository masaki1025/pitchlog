...
| 1.9 | 2026-08-11 | **WSL2 移行(2026-08-07 PO 決定・ハーネス設計書 論点C)への追随(3点)**: NFR-021 を「Windows 11上のWSL2で完結（Windowsネイティブでの開発は対象外。PostgreSQLはWSL2内にネイティブ導入するかDockerで立てる）」へ改訂し、測定方法を「WSL2クリーン環境でのセットアップ再現」に、セットアップ手順の正を README → `docs/development/onboarding.md` へ／7.1 の技術スタックの開発DB配置を **WSL2内**へ統一／8章 DoD⑦ を「README（プロジェクト概要と導線）」と「開発者セットアップ手順（onboarding.md）」に分離。**なお v1.8 行が予定した「ハーネス設計書の確定ゲートでの一括検証」は実施されなかった**(要件書は in-review のまま残っていた) — 正本更新ゲートの唯一の決定表(設計書 7.6)に照らし、**本版で別途 7.3 の確定ゲートを通す**(計画: `docs/features/req-v1-9-nfr021-wsl2/plan.md`) | 山田 + Claude |
...
#### NFR-021: 開発環境
- **カテゴリ**: 互換性 ／ **優先度**: Must
- **要件**: 開発環境はWindows 11上のWSL2で完結する（Windowsネイティブでの開発は対象外。PostgreSQLはWSL2内にネイティブ導入するかDockerで立てる）。セットアップ・起動手順を `docs/development/onboarding.md` に整備する
- **測定方法**: WSL2クリーン環境でのセットアップ再現
...
### 7.1 技術・アーキテクチャ制約
- 技術スタック: Python / FastAPI + Vue.js + TypeScript / PostgreSQL（本番=Supabase、開発=WSL2内のネイティブ導入 or Docker）。変更には承認を要する（v1.8: フロントエンドを React から Vue.js へ変更 — 決定は [ADR-002](../adr/ADR-002-frontend-vue.md)。v1.9: 開発環境の配置を WSL2 内へ統一 — NFR-021）
...
7. **ドキュメント**: README（プロジェクト概要と導線）・開発者セットアップ手順（`docs/development/onboarding.md`）・運用文書（管理者PW再設定/バックアップ取り扱い/死活監視/接続プール/移行手順）・利用チーム向けガイド（紛失時PW変更含む）が現行化されている
