...
| 与死球 | 死球 | 整数 |
| 被安打 | 当該投手の投球中に相手打者へ記録された安打の合計。**安打 = 打撃結果が単打・二塁打・三塁打・本塁打のいずれか(FR-022 の「安打種別」)**(v2.6 仮追補 — FR-021/FR-038 が参照し被打率・WHIP の入力となる定義の欠落の是正・PO スコープ裁定 2026-09-03) | 整数 |
| 被本塁打 | 打撃結果=本塁打 | 整数 |
...
| チャート | 定義 | 表示書式 |
|---------|------|------|
| コース分布 | 全投球の付録B座標を対象（結果球に限らない）。密度表示またはゾーン区画別カウント。母数（球数）併記 | ゾーン区画別カウント = 整数(度数)/ 母数(球数)= 整数(v2.6 仮追補 — 密度表示の色階調は UI 層で binding の対象外) |
...
>
**A-4 の指標(v2.6 仮追補 — PO 裁定 2026-09-03 で射程に追加。本表の「表示書式」列が binding の正 — A-1 の共通規定・D-10 の被覆対象)**:
