...
- **空欄規則**: まだ開始していない各チームの攻撃回は空欄とし、その攻撃回が開始した時点で0を表示する。開始済みの回で得点がなければ、進行中・終了済みを問わず0とし、コールド成立または試合終了宣言等によって行われなかった将来の回は空欄のままとする。FR-005が定める、後攻チームがリードしたまま最終回の表を終えて不要となった裏の「X」だけを空欄以外の未実施表記とし、サヨナラで途中終了した裏はXではなくその回の得点を表示する。H/E/K/Bは試合開始時から0以上の整数を表示し、空欄にしない
...
コールド条件の各段を`{点差: M, 適用開始回: N}`とする。`M`は首位側得点から劣位側得点を引いた0以上の点差である。`N`はその回の開始時点からコールドを許す指定ではなく、**第N回の攻撃機会を基準に成立判定を開始する完了時点の指定**とする。すなわち、公認野球規則7.01(g)の終了タイミングと同じく、先攻チームのリードでは後攻チームの同回の攻撃完了を待ち、後攻チームのリードでは同回表の完了後に裏を行わず成立でき、または同回裏の途中で必要な点差に達した確定プレイの直後に成立できる。
