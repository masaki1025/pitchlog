...
- **達成条件**:
  ...
  - **(e) 検査基盤の導入期における経過規定**（`BOOT-*` — v2.9新設）: **本項の達成条件(b)が要求する検査基盤が未整備である期間について次を定める。本規定は(b)の充足判定を変更しない — 変更するのはマージ可否の判定のみである。** **`BOOT-VOCAB`（語の定義）**: 本規定において**「未達」**とは、対象欄が列挙する対象のうち(b)が要求する層・検査が整備されていないものをいい、**「違反」**とは宣言と実在が食い違う状態（宣言外の入口の存在・生成物の手編集・派生物の陳腐化）をいい、**「既知債務」**とは移行開始時点で既に未宣言であった対象を全件列挙して封印したものをいう。**「判定不能」**は [ADR-003](../adr/ADR-003-domain-calc-method.md) D-11 の定義による。**本規定が扱うのは「未達」と「既知債務」のみであり、「違反」と「判定不能」には一切触れない** — これらは本規定の期間中も従来どおり不合格として扱う。 **`BOOT-STATE`（移行状態）**: 検査基盤の導入期を**移行状態**と呼び、次の二軸で定める。**同じ状態に2つの終了条件を持たせない**。 **`BOOT-PROGRAM`（軸① 移行プログラム）**: **未達数と既知債務数の合計が0より大きいあいだ継続**し、**合計が0になった時点をもって自動的に終了して以後は本項の達成条件をそのまま適用する**（**必須ゲートへの昇格**）。**昇格は不可逆であり移行状態へ戻る経路を設けない** — 戻すには本書の改訂を要する。 **`BOOT-GRANT`（軸② 緩和されたマージ授権）**: 軸①が継続しているあいだ**未達および既知債務を理由としてマージを妨げない**。ただし**①停滞エポック内で未達数と既知債務数の合計が1度も減少しないまま50回のマージが経過したとき失効し**、**②軸①が終了したときは終了する**。**①による失効中は本項の達成条件が無条件に復帰する** — 移行状態を理由としたマージ可否の別判定は行われない。**失効は人間の明示的な再承認によってのみ解除される**（再承認の記録形式・承認の機械的認識条件は検査基盤の実装が定める）。 **`BOOT-NO-CLAIM`（充足判定の禁止）**: **移行状態にあるあいだ、本項の達成条件(b)②を満たしたと判定してはならない。またNFR-019(a)を合格と判定してはならない。これは軸②が有効であるか失効しているかを問わない。** **移行状態におけるCIの緑は「移行状態の要求を満たしている」ことのみを意味し、本項またはNFR-019(a)の充足を意味しない。** **`BOOT-REPORT`（未達レポート）**: **移行状態にあるあいだ、CIは未達および既知債務の一覧を機械可読な形式で出力しなければならない。**出力のない緑は本規定の充足とみなさない。 **`BOOT-SEAL-BASE`（比較元）**: 既知債務の比較元は**移行開始を承認したコミットの完全なcommit OID**とし、**merge-baseを用いてはならない**。 **`BOOT-SEAL-IMMUTABLE`（比較元の不変性）**: 比較元のcommit OIDは**当該PRから変更できない**。**これを書き換える変更は無条件に不合格とする。** 更新には本書の改訂を要し、**軸②の再承認では更新できない**。 **`BOOT-SEAL-MONOTONE`（単調性）**: **①封印集合に無い対象が新たに未宣言となること ②一度解消した対象が再び未宣言となること ③封印集合から要素を削除すること**のいずれも**不合格**とする。**解消済みの記録は追加のみ可能とし削除してはならない。** **`BOOT-SEAL-SCOPE`（債務の範囲）**: 既知債務の対象は**ADR-003 D-11が要求する宣言のうち移行開始時点で未宣言であったものすべて**とし、**実行入口に限定しない**（表外既定の表示対応の突合を含む）。 **`BOOT-STALL`（停滞の測定）**: 停滞エポックは**①移行状態の発効時 ②未達数と既知債務数の合計が減少した時点 ③再承認の成立時点**のいずれかで開始する。**経過は時間ではなくマージ回数で測る。****50回目のマージと減少または昇格が同一の変更で成立した場合は減少・昇格を優先する。**（未達数の算出方式・段階2のPRの判定方式・マージ回数の母集合の具体は検査基盤の実装が定める。**ただし未達は安定した対象IDの単位で数え、宣言の更新だけでは減少しない** — 実在集合との差分で測る。） **`BOOT-ACTIVATION`（発効）**: 本規定は**移行判定器・機械可読な未達レポート・不変な比較元・停滞の観測機構・違反を不合格とする負例・自動昇格の負例・失効の負例**の**7点がすべて同一の変更でCIの必須経路へ接続された時点をもって発効する。****接続前は本規定を適用せず本項の達成条件をそのまま適用する。本書がapprovedとなることは発効ではない。**
