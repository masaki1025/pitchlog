...
| 2.9 | 2026-09-13 | **検査基盤の導入期の移行状態を条文化する改訂を起案(TSK-355・状態: in-review)** — `ADR-003` v0.3・開発ハーネス設計書 v1.15 と**単一の確定ゲート(7.3)で一括検証**する。`NFR-018` へ**移行状態の経過規定**を新設する — **充足判定は従来どおり不合格のまま、閉じた移行状態に限りマージ可否を別判定する**。 **射程宣言(7.3-7)** — **本改訂で確定する範囲**: ①**移行状態の二軸**の定義と機械判定可能な遷移条件(**移行プログラム** = 「未達 0 かつ債務 0」の成立で必須ゲートへ**自動かつ不可逆に昇格** / **緩和されたマージ授権** = 有効 → 失効 → **人間の明示的な再承認**。**失効するのは後者だけで前者は継続する**) ②「**未達**」「**違反**」「**債務**」の語彙と既存の「判定不能」との切り分け ③**既知債務の授権**(全件列挙・封印・**比較元の固定**・追加/再導入/降格の禁止)④**無期限の緩和にしないことの担保**(停滞をマージ本数で測る)⑤**fail-closed 原則との優先関係**と、**移行状態では `NFR-018(b)②` / `NFR-019(a)` を満たしたと判定できない**こと ⑥**発効原子性**(判定器・機械可読な未達レポート・不変な開始基準・停滞の観測機構・違反/自動昇格/失効の各負例が**同一変更で CI の必須経路へ接続される**まで移行状態は**未発効**であり、**それ以前は既存の fail-closed が維持される**)⑦**強制点への追随**(全強制点を「直接改訂 / 明示参照 / 非波及」へ割り当てる)。 **実装時に確定する範囲(TSK-235)**: 移行判定器・債務台帳・未達レポートの**ファイル名・スキーマ・フィールド名**と CI の具体的な配線。 **本改訂で変更しない範囲**: **fail-closed 原則そのもの** / **`NFR-018` の対象集合・区分 (α)(β)**(**対象からも外さない** — 債務台帳に載せた対象も**未達として計上し続ける**)/ **`NFR-018 (c)` 逐語移植の例外表** / **`NFR-019` の柱書**。 **v2.8 との関係**: `NFR-019(b)` の発効条項(v2.8・TSK-379)は**経路の実装をもって発効する**ことを定めるものであり、**本改訂の移行状態は検査基盤そのものの導入期を対象とする別の事柄である**。**両者が共有するのは「実体が無いものについて判定を下さない」という原則のみ**で、**向きは `NFR-018` 側(未完成のあいだ合格と判定しない)を維持する**。計画: `../features/adr003-bootstrap-transition/plan.md` | in-review |
...
- **達成条件**:
  ...
  - **(e) 検査基盤の導入期における経過規定**（`BOOT-*` — v2.9新設）: **本項の達成条件(b)が要求する検査基盤が未整備である期間について次を定める。本規定は(b)の充足判定を変更しない — 変更するのはマージ可否の判定のみである。** **`BOOT-COUNT`（履行義務と残務数）**: **「履行義務」とは [ADR-003](../adr/ADR-003-domain-calc-method.md) D-11 が要求する宣言の1件をいう。****対象欄が列挙する対象計算が実在集合に属さない場合、当該対象についてD-11が要求する宣言はすべて未履行の義務として数える。****実在集合に属するのは、当該対象についてADR-003 帰結1 段階2が要求する4点（宣言モデルの正本・生成物・製品経路の呼び出し部・その対象の3層検査）が揃い、かつ当該対象の3層検査が本規定による免除を適用しない生の実行結果として緑であるもの**に限る（**免除後の判定を実在の条件にしてはならない** — 循環するため）。**「残務数」は未履行の義務の要素数**とする。**対象計算が「未達」であるとは、当該対象について未履行の義務が1件以上あることをいう**（未達は履行義務から導出され、独立した計数単位を持たない）。**義務の粒度は本項が定め、件数の算出は検査基盤の実装が行う** — **義務の件数を定数として持たず、D-11が要求する宣言の一覧から導出する**。**残務数は義務の単位で数え、対象計算・行・表・ファイルの単位では数えない。****義務の統合・ワイルドカード化・改名・分類変更は減少に数えない。宣言の更新だけでは減少しない。****対象計算の識別子は、本項の対象欄が列挙する対象計算の名称に一対一で対応する安定した識別子とする**（列挙そのものは対象欄が正であり、本規定は識別子の安定性のみを要求する）。 **`BOOT-VOCAB`（語の定義と分類の優先順位）**: 本規定において**「未履行」**とは、当該義務が要求する宣言が存在しないことをいい、**「違反」**とは、**宣言が存在するにもかかわらず実体と食い違うこと**（宣言外の入口の存在・生成物の手編集・派生物の陳腐化）をいい、**「既知債務」**とは移行開始時点で未履行であった義務を全件列挙して封印したものをいう。**「判定不能」**は ADR-003 D-11 の定義による。**封印の対象は未履行の義務に限り、違反は封印の対象にならない** — **封印された義務を持つ対象であっても、宣言が存在する他の義務について実体と食い違えば、それは違反であり本規定は一切触れない。** **本規定が扱うのは「未履行」と「既知債務」のみであり、「違反」と「判定不能」には一切触れない** — これらは本規定の期間中も従来どおり不合格として扱う。 **`BOOT-STATE`（移行状態）**: 検査基盤の導入期を**移行状態**と呼び、次の二軸で定める。**同じ状態に2つの終了条件を持たせない**。 **`BOOT-PROGRAM`（軸① 移行プログラム）**: **残務数が0より大きいあいだ継続**し、**残務数が0になった時点をもって自動的に終了して以後は本項の達成条件をそのまま適用する**（**必須ゲートへの昇格**）。**昇格は不可逆であり移行状態へ戻る経路を設けない** — 戻すには本書の改訂を要する。 **`BOOT-GRANT`（軸② 緩和されたマージ授権）**: 軸①が継続しているあいだ、**①既知債務として封印された義務の未履行に由来する不合格**、および**②`BOOT-ACTIVATION`が列挙する10点の実装に由来する未履行の不合格**を理由として、マージを妨げない。**本規定が妨げないのはこの2つに限り、いずれも「未履行」に由来するものに限る** — **違反・判定不能に由来する不合格は、②の場合も含めて一切免除しない。** **段階2の部分追加の禁止**: **当該PRが、ある対象計算についてADR-003 帰結1 段階2が要求する4点のいずれかを追加した場合、同一PRで当該対象の4点すべてが揃い、かつ当該対象の3層検査が緑でなければ不合格とする**（**一部だけを追加して残りを既知債務のまま据え置く経路を閉じる**。この判定は当該PRが4点のいずれかに触れたかで発火し、実在集合へ加わったか否かでは発火しない）。ただし**①停滞エポック内で残務数が1度も減少しないまま50回のマージが経過したとき失効し**、**②軸①が終了したときは終了する**。**①による失効中は本項の達成条件が無条件に復帰する**。 **`BOOT-REAPPROVAL`（再承認の統制）**: 失効は**人間の明示的な再承認によってのみ解除される**。**再承認が成立するのは、軸①が継続中かつ軸②が失効している場合に限る** — **軸②が有効であるあいだの再承認は成立せず、停滞エポックを再始動しない**（**別々の承認記録を有効期間中に投入してカウンタをリセットする経路を閉じる**）。**再承認を行うのは8章の判定者**とする。**1つの承認記録は1つの停滞エポックのみを開始し、再利用できない。****再承認を記録する変更は他の変更を含まない。****失効中にマージできるのは、①残務数を減少させる変更 ②再承認を記録する変更 ③本項の対象計算および検査基盤に一切触れない変更**に限る。**軸①が終了した後の再承認は受け付けない。****再承認に時間的な期限は設けない**（経過はマージ回数で測る）。（承認記録の形式と、検査が承認を認識する条件は検査基盤の実装が定める。） **`BOOT-NO-CLAIM`（充足判定の禁止）**: **移行状態にあるあいだ、本項の達成条件(b)②を満たしたと判定してはならない。またNFR-019(a)を合格と判定してはならない。これは軸②が有効であるか失効しているかを問わない。** **移行状態におけるCIの緑は「移行状態の要求を満たしている」ことのみを意味し、本項またはNFR-019(a)の充足を意味しない。** **`BOOT-REPORT`（未達レポート）**: **移行状態にあるあいだ、CIは未履行の義務および既知債務の一覧を機械可読な形式で出力しなければならない。**出力のない緑は本規定の充足とみなさない。 **`BOOT-SEAL-BASE`（比較元）**: 既知債務の比較元は**移行開始を承認したコミットの完全なcommit OID**とし、**merge-baseを用いてはならない**。 **`BOOT-SEAL-IMMUTABLE`（比較元の不変性）**: 比較元のcommit OIDは**当該PRから変更できない**。**これを書き換える変更は無条件に不合格とする。** 更新には本書の改訂を要し、**軸②の再承認では更新できない**。 **`BOOT-SEAL-MONOTONE`（単調性）**: **①封印集合に無い義務が新たに未履行となること ②一度解消した義務が再び未履行となること ③封印集合から要素を削除すること**のいずれも**不合格**とする。**解消済みの記録は追加のみ可能とし削除してはならない。** **`BOOT-SEAL-SCOPE`（債務の範囲）**: 既知債務の対象は**ADR-003 D-11が要求する宣言のうち移行開始時点で未履行であったものすべて**とし、**実行入口に限定しない**（表外既定の表示対応の突合を含む）。 **`BOOT-STALL`（停滞の測定）**: 停滞エポックは**①移行状態の発効時 ②残務数が減少した時点 ③再承認の成立時点**のいずれかで開始する。**経過は時間ではなくマージ回数で測る。****「1回のマージ」とは、`develop` の第一親をたどった列に現れるPR統合コミットをいう**（**feature側が `develop` を取り込むマージは数えない**）。**50回目のマージと減少または昇格が同一の変更で成立した場合は減少・昇格を優先する。** **`BOOT-ACTIVATION`（発効）**: **本規定における「同一の変更」とは、`develop` へ統合される1つのPRをいう**（PR内のコミットの分割は妨げない）。本規定は次の**10点がすべて同一の変更でCIの必須経路へ接続され、かつ下記の負例がすべて当該変更に含まれた時点をもって発効する**: **①移行判定器（二軸の状態と残務数を判定する） ②機械可読な未達レポート ③不変な比較元 ④停滞エポックの観測機構 ⑤`BOOT-SEAL-*`4条項を検査する機構 ⑥`BOOT-NO-CLAIM`を検査する機構 ⑦`BOOT-REAPPROVAL`を検査する機構 ⑧`BOOT-COUNT`の原子単位と解消条件を検査する機構 ⑨`BOOT-GRANT`の2限定と段階2の部分追加の禁止を検査する機構 ⑩違反・判定不能を免除しないことを検査する機構**。**必須の負例**: **封印集合への追加・解消済みからの復活・比較元の書き換え・移行状態における充足の主張・8章の判定者以外による再承認・再承認記録の再利用・再承認と他の変更を混載したPR・段階2の部分追加・違反を免除する判定・自動昇格の成立・停滞による失効**が、**いずれも不合格となることを示す負例**。**接続前は本規定を適用せず本項の達成条件をそのまま適用する。本書がapprovedとなることは発効ではない。**
