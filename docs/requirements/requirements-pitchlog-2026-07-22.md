...
- **受け入れ基準**:
  ...
  - Given 付与あり / When 共有画面を表示 / Then 共有されるのは**付録A-2（投手）・A-3（打者）に定義のある指標のうち単一の数値で表せるもの**と、**付録A-5 のチーム概要7項目**（試合数・勝・敗・分・得点・失点・得失点差）に限られる。**コース分布・球速分布・被打球分布・打率マップ・打球方向などの構造化指標は共有しない**（1つの数値に還元できないため）
...
- **補足（範囲）**: 突合は**集計値の比較まで**とし、1球単位の突合は行わない。**身体プロフィールの共有は v2.0 では行わない**（共有対象の実体である選手プロフィール・身体測定履歴が本版に存在しないため。`プロフィール` 粒度と分布の最小集団数の規定はあわせて次版で扱う）
...
- **補足（テスト）**: NFR-019(a) の一致性テストは**拡張しない**。同テストはクライアント/サーバーの状況計算の一致が対象であり、チーム集計はサーバー側の集計であるため、**バックエンド集計の契約と単体テスト**を別建てで置く
...
| 異種grantの扱い | 参加テナントごとに付与粒度が異なりうるため、**粒度ごとにファイルを分ける**（1ファイルに混在させると列の有無がテナントごとに変わりCSVとして表現できない）。各ファイルには**その粒度を付与しているテナントの行のみ**を含める。**除外されたテナント名は出力しない** |
...
- **測定方法**: NFR-019のCIに「テナントの自由入力→管理コンソール/レポート表示」の越境XSS回帰テストを含める
- **補足**: 管理者はチーム無効化・PWリセット・選手統合・DB保守の全権を持つため、管理者画面での保存型XSSはテナント分離（NFR-010）を迂回する経路になる。外部提供時に後付けが困難な性質のため初日から要件化する
...
2. **CI全グリーン**: NFR-019 のテスト一式（**全ランナー**=pytest・Vitest・Playwright、および同項の**(a)一致性・(b)越境・(c)E2E主要分岐・(d)同期故障系**）がすべて成功。**(b) の合否は NFR-019(b) の定義に従う**（「越境が1件でもあれば fail」ではなく「FR-034 の認可行列どおりに通り、行列外はすべて404」。同項に列挙した組合せを網羅すること）
...
> **FR-041 が共有する範囲**: 上表のうち**試合数・勝・敗・分・得点・失点・得失点差の7項目のみ**。チーム打率・出塁率・長打率・イニング別得失点は**自チーム分析用であり共有対象外**である。
