...
> 対話の決定経緯は [`requirements-draft-pitchlog.md`](requirements-draft-pitchlog.md)（D-1〜D-41）を参照。
...
4. （プロトタイプからの課題）通信環境・データ蓄積・チーム固有前提がシステムの信頼性と汎用性を制約している → 改善台帳 I-1〜I-24 で解消
...
- **防御率の算出・表示**（自責点判定の複雑さに対し、失点+FIPで投手評価を代替。付録Aに明記）
- **勝敗投手・セーブの自動判定**（公認規則9.17/9.19相当の記録員判断を要するため、自責点と同じ理由でWon't。**手動指定欄はFR-030のCould**で提供）
- **イレギュラー判定の専用自動化**（打順間違いのアピールプレイの正規化・没収試合の特殊スコア等は、記録員の手動運用＝修正機能FR-007と手動上書きFR-003で扱う）
- **チーム単位の分析画面**（チーム打率・イニング別得点傾向等。将来検討 → 10章）
- **対戦相手チームとのデータ共有・突合**（テナント分離が価値の核。将来検討 → 10章）
...
| システム管理者（=運用者・プロダクトオーナー） | チーム登録/無効化/PWリセット・挙動付き語彙/区分デフォルト規則/設定値の管理・選手統合・ゴミ箱期限後復旧・DB保守・操作ログ閲覧 | オンボーディング、例外対応、保守 | 1名 |
| 対戦相手チーム（データ主体） | システムは利用しない。選手名・成績が記録される側 | — | — |

### 用語定義
- **テナント**: ログインする契約チーム。データ分離の単位（NFR-010）
- **対戦相手チームレコード**: テナントが対戦相手として登録したチーム・選手のデータ。**テナント内のレコードであり、テナント間で共有されない**（両チームが pitchlog を使う試合でも、それぞれが独立に記録する）
- **記録権**: 1試合の入力を1端末に限定するサーバー管理のロック（FR-013）
- **操作イベント**: 毎球入力・undo・交代・タイブレーク開始・試合終了宣言・選手のその場登録の6種。通信断中の操作はこの単位でキューに積まれる（FR-012）
...
stateDiagram-v2
    ...
    進行中 --> 進行中: 進行中に留まる操作イベント（毎球入力/undo/交代/TB開始/その場登録）
    進行中 --> 進行中: 通信断（クライアント計算で継続・復帰後同期）
    進行中 --> 終了: 操作イベント第6種=試合終了宣言（規定成立・コールド・サヨナラ・引き分け・途中打ち切り）
    終了 --> 終了: 過去プレイの修正（下流再計算）
    終了 --> 進行中: 追記のための再開（入れ忘れ等）
    終了 --> ゴミ箱: チームによる削除（論理削除）
    ゴミ箱 --> 終了: 30日以内の自己復元
    ゴミ箱 --> 非表示: 30日経過（DB残存・管理者のみ復旧可）
```
...
- **球種には「系統」属性を持たせる**（初期値: 直球系／スラ系／落ち系／未分類）。チームが球種を追加するときは系統を選択できる（未分類も可）。分析の球種グループ集計（ブロック4）はこの系統属性を軸とし、**球種名のハードコードによるグループ分けを実装しない**（旧版は分析コードに「スラ系=スライダー・カット・カーブ」等が焼き込まれており、追加球種が集計から漏れる構造だった）
- 語彙の初期値は付録D-4の一覧を投入する（旧版の値集合と同一内容 — 88列互換の値レベルの互換性を保つ）
- 表示順の管理・チームごとの非表示設定 = **Could**

#### 4.0-4 通信断中の動作（FR-012 の基盤）
- 断中可能な操作イベント（完全列挙）: **毎球入力・undo・選手交代・タイブレーク開始・試合終了宣言・選手のその場登録**。これ以外（過去プレイの修正・分析・カルテ編集・試合開始）はオンライン前提
- スコア・アウト・走者・打順・終了判定の状況計算はクライアント側でも実行できる（設計フェーズへの申し送り: サーバーとの単一実装/一致性担保は NFR-019 のCI一致性テストで検証）
...
- **受け入れ基準**（旧FR-001継承+以下を上書き）:
  - Given チーム・選手が登録済み / When スタメン（打順9名。DH制の場合は10人目として投手を別枠登録）と試合情報（日時・試合区分・大会名）を入力して確定 / Then 試合が作成され、**適用規則のスナップショット**（FR-014）が試合に保存され、毎球入力画面に遷移する
...
- **受け入れ基準**（旧FR-002継承+以下を上書き）:
  ...
  - Given 必須項目が未入力 / When プレイ確定 / Then 不足項目が明示されて確定できない。**必須項目 = コース・球種・打撃結果／任意項目 = 球速・構え**【暫定値・要確認: 現行実装のバリデーション準拠。運用感覚に合わなければ変更可】
...
#### FR-004: 走者・特殊プレイの記録
- **優先度**: Must ／ **関連する目的**: G-1
- **ユーザーストーリー**: As a スコアラー, I want 盗塁・牽制・暴投・捕逸・ボーク・エラー・打球情報・作戦を記録できる, so that 打撃結果以外のプレイも漏れなく蓄積できる
- **受け入れ基準**（旧FR-004継承）:
...
- **受け入れ基準**（旧FR-005継承+以下を上書き）:
  ...
  - Given サヨナラ成立 / When 得点計上 / Then 本塁打によるサヨナラは打者・全走者の得点を計上し、それ以外は決勝点までを計上する（公認規則準拠 → 付録A-1）。行われなかった裏の攻撃はスコアボードに「X」表記
...
- **受け入れ基準**（旧FR-007継承=幻の得点/アウトの禁止・差分プレビュー・楽観ロック。+以下を上書き）:
  ...
  - Given プレイの追記が必要（入れ忘れ等） / When 試合を進行中に戻す / Then 追記して再終了できる
...
- **補足**: 境界=DH解除・同一枠への再交代・直前交代の取り消し。**DH解除時は投手がDHの打順枠を引き継ぐ**（規則どおり）。連盟規定にない交代（リエントリー等）は対象外
...
- **受け入れ基準**:
  - Given 通信断発生 / When 4.0-4 の6操作を実行 / Then 端末内キュー（操作イベントの順序付き列）に保存され、入力を継続できる。未送信件数が常時表示される
...
- **受け入れ基準**:
  ...
  - Given 退避されたイベントの取り込み / When 実行 / Then **現在の正史の末尾への再番号付けと下流再計算を伴う明示的な操作**として行われる（衝突する連番のまま挿入しない。取り込み結果は差分プレビューで確認できる）
...
- **補足**: **在籍ステータスは対戦相手チームレコードの選手にも適用される**（メンテはそのチームを記録している側が行う）。在籍区分はカルテ・試合準備の画面からも変更できる（相手ロスターの棚卸し導線 — 一括出力FR-029の「現役のみ既定」を成立させる）。移行時は既存選手全員「現役」で取り込まれるため、**移行後の在籍棚卸し（歴代選手の一括OB化）を8章DoDに含める**
...
- **受け入れ基準**:
  ...
  - Given ゴミ箱UI / When 表示 / Then 「30日で復元できなくなる」旨が明示されている
...
- **受け入れ基準**:
  ...
  - Given プリフェッチの一部失敗（一部選手の取得エラー等） / When 試合開始 / Then **試合開始は妨げられず**、未取得の選手は断中「未取得」表示（FR-021③方式）になる。失敗分はバックグラウンドで再試行される
...
- **受け入れ基準**:
  - Given 対象投手のデータあり / When 投手・期間・試合区分・大会名を指定 / Then コース分布図・球種割合・球速分布・打球チャートが表示される（絞り込み未指定なら全データ）
...
- **受け入れ基準**:
  - Given 任意の選手 / When カルテを開く / Then 統計サマリー（付録Aの定義・母数併記・集計範囲ラベル必須）と所見が表示される。既定の集計範囲=全期間、分析画面と同じフィルタで絞り込み可
...
- **受け入れ基準**:
  - Given 対象選手を指定 / When PDF生成 / Then 統計＋所見入りPDF（集計範囲ラベル・母数併記・付録B座標系のチャート入り）がダウンロードできる
...
- **受け入れ基準**:
  - Given 正しいチーム名+パスワード / When ログイン / Then 自チームのデータのみ操作できるセッションが開始される（トークンは NFR-011）
...
- **ユーザーストーリー**: As a チームアカウント利用者, I want 他チームから自チームの記録・分析・カルテに一切アクセスできない状態が保たれてほしい, so that スカウティング情報の優位性が守られる
- **受け入れ基準**:
  - Given チームAでログイン / When チームBの試合ID・選手・カルテ・帳票をURL直叩き・API直叩きで指定 / Then 404または拒否となり、内容・存在が応答から判別できない
- **補足**: 対象はデータ資源。チーム名自体は登録時の重複チェック等で存在が判別され得るが、データ漏えいに当たらないものとする。越境アクセスの自動テストは NFR-019 に含む
...
- **受け入れ基準**:
  ...
  - Given 管理者認証済み / When チーム無効化 / Then 当該チームのログインが拒否され、**既存の全トークン・セッションも即時失効する**（サーバーは毎リクエストでテナント有効状態を検証する）。データは全保全され、再有効化で完全に戻る（可逆）
  ...
  - Given （Should）破壊的操作 / When 実行 / Then 日時・操作種別・対象が操作ログに記録され、管理コンソールで閲覧できる
...
- **受け入れ基準**:
  ...
  - Given 移行完了 / When 検証 / Then 件数突合・代表集計値の新旧一致・**コース分布チャートの右打者のみ/左打者のみ別の突合**がすべて一致する。**突合の基準**: 代表集計値の一覧（試合数・プレイ行数・選手数・チーム別通算打率・通算投球数・被安打数・本塁打数を最低限含む）は移行計画で確定し、**新旧いずれも「旧DBの生データから付録A定義で再計算した値」同士を完全一致で比較する**（旧システムの画面表示値との比較ではない——旧表示は独自ロジックのため定義差で不一致になり、検証として成立しない）
...
- **要件**: 分析・カルテ画面の初期表示が5秒以内（1チーム5年分≈15万プレイ蓄積時・ウォーム状態で維持）
...
- **要件**: 性能目標（NFR-001〜004）の前提負荷は「1テナントあたり同時3試合（各試合=入力1端末+閲覧10端末）+分析利用の並行」とし、15万プレイ蓄積まで維持する。集計はDB側絞り込み・集計（WHERE/GROUP BY+インデックス）を基本規律とし、全件読み込み型の集計を実装しない。毎球参照される画面に限り事前集計テーブルを採用可（元データ修正時の再集計を仕組みに含める）。一覧系画面はページングする
- **測定方法**: 15万プレイ+前提負荷での実測（DoD）。コードレビューで全件読み込みの不在を確認
- **補足**: チーム数増加時はインフラ増強+本要件の改定で対応（多チーム規模の性能試験は今回しない）
...
- **要件**: チームは自チームが記録したデータのみアクセスできる。他チームのデータ資源は内容・存在を含め参照できない（射程は FR-034 補足）。テナント分離は初日から全機能に適用し、管理者操作もチームスコープされる
...
- **要件**: ドメイン計算（状況判定・座標変換・捕球選手推定・成績集計の前処理・終了判定）は単一の共有実装とし、UI別・経路別のコピー実装を作らない（クライアント/サーバー両方で必要な状況計算の配置は設計課題として申し送り。一致はNFR-019で担保）
- **測定方法**: コードレビュー+一致性テスト

#### NFR-019: テスト規約のCI強制
- **カテゴリ**: 保守性 ／ **優先度**: Must
- **要件**: テストはpytestに一本化し、PRごとにCIで自動実行、全グリーンでないとマージ不可とする。CIには (a) クライアント/サーバー状況計算の一致性テスト（同一のプレイ入力列から同一状況を導く。**付録Eマトリクスのゴールデンケースを共通の正解ベクタとする**。断中に実行するドメイン計算＝状況判定・終了判定・**捕球選手推定・座標導出**を対象に含める） (b) 越境アクセステスト (c) 主要分岐のE2Eテスト（タイブレーク・コールド・引き分け・終了後修正→再集計・交代フルセット・通信断→復帰同期・**記録権の通常/緊急引き継ぎと退避経路**）を含む
- **測定方法**: CI構成の確認と実行履歴
...
#### NFR-021: 開発環境
- **カテゴリ**: 互換性 ／ **優先度**: Must
- **要件**: 開発環境はWindows 11で完結する（PostgreSQLはネイティブ版またはDocker）。セットアップ・起動手順をREADMEに整備する
- **測定方法**: Windowsクリーン環境でのセットアップ再現
...
| データ | 内容 | 量の想定 | 保持期間 |
|--------|------|---------|---------|
| 毎球データ | 新スキーマ（正規化・選手ID参照）。1試合250〜350行 | 年約3万行/チーム。設計上限15万行（1チーム5年） | 無期限 |
...
- **事前集計・キャッシュの無効化マトリクス**: 無効化→再計算のトリガーは次のすべての変更操作を含む——復帰同期・プレイ修正・undo・交代の記録/修正・試合の削除/復元/再開・終了後修正・選手統合/分割・設定値変更。設計時にトリガー×対象範囲の対応表として文書化し、各操作の受け入れテストに含める（削除した試合が事前集計にだけ残る類の取りこぼし防止）
- 「削除」はすべて論理削除（4.0-2）
...
### 6.4 個人情報・機密情報の扱い
- 選手名・背番号・成績・所見は**チーム内限定**（テナント分離 NFR-010 が担保。対外公開機能は持たない）
...
### 7.1 技術・アーキテクチャ制約
- 技術スタック: Python / FastAPI + React + TypeScript / PostgreSQL（本番=Supabase、開発=Windowsネイティブ or Docker）。変更には承認を要する
...
2. **CI全グリーン**: NFR-019 のテスト一式（pytest・一致性・越境・E2E主要分岐）がすべて成功
...
5. **移行完了**: FR-038 の検証がすべて一致し、**警告レポートの棚卸し**（得点・成績に影響する不整合行の修正または影響なし判断）と**在籍棚卸し**（移行で全員「現役」になった歴代選手の一括OB化）が完了している
6. **バックアップ復元リハーサル**: 1回実施し、前日時点のデータに復元できることを確認
7. **ドキュメント**: README（Windowsセットアップ）・運用文書（管理者PW再設定/バックアップ取り扱い/死活監視/接続プール/移行手順）・利用チーム向けガイド（紛失時PW変更含む）が現行化されている
...
|---|------|------|------|------|------------------|
| R-1 | 属人性 | リスク | 開発・運用が1名に依存 | 中 | 本書+改善台帳+README+運用文書で引き継ぎ可能性を確保 |
...
| R-3 | 二重計算の乖離 | リスク | 状況計算がクライアント/サーバー両方に存在（FR-012の代償） | 高 | 単一実装原則（NFR-018）+一致性テストのCI常設（NFR-019） |
...
| R-9 | チーム拡張語彙の表記ゆれ | リスク | 球種等の自由追加で「スライダー/横スラ」の混在があり得る | 小 | チーム内運用規律に委ねる。保全規律（無効化・改名）で事後統合可能 |
...
| セキュリティ詳細（レート制限の閾値・ロック時間・失効設計） | FR-033の暫定設計で着手 | 山田さん+関係者相談 | **リリース判定前** |
| バックアップ最終保管先 | 本命=S3互換ストレージ、暫定=運用者ローカルPC | 山田さん | **リリース判定前** |
| チーム管理者ロールの導入 | 導入せず（ゴミ箱方式+システム管理者で代替）。導入時は削除系の担い手を再配置 | 山田さん+チーム関係者 | 外部チーム増加時 |
| 対戦相手データの突合・共有 | Won't | 山田さん | 将来検討 |
| チーム単位の分析 | Won't | 山田さん | 将来検討 |
...
| 指標 | 定義 |
|------|------|
| 投球回 | アウト数÷3。1/3回単位で保持し「5回2/3」形式で表示（振り逃げはアウトが増えないため投球回に加算されない — 奪三振とは独立に数える） |
| 投球数 | 記録された投球イベント数 |
...
| FIP | (13×被本塁打 + 3×(与四球+与死球) − 2×奪三振) ÷ 投球回 + 定数（既定3.10・システム設定値）。**チーム内比較用であり、プロの防御率水準と直接比較できない旨を画面に注記する** |
...
**走者の責任投手**: 各走者には「出塁させた投手」を責任投手として保持する（データ要件6.1に含む）。**打席途中の投手交代**の帰属は公認規則準拠——交代時のカウントがボール優勢（2-0, 2-1, 3-0, 3-1, 3-2）で結果が四球の場合はその打者・出塁は**前任投手**の責任、それ以外の結果は救援投手に帰属する。
...
| 球速分布 | 球速入力のある投球を対象。ビン幅は既定2km/h【暫定・要確認】。球種・系統での絞り込み可 |
...
| 打球方向 | インプレー打球の付録B座標から**引っ張り／センター返し／流し打ち**の3バケットに分類（打者の左右で反転。区分角はB-6の領域シードで定義）【バケット粒度は暫定・要確認】。散布図表示は座標そのまま |

### A-3b カルテ掲載指標セット
- **投手カルテ** = A-2の全指標（当日+シーズン通算）+ A-2bのコース分布・球種割合（対左右分解含む）
- **打者カルテ** = A-3の全指標（対左右分解含む）+ コース別打率マップ + 打球方向
...
| エンドラン | 打者がインプレーにし、走者が進塁。**または打者が関与せず走者の進塁だけが成功した場合も成功（走者成功）に分類**（旧語彙の3値目「盗塁成功」に相当。二値では未分類落ちするため） | 空振りかつ走者アウト・併殺 |
...
5. **移行の変換式（確定）**: 旧263pxスケール座標からの変換は `x_norm = x_old / 263`、`y_norm = (263 − y_old) / 263`（旧Y軸は下向きのため反転）。端点は両端を含み、範囲外の値は [0,1] にクランプして警告レポートに記録する。**テストベクタ**（変換前後の具体値の組）をゴールデンフィクスチャとして収載し、変換後のコース分布チャートが**右打者のみ/左打者のみに分けて**新旧一致することを移行検証（FR-038）に含める
...
> システム管理者が管理コンソール（FR-037）で変更できる値。変更は以後の表示に反映され、関連キャッシュ・事前集計は無効化→再計算される。**本文中の数値はすべて「既定値」であり、実効値は本表の設定に従う**（本文と本表が食い違う場合は本表が正）。
...
1. **88列契約の実体**: 88列の列名・順序・型は現行リポジトリ `config.py` の `COLUMN_NAMES` を正とし、その固定コピー（機械可読なスキーマ定義ファイル）を pitchlog リポジトリに収載してバージョン管理する（「準拠」の判定基準を実装間の解釈に委ねない）
2. **移行（取り込み）**: 旧システムの88列「1プレイ=1行」を読み取る変換層を実装する。取り込み側の88列定義は固定（過去データの形は不変のため）
3. **CSV出力**: 88列互換の本体は不変とし、88列に収まらない新概念（交代履歴等）は**別ファイル方式（例: `substitutions.csv`）を正**として出力する。**サイドカー契約の固定**: 各サイドカーの列定義・本体との結合キー（試合ID+イベント連番）・直列化規則（NULL=空文字・日時=`YYYY-MM-DD HH:MM:SS`（JST）・小数=ピリオド区切り・エンコーディング/無害化は本体と同一）を**バージョン付きスキーマ定義ファイル**で固定し、ゴールデンフィクスチャ（正解サンプル一式）をテストに含める（実装間で「準拠しているのに互換性がない」ファイルを作れないように）
4. **語彙の初期値**: 値集合の出所は旧リポの UI 語彙定義（`frontend/src/lib/vocab.ts` および Streamlit 入力UI。※`config.py` は列名の定義であり値集合は含まない — v1.3で出所を訂正）。pitchlog では初期値一覧を機械可読なシードデータとしてリポジトリに収載し、それを正とする。**球種の初期値（10種）と系統の初期割り当て（v1.4修正）**:
   - 直球系: ストレート、**ツーシーム、シュート**（いずれも速球。旧版分析コードは落ち系に誤分類しており継承しない — 野球ドメイン監査による是正）
   - スラ系: スライダー、カット、カーブ
   - 落ち系: チェンジ、フォーク、シンカー
   - 未分類: 特殊球
...
### E-1 マトリクスのスキーマ（1行=1つの打撃結果/特殊プレイ）
| 列 | 内容 |
|----|------|
| 結果ID・表示名 | 語彙マスタの打撃結果/打撃結果2と1対1 |
...
| **振り逃げ** | **奪三振を計上しつつ打者は走者になり得る（アウトが確定しない）**。第3アウトを自動で作らない。振り逃げ成立条件（無死/一死は一塁空きのみ・二死は常時）はマトリクスの特記で表現 |
...
> FR-014 の規則セットの構造と、試合区分デフォルトの既定値。**既定値は旧版コードの判定ロジック準拠の暫定値であり【要確認】**——確認後に確定し、以後の変更は管理コンソール（FR-037）の運用作業。
