...
- **達成条件**:
  ...
  - **(b) 宣言と機械判定**: 対象ごとに**出所（正本の実装）・すべての実行入口・派生関係**を機械可読に宣言し、CIが ①**出所と派生物の乖離** ②**差分の検出可能性** ③**宣言された構成の完全性** を機械的に判定できること。**②を欠く担保は本要件を満たさない**（旧システムは共有モジュールを持ちながら「定義は共有・ロジックは別コード」の状態が解消せず、**片側だけが修正されて値がずれた**＝旧NFR-011）:
    ...
    - **② 差分の検出可能性**（v2.2で目的を変更 — **「再実装が存在しないこと」の証明から、「再実装があって値がずれたら必ず落ちること」の担保へ**）: **対象計算の結果として、宣言された正本の出力と異なる値が製品の経路に現れた場合、CIが必ず検出する**ことを担保する。担保は次の3層で構成し、**いずれか1層でも欠ければ本要件を満たさない**:
      - **網羅的な正解ベクタ**: 有限の状態空間を持つ対象は、**状態×イベント×主要フラグを全列挙**した正解ベクタを持つ（代表例だけで済ませない）
      - **プロパティテスト**: 不変条件（カウント上限・走者の一意性・得点の単調性等）と、**正本と各実行体の等価性**をランダム入力で検査する
      - **変異テスト**: 上記2層の**実効性そのものを測定**し、**生存率をCIの合否基準とする**（閾値は設計フェーズのADRが定める）。**「テストは通るが差分を捕まえられない」状態を許さない**
    ...
    - **残余の明示**: 上記をすべて満たしても、**正本と常に同一の値を返す再実装が存在する事実**は検出できない。ただし**値が同一である限り製品の挙動に差は生じず、片側だけが修正された時点で②が捕捉する**。この残余は本要件の適用範囲外とし、**(a)の単一性とレビューで抑止する**（v2.2で明示 — 検出できない範囲を要件が黙って含意しないため）
...
- **測定方法**: (b)の宣言に対する機械検査（①**出所と派生物の乖離の検出** ②**差分の検出可能性**＝網羅ベクタ・プロパティテスト・**変異テストの生存率** ③**宣言された構成の完全性**＝入口の全列挙・構成の照合・動的機構の禁止）+ NFR-019(a)の一致性テスト。(β)は上記「対象」欄が区分ごとに定める担保（①〜⑤・⑦＝付録Aの固定ベクタ／⑥＝付録B-5の変換ベクタとFR-038の移行突合／⑧＝FR-038の導出規則に対する単体テストと同条の移行突合）
