...
- **達成条件**:
  ...
  - **(b) 宣言と機械判定**: 対象ごとに**出所（正本の実装）・すべての実行入口・派生関係**を機械可読に宣言し、CIが ①**出所と派生物の乖離**（派生物が古い／手で修正された状態） ②**宣言された閉域の完全性**（下記）を機械的に判定できること。**②を欠く担保は本要件を満たさない**（旧システムは共有モジュールを持ちながら「定義は共有・ロジックは別コード」の状態が解消しなかった＝旧NFR-011）。**②が判定する対象は「宣言された正本・生成物・実行入口という構成」であり、任意のコードが対象計算を再実装していないことの意味的判定ではない**（v2.2で限定 — 後者は一般に決定不能であり、ADR-003 が現行言語構成では実現不能と結論した）:
    ...
    - **結果型の構築点の限定**（v2.2で新設 — **残余に対する新しい強制点**）: **対象計算の出力を表す型は、生成物のみが構築できる**ものとし、**生成物の外での構築を型と静的検査で拒否する**。これにより、宣言された構成の内側に再実装が書かれても、**その結果を製品の経路へ流し込めない**（再実装は「書けるが使えない」状態になる）
    - **残余の明示**: 上記をすべて満たしても、**生成物の外で対象計算を再実装し、その結果を対象計算の出力型を経由せずに画面や記録へ渡す経路**は検出できない。この残余は本要件の適用範囲外とし、**結果型の構築点の限定で経路を狭めたうえで、(a)の単一性・(b)①の乖離検出・NFR-019(a)の一致性テスト・レビューの併用で抑止する**（v2.2で明示 — 検出できない範囲を要件が黙って含意しないため）
...
- **測定方法**: (b)の宣言に対する機械検査（**出所と派生物の乖離の検出**および**宣言された閉域の完全性検査**＝入口の全列挙・構成の照合・動的機構の禁止・結果型の構築点の限定）+ NFR-019(a)の一致性テスト。(β)は上記「対象」欄が区分ごとに定める担保（①〜⑤・⑦＝付録Aの固定ベクタ／⑥＝付録B-5の変換ベクタとFR-038の移行突合／⑧＝FR-038の導出規則に対する単体テストと同条の移行突合）
...
| R-3 | 二重計算の乖離 | リスク | 状況計算がクライアント/サーバー両方に存在（FR-012の代償） | 高 | 単一実装原則（**NFR-018 の達成条件(a)〜(d)** — 実装は1系統・派生物は生成物・**宣言された閉域の完全性をCIで検査**〔入口・構成・動的機構・結果型の構築点〕）+一致性テストのCI常設（NFR-019(a)）。**本欄は対策の宣言であり、両側に計算が存在することを受容したものではない** |
