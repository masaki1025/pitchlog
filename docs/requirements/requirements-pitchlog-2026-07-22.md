...
6. **バックアップ復元リハーサル**: 1回実施し、前日時点のデータに復元できることに加え、①全試合の既存の記録権世代を失効させ、復元調整中の全変更をフェンスする、②復旧手順が回収対象として指定した端末の D1 付きイベントと P3 受理結果を適用・破棄せず退避する、③退避内容を管理コンソールで閲覧・書き出しでき、未回収の対象が 0 であることを確認する、④過去に発行した値を再利用しない新しい記録権世代 D4 を D3 = 0 で開始する、までのライフサイクルを完走する
