...
- **当日打席結果の定義**（v2.2で確定 — NFR-019(a) の比較面がこの列を参照する）: **打席ごとに1行**とし、次の7列を持つ:
  1. **打席番号**（その試合で当該打者が立った順。1始まり） 2. **イニング**（回・表裏） 3. **結果表示名**（打撃結果の表示名。見三振／空三振／`{捕球選手}{打球タイプ}` 等の合成表記） 4. **打数算入の可否**（犠打・犠飛・四球・死球・打撃妨害は打数に数えない → 付録A-3） 5. **安打種別**（単打／二塁打／三塁打／本塁打／なし） 6. **打点** 7. **出塁の別**（安打／四球／死球／失策／野手選択／打撃妨害／なし）
...
- **達成条件**:
  ...
  - **(b) 宣言と機械判定**: 対象ごとに**出所（正本の実装）・すべての実行入口・派生関係**を機械可読に宣言し、CIが ①**出所と派生物の乖離**（派生物が古い／手で修正された状態） ②**宣言された閉域の完全性**（下記）を機械的に判定できること。**②を欠く担保は本要件を満たさない**（旧システムは共有モジュールを持ちながら「定義は共有・ロジックは別コード」の状態が解消しなかった＝旧NFR-011）。**②が判定する対象は「宣言された正本・生成物・実行入口という構成」であり、任意のコードが対象計算を再実装していないことの意味的判定ではない**（v2.2で限定 — 後者は一般に決定不能であり、ADR-003 が現行言語構成では実現不能と結論した）:
    ...
    - **残余の明示**: 上記を満たしても、**宣言された構成の内側に手書きで再実装されたコードは検出できない**。この残余は本要件の適用範囲外とし、**(a)の単一性・(b)①の乖離検出・NFR-019(a)の一致性テスト・レビューの併用で抑止する**（v2.2で明示 — 検出できない範囲を要件が黙って含意しないため）
...
- **測定方法**: (b)の宣言に対する機械検査（**乖離の検出**および**宣言外実装の検出**）+ NFR-019(a)の一致性テスト。(β)は上記「対象」欄が区分ごとに定める担保（①〜⑤・⑦＝付録Aの固定ベクタ／⑥＝付録B-5の変換ベクタとFR-038の移行突合／⑧＝FR-038の導出規則に対する単体テストと同条の移行突合）
...
| **NFR-018 の正解ベクタの整備**（付録A固定ベクタ・付録E全表・付録B-6領域シード・付録B-5変換ベクタ）— **①実現手段と②検査方式は [ADR-003](../adr/ADR-003-domain-calc-method.md) で確定済み**（①＝制約された宣言モデルからPython・TypeScriptを生成／②＝宣言された閉域の完全性検査。**②は現行定義では実現不能とADRが結論したため本書v2.2で再定義した**）。**FR-022の列定義もv2.2で確定済み** | 暫定＝**ベクタの形式・置き場・CI配線・後続担当はADR-003 D-6で確定済み**。中身の作成が残る。**未完成の間はNFR-019(a)を合格と判定しない**（正解ベクタは完成をもって発効） | 山田さん + Claude（ADR起案） | **本書v2.2の承認後、NFR-018対象のドメイン計算コードを新規に追加または変更する最初の変更の前**（v2.2でADR-003の①②が確定したため、残るのはベクタの中身。既存の対象コード〔`courseInputView.ts` の座標変換〕は逐語移植の受入条件を解く時点で生成物へ移行する〔達成条件(c)の卒業〕） |
