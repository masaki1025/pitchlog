...
- **要件**: テストランナーを標準化する（バックエンド=pytest・フロントエンド=Vitest・E2E=Playwright。v1.8で明確化 — バックエンドのテストは pytest 以外に分散させない）。PRごとにCIで自動実行、全グリーンでないとマージ不可とする。CIには (a) クライアント/サーバーの一致性テスト（**対象は NFR-018 の区分(α)**＝クライアント・サーバーの双方で実行するドメイン計算とし、**列挙は NFR-018 を正とする**〔本項では繰り返さない〕。**NFR-018 の区分(β)〔サーバーでのみ実行するもの〕は本テストの対象としない** — 担保は **NFR-018 の「対象」欄が区分ごとに定める**（①〜⑤=付録A固定ベクタ／⑦=打球方向導出の専用ベクタ／⑥=付録B-5変換ベクタとFR-038移行突合／⑧=FR-038導出規則の単体テストと移行突合。FR-042 補足の別建て方針を維持）。**対象ごとに〈入力・出力・比較単位〉を定義してテストを構成する**。**比較する出力は、当該計算の結果を利用者へ見せる条項が定める全項目**（受入基準に加え、**ユーザーストーリー等で定める表示項目を含む**）とし、**一部だけを比較面に採ることはできない**（項目を除外するには本書の改訂を要する）。現時点の対応:
  ...
  - **断中の成績集計の前処理**＝**プレイ入力列＋語彙スナップショット**（ID→表示名。FR-022の結果表示名を完成した表示値として比較するため — [ADR-003](../adr/ADR-003-domain-calc-method.md) D-1）→**FR-021の当日成績の全指標**・**FR-022の当日打席結果の全列**（**列の定義はFR-022が正** — v2.2で確定済み。打席ごとの7列と、そこから導出する当日の集計）・**FR-023の球種割合と併記する球数**
...
- **測定方法**: 次の**受入プロファイル**を試験対象として事前に一意化し、approvedな `docs/development/onboarding.md` の手順を完走できることを2時点で8章の判定者が確認する
  - **受入プロファイル**: Windows 11 ホスト（**アーキテクチャは x64 / arm64 のいずれも保証対象とする**）＋**新規に作成したWSL2ディストリビューション「Ubuntu 26.04 LTS」の、ホストのアーキテクチャに対応する番号付きWSLイメージ（x64 / arm64）**（既定名 `Ubuntu` のディストリビューション、および他ディストリビューション・他版は保証対象としない）。**1回の受入は実施ホストのアーキテクチャ1つに一意化して行う**（両アーキテクチャでの同時実施は求めない — 測定方法柱書の「事前に一意化」はアーキテクチャを含む）。Windowsホスト側の事前導入は**WSL2の有効化のみ**を前提とし、その他の開発ツールはすべてonboardingの手順内で導入する（Docker Desktop の事前導入は前提としない）
  - **固定版の切替規則**: 新しいUbuntu LTSへの切替は、(1) WSL向けの正式提供開始 (2) `docs/development/onboarding.md` の更新 (3) 新版での受入再実施 の3点を満たした時点で本項の版を改訂して行う（「最新」を都度判定する運用はとらない）。**アーキテクチャの扱いは (1) と (3) で異なる**（v2.3）— **(1) は保証対象の全アーキテクチャ（x64 / arm64）について番号付きイメージが正式提供済みであることを要する**。片方のアーキテクチャ向けイメージしか存在しない時点で共通の固定版を切り替えると、**もう一方の保証対象環境を構築できなくなる**ため。一方 **(3) の受入再実施は、切替を実施するホストのアーキテクチャについて1回で足りる**（保証対象の全アーキテクチャでの再実施は求めない）— 未実施側については、**(1) により当該版の番号付きイメージの提供が確認済みであり、かつ本項の手順は onboarding の逐語実行のみでアーキテクチャ固有の分岐を持たない**ことをもって保証の根拠とする。**アーキテクチャ固有の分岐が手順に生じた場合は、その分岐を含む経路について受入を実施する**。**固定版の標準サポート終了前に必ず切替を完了する**
  ...
  - **証跡**: 日時・commit SHA・Windows版・WSL版・ディストリビューション版・onboarding版・**主要ツールの版（python / uv / node / docker）**・実行コマンドと終了コード・**各合格項目の期待値と実測値**（DB接続の応答・HTTP疎通の応答コード等）・**標準出力またはログ成果物への参照**（**参照先は候補コミットのツリーに収録されていること**。**かつ、実施したホストおよびWSLのアーキテクチャを識別できる出力**として `host_raw` / `host` / `wsl_uname_raw` / `wsl_uname` / `wsl_dpkg_raw` / `wsl_dpkg` の**6ラベルの実値を含むこと** — v2.3。両アーキテクチャを保証対象とするため、どちらで完走したかが証跡から一意に定まる必要がある。採取手順は `docs/development/onboarding.md` が定める）・判定者を記録する
