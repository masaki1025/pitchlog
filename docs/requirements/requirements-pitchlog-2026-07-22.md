...
- **受け入れ基準**:
  ...
  - Given 退避されたイベントの取り込み / When 実行 / Then **現在の正史の末尾への再番号付け（現行世代の連番として採番し直す）と下流再計算を伴う明示的な操作**として行われる（旧世代の連番のまま挿入しない。取り込み結果は差分プレビューで確認できる）
...
#### NFR-019: テスト規約のCI強制
- **カテゴリ**: 保守性 ／ **優先度**: Must
- **要件**: テストはpytestに一本化し、PRごとにCIで自動実行、全グリーンでないとマージ不可とする。CIには (a) クライアント/サーバー状況計算の一致性テスト（同一のプレイ入力列から同一状況を導く。**付録Eマトリクスのゴールデンケースを共通の正解ベクタとする**。断中に実行するドメイン計算＝状況判定・終了判定・**捕球選手推定・座標導出**を対象に含める） (b) 越境アクセステスト (c) 主要分岐のE2Eテスト（タイブレーク・コールド・引き分け・終了後修正→再集計・交代フルセット・通信断→復帰同期・**記録権の通常/緊急引き継ぎと退避経路**） (d) 同期プロトコルの故障系テスト（**墓標/改訂の適用・サーバー適用の原子性（クラッシュ注入）・複数タブの単一書き手競合**）を含む
- **測定方法**: CI構成の確認と実行履歴
