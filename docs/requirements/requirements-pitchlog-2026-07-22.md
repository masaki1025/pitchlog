...
- 0除算となる指標は「−」を表示する（エラー・0.00にしない）。**「−」は MINUS SIGN(U+2212)1 文字**とする(v2.6 仮追補: 文字種の確定)
...
- **表示書式の共通規定**(v2.6 仮追補 — 各指標表の「表示書式」列が対象出力ごとの binding の正): **丸めは全対象で四捨五入**(round half up。別方式を使う場合は当該行に明記する — 現時点で該当なし)/ **数字・小数点・`%`・括弧は半角**、単位・助数詞(「回」「球」等)は日本語 / **負値の符号は半角 `-`・正値に符号を付けない**(現時点で負値を取り得る表示対象は**得失点差と FIP**)/ 0 除算の「−」(全角ダッシュ)は上記のとおり / **本規定と書式列は ADR-003 D-1 の宣言的表示規則(数値表示 primitive)で実装され、比較面には完成表示値が載る**(NFR-019(a)・NFR-018)/ **本付録の表に書式列を持たない利用者可視の数値**(スコアボードの得点・カウント・打席番号・イニング等)**は整数の既定表示(固定小数 scale=0・符号なし)とする** — これも ADR-003 ③の fixed-decimal を通し、言語既定の文字列化は用いない(v2.6 仮追補)
...
| WHIP | (被安打 + 与四球) ÷ 投球回（与死球は含めない） | 小数2桁・四捨五入(例 `1.25`)+ 母数併記 `({投球回})`(投球回は混合分数形式 — 例 `(45回1/3)`) |
| FIP | (13×被本塁打 + 3×(与四球+与死球) − 2×奪三振) ÷ 投球回 + 定数（既定3.10・システム設定値）。**本システム内での相対比較用**であり、プロの防御率水準と直接比較できない旨を画面に注記する（v2.0: FR-041 の共同分析グループでチーム合算FIPを共有するため、比較の射程は自チーム内に限らない。定数が同一で計算式が本付録に固定されている範囲での相対比較という位置づけは変わらない） | 小数2桁・四捨五入(例 `3.10`)・**負値は半角 `-`**(FIP は定義上負値を取り得る)+ 母数併記 `({投球回})`(混合分数形式) |
...
>
> **状況別割合の表示 binding(v2.6 仮追補 — PO 裁定 2026-09-03 で射程に追加)**: FR-027 の傾向集計(回数・状況別割合)の割合は**百分率・整数・四捨五入 + 回数併記 `({回数}回)`**(A-2b 球種割合と同型の template 合成)とする。
