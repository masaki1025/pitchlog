...
6. **負例 14 件を固定**(N1〜N14 — design.md 6 節。**`N9` 初期値すり替え / `N10` `F-7` の比較元 /
   `N11` allow-list の識別単位 / `N12` 古くなった登録 / `N13` 例外枠の期限 / `N14` `fetch-depth: 0` の欠落**)
...
| [ハーネス運用評価台帳](../../development/harness-evaluation.md) | **`H-85` へ対応の追記** + **候補「2 つの機械検査が正面から矛盾し…」へ解消の追記** + **候補 6 件を新規**(**完了コミットを持たないステップで現在地導出が壊れる** / `function-bodies` の `source_commit` / **一度きりの移行検査(`F-7`)がマージ後に到達不能な分岐として残る問題** / **レビュー指摘の訂正を 1 箇所だけ直して「反映した」と数える型**〔3・4 周目の非起因がこれ〕/ **新設した要素を既存の要件文・述語表・ステップ順序・負例の向きと突き合わせない型**〔4 周目 `P1` 6 件中 3 件〕/ **述語を足したとき「破る例」の負例だけ落とす型**〔4・6 周目 — design.md 8 節の対策表〕)+ 変更履歴 1 行。**`H-*` の新規採番はしない・版は上げない** | PR レビュー(7.6-3 前段) |
...
**`N13` の割り当て漏れ**(**実装時に判明した計画の穴**): **`N13`(base に台帳がある状態で
`pending_removal: true` を置くと red)は確定ゲート 6 周目 `P2` で design.md 6 節と DoD へ足したが、
どのステップで作るかを割り当てていなかった。** **述語の実装はステップ 5 で入っている**が
**負例テストが無い**。**ステップ 11 で補う**(射程の拡大ではなく、DoD が既に要求している項目の
実装漏れの補修)。**ステップ 11 の実装前確認で Codex が検出して停止した。**

**ステップ 8〜10 は二段コミットの区間である**(**実装時の明確化** — 承認済み射程の変更ではない)。
**`corpus_version` を母集合へ足した時点で入力資産が変わる**ので、
**`check_authz_catalog.py` はステップ 8 から red になり、ステップ 10 の再封印で緑に戻る。**
**したがってステップ 8・9 の合格条件に「`check_authz_catalog.py` が ok」を含めない** —
代わりに **「red の理由が入力 blob の不一致だけであること」**を確認する。
**seal が指すべきコミットはステップ 9 の完了コミット**である(新 blob を含む最後のコミット)。

**ステップ 3 が最も重い。** ハーネス設計書は approved 正本で、**確定ゲートが要る**。
**ステップ 1 を最初に置くのが要**である — **朝にこの領域で防御を落としている**ので、
**変更前の物差しを先に作る**。

### 差し戻し修正(**ステップ表の外** — PR #63 の敵対レビュー `否決 P0 3 / P1 1`)

**ステップ表へ行を足さない。** **ステップ 12 まで完了済みで `(ステップ 12/12)` のコミットが
既に存在する**ため、総数を動かすと現在地導出と食い違う
(**太字の行番号は機械がステップ行として認識しない** — 実測で確認した)。
**修正コミットはステップ記法を付けない**(`/pr` の差し戻し手順)。

| # | 指摘 | 直すこと | 合格条件 |
| --- | --- | --- | --- |
| ① | `7.7-6` の 3 指定が宣言に無い(`P0`) | **台帳へ `declarations` を新設し、実装は導出する**(design.md 3-4-2)| **実装側に 3 指定の固定値が 0 件**(機械走査)/ **検査が「現に用いた 3 指定」を出力し宣言と完全一致** |
| ② | `corpus_versions` に `7.7-4` の必須項目が無い(`P0`) | **`supersedes`(直前の `version`)を足す**(PO 判断 2026-09-16)| **`F-2` が version 型にも効く**(不連鎖で red)|
| ③ | `F-5` が到達可能性を見ていない(`P0`) | **`merge-base --is-ancestor` を足す** | **dangling commit を基準にすると red**(`72802003…` で実証)|
| ④ | `.git` 無しで履歴照合を丸ごと省略(`P0`) | **省略を出力で明示し、保証の対象外と宣言する** | **`.git` 無しの実行が「履歴照合を省略した」と出力する** |
| ⑤ | `oracle_meaning` の更新経路が閉塞(`P1`) | **確定集合を基準から導出する**(design.md 3-4-3)| **台帳の `oracle_meaning` を現在へ進めると green**(7.7-2 の実証)|

**共通条件**: **既存の負例 14 件が引き続き red** / **両検査器が green** / **/check 全層 green**。
...
- [ ] **fail-closed 化が作る `fetch-depth: 0` 依存を `F-8` が固定している**。
      **`fetch-depth: 0` を外すと red**(N14 — 台帳 `:2469` の PR #52 の再発を塞ぐ)
- [ ] **既存の性質を落としていない** — N1・N2 が**変更前と同じく red**
- [ ] **負例 14 件が exact-set で数えられている** — **`backend/tests/` と `tests/` の両 root を横断**
      (ステップ 1 の marker + exact-set は前者のみ。**片方だけだと、もう片方で負例を消しても通る**)
...
- [ ] **台帳へ `H-85` の対応・候補の解消・新規候補 6 件を追記した**(過去記録は書き換えない)
...
| **負例(新設)** | **N3** 承認欄が空 / **N4** 到達不能な commit / **N5 既存記録の書き換え(最重要)** / **N6** `supersedes` の不連鎖 / **N7** 版の上げ忘れ / **N8** 派生の追随漏れ(`G-5`)/ **N9** 移行 PR の初期値すり替え(`F-7`)/ **N10** `F-7` の比較元が base 側ソースであること / **N11** allow-list の識別単位が `(パス, 値)` であること / **N12** 走査から消えた登録が残っていること(**`N11` と逆向き** — 4 周目 `P1-4`)/ **N13** base に台帳がある状態の `pending_removal: true`(**例外枠の禁止側** — 6 周目 `P2`)/ **N14** `fetch-depth: 0` を外した複製(**fail-closed が届く経路の検査** — 自己点検) |
...
| 負例が守りたい性質に対応していること | **N1〜N14** のそれぞれが、**どの性質が壊れたときに鳴るか**を対応づける |
