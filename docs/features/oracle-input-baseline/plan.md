...
| 11 | **負例 14 件を通しで確認し、効果を実測する** | **N1〜N14 がすべて red** / **digest 辺を機械が変更前後で出力し `21 → 16` を記録** / **手で計算する digest が 6 → 1**(台帳の `canonical_sha256` が残る)/ **要件書を 1 バイト変えて追随し、検査器のソースを 1 行も編集せずに済むことを実測** |
