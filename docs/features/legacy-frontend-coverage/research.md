...
**この調査は受入判定ではない**。「打席入力画面の逐語移植の受入判定」は別タスク(**TSK-307 として 2026-09-02 に起票済み** — 申し送り節)であり、TSK-226 とは明文で切り分けられている(`docs/features/course-coordinate-contract/plan.md:110`)。
...
5. **要件書側にも受入判定できない穴が 27 件**ある(10 章既載 2・未収載 25 — §1-3)。特に **H/E/K/B の定義が要件書のどこにも無い**のに NFR-019 がそれを一致検証の比較面に指定しており、比較面が確定しない。
...
**この差分の追跡は本タスクの射程に入れない。** 既存の記録が理由を述べている —「**『旧に追いつく』を要件のゴールに据えると終端がない。v2.2 のカットオーバー日の確定がこの追いかけを終わらせる唯一の手段**」(`docs/worklog/2026-08-13-req-v2-legacy-parity.md:70-74`)。カットオーバー日・移行対象の線引きは**移行仕様を策定する別タスクで決定する**(`REQ:988` 末尾。起点の worklog では「v2.2 送り」と記録されていたが、現行条文はタスク送りの形)。
...
**27 件**を検出(S1 由来の G-1〜G-20 = 20 件 + S2 由来の 7 件〔FR-025 表現形式の二択・FR-032 ページ構成・FR-033 レート制限・FR-035 通知手段・FR-038 人手介入 UI・FR-041 共有画面レイアウト・`プロフィール` 共有粒度〕)。10 章(未決事項)と突合した結果、**10 章に載っているのは 2 件だけ**(FR-033・プロフィール粒度)で、**残り 25 件は未収載の穴**。重いもの:
...
| 残り 21 件 | 【G 系 16】構えの入力 UI(G-1)/ 入力補助の対象キー・略号(G-2)/ 走者手動変更の操作形態(G-3)/ 終了宣言の促し方(G-5)/ タイブレークの起動導線(G-8)/ 9 ポジション警告の形態(G-9)/ 未送信件数の表示位置(G-10)/ 「編集して再送」の編集 UI(G-11)/ 通常引き継ぎの導線(G-12)/ 規則設定画面の所在と担い手(G-13)/ 「明示操作」の形態(G-14)/ 断中注記の基準時刻(G-16)/ 次打者の範囲(G-17)/ カルテ一式の範囲(G-18)/ 類似名の判定基準(G-19)/ 補正入力の形態(G-20)【S2 系 5】コース分布の表現形式が二択(FR-025)/ 分析 PDF のページ構成(FR-032)/ グループ終了通知の手段(FR-035)/ FR-038 の人手介入 UI / 共有画面のレイアウト(FR-041) | 各 FR 受入基準 | 無 |
...
- **出力は充実**: 投手分析 PDF・PPTX / 打者分析 PDF(打者は PDF 固定 — `LEGACY:api/routers/stats.py:408-420`)、チーム攻撃分析 PDF、試合用カルテ PDF/HTML(**headless Chrome の `--print-to-pdf`**)、一括レポート(PDF 20 名 / PPTX 8 名上限、同時実行は 429)、スコア表 PDF、88 列 CSV(1 試合 / 最大 100 試合 ZIP)。分析系の出力エンドポイントは**すべて管理者限定**。**数式インジェクション対策があるのはフロント生成の分析 CSV のみ**(先頭 `= + - @` をクォート・BOM + CRLF — `LEGACY:frontend/src/lib/analysisCsv.ts:103-115`)で、**サーバー生成の 88 列 CSV には無い**(§3 FR-031)。
...
> 集計は §3-0 の基準による再判定後の値。**要件書側の受入条件の穴(§1-3 の 27 件)は判定を甘くしない**: 実装の不在・逆実装が確定している FR は穴があっても ×(FR-012・013・014・024・035 等)。穴のせいで条項の合否を確定できない場合に限り △ に寄せる。
...
検査: × 8 件(FR-012・013・014・018・019・024・035・037)は ①〜⑦ に全件現れる。本表に現れる FR は × 8 種 + △ 8 種(FR-007・021・022・026・028・040・041・042)でいずれも突合規則 (b) を満たす。
...
| 作戦語彙のハードコード(FR-027) | SPA 接続 API の集計カテゴリが frozenset 固定(`LEGACY:api/strategy_service.py:24-27`) | 作戦は**チーム拡張可**のラベル語彙(`REQ:164,167`)で、追加作戦も傾向集計の対象(`REQ:481`)。集計コードへの焼き込みは `REQ:168` と同型の禁止対象 |
...
| 既決 | 内容 | 本調査との関係 |
| --- | --- | --- |
| **PO 裁定「完全に同じもの」** | 「似せる」のではなく機械的な逐語移植 | **§4-1 のうち SPA 移植に直撃する 6 件で成立しない**(旧を写すと要件違反になる。残る 1 件〔球種系統・ツーシーム〕は Streamlit 専用層のみで移植では踏まない — §4-1 の限定) |
...
| **D-9** | 新機能追加は初回リリース後に回す(スコープ肥大防止) | §4 の 15 項目が「新機能」に当たるかは PO 判断 |
...
1. **「打席入力画面の逐語移植の受入判定」タスク** — 要件書 NFR-018 (c) 例外表の「終了証跡」欄が要求(`REQ:906`)し、TSK-275(卒業タスク)の着手前提だった。**本タスクのステップ 6 で TSK-307 として起票済み(2026-09-02)— 鎖の先頭は解けた**。
...
| **TSK-306** 要件書 10 章への穴の反映 | 画面の受入判定ができない**未収載 25 件**(G-15 の H/E/K/B 定義不在を含む — 起票時は「18 件」としたが成果物レビュー 2 周目の再集計で 25 件に確定。Notion 側の DoD も更新済み)。確定ゲート案件 | §1-3 |
...
§1-3 の 27 件のうち未収載の 25 件(→ TSK-306)。特に **G-15(H/E/K/B の定義不在)は NFR-019 の比較面を確定不能にしており、要件書の改訂を要する可能性がある**。本調査は事実の指摘に留め、要件違反に当たるかの判定は行わない(なお旧実装に事実上の定義がある — §3 FR-020 行)。
