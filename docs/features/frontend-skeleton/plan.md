...
確定ゲート周回: 18         # 指摘反映を伴う敵対レビュー 1 周ごとに +1(同前。/finalize-doc が更新)
...
**A. 設計書の改訂(確定ゲート対象)** — **13 章 + 10.1 + 8.4**(確定ゲート 5・6 周目で 10.1・8.4 へ範囲拡張)+ **`.claude/skills/release/SKILL.md`・`.claude/skills/task-done/SKILL.md`・`.claude/core-areas.json` の同期**(同 6・7・8 周目)。**確定ゲート 9〜15 周目で起案した運用機構は PO 裁定 2026-08-16 で Phase 4-4 / 4-5 へ送った**(設計書 13 章「本改訂で意図的に定めなかったもの」)
...
| [ハーネス運用評価台帳](../../development/harness-evaluation.md) | **H-59・H-60 の新設**(① 逐語移植の検証で照合項目を列挙しないと未照合項目が通る ② ステップ完了後に見つかった欠陥のコミット記法が規約に無い)+ **H-51 の再発を 4 → 5 件へ** + **H-58 の再発を 1 → 2 件へ**。**版は上げない**(7.6-3 前段 — `H-*` の追記)。**番号は develop 取り込み時に H-54・H-55 から改番**(develop 側の PR #14 が先に使用) | PR レビュー |
...
| `.claude/core-areas.json` | **`guard_paths` へ `release/SKILL.md`(確定ゲート 6 周目 P1)と `task-done/SKILL.md`(同 8 周目 P1)を追加**(計 7 件)。**同ファイル自身も `guard_paths`** |
