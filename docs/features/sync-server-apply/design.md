...
**計画レビュー(`sol xhigh`)7 周分の指摘を反映済み** — 1〜5 周目は詳細設計そのものへの指摘
(1: P0 5・P1 5 / 2: P0 5・P1 2・P2 1 / 3: P0 4・P1 4 / 4: P0 0・P1 3・P2 1 / 5: P0 1・P1 3・P2 1)、
**6〜7 周目は射程分割の波及是正**(6: P0 0・P1 2・P2 2 / 7: P0 0・P1 2・P2 2)。

> ## ⚠️ 章ごとの射程
>
> **5 周目の人間の裁定 10 で射程を分割した。**
>
> | 章 | 射程 |
> | --- | --- |
> | **1・2・4・5・7 章** | **本タスク(TSK-321)** |
> | **3 章(故障系契約)・6 章(母集合と被覆)** | **射程外 — [TSK-332](https://app.notion.com/p/3d493b75e687816eb9fbd19dda889d5e) への申し送り**。5 周分のレビュー知見を保存してあるので、**TSK-332 の /investigate はここから始める**。ただし **5 周目の未反映指摘 4 件**(下記)が残っている |
>
> **TSK-332 が最初に処置すべき 5 周目の指摘**(本タスクでは反映していない):
>
> - **[P0]** `_v2` 移行後に同じ版の `faultInjection` を破壊的変更する手順は版付け規則(10-3 `:1689`)に反する → **case 配列・P5 union を含む最終構造へ一度だけ移行する**
> - **[P1]** `p5-b3b-unreached-t9` が「`T9` 未到達」を契約上表現できない → **`expectedReachability: "unreached"` 等の判別子と hook 呼出 0 回の期待値**を足し、正の 18 組被覆からは除外する
> - **[P1]** 複数 `case` と期待観測点の対応がなく、宣言だけで境界被覆を満たせる → **各 case に stable `caseId`** を持たせ観測点を case に結び付け、**孤立を許さない exact-set**にする
> - **[P1]** 2 つの注入被覆が和集合のままで重複と誤写像を検出できない → **`(scenarioId, caseId, 注入対象)` のペア集合**で照合し、**重複を明示的に拒否**する。トランザクション内 case は資産の `applicationPath` とも一致させる
