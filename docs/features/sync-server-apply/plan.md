...
7. **正本必須資産 ID の母集合(観点被覆つき)と 2 つの繰り延べ表** + **シナリオ資産 10 件**
...
**台帳と索引の更新は `/pr` のクローズ処理で行う**(pr スキル手順 1-3 — ①計画書 3 節の宣言 → ②台帳 → ③変更履歴 → ④索引を**同一コミット**)。
**実装ステップ表には置かない**(クローズ処理はステップ表の外側にある — 設計書 6.1)。
...
| 5 | **故障系契約の再設計**。期待フィールドを**共通 10 件(無条件)** — 既存 5 + `prefix`・`idempotentResult`・`temporaryIdMapping`・`recordingRight`・**`tElementCommitment`** — にし**省略可否だけを `applicationPath` で条件付き**に。**`schemaVersion: 2`** へ繰り上げ**構造との組を照合**。**契約検査と結果検査を分離**。**`tElementCommitment` は経路 reader から導出した T 集合について「全要素 `committed` または全要素 `notCommitted`」を要求**する。既存 4 資産を **`_v2`** へ移行 | `[機械]` 全 green / **部分確定(T 集合の一部だけ `committed`)が red** / 旧 `schemaVersion` + 新構造 / 新 `schemaVersion` + 旧構造がいずれも red / **`applicationPath` が `P1`〜`P5` の資産で後半 5 件を省略すると red** / **既存 runner が green のまま**(結果検査の分離)/ **サーバー経路で実値が欠落すると red** / 旧 `_v1` への参照が残っていない / `[手動]` `_v2` 移行が 10-3 `:1689` を満たす(不変条件は「**旧版参照が存在する間に旧資産を削除しない**」) |
| 6 | **P5 の値関係検査と注入点の 18 組**。`P5分岐` を discriminated union にし、**`B3b` は先着原本が初期永続状態と一致・先着と後着の D5 が同一・内容が異なる**、**`B3a` は対象 D5 が初期べき等集合に存在しない**ことを実行時検査。注入点は経路型・`T1`〜`T9`・**有効な 18 組**・P3 のトランザクション外 3 注入点を閉じる(**18 組と T 集合はステップ 1 の reader から導出し二重定義しない**) | `[機械]` 全 green / **異なる D5・同一内容・既存 D5 の各変異が red** / **無効な T ID・`P1` に `T9`・誤字のある P3 注入点が red** / `[手動]` 18 組が正本 8-1 と一致 |
| 7 | **母集合・観点被覆・2 つの繰り延べ表 + 資産 2 件**。① **正本必須資産 ID の母集合**を stable な小文字 scenarioId で全件定義 ② **10-2 の観点にも stable key を与え、`観点集合 = 各 scenarioId の covers の和集合`** を exact-set 化 ③ `作成済み ∪ 未作成繰り延べ = 母集合` ④ `SCENARIO_RUNNERS ∪ runner 繰り延べ = FAILURE_SCENARIO_IDS` ⑤ 資産 `tombstone-application`・`revision-application` を作成 | `[機械]` **4 つの exact-set のいずれからも 1 件消すと red** / **観点を 1 つ covers から外すと red** / 契約検査 green かつ実行ケースが増えない / `FIXTURES` と件数 assert を更新 / `[手動]` 母集合が 10-2 の全行を覆い、繰り延べ行に**必要な契約拡張と TSK-330** が書かれている |
| 8 | **資産 8 件**を追加し繰り延べ表から外す — `p1-crash-boundaries`・`p2-crash-boundaries`・`p3-crash-boundaries`・`p4-crash-boundaries`・**`p5-b3a-crash-boundaries`**・**`p5-b3b-unreached-t9`**・`d1-mixed-batch`・`b3b-after-gap`。**`FAILURE_SCENARIO_IDS` の末尾に追加** | `[機械]` 4 つの exact-set が保たれる / 各資産が経路別の必須項目と `tElementCommitment` を満たす / **`p5-b3a-…` と `p5-b3b-…` が P5 union の両分岐をそれぞれ固定** / `[手動]` 注入点が**ステップ 6 で閉じた 18 組**から選ばれ、正本 10-2 の各行の期待する不変条件と対応している |
...
| R10 | **`tElementCommitment` を足しても部分確定を捕まえられない** | 「全要素 `committed` または全要素 `notCommitted`」を契約検査で要求し、**部分確定の変異が red** になることを合格条件に |
...
- [ ] **`tElementCommitment` が経路の T 集合について全確定/全非確定を要求**し、**部分確定が red** になる
...
- [ ] **4 つの exact-set**(母集合 / 観点被覆 / 未作成繰り延べ / runner 繰り延べ)が宣言され、**どれも 1 件消すと red**
...
| **(c) E2E** | **足さない** | 列挙が閉じている |
| **(d) 故障系** | **契約・注入点・資産 10 件を足す。runner は TSK-330 へ送る** | 共通 10 件 / `tElementCommitment` / `schemaVersion: 2` / 契約検査と結果検査の分離 / P5 の値関係 / 18 組 / 4 つの exact-set |
