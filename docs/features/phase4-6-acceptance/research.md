...
> - **結論(要約)の 3**(「win-setup 選定は部分的に決着済み・未決はどの案を採るかのみ」)— **選定は (d) で確定済み**(GitHub-hosted 不採用 / 継続検証は要件書 NFR-021 の 2 時点の手動再現へ一本化)。**現行は設計書 10.1「NFR-021 の継続検証基盤の選定結果」が正**
> - **C 節の「未決」表現**(同上)— G-1 に決定を追記済み
