...
**差分方式が相殺しているので見えていない**が、**「ファイルを触ると全部出る」わけではない**
(`scan_source_change` `:3452-3485` を読んで確認):
...
**「`authz` を触ると 172 件が一度に出る」は誤り。「その行を動かした分だけ出る」が正しい。**
なお **363 件という数は `--base-ref` を root commit にして基準版を空にした結果**であり、
**「現在のツリーに存在する違反の総数」であって「触ると出る地雷の数」ではない。**
→ **新設パッケージだけの問題ではない。**
