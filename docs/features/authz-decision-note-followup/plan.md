...
| **`8a72a93`** | **`source_commit` を `14973f6` へ再導出**(manifest 4 行) | **② の起因ではない**(敵対レビュー 2 周目 `P2-2` の是正 — **当初「解決対象の SHA が tip 以外になった」と書いたが誤り**)。**`5f668bf` の manifest は既に `635a595`(自コミットではない別の SHA)を指しており、depth 1 で解決できない条件は `5f668bf` の時点で成立していた**。本コミットは**参照先を `14973f6` へ差し替えただけ**である |
