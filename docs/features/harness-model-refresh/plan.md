...
3. **approved 化後に機構を同期**(文書・設定は Claude、コードとテストは Codex): `.claude/agents/*.md` / `CLAUDE.md`(`:30` の「実装は sol xhigh」・`:35` の「Opus 5・effort high 固定」)/ スキル本文の旧モデル名(`implement:50`・`research:8`)を「ラッパーが ADR-001 どおりに固定」へ一般化 / **`.claude/skills/finalize-doc/SKILL.md:15` の括弧書き「sol xhigh」を「ラッパーが ADR-001『敵対レビュー・コア領域 PR・正本確定ゲート』行どおりに固定」へ一般化**(guard_paths — 人間の逐行確認・PR の実施記録行・SHA 拘束マージ〔github-setup 2 章〕を適用)/ プロジェクト `.claude/settings.json` に主セッションの `model`・`modelSettings`(PO 判断③が可の場合)→ その後 `codex_run.py` の定数(`MODEL_MAP` / `RESEARCH` / `RESEARCH_DEEP` / `REVIEW_NORMAL` / `REVIEW_ADVERSARIAL`)を新表へ / **ADR-001 の決定表 ↔ 定数の構造的同期テスト**(固定構文 — 4 節)・**ラッパー各経路の引数テスト**・**agents frontmatter の固定テスト**を新設
...
- **変更しない guard_paths**: `.claude/skills/pr/SKILL.md`(モデル名を含まないため変更不要)。`plan/SKILL.md` は guard_paths ではなく、「`重さ分類` を 4 値のいずれかへ必ず置換する」指示を追加する(ステップ 2 — 変更対象)。`finalize-doc/SKILL.md:15` は**変更対象**(2 節「やること」3 — guard_paths の手続を適用)
...
**正本体系外だが同一 PR で更新するもの**: `.claude/scripts/codex_run.py`・`tests/test_codex_run.py`(同期テスト・経路テストの追加)・`tests/test_agents_frontmatter.py`(新規)・`.claude/agents/{spec-checker,legacy-analyst,decision-tracer}.md`・`CLAUDE.md`(`:30`・`:35`)・`.claude/skills/{implement,research,setup-dev,plan}/SKILL.md`(setup-dev は版下限の確認手順・plan は `重さ分類` の必須置換・implement は fast 適用時の 2 キー一括更新)・`docs/development/templates/plan-template.md`(`重さ分類` を空値へ)・`.claude/settings.json`(PO 判断③が可の場合のみ — `model`・`modelSettings`・`env.CLAUDE_CODE_DISABLE_FAST_MODE` の 3 キー)・`docs/features/harness-model-refresh/{plan,research}.md`・worklog

## 4. 実装方針

- **重さ分類 = 通常** の根拠: 変更対象はハーネスのラッパー定数・設定・文書であり、コア領域 5 領域(設計書 6.3)の `paths` には該当しない(research.md A-4・C-7 #8)。**`.claude/skills/finalize-doc/SKILL.md` は `guard_paths` に該当する**(`.claude/core-areas.json`)が ADR-001 の重さ分類上のコア領域ではないため `重さ分類 = 通常` とし、guard_paths の手続(人間の逐行確認・PR の実施記録行・SHA 拘束マージ — `.claude/skills/pr/SKILL.md`・github-setup 2 章)を PR で適用する。`pr/SKILL.md`(guard_paths)は触らない。typo 級の軽微でもない(ラッパーの機械適用値と正本 2 本の版繰り上げを伴う)

### 阻止条件 0(コミットなし — ステップ 1 の前に人間 + Claude で確認)

1. 人間が Codex CLI を **0.157.x 以上**へ更新する(GPT-6 Sol/Luna の `minimal_client_version` = 0.155.0。カタログ追加は公式変更履歴で 0.156.1 hotfix・GitHub リリースノートで 0.157.0 — research.md B-2「Codex CLI の要更新」。モデル定義の最低版は B-3「CLI 対応」を参照。運用下限は本機確認済みの 0.157.0)
...
| --- | --- | --- | --- | --- |
| 案 A | `gpt-6-sol` × {medium, high, xhigh, max}・`gpt-6-luna` × {xhigh} | `probe` で上記 5 組すべて + 本番様式で `REVIEW_NORMAL` = (`gpt-6-sol`, max)/ `REVIEW_ADVERSARIAL` = (`gpt-6-sol`, xhigh) | 対応表を変えるなら **ADR-001 v1.2**(astra 昇格を含む)を起案し新規の確定ゲート | `.claude/skills/finalize-doc/SKILL.md:15`(guard_paths — 逐行確認・実施記録行) |
...
| Web 調査(深い技術検証 `--deep`) | `gpt-5.6-sol` xhigh | `gpt-6-sol` xhigh | `gpt-6-astra` medium |
  ...
  - **根拠(案 A を推奨する理由)**: gpt-6-sol は 5.6-sol の後継 tier で API 単価が半額(✔ 照合済み — research.md B-2)。公式発表は "improves substantially over GPT-5.6 Sol"(FrontierCode)・"higher usage limits and lower cost" と述べる(OpenAI 発表ページを Codex が 2026-09-27 に再照合済み — ADR-001 v1.1「文脈」・research.md B-3。**Astra 対 5.6-sol の数値のみ未照合**)。利用枠の試算(現行比 **−35%** / 案 B **+97%**)は **Business/Enterprise の credit rate card を代理指標にしたもの**で、Plus/Pro の係数は非公開(B-3)。したがって推奨の根拠は「単価半額(照合済み)+ 旧フロンティア以上との公式声明(再照合済み)」であり、**品質の同等性は移行後の実測で確認する**(下記の品質下限・実測期間)
  ...
  - **品質下限・実測期間・巻き戻し条件**(PO 判断①の一部として確定): 開始・終了・8 週間時の扱い・記録者・集計者・判定者・記録項目および巻き戻し条件 (a)〜(c) の閾値の**唯一の正は ADR-001 v1.1「帰結」**とし、本計画では再掲しない(確定ゲート 2 周目 P1 — 二重定義の解消)。品質下限の判定は周回数でなく検出実例の有無で行う(H-30)
...
**③ 主セッションの機構化**(推奨 = 機構化): プロジェクト `.claude/settings.json` に `"model": "claude-opus-5-5"` と `"modelSettings": {"claude-opus-5-5": {"effortLevel": "high"}}` を追加する
  ...
  - **副作用の統制**: `permissions`・`hooks` ブロックは**差分不変**(ステップ 2 の合格条件で `git diff` が `model`・`modelSettings`・`env` の 3 キー追加のみであることを確認)。`tests/test_hooks.py` のフック実在検査が引き続き緑。**確定ゲート 1 周目で追加**: Fast mode は `/fast` の ON が次セッションへ持続するため、共有設定の `env` に `CLAUDE_CODE_DISABLE_FAST_MODE=1` を置いて機構的に無効化する(公式 fast-mode)。優先順位(公式): 現セッション中は `/model`、新規セッションは起動時の `--model` → `ANTHROPIC_MODEL` → settings の `model`(settings 間は managed → local → 共有 → 利用者。managed の `availableModels` は全経路を制約)。前提 = Claude Code 2.1.280 以上(Opus 5.5 の最低版 — `/setup-dev` で確認)
...
- 設計書 9.4 は値・行キーを再掲せず ADR-001 の「決定」節を参照する(同期テストも ADR-001 のみを読む — 確定ゲート 1・2 周目)
...
| `.claude/skills/finalize-doc/SKILL.md` | `sol xhigh`・`gpt-5\.6` | 「ADR-001『敵対レビュー・コア領域 PR・正本確定ゲート』行どおり」参照 |
...
| 2 | **Claude 側設定・文書の同期**(Claude — コードには触れない): `.claude/agents/*.md` を `model: claude-opus-5-5`・`effort: high`(`effortLevel` 行削除)/ `CLAUDE.md:30` の「実装は sol xhigh」を「ADR-001 の『コア領域の実装』行のモデル・effort」参照へ・`:35` を「Opus 5.5・effort high 固定」へ・**主セッションの共有既定(Opus 5.5・high・Fable への一時昇格条件・Fast 禁止)を追記**(設計書 8.1 の CLAUDE.md 項目)/ `.claude/skills/{implement,research}/SKILL.md` の旧モデル名(`:50`・`:8`)を「ラッパーが ADR-001 どおりに固定」へ一般化/ `finalize-doc/SKILL.md:15` の括弧書きを「ラッパーが ADR-001『敵対レビュー・コア領域 PR・正本確定ゲート』行どおりに固定」へ(guard_paths — 逐行確認・実施記録行)/ `.claude/skills/setup-dev/SKILL.md` に版の下限確認(Claude Code 2.1.280・Codex CLI 0.157.0)の手順を追加 / `docs/development/templates/plan-template.md` の `重さ分類` を空値へ・`.claude/skills/plan/SKILL.md` に「4 値のいずれかへ必ず置換」・`implement/SKILL.md` の fast path を「① 非コア領域 ② 小差分(目安 50 行以下)かつ正本影響なし ③ typo・コメント・小さなテスト修正・設定微調整の類 — **全 3 条件**を満たす場合に限定し、人間の事前 OK 後に `重さ分類: 軽微` と `実行方式: fast` を一括更新してから実行する」手順へ置換 / `.claude/settings.json` に `model`・`modelSettings`・`env.CLAUDE_CODE_DISABLE_FAST_MODE`(PO 判断③が機構化の場合) | 「旧記述の残存検査」表の agents・CLAUDE.md・skills(finalize-doc を含む)・settings.json の行がすべて満たされる / `uv run pytest -c pyproject.toml tests/` green(既存テストのみ — `test_hooks.py` のフック実在検査を含む)/ 人間が新規セッションで、managed 設定・起動時指定(`--model` / `ANTHROPIC_MODEL`)・`.claude/settings.local.json` の有無と実効値を worklog に記録し、**高優先層が無い環境では** `/model` が `claude-opus-5-5`・effort high、`/fast` が無効(disabled)であることを確認する。高優先層がある場合は実効値の差を不合格にせず、その由来と共有設定 3 キーの存在を確認する / **逐行確認 3 点**: `docs/development/templates/plan-template.md` の `重さ分類` が空値・`plan/SKILL.md` が通常経路で 4 値のいずれかへの置換を必須化・`implement/SKILL.md` が fast path の全 3 条件と 2 キー一括更新を明記 |
| 3 | **ラッパーの対応表更新 + 版検査 + `probe` + テスト**(Codex): `codex_run.py` の 5 定数を approved の ADR-001 v1.1 へ更新 / **起動前の Codex 版検査**(`codex --version` を解析し 0.157.0 未満・解析不能なら fail-closed で停止し onboarding 1-6 の更新手順を示す — 全モードに適用)/ **`重さ分類` の必須化**(欠落・空値・不正値は「通常」へ既定せずモデル実行前に停止 — ADR-001 決定節・設計書 6.3 の fail-closed)/ **`fast` 経路の計画書 3 値検査**(登録済み worktree の現在 branch `feature/<slug>` / `fix/<slug>` から**正規位置 `docs/features/<slug>/plan.md` を一意に導出し、その計画書だけを受理**して `status: active`・`重さ分類: 軽微`・`実行方式: fast` の 3 値を検査する〔`status: in-review` のままの fast 実行を排除 — 差し戻しの往復規約〕 — 設計書 6.1 `:302`・`feature_status.py` の正規配置判定と同じ規則。計画書なし・読取不能・frontmatter 不正(非閉止・8 KiB 超過・UTF-8 不正・厳密 status 不適合 — `feature_status.py` と同じ判定)・機構読取キーの重複(全機構読取キーをパラメータ化)・`branch` 不一致・同一 branch を持つ別位置の計画書・`status: active`・`重さ分類: 軽微`・`実行方式: fast` の 3 値の欠落/空値/不一致(`status: in-review` を含む)はモデル実行前に停止)/ **read-only の `probe` モード**(ADR-001 の決定表に現れる一意な `(model, effort)` 組を列挙し、各組でラッパー内蔵の固定短文(stdin を読まない)を `exec` 経路に流して受理・応答を表示。sandbox は `read-only`・web_search は `cached`)/ `tests/test_codex_run.py` に ① 固定構文で ADR-001 の決定表を読み(`## 決定` 直下の最初の表・ヘッダ固定・8 行キー・code span 1 個・effort トークン 1 個)、行キー → 定数の対応表で `(model, effort)` を照合する同期テスト ② 負例 6 種(モデル不一致・effort 不一致・行キー欠落・構文違反〔code span 2 個 / 未知の effort トークン〕・行順逆転・行キー重複を一時的な表文字列で検出)③ 経路テスト(`run_codex` を差し替え、`implement`〔重さ 4 分類〕・`fast`・`research`・`research --deep`・`review normal`・`review adversarial`・`probe` の各経路が期待する `-m <model>` と `-c model_reasoning_effort=<effort>` を組み立てることを既存の fixture 方式で検証)④ 版検査テスト(`codex --version` の出力を差し替え、0.157.1 は通過・0.153.4 は停止・解析不能は停止の 3 例)⑤ `重さ分類` の fail-closed テスト(4 つの正常値は通過・欠落・空値・不正値は停止)+ `fast` の計画書検査テスト(正規位置で `status: active` と 2 キーが一致 = 通過 / 計画書なし・読取不能・frontmatter 不正(非閉止・8 KiB 超過・UTF-8 不正・厳密 status 不適合 — `feature_status.py` と同じ判定)・機構読取キーの重複(全機構読取キーをパラメータ化)・`branch` 不一致・同一 branch を持つ別位置の計画書・`status: active`・`重さ分類: 軽微`・`実行方式: fast` の 3 値の欠落/空値/不一致(`status: in-review` を含む) = 停止 — 負例は停止対象の各項目に 1 例以上)⑥ `probe` の経路テスト: 全 5 呼出しが**同一のラッパー内蔵固定短文**を `run_codex` に渡すこと・**stdin 読取りを失敗させるスタブに置換しても完走する**こと(stdin 未読)・各呼出しが `-s read-only` と `-c web_search="cached"` を含み `workspace-write`・`web_search="live"` を含まないことを検査/ `tests/test_agents_frontmatter.py` を新設(3 ファイルの `model` = `claude-opus-5-5`・`effort` = `high`・`effortLevel` 不在・`tools` = Read, Grep, Glob — ステップ 2 の状態を固定) | `uv run pytest -c pyproject.toml tests/` green・`uv run ruff check .`・`uv run ty check` green / 負例 6 種が実際に落ちることをテスト内で確認 / 定数に旧 ID `gpt-5.6-*` が残らない(`grep -P 'gpt-5\.6' .claude/scripts/codex_run.py` = 0) |
...
- [ ] `codex_run.py` の対応表・`.claude/agents/*.md`・`CLAUDE.md`・`implement`/`research`/`setup-dev`/`plan`/`finalize-doc` スキル・計画書テンプレート・(機構化の場合)`.claude/settings.json` が approved の ADR-001/8.5 と同期し、固定構文の同期テスト・負例・経路テスト・frontmatter テストを含むハーネスのテスト・ruff・ty が緑
