...
| **24**(**分割しない**) | staging の木の本番移設 + `profiles/registry.json` の `data-model` entry + `tests/test_ci_wiring.py` の docs-lint 系 + 設計書 10.1 の **`docs-lint` 行** | **本タスクに残す** | `R-8`。**ステップ 24 が触る 10.1 は `docs-lint` 行**であり ORM 条項ではない |
