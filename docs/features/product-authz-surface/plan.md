...
- 1 → 2 → 3 は順に依存する。**4(表分類)と 5(capability カタログ)は 1〜3 と独立**(spec を使わない)なので、**先に入れて帯 2 へ早く渡す**ことができる
- 6 → 7 → … → 19 は順に依存する(6 は 1〜5 の後)
- 20(移行バッチ用ロール)は 14 の後、21(写像)は 20 の後
