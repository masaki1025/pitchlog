...
| **規則セットを読む・大会へ結ぶ関数**と、**`rule_sets` の帰属の列の追加** | **U-X1**(FR-014 の主所有・帯 4 で凍結中 — `../product-impl-unit-split/plan.md:234`。PO 決定 2026-09-24。解凍まで規則セットには直接届かない) |
...
**ステップ 1 を始める前の外部の前提**(承認の後に済ませる — 12 周目 12-P2-1): ① PR A2・PR B・PR C・3-2 節の典拠の訂正の Notion タスクを起票し、ID を得る(ステップ 7 の `pending_switch` が PR B のタスク ID を使う)② FR-014 の到達経路の所有単位は **U-X1(凍結中)に確定済み**(PO 決定 2026-09-24)
...
- [ ] **12-8 節に、A1 の範囲だけを「確定・未発効」と書き、残件と所有者を記録した**(PR A2 / PR B / PR C の各タスク ID・U-A1 / U-A2 / U-C1〔`analysis_groups` の名称の列を含む〕/ U-C2〔制御情報の読み取りの制限関数〕/ U-C3 / U-X1(FR-014 の主所有) / TSK-344 / TSK-349 / 3-2 節の典拠の訂正タスク)
- [x] **FR-014 の到達経路(規則セットを読む・大会へ結ぶ関数)と `rule_sets` の帰属の列の所有を確定した**: U-X1 = TSK-405(カードに申し送りを記録。PO 決定 2026-09-24)
