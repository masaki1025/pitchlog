...
- **分担**: [FR-019](../../requirements/requirements-pitchlog-2026-07-22.md#FR-019) の**キャッシュ無効化契約**(主所有は U-D1)。**契約の一次典拠は要件書 6.2 `:996`**(事前集計・キャッシュの無効化マトリクス — 14 トリガーを列挙する条項)であり、**FR-019 本文にキャッシュの語はない**(FR-019 が供給するのは 14 件中「試合の削除/復元/再開」の 1 件)。**分担の器としては FR-019、要件根拠としては 6.2** である(Claude 一次レビューの是正)
...
| **凍結基準の更新経路(設計書 7.7)への完全な適合** | **[TSK-431](https://app.notion.com/p/3e193b75e68781b7a4ecee5917494f11)** へ分離(**2026-09-20・山田正輝の判断**)。7.7 は本単位の実装進行中に develop へ入り(`5e9ffd9` / PR #71)、取り込みで適用対象になった。`baseline_control` の追加までは本単位で行い、**敵対レビュー 3 周目の残り 4 件(7B・7C・7D・7E)は TSK-431 が閉じる**。**7A は 2026-09-21 に射程へ戻して本 PR で閉じた** — 下記「残る既知の欠陥」表 |
...
### 後続単位への申し送り(Claude 一次レビューの反映)

| 申し送り先 | 内容 | 典拠 |
| --- | --- | --- |
| **各トリガー所有単位**(U-D1 ほか) | 要件書 6.2 `:996` の**後段**「トリガー×対象範囲の対応表として文書化し、**各操作の受け入れテストに含める**」の**後半が未充足のまま残る**。本単位は契約と API までで**発火を持たない**ため、受け入れテストへの組み込みは各トリガーの所有単位が負う | 要件書 `:996` / `docs/design/data-model.md:2046` |
| **U-01**(DTO 基盤) | リポジトリ基底は結果を**全行 materialize** し、**行数上限・ページング契約を持たない**。現時点では一覧経路を 1 つも公開しないので NFR-005 違反ではないが、**一覧 operation を載せる単位が `:818` 後段(一覧系画面はページングする)を満たす責務を負う** | 要件書 `:818` / `backend/src/pitchlog/repositories/base.py` |
| **後続の実装 PR** | 12-4 の「対象入口なし」判定について、**なぜ `data-model.md:2527`(ゲートの対象 = DB を利用する製品機能の実装 PR)に当たらないか**を PR の判定記録に残す。本 PR の読みは「**製品の DB 到達コードは入るが登録 operation が 0 件なので『製品機能』に当たらない**」である。後続 PR での再判定を容易にする | `docs/design/data-model.md:2527` `:2531` `:2534` |

### 残る既知の欠陥(**マージ時点で開いている** — 2026-09-20・山田正輝の受容)
...
| **7E** | 外部ファイル射影の比較元と現側でバイト正規化が異なる | 同内容でも射影移動と誤判定され得る(P2) | TSK-431 |

**7A・7D・および本単位で閉じた 2 周目 P0-3 は同じ形である** — **検査対象が検査の条件を決められる**。**7A は本 PR で閉じた**(比較元を CI が与える形へ)。**同型で残るのは 7D だけである。**
個別に塞ぐ前に「**基準の値**」と「**検査の手続き**」の線引きを定める必要がある可能性があり、
TSK-431 にその観点を申し送っている。
...
アプリ用ロールの真正性検査は、物理接続の新規作成時だけでなく **SQLAlchemy pool の
checkout ごと**に実施する。カタログ照合の往復コストは増えるが、プール返却後に管理経路から
変更された 7 属性や直接・多段・混合メンバーシップを次の業務 SQL より前に拒否する安全性を
優先する。検査失敗時は当該物理接続を invalidate してプールから排除する。検査頻度を下げる
最適化は、安全性の同等性を示す別の設計判断なしには行わない。
...
| NFR-019 の項 | 割り当て |
| --- | --- |
| **(a) 一致性** | **該当なし**(本単位はドメイン計算を持たない。NFR-018 の対象列挙に認可機構は入らない — 要件書 `:892`–`:894`。`:887` は節見出しで、列挙の実体はこの 3 行) |
...
| **NFR-015(失敗の黙殺防止)** | **該当なし(未発効)** — 実装は例外で顕在化する(`engine.py` は接続を invalidate して再送出、`binding.py`・`base.py` は `TenantBindingError`)が、要件書 `:874` が求める「**利用者または管理者が気づける形**(画面通知・ログ)」の到達確認は**入口が無いため本単位では判定できない**。入口を開く単位が負う(Claude 一次レビューの是正) |
