...
| **凍結基準の更新経路(設計書 7.7)への完全な適合** | **[TSK-431](https://app.notion.com/p/3e193b75e68781b7a4ecee5917494f11)** へ分離(**2026-09-20・山田正輝の判断**)。7.7 は本単位の実装進行中に develop へ入り(`5e9ffd9` / PR #71)、取り込みで適用対象になった。`baseline_control` の追加までは本単位で行うが、**敵対レビュー 3 周目の P0 4 件 + P2 1 件は TSK-431 が閉じる**。**うち 7A(比較元 `base_ref` を資産が自己申告できる)は既知の fail-open のままマージする** — 下記「残る既知の欠陥」 |
...
**7A・7D は、本単位で閉じた 2 周目 P0-3 と同じ形である** — **検査対象が検査の条件を決められる**。
