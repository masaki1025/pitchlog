...
| **発行証跡** | `TenantContext` は構築時に `tenant_id` から導出した証跡を持ち、`execute()` が型検査の直後に検証する。**得られるのは完全性のみ**(下記) |

`TenantContext` は `tenant_id` とプロセスローカルな秘密から導出した発行証跡を保持し、
公開 `execute()` は型検査の直後、token 解決・プロパティ参照・サブクラスフックより前に
証跡を検査する。ここで得られる性質は**構築後に値が改竄されていないこと（完全性）だけ**である。
その `tenant_id` が当該要求の認証済み主体にとって正しいという**真正性は得られず**、
従来どおり TSK-217 / U-A1 の責務である。
