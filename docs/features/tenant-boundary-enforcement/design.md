...
**TSK-440 改訂**: 条件 5 の保証縮小と残余リスクの受容範囲を正式化した(2026-09-25)。
...
| **生成箇所の allowlist** | 条件 5 の判定には**機械可読の allowlist** を用いる。保証単位と検査結果の境界は直下の共通逐語ブロックを正とする |
...
### 6-0. この検査器の**脅威モデル**(敵対レビュー 4 周目の反映)

**この節を書く理由**: 3 周目と 4 周目の指摘が**互いに矛盾した**。
3 周目 P1 は「receiver の型に関係なく危険メソッド名を拒否するのは**過剰拒否**で、
正当な非 DB 実装が書けない。**由来を追跡して既知の非 DB 型は通せ**」と求めた。
4 周目 P0 は「**型注釈を信用するから** DB receiver が非 DB 扱いになりうる」と求めた。
**両方を同時に満たすには、注釈を信用せず型を実際に解決する一般的な型推論器が要る。**
検査器がそこへ向かうと、周回ごとに射程が広がって収束しない。**線を引く。**
...
- **悪意ある committer による意図的な偽装**。検査器を欺くために inventory の型名を shadow するクラスを定義する /
  非 DB 注釈を付けた変数へ未解決値を再束縛する、といった経路は、静的解析では原理的に閉じきれない。
  これは**代償なしの保証縮小であり、人間が残余リスクを明示的に受容する**
- **実行時の型**。Python の注釈は実行時に強制されない。検査器が見るのは**宣言**であって事実ではない
- **同一プロセス内の攻撃者**([1-1](#1-1-真正性の引き渡しを機械化するレビュー-1-3-の反映) の
  発行証跡と同じ境界。証跡も検査器も、同一プロセス内の任意コードに対する信頼境界ではない)

本来の解(発行専用モジュールへの機械的封じ込め)は別タスクで起票する。

**したがって、以後の周回で「静的解析で意図的な偽装を完全に閉じよ」という要求は
本単位の射程外として扱う。** 個別に実測で素通りが確認された経路は閉じる(4 周目はそれを 3 件行った)。
**一般的な型解決器は作らない。**
