...
| **発行証跡** | `TenantContext` は構築時に `tenant_id` から導出した証跡を持ち、`execute()` が型検査の直後に検証する。**弾けるのは証跡を更新しない単一フィールド改変だけ**(下記) |

`TenantContext` は `tenant_id` とプロセスローカルな秘密から導出した発行証跡を保持し、
公開 `execute()` は型検査の直後、token 解決・プロパティ参照・サブクラスフックより前に
証跡を検査する。この検査は完全な改竄耐性ではなく、弾けるのは
**`tenant_id` だけを書き換えて証跡を更新しなかった場合**に限られる。
同一プロセス内で導出経路へ到達できるコードは証跡を作り直せるため、この証跡は
**同一プロセス内の攻撃者に対する信頼境界ではない**。導出経路への参照を拒否する静的検査も、
静的に到達を解決できる範囲だけを検出する。その `tenant_id` が当該要求の認証済み主体にとって
正しいという**真正性は得られず**、従来どおり TSK-217 / U-A1 の責務である。
