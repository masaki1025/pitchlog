...
**`ADR-004` と `data-model.md` v0.3・要件書 v2.8 は未マージ**のため、
worktree `../pitchlog-worktrees/feature-merge-gate-clause`(HEAD `96d0f86`)の版を典拠とした。
...
> (`data-model.md:10` の変更履歴。**ただし未マージ**)。
...
| **6** | **未マージ依存** — `ADR-004` / `data-model.md` v0.3 / 要件書 v2.8 は未マージ。`ADR-004:161`「**本 ADR がマージされるまで、製品機能 PR に効くのは現行の 12-4 である**」。**計画書が引用する場合は「マージ後に発効」の注記が要る** | **計画書で注記** |
