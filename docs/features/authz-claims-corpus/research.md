...
- 例外表 2 セル更新のゲート区分(7.6-3 前段)は TSK-233 の人間判断が根拠。計画承認時に PO の再確認を 1 行入れる
- H-85 対応案②(digest 連鎖の 1 段化)・③(件数の非ハードコード化)は「TSK-270 計画改訂 2 の射程」と記録されている(`docs/development/harness-evaluation.md:761-764`)。**本タスクで扱うか(スコープ判断)は /plan で PO に諮る** — ③は本タスクの期待件数更新と密接(直さないと今回も 4 箇所+の手更新)
