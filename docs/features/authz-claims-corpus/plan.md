...
| 5 | **母集合・lock・派生 3 資産の追随**(codex 委任)— 裁定リストの分類・`decidable_at` 是正 + 採取追随(kind/source_id 変更 3 件)+ 例外表セルの source_text 追随 + **変更履歴の新規行(total 1063・table_row +1・分類は §2-4 の既定)** + `input_manifest.commit` = ステップ 2 コミット SHA + **§4-(2) の reseal 実行順(①`--reseal --skip-derived --skip-oracle` ②派生更新 ③`--reseal-derived --skip-oracle`)** + `tests/` 期待件数の更新 | **`check_authz_catalog.py --skip-oracle` green** / **Claude が python で直接計数した分布(classification・kind・location・layer)と全 ID 集合の照合が一致**(checker 出力に依存しない)/ **変更履歴の新規行が既存変更履歴行と同一規則の `out_of_scope` に分類されていることを当該レコード単位で個別照合**(集計分布だけでは誤適用を検出できない — 計画レビュー 5 周目 P1-2)/ **帰属表(§4-(4))で帰属のない変更 0** / 失敗集合が**期待失敗集合(§4-(2) の方式で委任前に固定 — 裁定による分類・DB 判定点の変化で 2 本の見込みを超え得るため、正は期待集合)**と一致 |
...
| 7 | **総合検証と後片付け**(Claude)— /check 一式・**最終成果物 diff(基準 `41884a9`・§4-(4) の対象)と attribution.json の双方向全件再照合**(diff→帰属表の「帰属なし 0」と、帰属表→diff の「失効・架空エントリ 0」の**両差集合 0** — 計画レビュー 4 周目 P1-5・5 周目 P1-3。ステップ 6 で加わった oracle・seal 差分を含む最終状態で機械検査)・**帰属表から人間の逐行確認用の突合シートを機械生成**・H-85 案②③の別起票(Notion)・worklog 締め | /check green / **最終 diff と帰属表の双方向再照合で両差集合 0** / 突合シートが attribution.json の全エントリを覆う / 起票 URL が worklog にある |
