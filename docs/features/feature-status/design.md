...
| キー | 値文法 | 既定 | 更新責務 |
| --- | --- | --- | --- |
| `計画レビュー周回` | 半角整数 | 0 | /plan — レビュー(review normal/adversarial)の指摘反映を 1 周終えるごとに +1 |
| `確定ゲート周回` | 半角整数 | 0 | /finalize-doc — 対象正本の敵対レビュー 1 周(指摘反映まで)ごとに +1 |
| `実行方式` | `通常` \| `fast`(**これ以外は不正** — 8 周目 P1) | 通常(省略可) | fast path 適用の**人間事前 OK** 時に `fast` へ(fast 手順 — implement/SKILL.md の fast 節に記入手順を追記)。導出側はこれで「承認なし・ステップ表なし」を正当な fast と識別する(7 周目 P1)。**キーが存在して値が列挙外(空値・`fastt` 等)の場合は stage 判定せず「未取得(実行方式不正)」と顕在化**(無言で通常扱いにしない) |
...
- **評価順**(3 周目 P2): ①保護ブランチ worktree を無言除外 → ②frontmatter/status の解析(**失敗は「未取得(frontmatter 解析失敗)」として顕在化**)→ ③解析成功後に branch 欠落/不一致を無言除外(既存決定 v0.15/v0.16 の踏襲)。解析失敗と branch 不一致が同時に成立し得るケース(例: `activeX` + branch 不一致)は**解析が先 = 表示される**
...
| 3 | `status: active` かつ base..HEAD の履歴中に plan.md が `status: in-review` だったコミットが存在(= 直近の in-review → active が**未解消**) | **実装中(差し戻し修正)**(9 周目 P1 — k 判定より優先。全ステップ完了済みでも「実装完了・/pr 前」と誤表示しない) |
| 4 | 承認が `済` で始まらない | **計画段階**(計画レビュー周回 n・承認待ちを併記) |
...
- **k(完了ステップ)の導出**: 表検証を通過した後、worktree で `git log <base>..HEAD`(base = `git merge-base origin/develop HEAD`)の件名から完全トークンを**全収集**した集合を S とし、進捗 3 値(§3.2 で使用)を決める:
  ...
  - S が空 → **計画系コミットを除いて判定**(4/5 周目 P1 — 承認直後の通常経路を「実装前」と正しく導出する): **計画系コミット** = 変更パスが **1 件以上**あり、かつ**全件**が当該 feature の `docs/features/<slug>/`・`docs/worklog/` 配下にあるもの(`git diff-tree --no-commit-id --name-only -r` で判定)。**空集合・取得不能・マージコミットは保守的に実装系**として扱う(2 親マージでは diff-tree が名前を返さないことがある — 空集合を「計画系」に倒して誤 known(0) にしない)。計画系を除いた**実装系コミット**が 0 件 → **known k = 0**(実装前)/ 1 件以上 → **unknown**(黙って 0 と断定しない)。base..HEAD が 0 件の場合も known k = 0
