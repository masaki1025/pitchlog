...
確定ゲート周回: 16         # 指摘反映を伴う敵対レビュー 1 周ごとに +1(同前。/finalize-doc が更新)
...
1. **設計書 13 章の改訂とそのゲート(ステップ 5・8)は onboarding のゲート(ステップ 10)より前**。先行 PR を許す規範が立ってから onboarding を approved にする。前例 `docs/worklog/2026-08-11-req-v1-9-nfr021-wsl2.md:126`「波及追随を両方の確定ゲートより前へ」
2. **確定ゲートは「レビュー対象の現状態を approved にできるか」を判定する**(同 `:32`)。既知の矛盾を未来ステップへ送らない
3. **テストの反転(ステップ 10)は onboarding の approved 化と同一コミット**。`tests/test_verify_nfr021_evidence.py:2403` は実 HEAD の onboarding が approved **でない**ことを assert しており、approved 化すると必ず落ちる(実測確認済み — [research.md](research.md) D-3)。`draft`・`in-review` の間は落ちないので、ステップ 9 までは触らない
...
| 8 | **設計書の確定ゲートと approved 化** — `/finalize-doc docs/development/dev-harness-design-2026-08-07.md` で収束させ人間承認を得たうえで、**同一コミットで** frontmatter を `approved` / 変更履歴へ **`1.9` / `approved` 行**を実承認日で追記(`in-review` 行は原文保存)/ **索引を `approved`・版 1.9・承認日へ**。**ゲートの反映は `反映<r>周目` コミットで行い、ステップ記法を付けない**(本ステップのステップコミットは approved 化の 1 本 — 確定ゲート 9 周目で機構の欠番判定により是正) | 収束記録が worklog にある / `反映<r>周目` コミットが `{1..確定ゲート周回}` と**集合として**一致(設計書 `:295`)/ **人間の承認を明示的に取得** / frontmatter がちょうど 3 行で `approved` / **`1.9` の行が 2 行(in-review / approved)**(人手確認)/ 索引 3 セルが一致し `check_docs_status.py` exit 0 / **v1.8 以前の変更履歴行が未変更**(`git diff`) |
| 9 | **受入証跡 README の確定ゲートと approved 化** — `/finalize-doc docs/ops/nfr021-acceptance/README.md`(**v1.1**)。反映ループと approved 化はステップ 8 と同じ形 | ステップ 8 と同じ(収束記録・周回カウンタの集合一致・人間承認・frontmatter・変更履歴 2 行・索引一致・`check_docs_status.py` exit 0)/ **v1.0 行が未変更** |
| 10 | **onboarding の確定ゲートと approved 化 + テストの正例反転(`/implement` — Codex 委任)** — `/finalize-doc docs/development/onboarding.md`(**v1.0**)。開始時に**変更履歴へ `1.0` / `in-review` 行・frontmatter・索引を同一コミットで**同期する(`/finalize-doc` 手順 1 は**変更履歴表も**対象)。収束・人間承認後、下記「ステップ 10 の実行手順」に従い**単一の未 push コミット**として ① frontmatter を `approved` ② 変更履歴へ **`1.0` / `approved` 行** ③ 索引を `approved`・版 `1.0`・承認日へ ④ `tests/test_verify_nfr021_evidence.py:2403` を**実 HEAD が approved であることを assert する正例へ反転**(合成負例 `:2208`・`:2225` は変更しない)。**分割しない**(理由は下記) | 下記「ステップ 10 の実行手順」の **4 段すべてを実施**し、**双方向の変異確認がいずれも赤を観測**している(H-81)/ frontmatter がちょうど 3 行で `approved`(設計書 7.1-5)/ **`1.0` の行が 2 行** / 索引 3 セルが一致し `check_docs_status.py` exit 0 / **v0.1〜v0.5 が未変更**(`git diff`)/ **コミット後の HEAD に対して** `uv run pytest tests/` 全 green / `uv run ruff check .`・`uv run ty check` exit 0 |
...
**ステップ 9 のコミットを `B` として、以下の 4 段で行う**。`--amend` は**常に自分自身**を置換するので、子コミットを作ってから amend しても `B` の状態には戻らない(3 周目 P1-2 — 2 周目の反映時に私が誤っていた点):
