...
> **現在は改訂 4(PR #3)で、その承認範囲は `S-9` + `S-10`・実装ステップは 1 本である**(裁定 `D-22`。
> **`plan.md` が契約の正** — **本書は詳細設計であって射程を定めない**)。
>
...
>
> **【4 周目 `P1` で明示】試験設計 A〜F 節も「現行 PR」ではない。**
> **A〜F は第 2 弾(PR #59)の試験設計であり、改訂 4 の射程ではない。**
> **改訂 4 の合格条件は `plan.md` 4 節の「層①〜④」が正**で、**本書は参照しない。**
...
### `<BASE>` の定義(**25 周目 `P1-4` の是正**)

**本節以降の `<BASE>` は、本改訂の実装の最初のコミットの親コミット SHA を指す。**
**`origin/develop` を使ってはいけない** — **可変参照で、承認後も動く**。
**実測(2026-09-12)**: **merge-base は `9c4791a` だったが `origin/develop` は `e6e2e20` へ進み、
その差分に `.claude/core-areas.json` が含まれていた**(**導出器 `f` の入力そのもの**)。
**`<BASE>` の実 SHA は実装開始時に確定し、worklog と各ステップのコミット本文へ記録する。**
**現時点の merge-base**: `e6e2e20d868c4b7e684cfb6f7acb80f7ff0be515`(**追従のたびに変わるので、これを計画書へ焼き込まない**)。
...
4. **母集団は列挙せず「導出元 + 導出器の全数性」で書く**(**型 A の是正**)。
   ...
   **① 兄弟要素の全数反復** — **系列の各段について、その段の容器を要素 `w` 個にし、2 個目以降のすべてに探針を置く**。
   **`w` = `<BASE>` の凍結 15 パスに実在する容器の最大要素数**(**テストが計算する。定数で持たない** — 25 周目 `P1-5`)。(23 周目 — **末端だけでは中間 `list` の 2 個目以降や `dict` の 2 番目以降の値を降りない走査が通る**)。
...
| **先頭要素だけを見る** | **2 個目以降の要素を複製した資産**で red(**負例も「いずれかの要素」へ揃える** — 25 周目 `P1-3`)|
...
| **分岐単位で例外を握り潰す** | **走査が到達した配列パスの集合を、独立した比較元と突き合わせる** — **比較元は `json.loads` の結果を `object_pairs_hook` なしで再帰的に列挙した集合**(**被検査の走査とは別実装**)。**同じ走査から期待集合を作らない** — **検出対象と期待集合が同時に縮んで green になる**(25 周目 `P1-6`)|
...
**層 ① の対象配列も同じ形で導く**(**`S` は上表のとおり 7 パス** — **凍結 15 パスではない**。25 周目 `P1-7`)— **導出元 `S` = `git show <BASE>:contracts/authz/oracle-seal.lock.json` が挙げる
...
| 固定するもの | 版 |
| --- | --- |
| 資産 | `git show <BASE>:contracts/authz/<name>` |
| 検査器 | `git show <BASE>:scripts/check_authz_catalog.py` |
...
**述語の本体**:**「`contracts/authz/` 配下のファイルを開く、または `git show <BASE>:.claude/core-areas.json` の `areas[].paths` にマッチするファイルを開く `scripts/check_*.py` と、その**名前の対** `tests/test_<検査器のファイル名>`」**。
...
**名前の対応 `scripts/check_X.py` ↔ `tests/test_check_X.py` は実測で全 9 本に存在する**(**対の無い検査器 0 件**)。
...
**対の有無も分岐である** — **対の無い検査器を 1 本足すと母集団が 1 しか増えない**ことも示す。**`f` の出力は既登録分を含む**(**実測 2026-09-12: 検査器 9 本 = 18 パス。うち `check_design_propagation` / `check_doc_coverage` / `check_processing_stages` の 6 パスは既登録**)。**本改訂が追加するのはその差分 12 パスである。****合格条件は「`f` の出力すべてが `guard_paths` に登録されている」を本改訂のテスト内で 1 度確かめるところまで**である。**これを恒久テストとして常時発火させると、未登録の 13 本目を検出する導出型検査そのものになり、裁定 `D-13` の送り先と衝突する**(15 周目 `P1-7`)。**恒久化は別タスクの射程**([core-areas.json の登録を導出型の検査で閉じる](https://app.notion.com/p/3d993b75e68781e6bcb4e42c79ddd789))。**本改訂は `f` の出力を「実装時点で 1 度測り、その結果を worklog へ記録する」** —**`f` を呼ぶ恒久テストを `tests/` へコミットしない**(**コミットすれば常時発火する導出型検査そのものになる** —16 周目 `P1-3`)。**`tests/` へ入れるのは「C 節の表が列挙する 21 件が登録されている」という固定の検査だけ**で、**13 本目の検出は別タスクが恒久化する。****下表は差分の内訳であって母集団の定義ではない。****母集団の各要素を 1 件ずつ外すと red**(**「いずれか 1 件」ではなく全件**)。**`guard_paths` は完全一致なので、パスの大小文字を変えるだけでも red**。**規律 5 により、当該検査を外すと 21 件の負例が red でなくなることを示す**。`[手動・外部]` **6.3 規則⑤の敵対レビュー + 人間承認**

**`guard_paths`(完全一致・`frozenset`)へ 12 パス** — 規則「**凍結資産またはコア領域の成果物を検査する検査器とその対**」:

> **【2026-09-12 訂正】当初 10 パスとしていたが、監査フックの実測で 12 パスだった。**
> **`check_docs_status.py` が `docs/design/data-model.md` / `sync-protocol.md` / 要件書を開く**
> (**いずれも `tenant-isolation.paths` に登録済み**)ので、**述語 `f` はこれと対も返す。**
> **私の当初の測定が「静的な文字列検索」だったため取りこぼしていた** —
> **本節自身が禁じている方法で測っていた。**
> **述語を曲げて除外しない** — **それは「人が母集団を選ぶ」へ戻ることである。**
...
| **`scripts/check_docs_status.py`** | **`tests/test_check_docs_status.py`** |
...
**本改訂は `f` を負例の母集団の導出にだけ使い、「規則に合致するのに未登録の 13 本目」を本番検査として常時発火させることはしない**(裁定 `D-13`)。
...
**本改訂の合格条件は「`f` の出力すべてが登録されており、1 件外すと red」である**(**`f` の出力 = 既登録 6 パス + 本改訂が足す 12 パス**。**下表はその差分 12 パスの内訳であって、合格条件の母集団ではない**)。

## D. owner の母集団と全数性(**plan 4 節から移した**)

**負例の母集団**(規律 4): **導出元 `S` = `git show <BASE>:contracts/authz/boundary-proposal.json` の全キー**
...
**負例の母集団**(規律 4): **導出元 `S` = `git show <BASE>:contracts/authz/oracle-seal.lock.json` の全 leaf**
...
| 導出元 `S` | 導出器 `f` | 全数性の示し方 |
| --- | --- | --- |
| **`git show <BASE>:contracts/authz/boundary-proposal.json` の全 leaf**(**base のみ** — **変更後を入れると本ステップの diff が入力になり規律 4 の問 1 に no**。16 周目 `P1-2`) | **base を再帰的に走り、全 leaf のパスを返す** | **探針の置き場所を容器の型の並びから生成する**(規律 4 の「探針の生成」)。**`dict` の中だけへ足す探針では `list` を降りない走査器も +1 になる**(**実測: 完全な走査 88 leaf に対し `list` を降りない走査は 11 leaf しか見ない**) |
