...
**`--skip-derived` / `--skip-oracle` というオプションは存在しない**(計画書の記述は誤り)。
...
**改訂 4 は ② を機械化する。** 「ラベル件数 vs 行の実数」の突合は今回のスクリプトがそのまま使える。
...
1. **`U-T1` が引き渡し 3 資産に依存するか** — **未裁定**(§4)。
   **TSK-363 は既にマージ済みでカード 23 枚も起票済み**なので、**是正するなら別タスク**。
   **PO 判断が要る。**
