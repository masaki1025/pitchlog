...
| `.claude/core-areas.json` | **本改訂で変更する**(ステップ 1)— **`guard_paths` へ 10 本**・**`tenant-isolation.paths` へ 9 パターン**([design.md](design.md) C 節が正)。**第 1 弾では変更しなかった**(裁定 `D-10` で第 2 弾へ送っていた) | **6.3 規則⑤(敵対レビュー + 人間承認)— 本改訂で払う** |
