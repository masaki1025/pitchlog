...
| **D-9** | ステップ 17(`contract_only` の runtime テスト 152 件)の合格条件が凍結資産の定義と矛盾する | **ステップ 17 を表から撤去し `S-10` として改訂 3 へ送る**(2026-09-10・山田正輝)。**旧 18〜21 を 17〜20 へ連番で振り直し、総数を 20 とする**。根拠は 3 つで、① **`R-7` の原文**が「`contract_only` は schema-drift kill + **受取タスクの** runtime テスト ID を必須にする」と定め、**同じ行が「契約 lint を実副作用 kill として数える抜け道」を名指しで警戒している**(`../pg-authz-verification/plan.md:414`)② **`contract_only` の 165 行には `runtime_target` キーが存在しない**(`probe_executable` の 33 行は持つ)= **資産が runtime のターゲットを定義していない** ③ 157 件の理由コードは `route_universe_pending` で、**製品の HTTP 経路は存在せず本計画は `product_schema: false` を自ら宣言している**。**あわせて `S-9` の置換先を 3 分割する**(一律 `TSK-317` 置換は**所有先が空白の claim を封印で固定する**)。**改訂 3 は本裁定の時点で開始する** — `feature_status.py:948-958` が完了ステップの欠番を `inconsistent` と判定するため、**17 を空けたまま 18〜21 を先行する経路が無い** | 
...
5. **越境テスト**(ステップ 7〜13・16 — 正例 / 拒否例 / 6 前提 × 認可行列各行 / 書き込み 8 権限 /
   管理経路 probe / TOCTOU / 信頼境界 / `contract_only` 158 件)
...
| `contracts/authz/operation-count-mapping.json` | **改訂 3 の射程**(`S-3`)— 要件の 7 単位と `operation_ids`(8)の 1:N 写像。**本計画では作らない** |
| `contracts/authz/handoff-manifest.json` | **改訂 3 の射程**(`S-4`)。**本計画では作らない** |
| `contracts/authz/` の**凍結 15 パス** | **本計画では変更しない**(`S-1`・`S-5`・`S-7` として改訂 3 の射程) |
...
**第 2 群後半・第 3 群は改訂 3 で確定する**(4 節の `S-1`〜`S-10`)。
...
### 第 2 群後半・第 3 群 — **本計画の承認範囲外。計画改訂 3 で確定する**
...
#### 改訂 3 が満たすべき要件(3 周分のレビューで確定した実質的な指摘。**失わないためにここへ記録する**)
...
| `S-9` | **プレースホルダ受取先を実 ID へ置換する**(4 周目 `P1-9`)— 凍結資産に `receiving_task_id: TSK-270-GROUP-2` が **178 件**残っている(**実測** — `contract_only` 158 + `probe_executable` 20。5 周目 `P1-2` の訂正)。**受取先を 2 つに分けて置換する**(**裁定 `D-9`・2026-09-10 で改訂 → 敵対レビュー 1 周目 `P0-2` で 3 分割から 2 分割へ再是正**): **① `probe_executable` 20 件 = `TSK-317`**(`runtime_target` を持ち本タスクの probe 経路で実行できる)**② `contract_only` 158 件 = 製品経路の実装タスク(未起票 — 本改訂で起票する)**(`runtime_target` が無く、本計画は `product_schema: false` を宣言しているので TSK-317 は受取先になれない。**一律 `TSK-317` 置換は所有先が空白の claim を封印で固定する**)。**当初案の ③(`no_db_decision_point` の 1 件を別受取先にする)は取り下げた** — **実測で `FR-041/list_item-014#request-validation`(`no_db_decision_point`)と `FR-041/list_item-014#participant-authorization`(`route_universe_pending`)が同一の `runtime_test_owner.id`(`TSK-270.group2.runtime.FR-041/list_item-014`)を共有している**ため、**分けると 1 つのテスト ID を 2 タスクが所有する**(共有 6 組のうち理由コードが混在するのはこの 1 組だけ — 残り 5 組は `route_universe_pending` のみ)。**したがって受取先の分割単位は claim 行ではなく `runtime_test_owner.id` である。****不変条件(機械検査)**: **同一 `runtime_test_owner.id` を共有する行の `receiving_task_id` は一致しなければならない**(`S-10` ③ で検査する)。**残存 `TSK-270-GROUP-2` は 0 件**を条件にする。封印資産の変更なので `S-1` の基準コミットに含める。**`S-6` は TSK-250 / TSK-217 の受取契約しか扱わないため、本要件が無いと `R-7` の所有先が古い ID のまま封印される** | 4 周目 `P1-9` / **裁定 `D-9`** |
| `S-10` | **`contract_only` の runtime テストの所有先を確定する**(**裁定 `D-9`・2026-09-10 で新設** — 旧ステップ 17 の置き換え)。**`R-7` の原文が「受取タスクの runtime テスト ID」を要求しており、TSK-317 が 152 個の実テストを書くのは `R-7` 自身が警戒する「契約 lint を実副作用 kill として数える抜け道」になる**。したがって **TSK-317 側で機械固定するのは次の 6 点に限る**(**敵対レビュー 1 周目 `P0-3`・`P1-4` で 4 点から書き換えた** — 旧案は比較元と基準コミットが未指定で、**受取先はリテラルを置くだけで通り**、旧ステップ 17 の「158 claim との対応」「常時成功にすると red」を落としていた): **① 158 行の `execution_class` / `classification_rule_id` / `contract_only_reason_code` が資産と exact-set**。**比較元を明示する** — `contracts/authz/claim-mutant-map.json` の **`S-1` の基準コミット直前の blob**(`git rev-parse <base>^:<path>`)を期待値とし、**現ファイルとの差分が `receiving_task_id` の列だけ**であることを要求する(**分類を書き換えながら「exact-set」を自称できない**)。**② `receiving_task_id` が非空であることに加え、`S-9` の 2 分割が定める許容集合(`TSK-317` または起票した製品経路タスクの実 ID)に属する**。**プレースホルダ `TSK-270-GROUP-2` が 0 件**(現行検査器は `check_authz_catalog.py:3803` で**非空文字列しか見ない**ため、**許容集合の検査は本タスクが足す**)。**③ 同一 `runtime_test_owner.id` を共有する行の `receiving_task_id` が一致する**(`S-9` の不変条件 — **`P0-2` の是正**。リテラルでは通せず、**分割を誤ると必ず red**)。**④ `TSK-217` の 7 件を機械で分離できる**。**⑤ 158 claim 行と 152 の一意 ID の対応表が資産由来で導出されている**(旧ステップ 17 から引き継ぐ — **6 組の共有を取りこぼさない**。件数は定数で持たない)。**⑥ 凍結 15 パスの差分**: **`S-1` の基準コミットより前の全期間で 0**(`origin/develop...<base>^`)。**基準コミット以降は `S-1` と `S-9` が明示宣言した資産のみ**が変わり、**それ以外の凍結パスの差分は 0**(`P1-4` の是正 — `S-9` は封印資産 `claim-mutant-map.json` を変更するため、**旧基準 `origin/develop...HEAD` では ② と ⑥ が同時に成立しない**)。**`runtime_test_owner.status` は `planned` のまま動かさない**(実テストは受取タスクが書く)。**あわせて ② の受取先タスクを Notion へ起票し、その DoD へ次の 3 つを登録して相互リンクする**(`S-6` と同じ受取契約の形): **(a) `runtime_test_owner.id` 152 個を安定 ID として登録** **(b) 152 ID が 158 claim 行のすべてを覆うこと** **(c) 各テストについて「claim の述語が assertion に現れ、常時成功にすると red」**(旧ステップ 17 の負例条件を**受取契約へ移す** — **受取側が空実装や無条件成功のテストで契約を満たせないようにする**。TSK-317 は実テストを書かないので、この条件を検査するのは受取タスク側である) | **裁定 `D-9`** / 1 周目 `P0-2`・`P0-3`・`P1-4` |

**改訂 3 の進め方**: **裁定 `D-9`(2026-09-10)により、改訂 3 は本計画のステップ 16 完了時点で開始した**(旧予定は「ステップ 21 の完了後」)。`status` は `active` のまま、`承認` を `未` へ戻して改訂し、**コア領域なので敵対レビューを通してから人間の承認を求める**。**ステップ 1〜16 は番号が動かないため履歴の書き換えを伴わない。**

## 5. DoD(受け入れ基準)

**本計画の承認範囲は第 2 群前半(ステップ 1〜20)**であり、下記はその受け入れ基準である(裁定 `D-8`・`D-9`)。
**第 2 群後半・第 3 群の DoD は計画改訂 3 で確定する**(4 節の `S-1`〜`S-10`)。
...
- [ ] **`contract_only` 158 行の受取契約を機械で固定した**(**裁定 `D-9` で旧ステップ 17 の「152 個の実テストを書く」を撤去し `S-10` へ送った** — `R-7` が runtime テストを**受取タスクの所有**と定めており、TSK-317 が書くと同条が警戒する「契約 lint を実副作用 kill として数える抜け道」になる)。**本計画の承認範囲では実装しない** — 受取契約の確定は改訂 3 の `S-10`
