...
created: 2026-09-09
計画レビュー周回: 20        # 指摘反映を伴うレビュー 1 周ごとに +1(収束確認周は数えない。/plan が更新)
...
5. **すべての `[機械]` 条件を 3 点組で登録し、効きは機構が機械適用する**(**型 B の是正**)。
   ...
   **登録漏れは「`[機械]` 条件の数 = 登録ペアの数」で red にする。**
...
| # | ステップ | 合格条件 |
| --- | --- | --- |
| 1 | **検査基盤**(裁定 `D-17`)— **① 母集団の導出器**(規律 4 の形を満たす関数群。**導出元は言語の文法・スキーマ・資産の構造**で、**自分が列挙した集合を使わない**)**② 3 点組ハーネス**(`[機械]` 条件を `(検査関数, 正例, 負例)` で登録し、**(c) を全ペアへ機械適用する**)。**封印資産に触れない** | `[機械]` **本改訂が追加する全 `[機械]` 条件がハーネスへ登録されている**(**「`[機械]` 条件の数 = 登録ペアの数」で突合し、登録漏れは red**)・**ハーネスが全ペアへ (a) 正例で green (b) 負例で red (c) 当該検査だけ無効化で負例が通る を適用する**・**負例の母集団が導出元から得られている**(**列挙した定数を母集団にしていない**)・**負例: (c) を満たさないペアを 1 件登録すると red**(**digest 不一致だけで落ちる負例を弾けることを示す**)・**負例: 導出器が導出元を取りこぼす合成入力で red** |
| 2 | **`core-areas.json` への登録**(裁定 `D-13` + `S-8` の一部)— **`guard_paths` へ 10 本**・**`tenant-isolation.paths` へ 9 パターン**(いずれも下表で確定済み)。**封印資産に触れない** | `[機械]` **下表の 10 本と 9 パターンが登録されている**(exact-set)・**`tenant-isolation.paths` に追加した各パターンが実在ファイルへ 1 件以上マッチする**(**空振りのパターンを書けない**)・`uv run pytest tests/test_core_guard.py tests/test_ci_wiring.py` green。**負例の母集団 = 裁定 `D-13` の規則を実在ファイルへ当てた結果**(規律 4 — **導出元 `S` = `scripts/` と `tests/` の実在ファイル**、**導出器 `f` = 「凍結資産またはコア領域の成果物を検査する検査器とその対」**。**全数性は「`S` に検査器を 1 本足すと母集団が 2 増える」合成入力で示す**)。**下表は現時点の出力であって母集団の定義ではない**— **母集団の各要素を 1 件ずつ外すと red**(**「いずれか 1 件」ではなく全件**)・**`guard_paths` は完全一致なので、パスの大小文字を変えるだけでも red**・**規律 5 により、当該検査を外すと 19 件の負例が red でなくなることを示す**。`[手動・外部]` **6.3 規則⑤の敵対レビュー + 人間承認** |
...
**負例の母集団 = 上表の 3 境界 × 当該フィールド + `deferred_equivalence_contract` の 1 件 = 4 箇所**(**資産の `boundaries[].boundary_id` と `deferred_equivalence_contract` から導出する**)。
**4 箇所それぞれについて、期待から外れる変異で red になる**:
...
| 導出元 `S` | 導出器 `f` | 全数性の示し方 |
| --- | --- | --- |
| **`check_authz_catalog.py` が `boundary-proposal` に対して literal 比較しているフィールド** | **検査器の AST を走り、`raw[...]` 由来の値と定数の比較をすべて拾う** | **literal を 1 つ増やした合成検査器で、母集団が 1 増えることを示す** |
| **本計画の「確定値」表で「不変」と書いた全フィールド** | **表の行をすべて取る** | **表に 1 行足すと母集団が 1 増えることを示す** |

**母集団の各要素について、期待から外れる変異で red になる。**
**現時点で導出される要素は下表**(**参考 — 合格条件は導出器の出力が正**):
...
**入口は「対象パスの exact-set」を引数に取り、その集合自体を DoD で固定する。**
