...
created: 2026-09-09
計画レビュー周回: 27        # 指摘反映を伴うレビュー 1 周ごとに +1(収束確認周は数えない。/plan が更新)
...
5. **封印資産の値を変える負例は、semantic validator を直接呼ぶ**(**型 B の是正**)。
   ...
   **それぞれ「当該コードを壊すと負例が red でなくなる」「CLI を 2 回回して差分を見る」で確かめる。**
...
**全数性**: **述語の分岐ごとに探針を作る**(19 周目 `P1-1`)— **`owner` だけを含むキー**(例 `extra_owner`)と**`task_id` だけを含むキー**(例 `extra_task_id`)を**それぞれ 1 つ足すと母集団が 1 増える**。
**`*_owner_task_id` のような両方を含むキーだけでは、片方しか見ない導出器も通る**(実測で確認)。
...
| 導出元 `S` | 導出器 `f` | 全数性の示し方 |
| --- | --- | --- |
| **`git show origin/develop:contracts/authz/boundary-proposal.json` の全 leaf**(**base のみ** — **変更後を入れると本ステップの diff が入力になり規律 4 の問 1 に no**。16 周目 `P1-2`) | **base を再帰的に走り、全 leaf のパスを返す** | **走査の分岐ごとに探針を割る**(自己監査 2026-09-12)— **① `dict` の中へ leaf を 1 つ ② `list` の中の要素へ leaf を 1 つ ③ 新しい `list` の要素として leaf**。**それぞれ +1**。**① だけでは `list` を降りない走査器も +1 になる**(**実測: 完全な走査 88 leaf に対し `list` を降りない走査は 11 leaf しか見ないのに、① の探針では同じ +1 を返す**) |
...
**全数性**: **base の seal へ資産を 1 本足すと対象が 1 増えることを示す**。
...
**全数性**: **走査の分岐ごとに探針を割る** — **① `dict` の中 ② `list` の中の要素 ③ 新しい `list` の要素**へ leaf を 1 つずつ足し、**それぞれ +1**(**① だけでは `list` を降りない走査器も通る** — 自己監査 2026-09-12)。
...
- [ ] **5 つの導出器それぞれに全数性試験があり、述語の分岐ごとに探針を割っている**:
      **① core `f`** = **`contracts/authz/` 配下だけ**を開く検査器と**`areas[].paths` だけ**を開く検査器で**それぞれ +2**・**対の無い検査器では +1 しか増えない**
      **② owner** = **`owner` だけ**を含むキーと**`task_id` だけ**を含むキーで**それぞれ +1**
      **③ `S-5` の leaf** = **`dict` の中 / `list` の中の要素 / 新しい `list` の要素**へ leaf を足して**それぞれ +1**
      **④ seal の全 leaf**(`2-e`)= 同じ 3 探針で**それぞれ +1**
      **⑤ `g`** = **容器の型の並びを全部訪れる**。**深さ `d` はテストが base の凍結 15 パスから計算する**(**定数で持たない**)
