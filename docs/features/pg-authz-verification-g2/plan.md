...
created: 2026-09-09
計画レビュー周回: 21        # 指摘反映を伴うレビュー 1 周ごとに +1(収束確認周は数えない。/plan が更新)
...
4. **母集団は列挙せず「導出元 + 導出器の全数性」で書く**(**型 A の是正**)。
   ...
   **`S` は自分が作っていないものにする** — **言語の文法・スキーマ・資産の構造・正本の表**。
...
5. **すべての `[機械]` 条件を 3 点組で登録し、効きは機構が機械適用する**(**型 B の是正**)。
   ...
   **判定の時点はステップごとに分ける** — **ステップ 1 は登録簿の機構と条件 ID の体系を作るところまで**、
   **各条件が実際に登録されているかは、その条件を実装するステップの完了時点で判定する**
   (**ステップ 1 完了時点では対象の検査がまだ存在しない** — 13 周目 `P1-1`)。
...
**負例の母集団**(規律 4): **導出元 `S` = `boundary-proposal.json` そのもの**。
...
**規律 5 により、当該検査を外すと上記 4 箇所の負例が red でなくなることを示す。**
...
| 導出元 `S` | 導出器 `f` | 全数性の示し方 |
| --- | --- | --- |
| **`boundary-proposal.json` の全 leaf**(**資産そのもの** — **本計画の表でも検査器でもない独立した `S`**) | **資産を再帰的に走り、全 leaf のパスを返す** | **合成資産へ leaf を 1 つ足すと母集団が 1 増えることを示す** |
...
| 層 | 何を守るか | 本改訂の射程 |
| --- | --- | --- |
| **① 検査器**(`check_authz_catalog.py`) | **検査器が配列を畳んで多重度を潰す**こと自体 | **下表の 5 配列だけ**。**残る 10 配列と重複キーは別タスク** |
...
**導出元 `S` = `oracle-seal.lock.json` の `sealed_assets[].path` と `input_assets[].path`**、
**導出器 `f` = 「本改訂の差分(`origin/develop...HEAD`)に含まれるもの」で絞る**。
**全数性**: **差分へ資産を 1 本足すと対象が 1 増えることを示す**。
**`5 配列` も同じ** — **導出元 `S` = 3 資産の全コンテナ**、
**導出器 `f` = 「先頭行を複製しても検査器が green のまま通る配列」**(**変異で判定する**)。
**下表は現時点の `f` の出力であって定義ではない。**
...
- **層 ①**: **検査器側の残り**(10 配列 + 重複キー。`_read_json` の読み取り経路の変更を含む)
- **層 ②**: **恒久検査を残る 12 資産へ拡大する**

**本改訂は「層 ① を 5 配列」+「層 ② を 3 資産」である。**
...
# 2-f. 層 ① の 5 配列の多重度検査が実効を持つこと(seal の digest 不一致で判定していない)
