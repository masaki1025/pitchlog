...
**導出器 `g` = 「先頭要素を複製しても `validate_oracle_assets(..., verify_seal=False)` が
green のまま通る配列」**(**変異で判定する**)。

**入口は 1 つに固定する**(21 周目 `P1`)— **`validate_oracle_assets` は
6 資産と seal を依存関係つきで 1 箇所から呼ぶ**ので、**資産ごとに入口を選ぶ余地が無い**。
**`verify_seal=False` で digest 検査を外す** — **付けたままだと全配列が digest 不一致で red になり、
多重度の判定にならない**(規律 5 と同じ理由)。
**資産ごとに別の validator を呼ぶ形にしてはいけない** — **入口の選び方で `g` を縮小でき、その `g` を使う DoD も通る。**
...
**実測(2026-09-12・単一入口)**: **非空配列 1676 本のうち `g` の出力は 19 本。**
...
| `oracle-seal.lock` | `input_assets` / `sealed_assets` |

**過去の「15 本」「16 本」はどちらも不足だった** — **前者は走査の入口を `dict` 限定にし、後者は資産ごとに別の validator を呼んで
5 資産しか測れていなかった**。**単一入口にして初めて `claim-mutant-map` / `attack-tree` /
`verification-evidence` / `sealed_assets` の 4 本が出た。**
