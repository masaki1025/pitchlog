...
created: 2026-09-09
計画レビュー周回: 26        # 指摘反映を伴うレビュー 1 周ごとに +1(収束確認周は数えない。/plan が更新)
...
| 導出元 `S` | 導出器 `f` | 全数性の示し方 |
| --- | --- | --- |
| **`git show origin/develop:contracts/authz/boundary-proposal.json` の全 leaf**(**base のみ** — **変更後を入れると本ステップの diff が入力になり規律 4 の問 1 に no**。16 周目 `P1-2`) | **base を再帰的に走り、全 leaf のパスを返す** | **base へ leaf を 1 つ足すと母集団が 1 増えることを示す** |
...
- [ ] **負例を 3 つの型へ分け、それぞれの確かめ方で示している**(**規律 5 の射程** — 17 周目 `P1-4`):
      ...
      **③ CLI の副作用を見る負例**(通常検証で reseal されない)= **CLI を 2 回回して差分を見る**
...
- [ ] **5 つの導出器それぞれに全数性試験がある**:
      **① core `f`** = 述語を満たす検査器を 1 本足すと母集団が 2 増える
      **② owner** = `*_owner_task_id` を 1 つ足すと 1 増える
      **③ `S-5` の leaf** = base へ leaf を 1 つ足すと 1 増える
      **④ seal の全 leaf**(`2-e`)= 合成 seal へ leaf を 1 つ足すと 1 増える
      **⑤ `g`** = **容器の型の並び**(**深さ = base の最大容器深さ**)を全部訪れる
