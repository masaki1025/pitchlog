...
created: 2026-09-09
計画レビュー周回: 22        # 指摘反映を伴うレビュー 1 周ごとに +1(収束確認周は数えない。/plan が更新)
...
**未登録は `backend/src/pitchlog/authz/*` と新設検査器 8 本**で、**本改訂のステップ 2 で登録する**(4 節の表が正)。**現在のステップ表は本改訂の 1〜2 である**(第 1 弾の 1〜20 は「第 1 弾の実施記録」へ移した)。
...
5. **すべての `[機械]` 条件を 3 点組で登録し、効きは機構が機械適用する**(**型 B の是正**)。
   **3 点組** = **(a) 正例で green (b) 負例で red (c) 当該検査だけを無効化すると負例が通る**。
   **(c) が要る理由**: **封印資産の変異は `seal` の digest 不一致だけでも必ず red になる**ため、
   **検査を 1 行も書かなくても (b) が満たせてしまう**(5・8・10・11・12 周目の `P1`)。
   **(c) を条件ごとに文章で書かない** — **ステップ 1 の検査基盤が全ペアへ機械適用する**。
   **`[機械]` 条件には一意な条件 ID を振る**(`CHK-<ステップ>-<連番>`)。
   **「条件の数」を数えない** — **`[機械]` 記号 1 つに複数条件が連なるので数が定義できない**
   (13 周目 `P1-1`)。**代わりに条件 ID の集合で exact-set 突合する**:
   **計画書の条件 ID 集合 = ハーネスの登録 ID 集合**。
   **両方から同じ ID を落としても、計画書側の集合は本文の走査で得るので検出できる。**
   **負例の枠も `S` から導く** — **負例の母集団 = `S` の全要素 × 変異操作**。
   **負例を人が選ぶと、検査器と負例が同じ枠から作られ、枠の外が両方で盲点になる**
   (**13 周目 `P1-4` の実例**: **`trust_boundary.verification_owner_task_id` は
   現行 validator も私の負例 4 件も見ていなかった** — **型 A と型 B は同じ根から出ている**)。
   **登録は 2 段で行う**(**13・14 周目 `P1`**):

   | 段 | 何をするか |
   | --- | --- |
   | **ステップ 1** | **計画書の `[機械]` 条件から条件 ID を機械抽出し、全件を「未実装」として登録する** |
   | **各実装ステップ** | **実装した条件の ID を「登録ペアあり」へ進める** |
   | **最終ステップ** | **「未実装 0 件」かつ「登録 ID 集合 = 計画書の条件 ID 集合」を exact-set 突合** |

   **件数一致では両側から同時に落とせるが、ID の集合突合なら片方だけ落ちても red になる。**

> **なぜ「測ったのに閉じたと書く」のか**(**裁定 `D-17` の根拠** — 2026-09-12)。
...
>
> **したがって「閉じた」と書く直前に「この枠を選んだのは誰か。枠の外にある欠陥はどんな形か」を
> 1 行書く。答えが「自分が列挙した」なら閉じていない。**
> **本 PR の 12 周で出た抜け道 8 件は、すべて枠の縁に出ている** —
...
**裁定 `D-8` と `D-9` により、第 1 弾の承認範囲は 20 ステップ(第 2 群前半)だった。****本改訂の承認範囲は 3 ステップ(検査基盤 + 封印系)である**(裁定 `D-14`・`D-16`・`D-17`)。**以下は第 1 弾についての記述である。** コア領域なので全ステップに人間の逐行確認が掛かり、
...
**3 本に分けた理由 — 封印は原子単位であり、検査基盤が先に要る**(**4 周目 `P0-4` の実測 + 裁定 `D-17`**):
...
**そこで封印資産を変える作業をすべてステップ 3 の 1 コミットへ収め、
封印資産に触れない 2 件(検査基盤・`core-areas.json` の登録)を先行ステップにした。**
**これは 1 ステップ = 1 コミットの例外ではなく、3 ステップとも 1 コミットである。**

**検査基盤を先頭に置くのは裁定 `D-17` による** — **規律 5 の (c)(当該検査だけを無効化すると負例が通る)を、後続ステップの全条件へ機構が機械適用するため**。**基盤が無いと (c) を条件ごとに文章で書くことになり、書き漏らした箇所が次のレビュー指摘になる**(**12 周で 8 回再生産した**)。

> **期待件数の撤去は裁定 `D-16` で別タスクへ送った**(1 節)。**本改訂の先行ステップは `core-areas.json` の登録 1 件だけになった。**

| # | ステップ | 合格条件 |
| --- | --- | --- |
| 1 | **検査基盤**(裁定 `D-17`)— **① 母集団の導出器**(規律 4 の形を満たす関数群。**導出元は言語の文法・スキーマ・資産の構造**で、**自分が列挙した集合を使わない**)**② 3 点組ハーネス**(`[機械]` 条件を `(検査関数, 正例, 負例)` で登録し、**(c) を全ペアへ機械適用する**)。**封印資産に触れない** | `[機械]` **`CHK-*` の条件 ID 体系と登録簿の機構がある**・**計画書の本文から `CHK-*` を走査して集合を取れる**(**人が数えない**)・**ハーネスが登録済みペアへ (a) 正例で green (b) 負例で red (c) 当該検査だけ無効化で負例が通る を適用する**・**負例: (c) を満たさないペアを 1 件登録すると red**(**`seal` の digest 不一致だけで落ちる負例を弾けることを示す**)・**負例: 導出器が導出元を取りこぼす合成入力で red**・**本ステップでは登録の完全性を判定しない**(**対象の検査はステップ 2・3 で実装されるため** — 13 周目 `P1-1`。**完全性は各ステップの完了時点で「計画書の `CHK-*` 集合 = 登録 ID 集合」を exact-set 突合する**)|
| 2 | **`core-areas.json` への登録**(裁定 `D-13` + `S-8` の一部)— **`guard_paths` へ 10 本**・**`tenant-isolation.paths` へ 9 パターン**(いずれも下表で確定済み)。**封印資産に触れない** | `[機械]` **下表の 10 本と 9 パターンが登録されている**(exact-set)・**`tenant-isolation.paths` に追加した各パターンが実在ファイルへ 1 件以上マッチする**(**空振りのパターンを書けない**)・`uv run pytest tests/test_core_guard.py tests/test_ci_wiring.py` green。**負例の母集団**(規律 4): **導出元 `S` = `scripts/*.py` と `tests/test_*.py` の実在ファイル**。**導出器 `f` = 機械判定できる述語**(**日本語の規則をそのまま条件にしない** — 13 周目 `P1-2`):**「`contracts/authz/` 配下のファイルを開く、または `core-areas.json` の`areas[].paths` にマッチするファイルを開く `scripts/check_*.py` と、それを import する `tests/test_check_*.py`」**。**全数性**: **その述語を満たす検査器を 1 本足した合成ツリーで、母集団が 2 増えることを示す。****`f` の出力は既登録分を含む**(**実測 2026-09-12: 検査器 8 本 = 16 パス。うち `check_design_propagation` / `check_doc_coverage` / `check_processing_stages` の 6 パスは既登録**)。**本改訂が追加するのはその差分 10 パスである。****合格条件は「`f` の出力すべてが `guard_paths` に登録されている」**であり、**下表は差分の内訳であって母集団の定義ではない。****母集団の各要素を 1 件ずつ外すと red**(**「いずれか 1 件」ではなく全件**)・**`guard_paths` は完全一致なので、パスの大小文字を変えるだけでも red**・**規律 5 により、当該検査を外すと 19 件の負例が red でなくなることを示す**。`[手動・外部]` **6.3 規則⑤の敵対レビュー + 人間承認** |
| 3 | **封印資産の確定と再封印**(owner 移管 + `S-5` + `S-7` + 検査器追随(**重複行で潰れる配列の検査を含む**)+ `--reseal-oracle` **1 回**)— **3-a 〜 3-e を 1 コミットに収める**(上記の理由)。**`oracle_commit` は動かさない**(入力資産が不変のため — 2 節の実測) | `[機械]` **3-a 〜 3-e の全条件を満たす**(下記)・**reseal 後に `check_authz_catalog.py` が rc=0**・**`--reseal-oracle` を付けない通常検証では reseal されない**。`[手動・外部]` **差分敵対レビューと人間査読**(`reseal_policy.human_review_required`) |

##### ステップ 2 の登録先 — **`D-13` の規則を当てた結果**(**実測で確定済み**)
...
**3-a owner の移管と `CONTROL-READS` の事実是正**(裁定 `D-11` `D-12`)
...
**全数性**: **合成資産へ `*_owner_task_id` を 1 つ足すと母集団が 1 増えることを示す**。
...
**規律 5 により、当該検査を外すと母集団全件の負例が red でなくなることを示す。**
...
**3-b `S-5` — `boundary-proposal.json` の裁定後の型を確定**

**資産と検査器を同時に変える**(下記「資産と検査器が二重に固定されている」)。

`CHK-3-02` `[機械]` **`frozen_value` の差分が 0**・**`alternative_value` と `review_id` が不変**・
**検査器の literal が資産と一致**・**`tests/test_check_authz_catalog.py` が追随している**・
**負例の母集団 = 導出元 2 つの和**(規律 4 —**自分で列挙しない**):

| 導出元 `S` | 導出器 `f` | 全数性の示し方 |
| --- | --- | --- |
| **`git show origin/develop:contracts/authz/boundary-proposal.json` の全 leaf と、変更後の全 leaf の和集合**(**版を base へ固定し、削除・新設した leaf も母集団に残す** — 14 周目 `P1-4`) | **両版を再帰的に走り、全 leaf のパスの和を返す** | **どちらかの版へ leaf を 1 つ足すと母集団が 1 増えることを示す** |

**全 leaf を 3 つに分類する**(**分類も資産と本計画の確定値表から機械的に決める**):
...
**3-c `S-7` — `ddl-elements.json` の `scope` を確定**
...
`CHK-3-03` `[機械]` **4 キーが確定値と完全一致**・**検査器の literal が資産と一致**・
...
| 層 | 何を守るか | 本改訂の射程 |
| --- | --- | --- |
| **① 検査器**(`check_authz_catalog.py`) | **検査器が配列を畳んで多重度を潰す**こと自体 | **`g` の出力すべて**(実測 15 本)。**残るのは重複 JSON キーだけ** |
...
| **対象資産の集合を狭める**(3 資産のうち 1 つを外す) | **3 資産それぞれに重複を入れた 3 つの負例で red**(**1 資産ぶんを外すと 1 件が通ってしまう**) |
| **深さ制限つきの走査で通す** | **`ddl-elements.json` は深さ 5 の配列を持つ**(実測)。**その最深部に重複を置いた負例で red** |
...
**3 資産は同じ入口へ通す**(**資産ごとに別の呼び出しを書くと、1 つを外しても気づけない**)。
**対象資産の集合も列挙しない**(13 周目 `P1-5`)— 
**導出元 `S` = `git diff --name-only origin/develop...HEAD -- contracts/authz/` の出力**
(**本改訂が実際に変更する資産そのもの**)。
**導出器 `f` = そのうち JSON であるものを返す**。
**`seal` の `sealed_assets[]`/`input_assets[]` を `S` にしてはいけない** —
**`oracle-seal.lock.json` 自身はどちらにも登録されていないので、seal が母集団から漏れる**
(14 周目 `P1-5` の実測)。
**全数性**: **差分へ資産を 1 本足すと対象が 1 増えることを示す**。
**層 ① の対象配列も同じ形で導く** — **導出元 `S` = 上記 `f` が返す資産の全コンテナ**、
**導出器 `g` = 「先頭要素を複製しても検査器が green のまま通る配列」**(**変異で判定する**)。
...
**実測(2026-09-12)— 3 資産の非空配列 95 本のうち `g` の出力は 15 本。**
**「5 配列」(7 周目)も「10 配列」(8 周目)も誤りだった** —
**前者は走査の入口を `dict` 限定にしていたため、後者は `seal` の `input_assets` を数えていなかったため。**
...
- **層 ②**: **恒久検査を残る 12 資産へ拡大する**

**本改訂は「層 ① を `g` の出力(実測 15 本)」+「層 ② を変更する 3 資産」である。**

`CHK-3-04` `[機械]` **`f` が出力する全配列に行数と ID 一意性の検査がある**・
...
`CHK-3-05` `[機械]` **seal の `oracle_commit` が `dd2cb92...` のまま**・
...
**負例の母集団 = seal が持つ全フィールド = `oracle_commit` / `oracle_commit_semantics` /
`input_assets` 8 行 / `sealed_assets` 6 行 / `review_policy` / `reseal_policy`**
(**`oracle-seal.lock.json` のトップレベルキーから導出する** — 実測で 8 キー)。
...
**規律 5 により、当該検査を外すと上記の負例が red でなくなることを示す。**
...
### 第 2 弾(ステップ 1〜3)— **本改訂の受け入れ基準**

> **第 1 弾(上記)は PR #52 で達成済み。本節が本改訂の受け入れ基準である。**
> **ステップ 1・2 は封印資産に触れず、ステップ 3 が封印資産の変更と reseal を 1 コミットで行う。**

#### ステップ 1 — 検査基盤(裁定 `D-17`)

- [ ] **母集団の導出器が規律 4 の形を満たす**(**導出元は言語の文法・スキーマ・資産の構造**・**自分が列挙した集合を使っていない**)
- [ ] **導出器が導出元を取りこぼす合成入力で red**
- [ ] **本改訂が追加する全 `[機械]` 条件がハーネスへ登録されている**(**「条件の数 = 登録ペアの数」で突合し、登録漏れは red**)
- [ ] **ハーネスが全ペアへ 3 点組を機械適用する**((a) 正例で green (b) 負例で red (c) **当該検査だけ無効化で負例が通る**)
- [ ] **(c) を満たさないペアを 1 件登録すると red**(**seal の digest 不一致だけで落ちる負例を弾けることを示す**)
- [ ] **規律 5 の (c) を条件ごとに文章で書いていない**(**機構が適用する**)
...
- [ ] **`g` の出力する全配列に行数と要素一意性の検査を置いた**(**実測 15 本 — 数を計画書から取らず `g` を回して得る**)
- [ ] **`g` の出力それぞれで、多重度検査が red を出す**(**seal の digest 不一致で判定していない** — **多重度検査を外すと負例が red でなくなる**)
- [ ] **既に red だった配列の検査を変えていない**
- [ ] **層 ②: 変更する 3 資産の全配列 + 全 JSON キー対を走る恒久検査を `tests/` へ置いた**
- [ ] **その検査が再封印の直前にも走り、重複 0 件であることを確認した**
- [ ] **走査自体を壊した 6 種の負例で red**(再帰しない / `json.loads` の last-wins / 無効化 / **対象資産の集合を狭める** / **深さ制限つきの走査** / **分岐単位の例外握り潰し**)
- [ ] **3 資産を同じ入口へ通し、対象パスの exact-set を DoD で固定した**
- [ ] **走査が実際に到達した配列パスの集合を assert している**
- [ ] **負例の資産は一時ディレクトリの写しで作り、本物の凍結資産を変異させていない**
- [ ] **層 ① と層 ② の射程の違いを計画書で書き分けた**(① = 検査器の `g` 出力 / ② = 3 資産の恒久検査)
- [ ] **別タスクへ送った 2 件を計画書に書いた**(① の重複 JSON キー / ② の残る 12 資産)
