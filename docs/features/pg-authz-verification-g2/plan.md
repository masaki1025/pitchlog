...
created: 2026-09-09
計画レビュー周回: 24        # 指摘反映を伴うレビュー 1 周ごとに +1(収束確認周は数えない。/plan が更新)
...
**負例の母集団**(規律 4): **導出元 `S` = `scripts/*.py` と `tests/test_*.py` の実在ファイル**。**導出器 `f` = 機械判定できる述語**(**日本語の規則をそのまま条件にしない** — 13 周目 `P1-2`):**「`contracts/authz/` 配下のファイルを開く、または `git show origin/develop:.claude/core-areas.json` の `areas[].paths` にマッチするファイルを開く `scripts/check_*.py` と、それを import する `tests/test_check_*.py`」**。
...
**規律 5 により、負例は当該 validator の関数を直接呼び、その関数が投げることを確かめる**(**`check_authz_catalog.py` 全体を回して red を見ない**)。
...
**層 ① の対象配列も同じ形で導く** — **導出元 `S` = 上記 `f` が返す資産の `origin/develop` 版の全コンテナ**、
...
**`g` の全数性**(規律 4 の問 3 — **自己監査 2026-09-12 で欠けていた**): **走査が全コンテナへ到達することを、既知の入れ子を持つ合成資産で示す** — **`dict` の中の `list`・`list` の中の `list`・`list` の中の `dict` の中の `list` をそれぞれ 1 つ持つ資産で、走査が 3 つとも訪れる**。**要素の型で入口を絞らない**(**7 周目に `dict` 限定で 10 本取りこぼした**)。
