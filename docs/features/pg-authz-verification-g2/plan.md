...
created: 2026-09-09
計画レビュー周回: 28        # 指摘反映を伴うレビュー 1 周ごとに +1(収束確認周は数えない。/plan が更新)
...
- [ ] **4 つの導出器それぞれに全数性試験があり、分岐ごとに探針を割っている**(**core `f` はステップ 1 の DoD**):
      ...
      **④ `g`** = **容器の型の並びを全部訪れる**・**入口の 3 分岐**(`sealed_assets` だけ / `input_assets` だけ / seal 自身を外す)**にも探針**。**深さ `d` はテストが base の凍結 15 パスから計算する**(**定数で持たない**)
