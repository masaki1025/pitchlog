...
created: 2026-09-09
計画レビュー周回: 32        # 指摘反映を伴うレビュー 1 周ごとに +1(収束確認周は数えない。/plan が更新)
...
4. **母集団は列挙せず「導出元 + 導出器の全数性」で書く**(**型 A の是正**)。
   ...
   **① 兄弟要素の全数反復** — **各系列の末端を要素 2 個以上の配列にし、2 個目以降だけに差を置く**(**先頭要素しか見ない走査を落とす**)。
...
5. **封印資産の値を変える負例は、semantic validator を直接呼ぶ**(**型 B の是正**)。
   ...
   **負例の形**: **資産を 1 つ変えて seal を古いままにし、通常検証を回す** →
   **seal が書き換わったら red**(**`--reseal-oracle` でだけ書き換わること**)。
...
**「ファイルを開く」の判定方法も固定する**(21 周目)— **検査器を実行し、その間に開かれたファイルを実測する**(**`sys.settrace` や `strace` ではなく、`builtins.open` と `pathlib.Path.read_text` / `read_bytes` をテスト内で差し替えて記録する**)。
**静的な文字列検索にしない** — **`Path` 定数・別名・helper 経由・動的結合を取りこぼす**。
...
**導出器 `g` = 「先頭要素を複製しても、その資産の semantic validator が green のまま通る配列」。**
...
**広げるコストは小さい**(**当時の測定で 3 資産 15 本 → 5 資産 16 本**。**その後 22 周目に入口を是正して 7 パス 18 本と確定した** — 下記の実測表が正)。
...
- [ ] **負例を 3 つの型へ分け、それぞれの確かめ方で示している**(**規律 5 の射程** — 17 周目 `P1-4`):
      **① 封印資産の値を変える負例** = **下表の validator を直接呼んで投げる**
...
- [ ] **5 箇所それぞれに逐項の負例がある**(4 節の表)・**形式だけ似た別 ID / 空文字でも red**
...
- [ ] **`g` の入口を資産の区分ごとに 1 つへ固定した**(**`sealed_assets` の 6 資産 = `validate_oracle_assets(verify_seal=False)` / **seal 自身 = `validate_oracle_seal`**。**資産ごとに別の入口を選べない**)
- [ ] **`g` の `S` が 7 パスであることを書いた**(**`input_assets` の 8 資産は別の入口で検査され本改訂は触らない**)
