...
5. **越境テスト**(ステップ 7〜13・16〜17 — 正例 / 拒否例 / 6 前提 × 認可行列各行 / 書き込み 8 権限 /
...
| # | 要件 | 由来 |
| --- | --- | --- |
| `S-1` | **統合コミットを 2 段に分ける** — ① 入力・封印資産・検査器・追随テスト・derived lock を変更する基準コミット ② その**基準コミット SHA** を 6 資産の `oracle_context` と seal へ設定し `--reseal-oracle` するコミット。**`oracle_commit` に自コミットの SHA は入れられない**。各層の reseal は一度ずつでよく、差分敵対レビューは 2 コミットの範囲に対して 1 回にできる | 3 周目 `P1-2` |
...
| `S-9` | **プレースホルダ受取先を実 ID へ置換する**(4 周目 `P1-9`)— 凍結資産に `receiving_task_id: TSK-270-GROUP-2` が **178 件**残っている(**実測** — `contract_only` 158 + `probe_executable` 20。5 周目 `P1-2` の訂正)。**受取先を 3 つに分けて置換する**(**裁定 `D-9`・2026-09-10 で改訂** — 一律 `TSK-317` 置換は誤り): **① `probe_executable` 20 件 = `TSK-317`**(`runtime_target` を持ち本タスクの probe 経路で実行できる)**② `contract_only` かつ `route_universe_pending` の 157 件 = 製品経路の実装タスク(未起票 — 本改訂で起票する)**(`runtime_target` が無く、本計画は `product_schema: false` を宣言しているので TSK-317 は受取先になれない。**一律置換すると所有先が空白の claim を封印で固定する**)**③ `contract_only` かつ `no_db_decision_point` の 1 件(`FR-041/list_item-014#request-validation`)= 要判定**(DB 判定点が無いので `TSK-217` の 7 件と同型に見えるが、`receiving_task_id` が GROUP-2 のままである理由が資産に無い)。**残存 `TSK-270-GROUP-2` は 0 件**を条件にする。封印資産の変更なので `S-1` の基準コミットに含める。**`S-6` は TSK-250 / TSK-217 の受取契約しか扱わないため、本要件が無いと `R-7` の所有先が古い ID のまま封印される** | 4 周目 `P1-9` / **裁定 `D-9`** |
| `S-10` | **`contract_only` の runtime テストの所有先を確定する**(**裁定 `D-9`・2026-09-10 で新設** — 旧ステップ 17 の置き換え)。**`R-7` の原文が「受取タスクの runtime テスト ID」を要求しており、TSK-317 が 152 個の実テストを書くのは `R-7` 自身が警戒する「契約 lint を実副作用 kill として数える抜け道」になる**。したがって **TSK-317 側で機械固定するのは次の 4 点に限る**: **① 158 行の `execution_class` / `classification_rule_id` / `contract_only_reason_code` が資産と exact-set** **② `receiving_task_id` が `S-9` の 3 分割どおりに全件埋まっている**(空・プレースホルダが 0 件) **③ `TSK-217` の 7 件を機械で分離できる** **④ 凍結 15 パスの差分が 0**。**`runtime_test_owner.status` は `planned` のまま動かさない**(実テストは受取タスクが書く)。**負例: 受取先を 1 件でも空にすると red / 分類を 1 件変えると red**。**あわせて ② の受取先タスクを Notion へ起票し、その DoD へ `runtime_test_owner.id` 152 個を安定 ID として登録して相互リンクする**(`S-6` と同じ受取契約の形にする) | **裁定 `D-9`** |
...
- [ ] **`contract_only` の runtime テストがある** — **claim 158 行 / 一意な論理 ID 152 個**の双方を覆う(6 組が ID を共有)。`TSK-217` の 7 件は実装しない。**資産の status も ID も変えない**
...
- [ ] **改訂 3 が満たすべき要件 `S-1`〜`S-9` を 4 節に記録した**(失わないため)
...
| 種別 | 追加内容 |
| --- | --- |
| **(b) 越境アクセステスト** | ステップ **7・8・10・11・12・16・17** — 正例 / 拒否例 / **6 前提 × 認可行列各行** / 書き込み 8 権限 / 管理経路 probe / 信頼境界。**DB レベルの「0 行 / 権限拒否」まで**で、`404` の判定と 13 組合せクラスの残り 12 は TSK-217 |
