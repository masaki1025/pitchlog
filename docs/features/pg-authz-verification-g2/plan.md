...
created: 2026-09-09
計画レビュー周回: 33        # 指摘反映を伴うレビュー 1 周ごとに +1(収束確認周は数えない。/plan が更新)
...
- [ ] **負例を 3 つの型へ分け、それぞれの確かめ方で示している**(**規律 5 の射程** — 17 周目 `P1-4`):
      ...
      **③ CLI の副作用を見る負例**(通常検証で reseal されない)= **semantic validation を通る変更を資産へ入れ、seal を古いままにして通常検証を回し、seal がバイト不変であることを示す**
      (**整合済みの seal でバイト不変を見ても差が出ない** — 21 周目。**semantic に不正な変更では検査器が reseal 処理へ達しない** — 23 周目)
