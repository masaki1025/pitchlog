...
created: 2026-09-09
計画レビュー周回: 29        # 指摘反映を伴うレビュー 1 周ごとに +1(収束確認周は数えない。/plan が更新)
...
4. **母集団は列挙せず「導出元 + 導出器の全数性」で書く**(**型 A の是正**)。
   ...
   **探針の置き場所も容器の型の並びから生成する**(20 周目 `P1-2` — **`g` と同じ機構**)。
...
5. **封印資産の値を変える負例は、semantic validator を直接呼ぶ**(**型 B の是正**)。
   ...
   **それぞれ「当該コードを壊すと負例が red でなくなる」「実行前の seal と各実行後の seal がバイト不変」で確かめる**
   (**「CLI を 2 回回して差分を見る」では足りない** — **1 回目に誤って reseal し 2 回目が無変更な実装でも通る**。19 周目 `P1-4`)。
...
**負例の母集団**(規律 4): **導出元 `S` = `scripts/*.py` と `tests/test_*.py` の実在ファイル**。**導出器 `f` = 機械判定できる述語**(**日本語の規則をそのまま条件にしない** — 13 周目 `P1-2`):**「`contracts/authz/` 配下のファイルを開く、または `git show origin/develop:.claude/core-areas.json` の `areas[].paths` にマッチするファイルを開く `scripts/check_*.py` と、その**名前の対** `tests/test_<検査器のファイル名>`」**。
...
**② 再帰の分岐**(20 周目 `P1-1`)— **探針の置き場所を容器の型の並びから生成する**(下記「探針の生成」)。**トップレベルの `dict` へ足すだけでは、配列の中を降りない導出器も通る**。
...
**ステップ 1 は封印資産に触れないので規律 5 の対象外**。**負例は `core_guard.py` の判定関数を直接呼んで確かめる。**
...
| 導出元 `S` | 導出器 `f` | 全数性の示し方 |
| --- | --- | --- |
| **`git show origin/develop:contracts/authz/boundary-proposal.json` の全 leaf**(**base のみ** — **変更後を入れると本ステップの diff が入力になり規律 4 の問 1 に no**。16 周目 `P1-2`) | **base を再帰的に走り、全 leaf のパスを返す** | **探針の置き場所を容器の型の並びから生成する**(下記「探針の生成」)。**`dict` の中だけへ足す探針では `list` を降りない走査器も +1 になる**(**実測: 完全な走査 88 leaf に対し `list` を降りない走査は 11 leaf しか見ない**) |
...
**凍結 15 パスは同じ入口へ通す**(**資産ごとに別の呼び出しを書くと、1 つを外しても気づけない**)。
**対象資産の集合も列挙しない**(13 周目 `P1-5`)—
**導出元 `S` = base の seal が挙げる `sealed_assets[].path` と `input_assets[].path`、および seal 自身**(= **凍結 15 パス**)。
**層 ① と層 ② は同じ `S` を使う**(18 周目 `P1-2` — **表と詳細条件で別の導出を書かない**)。
**`seal` 自身を足すのを忘れない** — **`oracle-seal.lock.json` はどちらの配列にも登録されていない**(14 周目 `P1-5` の実測)。
**差分を `S` にしてはいけない** — **検証・コミットの時点でまだ変更が `HEAD` に無い**(17 周目 `P1-2`)。
**全数性**: **入口の 3 分岐それぞれに探針を置く**(20 周目 `P1-3`)— **① `sealed_assets[]` だけへ資産を 1 本足す ② `input_assets[]` だけへ 1 本足す ③ seal 自身を対象から外す**。**① ② はそれぞれ +1、③ は母集団が 1 減る**。
...
**広げるコストは実測で +1 本**(**本改訂が変更する 3 資産**で 15 本 → 測定できた 5 資産で 16 本)。
...
**実測(2026-09-12)**: **変更する 3 資産だけなら非空配列 95 本のうち 15 本**。
**凍結 15 パスへ広げると、測定できた 5 資産で 107 本のうち 16 本**(**残る資産は validator の呼び出し形が違い未測定** — **実装時に全件を測って worklog へ記録する**)。
**下表は本改訂が変更する 3 資産だけを測ったときの内訳であって、母集団の定義ではない。**
**`S-8` の「引き渡し 3 資産」(`ddl-elements` / `auth-catalog` / `rejected-configs`)とは別物である。**
...
**全数性**: **探針の置き場所を容器の型の並びから生成する**(下記「探針の生成」)。
...
- [ ] **`f` の出力すべてが `guard_paths` に登録されている**(**`f` は base の `core-areas.json` を入力にする** — 4 節)
...
- [ ] **core `f` の全数性試験がある** — **`contracts/authz/` 配下だけ**を開く検査器と**`areas[].paths` だけ**を開く検査器で**それぞれ +2**・**対の無い検査器では +1 しか増えない**
- [ ] **`f` の出力を実装時に測って worklog へ記録した**(**`f` を呼ぶ恒久テストを `tests/` へコミットしていない** — 恒久化は別タスク)
...
- [ ] **負例を 3 つの型へ分け、それぞれの確かめ方で示している**(**規律 5 の射程** — 17 周目 `P1-4`):
      **① 封印資産の値を変える負例** = **当該 validator の関数を直接呼んで投げる**
      (**`check_authz_catalog.py` 全体を回して red を見ない** — **digest 不一致だけでも必ず red になるため**)
      **② 検査コードを変異させる負例**(走査の無効化・深さ制限・対象集合の縮小)= **当該コードを壊すと負例が red でなくなる**
      **③ CLI の副作用を見る負例**(通常検証で reseal されない)= **実行前の seal と、各実行後の seal がバイト不変であることを示す**
      (**「1 回目と 2 回目の差分」では足りない** — **1 回目に誤って reseal し 2 回目が無変更な実装でも通る**。19 周目 `P1-4`)
- [ ] **`boundary-proposal.json` の 3 境界が 4 節の閉じた表と完全一致**
      (**`REPRESENTATIVE-MANAGEMENT` の `TSK-250` は不変**)
- [ ] **`deferred_equivalence_contract.owner_task_id` が実 ID と完全一致**
- [ ] **owner の母集団を base の資産から導いた**(**キー名に `owner` または `task_id` を含む全箇所 — 実測 5 箇所**)
- [ ] **`trust_boundary.verification_owner_task_id` の検査を足した**(**現行 validator が見ていなかった**)
- [ ] **`proposal_status` と 2 件の `status` が確定値**・**`frozen_value` / `alternative_value` / `review_id` は不変**
- [ ] **`S-5` の母集団を base の全 leaf から導き、「変える / 変えない / 削除する」へ分類した**
- [ ] **base に無い leaf が変更後にある場合は red**(**新設は計画違反**)
- [ ] **`scope` の 4 キーが確定値**で、**検査器の literal が資産と一致**し、**4 キーそれぞれの負例がある**
- [ ] **4 つの導出器それぞれに全数性試験があり、分岐ごとに探針を割っている**(**core `f` はステップ 1 の DoD**):
      **① owner** = **述語の分岐**(`owner` だけ / `task_id` だけ)と**再帰の分岐**(容器の型の並び)の**両方**
      **② `S-5` の leaf** = **容器の型の並びから生成した探針すべてで +1**
      **③ seal の全 leaf**(`2-e`)= 同じ形
      **④ `g`** = **容器の型の並びを全部訪れる**・**入口の 3 分岐**(`sealed_assets` だけ / `input_assets` だけ / seal 自身を外す)**にも探針**。**深さ `d` = base の凍結 15 パスの最大容器深さ**を**テストが計算する**(**定数で持たない**)
- [ ] **`g` の導出で資産と検査器の両方を base へ固定した**(**作業コピーの検査器を使うと `g = ∅` になる**)
- [ ] **`S-5` の全 leaf それぞれで逐項 red**・**資産だけ変えても検査器だけ変えても red**
- [ ] **seal の全 leaf それぞれに逐項の負例がある**(**`2-e` の表** — 実測 51 leaf)
- [ ] **`g` の対象資産を base の seal から導いた**(**凍結 15 パス** — 差分でも列挙でもない)
- [ ] **`g` の出力する全配列に行数と要素一意性の検査を置いた**(**数を計画書から取らず `g` を回して得る**)
- [ ] **`g` の出力それぞれで、多重度検査が red を出す**(**seal の digest 不一致で判定していない** — **多重度検査を外すと負例が red でなくなる**)
- [ ] **既に red だった配列の検査を変えていない**
- [ ] **層 ②: 凍結 15 パスの全配列 + 全 JSON キー対を走る恒久検査を `tests/` へ置いた**
- [ ] **その検査が再封印の直前にも走り、重複 0 件であることを確認した**
- [ ] **走査自体を壊した 6 種の負例で red**(再帰しない / `json.loads` の last-wins / 無効化 / **対象資産の集合を狭める** / **深さ制限つきの走査** / **分岐単位の例外握り潰し**)
- [ ] **凍結 15 パスを同じ入口へ通し、対象パスを base の seal から導いた**
- [ ] **走査が実際に到達した配列パスの集合を assert している**
- [ ] **負例の資産は一時ディレクトリの写しで作り、本物の凍結資産を変異させていない**
- [ ] **層 ① と層 ② の射程の違いを計画書で書き分けた**(① = 検査器の `g` 出力 / ② = 凍結 15 パスの恒久検査)
- [ ] **別タスクへ送った 1 件を計画書に書いた**(**重複 JSON キー** — 観測層を下げる作業が要る)
