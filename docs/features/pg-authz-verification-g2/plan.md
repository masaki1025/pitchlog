...
created: 2026-09-09
計画レビュー周回: 25        # 指摘反映を伴うレビュー 1 周ごとに +1(収束確認周は数えない。/plan が更新)
...
**深さ `d` までの容器の並びは `2^1 + ... + 2^d` 通りで閉じる**(**深さ 3 なら 14 通り** — **言語の仕様から来る集合で、私が選んでいない**)。
**14 通りそれぞれの末端へ配列を置いた合成資産を作り、走査が全部を訪れることを示す。**
...
**実測(2026-09-12)— 変更する 3 資産の非空配列 95 本のうち `g` の出力は 15 本。**
**凍結 15 パスへ広げると、測定できた 5 資産で 16 本**(**+1 本**)。
...
- [ ] **本ステップの全負例が、当該 validator の関数を直接呼んで投げることを確かめている**
      (**規律 5** — **`check_authz_catalog.py` 全体を回して red を見ていない**。
      **封印資産の変異は `seal` の digest 不一致だけでも必ず red になるため**)
...
- [ ] **4 つの導出器それぞれに全数性試験がある**(**core `f`**= 述語を満たす検査器を 1 本足すと母集団が 2 増える / **owner**= `*_owner_task_id` を 1 つ足すと 1 増える / **`S-5`**= base へ leaf を 1 つ足すと 1 増える / **`g`**= **容器の型の並び 14 通り**を全部訪れる)
