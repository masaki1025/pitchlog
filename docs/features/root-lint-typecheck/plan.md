...
| 除外するもの | 理由 |
| --- | --- |
| **`.claude/**` の検査**(hooks 7 本 + `codex_run.py`) | ruff **73 件** / ty **19 件**。**当初「`codex_run.py` の 5 件は実バグ疑い」と報告したが、実測により訂正する**(2026-08-23): 5 件はいずれも **`die()` の注釈が `-> None`** であること(実体は `sys.exit(2)` で戻らない)に起因し、**`-> NoReturn` へ直すと 19 → 14 件・`codex_run.py` は 7 → 2 件になる**。**実バグではなく注釈の欠陥 1 件**である。残る 14 件はすべて `sys.stdout.reconfigure`(typeshed が `sys.stdout` を `TextIO` と型付けするための誤検知)で、いずれも `try` / `except Exception` の中にあり実害はないが、**コードでは直せないため `# ty: ignore` か設定での扱いを人間が裁定する必要がある**(2026-08-19 の裁定)。**「優先度 高相当」の根拠(実バグ疑い 5 件)は失われたので、優先度は再判定を要する**。いずれにせよ**本タスクに混ぜると差分がレビュー不能**なので**別 follow-up** で扱う。**台帳 H-13 は「部分対応」に留める**(「対応済み」にしない) |
