...
| 3 | TSK-235 | `git_guard` | **`git -C "$W" fetch && git -C "$W" rebase`** → 「現在のブランチを解決できませんでした」。**両方 `-C` で明示パスを持つ正当な形** | 記載どおり |
...
- **回避はいずれも「正しい書き方に直す」ではなく「ガードを避ける」**(Write ツールへ切替 / 分割実行 /
  別の取得経路)。**1・2・5 は heredoc 本文**、**3 は複合コマンドの解析失敗**、**4 は 4 文字の部分一致**
...
- **昇格条件 / 対応案**: (a) **heredoc 本文を走査対象から外す**
  (b) **`git -C <絶対パス>` の複合を許可する**(`-C` があればブランチ判定は可能)
