...
| ⑦ | **時点の事実を記録へ焼き込むと後の変更で古くなり、訂正経路が無いと直せない。** 対策は件数や固有名詞でなく**導出される定義**で書くこと | 実例 3 件: (a) **TSK-450 のカードが対象を「7 件」と固定していた**(訂正経路を定めるカード自身が同じ型で古くなりかけた — TSK-431 が是正) / (b) **本 PR の `movement_fact`** が資産の宣言を根拠として引いている / (c) **③ の marker**。**(b) は TSK-450 の種別 B として射程に入った** |
