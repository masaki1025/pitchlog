...
**marker の送り先**: **TSK-450「凍結履歴のセンチネル 7 件を訂正する経路を定める」**
(`https://app.notion.com/p/3e593b75e68781c4aadad43595dbaccc`・未着手)。**原典で確認した。**
既存記録の訂正経路が設計書 7.7 に無く(7.7-2 は追記のみを定める)、
**条文の改訂か PO 裁定が先**という整理である。

**ただしカードの対象は「v1 の既存 7 件だけ」と明記されている。**
**本 PR が足す 2 件は含まれていない。** 本 PR が TSK-431 より先にマージされれば、
本 PR の 2 件は 431 の merge-base 側 prefix に入って v1 扱いになるため、
**TSK-450 の対象は 7 件から 9 件へ増える**。**カードの更新が要る**(起票は 431 側)。
...
**射程の切り分け**: 431 は「TSK-450 のカードへ 2 件並べる」としているが、
**TSK-450 の DoD は承認欄のセンチネル(v1 の既存 7 件)に限定されており、
`movement_fact` の陳腐化は現在の DoD に含まれない**。
「既存記録を訂正できない」という根は同じだが、**カードの射程を広げる判断は人間のもの**である。
