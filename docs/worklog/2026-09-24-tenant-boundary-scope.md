...
**marker の送り先**: **TSK-431 によれば、この marker 問題は TSK-450 へ送り出し済みで、
本 PR の 2 件も同じタスクの射程に入る**(既存記録の訂正経路が確定ゲートの決定表に無いため)。
**431 の主張であり、こちらでは TSK-450 を実測していない。**
...
| # | 候補 | 典拠 |
| --- | --- | --- |
| ① | **新設した任意パラメータが未供給のとき、機構が黙って無効化される。** 「本番経路が必ず供給すること」をテストで固定する手当てが有効だった | ステップ 5 の差し戻し。**TSK-431 は同型を敵対レビュー 1 周目まで残した**(431 の自己申告) |
| ② | **「関数は正しいが本番経路へ結線されていない」型は、機構を壊す変異を入れて本番経路テストが落ちるかで検出できる。** 関数を直接呼ぶテストでは 1 件も捕まらない | 本 PR の M1〜M4 の実測。TSK-431 の 1 周目 P0 2 件がこの型 |
| ③ | **marker に固有名詞(PR 番号)を焼き込むと、後続の未承認記録が自分の出所を名乗れない。** append-only 制約により定数の一般化も塞がれる | `check_tenant_boundary_bypass.py:64`。**TSK-431 によれば TSK-450 の射程** |
...
**append-only により訂正できない** — これも ③ と同じく TSK-450 の射程である。
