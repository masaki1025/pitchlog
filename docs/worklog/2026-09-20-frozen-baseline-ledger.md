...
### ステップ 6/8 — `.claude/core-areas.json` へ未保護の新設物を登録(設計書 6.3-⑤)

**成果物**: `.claude/core-areas.json` の `guard_paths` へ 10 件追加(42 → 52)/ `tests/test_core_guard.py` の `AUTHZ_GUARD_PATH_ADDITIONS` へ同じ 10 件を宣言。

#### 未一致集合は機械導出した(件数を先置きしない)

`git diff --diff-filter=A origin/develop...HEAD` で新設ファイルを導出し、`core_guard.matched_paths()` へ通した:

| 区分 | 件数 | 内訳 |
| --- | --- | --- |
| 新設 | **16** | — |
| 既存保護に一致 | **2** | 台帳 2 ファイル(領域 glob `contracts/authz/*`) |
| 未一致 | **14** | — |
| うち**登録した** | **10** | 検査器 2・データ 1・テスト 7 |
| うち**登録しない** | **4** | docs 3 + `.gitkeep` |

> **草案は「新設 4 ファイル」としていたが、機械導出では 10 件だった。**承認済み計画書が「件数を先置きしない」としているのが効いた。

#### 登録の機構(自分で確認)

`guard_paths` は **凍結 base 版 ∪ `AUTHZ_GUARD_PATH_ADDITIONS`** と exact 一致が要求される(`test_guard_paths_population_remains_unchanged`)。base 側は `AUTHZ_GUARD_BASE_REVISION`(`56c281c4…`)から **git で読む凍結値**なので編集できない。

→ **`core-areas.json` に足すだけでは通らず、`AUTHZ_GUARD_PATH_ADDITIONS` にも宣言が要る。**空洞化していない。

**`AUTHZ_GUARD_BASE_REVISION` は動かしていない**(`pending_removal` の 1 件・移設は後続タスク)。

#### 台帳を登録しない判断 — ①だけでなく②まで実測した(自分で)

「登録しなくても検知される」だけでは **glob が効いている証拠にならない**。実設定を複製し `path_patterns` から `contracts/authz/*` **だけ**を外して `matched_paths()` を再実行した:

| 状態 | 台帳 2 ファイルの検知 |
| --- | --- |
| glob あり | **2 件**(`guard_paths` 完全一致は False = 二重登録していない) |
| **glob を外す** | **0 件** |

**glob が実際の検知要因である。**二重登録は不要。

#### 除外 4 件の根拠(機械で決着)

| 除外 | 根拠 |
| --- | --- |
| docs 3 件 | **既存 `guard_paths` 42 件のうち docs 配下は 0 件**。docs は docs 側の確定ゲートが担当する |
| `tests/frozen_negatives/.gitkeep` | **0 バイト**の位置マーカーで検査経路ではない |

#### 登録 10 件が既存の類型に収まることの確認(機械で決着)

既存 `guard_paths` の内訳は 検査器スクリプト 10 / そのテスト 14 / 検査器が読むデータ 7 / 設定・workflow 7 / その他 tests 5 / ルート設定 4 / その他 scripts 3 / fixture 2。**登録 10 件はすべてこの既存類型に収まり、新しい類型を持ち込んでいない。**

#### 人間の逐行確認②(設計書 6.3-⑤)— 実施記録

**実施: 2026-09-23・山田正輝。**

突合シートを**機械生成**して提示した。**機械が確定させられる分は私が検証してチェック済みにし、判断だけを残した**(PO の指示 — 「確認して明らかに問題ないものはチェックしてほしい」)。

**機械が確定させた分**(確認者の目視対象外):

| 項目 | 結果 |
| --- | --- |
| 新設 16 件の分類と取りこぼしの有無 | ✅ 取りこぼしなし |
| `core-areas.json` と `AUTHZ_GUARD_PATH_ADDITIONS` の exact 一致 | ✅ 10 / 10 |

**人間が確認した分**:

| | 内容 | 結果 |
| --- | --- | --- |
| A | 登録 10 件をコア領域の検査経路として扱う(既存類型に収まる) | ✅ 承認 |
| B | docs 3 件と `.gitkeep` を除外する | ✅ 承認 |
| C | 台帳 2 件は glob 一致で足りるので二重登録しない | ✅ 承認 |

**登録の実内容が、提示した 10 件・除外 4 件と一字一句一致していることを突合したうえでの承認である。**

**機械が保証しないこと**: `.claude/core-areas.json` は **`check_plan_docs_sync.py` の除外対象**で、計画書の「影響する正本」宣言との機械突合を受けない。**ここは人間の確認が唯一の担保**である。

#### 委任先が報告した制約の衝突(記録)

`test_propagation_checker_and_claude_files_are_unchanged` は **`git diff --name-only HEAD -- .claude/` が空であること**を要求する。本ステップは `.claude/core-areas.json` を変更するので、**「コミットしない」という依頼と同時には満たせない**。

委任先は**テストを書き換えず・実ブランチへコミットせず**、複製へ同じ 2 ファイル差分を適用して一時コミットし、**コミット後は 1446 passed** であることを確かめて報告した。妥当な対処。

> **後続ステップへの申し送り**: `.claude/` を変更するステップでは、委任時点の全件実行が必ずこの 1 件で落ちる。**コミット後に再実行して判定する。**

#### 範囲の確認

差分は**許可した 2 ファイルのみ**。委任前スナップショットとの突合で、検査器 2 本・`check_authz_catalog.py`・allow-list・台帳が**すべて無変更**であることを確認した(**前 2 回にあった無断変更は、今回はゼロ**)。

#### 合格条件の判定

| 条件 | 結果 |
| --- | --- |
| 導出結果と登録内容が exact-set 一致 | ✅ 自分でも `matched_paths()` を回して確認 |
| `core_guard.py` が全新設ファイルを検知 | ✅ |
| 台帳は登録せず、**glob を外すと検知されなくなる**ことを実測 | ✅ **2 件 → 0 件**(自分で実測) |
| 既存 guard テスト green | ✅ `tests/test_core_guard.py` **143 passed** |
| `ruff check .` / `ty check` / `core_guard.py` / `--invariants-only` | ✅ |
| **人間の逐行確認②** | ✅ **2026-09-23・山田正輝** |

### ステップ 7/8 — 最終コア差分の逐行確認(bootstrap の信頼根の完成)

**対象** = 凍結基準台帳の規範状態 5 つ / **範囲** = final head `f668bf08eef0` 全体 / **方法** = 台帳の記録値と、突合元(base / final head)から機械抽出した実値の照合。

**実施: 2026-09-24・山田正輝。**突合シートを**機械生成**して提示し、**機械が確定させられる分は検証してチェック済みにし、判断だけを残した**。

#### 規範状態 5 つと突合元

| # | 規範状態 | 突合元 | 確認 |
| --- | --- | --- | --- |
| ① | `acceptance` | **base**(確認①で実施済み・以降**不変**) | 引き継ぎ |
| ② | `movement_rules` | **base**(同上) | 引き継ぎ |
| ③ | `declarations` | **base**(同上) | 引き継ぎ |
| ④ | `placements` | **base** で宣言・**final head** で真になった | **本確認** |
| ⑤ | `implementation_bindings` | **final head**(head の新設コード) | **本確認** |

**確認①(`1f5d905`)の時点の台帳と final head を機械比較し、①〜④が 1 バイトも動いていないことを確認した。**変化したのは `implementation_bindings` の **digest 2 件のみ**。`history` は 1 件のまま。

#### base が 3 回動いたが、①の結論は維持された

確認①は base `5e9ffd9` に対して行った。その後 **`871fd971`(PR #72)→ `e6bc0cc`(PR #75)** と 2 回動いた。**毎回、表記ではなく中身で確かめ直した**:

| 前提 | 確認方法 | 結果 |
| --- | --- | --- |
| bootstrap 経路が続くか | 新 base に台帳があるか | ✅ **無い**(3 回とも) |
| `movement_rules` の典拠(設計書 7.7) | **節の sha256** | ✅ `d0b95faa…` で**不変** |
| 凍結対象 7 件の値 | 資産から機械抽出 | ✅ `24ef4fcc…` で不変 |
| 差分閉包の early-return 条件 | `TSK-270-GROUP-2` の件数 | ✅ 0 件 |

**行番号や版表記ではなく節の内容ハッシュと実値で見ているので、他 PR の編集で表記が動いても判定がぶれない。**

#### ④ `implementation_bindings` — final head で突合(機械で確定)

| 項目 | 結果 |
| --- | --- |
| `code_assets` の digest 2 件 | ✅ final head の実ファイルと一致 |
| `strategy_keys` | ✅ 実 registry と一致(1 件のまま) |
| `registry.symbol` | ✅ 実在 |

束ねるファイルの集合(`CODE_ASSET_PATHS`)は**検査器側の定数**なので増減は機構で閉じている。**浮くのは digest 値だけ**で、それは実行時に再計算して照合される。

#### ⑤ `placements` — 宣言が final head で「真」であること(機械で確定)

JSON は確認①の時点から不変だが、**その宣言が真になったのはステップ 5**(実消費が入るまでは「これから台帳に置く」という宣言だった)。

| 確認項目 | 結果 |
| --- | --- |
| 移設元に直書きが残っていない | ✅ |
| 台帳リーダを import している | ✅ |
| 読み取り関数の参照が 2 箇所(定義 1 + 消費 1) | ✅ |

**宣言が飾りでないことの実測**(ステップ 5・復元はバイト一致): 台帳の値を誤った値へ変えると `check_authz_catalog.py` が **exit 1**、台帳読み取りを 40 桁定数へ戻すと**走査が red**。

#### 人間が確認した分

**この 5 状態で本タスクの凍結基準を表現しきれているか** — ✅ **承認**(2026-09-24・山田正輝)。

#### 機械が保証しないこと(明示したうえでの確認)

1. **本 PR は base に台帳が無い初回 bootstrap である。**PR は head 側の検査器を実行するので、**初回に「検査器が正しい」ことを機械は保証できない**。だから信頼根が人間の確認になる
2. 台帳は **base の最新性を保証しない**。`github-setup.md` 2 章 手続 2 の「base SHA の 3 点一致と head の拘束」が担う
3. **`target_correspondence`**(どの対象がどの値を持つか)は照合が集合どうしのため**観測できない**。`oracle_input` は 7 対象すべて同値なので空虚で、多値系列を扱う**後続タスクの DoD へ送る**(2026-09-21 合意)
...
### 差し戻し修正 — CI 失敗 1 件 + 敵対レビューの P0 1 件・P1 3 件・P2 1 件

#### CI の backend 失敗(私の見落とし)

ステップ 5 で `scripts/check_authz_catalog.py` へ同階層 import を足したが、このファイルは **`importlib` でパス指定ロード**されることがあり、`scripts/` が `sys.path` に無いと解決できない。**backend の収集が 4 errors で中断**した。

**harness 側のローダは直したのに backend 側の同じローダを直していなかった。**「backend は射程外」を「backend のテストは壊れない」と取り違えた。**射程は移設対象を限定するが、変更したファイルの影響範囲は限定しない。**

さらに **`cd backend && uv run pytest` を一度も回していなかった**。AGENTS.md は harness と backend を別コマンドとして挙げ、CI も別ジョブである。既存候補「**検証コマンドを人が選ぶと CI が走らせるコマンドとの差分が黙って残る**」の再発で、**差は「オプション」でも「層」でもなく root** にあった。

**直し方**: ローダを 1 つずつ直さず**検査器を自己充足させた**。ロードしうる箇所は複数あり今後も増えるので、ローダ側の個別対応は「母集団を人が列挙する検査は射程が動くと黙って古くなる」型になる。

#### 敵対レビュー — `P0` 受理不能を exit 0 で通していた

**自分で 4 経路すべて再現した**:

| 条件 | 修正前 | 修正後 |
| --- | --- | --- |
| `base.ref != develop` + でたらめな SHA | **exit 0** | **exit 1** |
| `base.ref == develop` + でたらめな SHA | **exit 0** | **exit 1** |
| **`.git` 不在** + `base.ref != develop` | **exit 0** | **exit 1** |
| HEAD の親が 1 つ | **exit 0** | **exit 1** |

**「評価しない」が環境の異常まで飲み込んでいた。**7.7-3 の「判定の保留・省略・中立では満たさない」に反する。

> **原因はステップ 3 の依頼文**。私は「受理遷移検査を行わない。その旨を**標準出力へ明示**する(黙って通らない)」と書いた。**明示しても exit 0 なら CI では通る。**「黙って通らない」を「標準出力に書く」と取り違えていた。
>
> **これは PR #72 が塞いだ「必須チェックはジョブが緑かしか見ないので `if: false` でステップを殺しても通過する」と同じ層の錯誤である。**あちらは条件でステップを殺しても緑、こちらは理由を出力しても緑。**どちらも「人間が見れば分かる」形を機械の保証と取り違えている。**

**直し方**: 親が 2 つでないのは **red**(`pull_request` で HEAD が synthetic merge でないこと自体が異常)。`base.ref != develop` の「評価しない」は policy として維持するが、**その判断の前に** `.git`・event の整形式・base/head SHA の解決可能性を検証する。**「評価できる状態にあることを確かめたうえで、対象外だから評価しない」**という順序にした。

#### `P1` — dispatch の変異テストが証明になっていなかった

`identity` / `granularity` のテストが **registry 全体を 1 要素へ交換**していたため、「キーの存在だけ確認し**常に registry 先頭を実行する**」実装でも通っていた。前任の「宣言はあるが実装が使っていない」を防ぐ退行試験になっていなかった。

**固定した 2 戦略 registry と 2 系列の宣言交換**で検証する形へ変えた。**逆向き変異を自分で入れて、当該 2 テストが落ちることを実測した**(復元はバイト一致)。

> **`frozen_targets` の②は自分で再現したのに、この 2 つは委任先の報告を受け取ったままだった。**同じ厳しさを当てていなかった。

#### `P1` — N29 が `guard_paths` から漏れていた

追随修正で追加した負例が `guard_paths` に無かった。

> **ステップ 6 で `matched_paths()` による機械導出を用意し「件数を先置きしない」ことを合格条件にまでしたのに、ファイルが増えた後に回し直していなかった。**既存候補「母集団を人が列挙する検査は射程が動くと黙って古くなる」の**変種** — **今回は機械で導出したのに実行時点が古かった**。**導出の仕組みがあることと、それが最新であることは別である。**

#### `P2` — 非保証として明記(PO 判断 2026-09-24)

隣接文字列連結など、**ソース上に連続して現れない形で再構成された定数は走査できない**。

```python
BASE = "24ef4fcc682b42b504ed" "c1d6264d380760c54929"   # 実行時は 40 桁だが走査に掛からない
```

**走査自体は直さない。**ステップ 4 で「AST 完全一致にすると docstring 埋め込みを見落とす」ことを実測して部分一致を選んだ経緯があり、両立には設計変更が要る。検査器のヘルプと PR 本文へ明記した。**PO 判断で台帳の候補へは追記せず、本 PR 限りの記述とする。**

#### あわせて — N15 を負例から正常系へ

「`base.ref != develop` で受理検査が走らない」は **exit 0 を要求する正常系**であり負例ではなかった。ステップ 4 で走査の正常系を分離したのと同じ扱いにし、`tests/test_frozen_baseline_acceptance_rules.py` へ移した。代わりに「壊れた環境なら red」「親が 2 つでなければ red」の負例を足し、**負例は 30 件**になった。

#### 検証(すべて自分で実測)

| 項目 | 結果 |
| --- | --- |
| `P0` の 4 経路 | **すべて exit 1**(修正前はすべて exit 0) |
| dispatch の逆向き変異 | 「常に先頭を使う」で **当該 2 テストが落ちる** |
| `guard_paths` の未一致 | **0 件**(新設 18 件すべて保護下) |
| backend の収集 | **585 件・エラー 0**(修正前は 4 errors で中断) |
| harness 全件(コミット後) | **1621 passed** |
| 触れないと明示した範囲 | **無断変更ゼロ** |

#### 人間の逐行確認のやり直し — 実施記録

**実施: 2026-09-24・山田正輝。**ステップ 7 の記録「確認後に差分が発生したら再確認する」に従い実施した。

**差分は最小限だった**(機械で確認):

| 規範状態 | ステップ 7 の確認以降 |
| --- | --- |
| `acceptance` / `movement_rules` / `declarations` / `placements` | **不変** → 前回の確認を引き継ぐ |
| `implementation_bindings` | **`check_frozen_baselines.py` の digest 1 件のみ** |
| `history` | 1 件のまま |

`guard_paths` は **10 → 12 件**(漏れていた `test_frozen_baseline_ci_dispatch.py` + N15 の移設先 `test_frozen_baseline_acceptance_rules.py`)。`core-areas.json` と `AUTHZ_GUARD_PATH_ADDITIONS` が **12 / 12 で exact 一致**、取りこぼし 0 件。

> **検査器を大きく直したのに規範状態が digest 1 件しか動かなかった。**台帳が「何を凍結するか」を実装から分離できている証拠である。宣言と実装が絡んでいれば `declarations` や `movement_rules` まで動いていたはずである。

**機械が保証しないことは 6 件へ**(`P2` の追加分を含む)。
