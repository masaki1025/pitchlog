...
## ステップ 3: 追跡是正の申し送り(2026-09-01)

### TSK-270 へ申し送る例外表の有効化

要件書 `NFR-018` 例外表について、次の 2 セルの更新を TSK-270 の要件主張母集合修正へ申し送る。

- **検証テスト**: `frontend/src/lib/courseCoordinateContract.spec.ts`
- **状態**: `有効`

**終了証跡欄と失効判定者欄は変更しない**。版は上げず、7.6-3 前段(実装追随の節更新)として扱う。

### TSK-228 へ申し送る `core-areas.json` の候補パス
...
設計書 6.3 の境界定義表は、状況計算の含む側に「座標変換」を明記している。この境界に基づく候補として、`core-areas.json` の paths 充填を担う **TSK-228** へ引き渡す。

### 構造比較の実行記録

使い捨てスクリプト `/tmp/tsk233_step3_structure_check.py` で、HEAD 版と作業版を節・表行・バイト列として比較した。

```bash
UV_CACHE_DIR=/tmp/codex-uv-step3 uv run python /tmp/tsk233_step3_structure_check.py
UV_CACHE_DIR=/tmp/codex-uv-step3 uv run python scripts/check_docs_status.py
```

結果:

1. **PASS** — 台帳は `H-61` 節と変更履歴 1 行以外が HEAD とバイト一致
2. **PASS** — `H-61` の状態語は不変で、`H-*` の見出し集合・順序も不変
3. **PASS** — ADR は後続表の対象行と変更履歴 1 行以外が HEAD と一致し、`D-12` もバイト一致
4. **PASS** — 旧 worklog は `new_bytes.startswith(old_bytes)` を満たし、追記部分は非空
5. **PASS** — `docs/requirements/**` と `contracts/**` は HEAD から無変更
6. **PASS** — 台帳・ADR の変更履歴はいずれも「版は上げない」(7.6-3 前段)を明記
7. **PASS** — `check_docs_status.py` は exit code 0(標準出力なし)

補助検査として、`docs/README.md` は対象 2 行の日付セルだけが `2026-09-01` へ変わり、版セルを含む他セルと他行が HEAD と一致することも assert した。

## ステップ 3 の Notion 処理(Claude が MCP で実施 — 2026-09-01)

**`[手動・外部]` の合格条件に対応する実値を記録する。**

| # | 対象 | 実値 |
| --- | --- | --- |
| 1 | **卒業タスクの起票** | **`TSK-275`** — `COURSE_COORDINATE_SIZE` の逐語移植例外の卒業(NFR-018 (a) への収束 — ADR-003 D-12 の 4 操作を原子的に)<br>`https://app.notion.com/p/3cd93b75e6878141be4cc5b816fb4d19`<br>page id `3cd93b75-e687-8141-be4c-c5b816fb4d19` / 取得日時 2026-09-01 / ステータス `未着手` / 優先度 `中` |
| 2 | **`TSK-228` への引き渡し** | コメント id `3cd93b75-e687-81df-8449-001d5426b35b`。候補パス 3 件(`courseInputView.ts` / `display_geometry_263_v1.json` / `courseCoordinateContract.spec.ts`)と根拠(設計書 6.3 の境界定義表)を記録 |
| 3 | **`TSK-270` への申し送り** | コメント id `3cd93b75-e687-81ce-8a35-001de470607a`。例外表 2 セルの変更内容・版を上げない根拠・実測した連鎖・インデント表の採取欠陥(P0 候補 8 件目)を記録 |
| 4 | **`TSK-233` の DoD 更新** | 「要件書 NFR-018 の例外表の状態が「有効」になった」に取り消し線を引き、**TSK-270 へ合流**と明記。**ADR-003 の書き分け**と**卒業タスクの起票(TSK-275)**を DoD へ追加 |

**`TSK-275` の本文には 2 節の必須事項 7 件をすべて記載した** — ① NFR-018 (a) を満たす生成物
② 座標ベクタ ③ ADR-003 D-12 の 4 操作を原子的に ④ 判定者 = システム管理者
⑤ 前提タスク(`TSK-233`・`TSK-238`)⑥ **画面受入タスクは未起票であり、その起票が着手前の前提条件**
⑦ 終了証跡欄への PR URL 記録は本タスク(TSK-275)の責務。

### 起票時に踏んだ機構の穴(記録)

**`notion-create-pages` でプロパティを指定してもタスク DB の行にならなかった**
(`ancestor-path` が空・タスクID が付かない)。**`notion-move-pages` で data source へ移動して
`TSK-275` が採番された**が、**移動でプロパティ(ステータス・優先度・プロジェクト・担当者)が失われた**ため
再設定した。**重複起票は作っていない**(TSK-256 の前例を踏まない)。

### 併せて判明したこと

**他の担当者が `TSK-273`「確定ゲート収束規則の再設計(ハーネス設計書 7.3 改訂)」を起こしている。**
本タスクの計画レビューが **4 周**かかり、**3 周目でレビュー自身が「H-22 型の閉じた輪」と判定して
打ち切りを提案した**経緯は、**そのタスクの入力になり得る**。
...
| ステップ | コミット | 成果物 |
| --- | --- | --- |
| 1 | `b89fb03` | **契約一致テスト** `frontend/src/lib/courseCoordinateContract.spec.ts`。契約 JSON を直接 import し `COURSE_COORDINATE_SIZE` と `DISPLAY_COORD_SIZE` がともに `geometry.size` であることを固定。**リテラル `263` を書かない**(263 と `version` の固定は既存 `displayGeometry.spec.ts` が所有) |
| 2 | `4b6a376` + `12b4121` | **射程の変更と記録**。要件書の更新が母集合の凍結に当たり、追随が **8 資産 + ハーネステスト + 複数の基準コミット**へ連鎖することを実測。正本を `develop` の状態へ戻し、**TSK-270 へ申し送った** |
| 3 | `154bb84` | **追跡の是正** — 台帳 `H-61` の現況更新 / **`ADR-003` 後続表を「有効化 = TSK-233 / 卒業 = 新 ID」へ書き分け** / 旧 worklog へ訂正追記 / 索引の現行化 / **卒業タスク `TSK-275` の起票** / `TSK-228` への引き渡し |

**製品コードは 1 行も変更していない。** `develop` との差分は **`.spec.ts` 1 本 + 文書 6 本**。

### 正本へ反映したもの

- **`docs/development/harness-evaluation.md`**: `H-61` の現況更新 / **`H-85` を新設** /
  **`H-22`・`H-42` へ実測** / `## 候補` へ 1 件 / 変更履歴 1 行。**版は上げない**(7.6-3 前段)
- **`docs/adr/ADR-003-domain-calc-method.md`**: 後続表の 1 行を書き分け + 変更履歴 1 行。**版は上げない**
- **`docs/README.md`**: 台帳行・ADR 行の最終更新日と説明を現行化
- **`docs/requirements/**`**: **反映なし** — 例外表の 2 セル更新は **TSK-270 へ合流**
- **`contracts/**`**: **反映なし**(1 バイトも変更していない)

### ハーネス運用評価台帳への追記(PR 作成者の判断 — 該当する)

**`H-85` を新設した**(機構の穴・追跡優先度 通常):
**oracle の入力凍結が、正本の 1 文字の変更を 8 資産 + ハーネステストの更新へ拡大する**。
**TSK-213 / 214 / 215(要件書 v2.2〜v2.4 改訂)も同じ壁に当たる**ため単発事象ではない。

**既存項目へ実測を追記した**(**新規採番は `H-85` の 1 件だけ**):

- **`H-22`**(自分が新設した規定の穴を次の周が突く)— 計画レビュー **4 周**。
  **3 周目の指摘 3 件はすべて 2 周目の修正箇所に対するもの**で、
  **レビュー自身が「H-22 型の閉じた輪」と判定して打ち切りを提案**した。
  **4 周目を差分限定の最終周とし「新しい観点を持ち込むな」と明示的に禁じたら P1 1 件まで落ちて収束**した。
  → **`TSK-273`(確定ゲート収束規則の再設計 — 設計書 7.3 改訂)の入力になり得る**
- **`H-42`**(是正作業が次の周の指摘を生む)— **3 例目**。
  **`H-85` の追記位置を `## 機能した点` の文字列一致で決めたところ、
  その最初の出現が変更履歴表のセル内**(**2026-08-15 の行が本項の是正内容を引用しているため**)
  **だったため偽の見出しを作った**。**本項を読んだ上で本項の欠陥を踏んだ。**
  構造検証(行頭 `^##+ ` だけを拾って節の帰属を確認)で検出し同一 PR 内で修復した

**`## 候補` へ 1 件**(切り分け未了・1 件のみの観測):
**Notion MCP の起票がタスク DB の行にならず、移動でプロパティが失われる**。

### 検証コマンドと結果(すべて Claude が worktree で実行)

- `frontend`: `pnpm exec prettier --check .` / `eslint .` / `vue-tsc --noEmit` green /
  `pnpm test -- --run` **4 ファイル 11 テスト**(9 → 11)
- ルート: `uv run ruff check .` / `uv run ty check` green / `uv run pytest tests/` **843 passed**
- `check_docs_status.py` / `check_doc_coverage.py` / `check_design_propagation.py` すべて OK

### 独立検証(委任先の報告を受け取らずに実測した)

| 対象 | 結果 |
| --- | --- |
| ステップ 1 の 4 変異 | **変異 1・2 で `T-COURSE` だけ失敗** / **変異 3 で新規 spec 単体 green・全体で `T-EXISTING` 失敗** / **変異 4 で `T-DISPLAY` 失敗**。**変異 3 では `StrikeZone` の 4 テストも失敗** — 「全体の red では新規テストの有効性を証明できない」を実測で裏づけた |
| 実行時の変異差分 | 各変異で変更されたパスが意図した箇所だけ(worklog の除外を差し引いて確認) |
| 復元検査 4 点 | working tree / `git diff --cached HEAD` / 種別・mode・SHA-256 の manifest / porcelain v2 すべて一致 |
| ステップ 2 の構造比較 | 例外表の**変更列集合が `{検証テスト, 状態}` と完全一致**・**残り 4 セルがバイト一致** |
| ステップ 3 の構造比較 | 7 点すべて PASS(台帳は `H-61` 節と変更履歴だけ・`H-*` の順序不変・ADR は 3 行・`D-12` バイト一致・旧 worklog は追記のみ・要件書と `contracts/` は無変更) |
| 母集合の連鎖 | 派生 3 資産 + 各 lock + `oracle-seal` + `tests/` の 4 箇所のハードコードを実測で特定 |

### 形式上の残余(隠さず記録する)

**要件書 `NFR-018` の例外表は「有効化待ち」のまま**で、**形式上は本要件の違反状態が続く**。
**ただし実リスクは解消済み** — 改善台帳 `I-17` の「座標系の二重管理による**静かなデータ破壊**」は
ステップ 1 のテストで検出可能になった(契約 JSON の `size` を変えれば必ず落ちる)。
**閉じるのは TSK-270 の母集合修正**(申し送り済み)。
