...
- develop マージ後: v1.2 統合マージコミットの**第一親 = アンカー**を確認 → その SHA を head とする `develop → main` ベースライン PR(本文に対象 SHA・アンカーを記録)→ CI 確認 → **マージ直前に develop・PR head の不変を再確認**(変化していれば未使用失効処理: PR クローズ・Notion「取り下げ」・実測 SHA 記録)→ 人間マージ → main 側マージコミット SHA を PR 本文・Notion に記録 → workflow_dispatch 起動確認 → /task-done
