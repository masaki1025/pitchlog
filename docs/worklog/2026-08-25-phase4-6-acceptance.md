...
- **最大の発見**: 失効対象パスの正本は `default: "invalidating"` で、allowlist は 3 ディレクトリのみ。**設計書・要件書・`.github/workflows/` はすべて失効対象**であるため、win-setup 選定の記録は **`T` を採るより前**に置くことが強制される(research.md E 節)。実装ステップの並びはこの制約で決まる
- **次の一歩**: research.md の申し送り G-1〜G-5 は**いずれも判定者(PO)の決定**が要る。特に G-1(ランナー 4 案の選択)は、採用時の再確認 2 点が Web 情報のため **/research(Codex 委任)を挟む必要がある**
- そのうえで /plan で実装計画書を作成し承認を得る
