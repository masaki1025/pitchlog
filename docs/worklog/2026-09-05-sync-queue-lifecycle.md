...
| 6 IndexedDB 永続キューと `Q1` の不可分性 | `e354f9b` | 281 |
| 7 `Q3`・`Q7` と 24 時間破棄 | `97eb50b` | 293 |
| 8 通知契約 6 種 | `07ba6fe` | 306 |
| 9 Web Locks による単一書き手 | `92fb5c2` | 322 |
| 10 ローカル書き出しと取り込み | `fe7cafe` | 332 |
| 11 `NFR-019(d)` 資産 4 件 + validator | `7fa3b89` | 344 |
| 12 (d) アダプタと 4 シナリオ実行 | `7b50e11` | 353 |
| 13 W4 の 3 前提合成入口 + `P-24` + 走査対象の是正 | `315674e` | 371 |
...
- **後続 δ を起票した**: [同期プロトコル — キュー通知の画面接続(後続 δ)](https://app.notion.com/p/3d293b75e68781438bdec5c79dcdcb93)
  (`Q4`・`Q5`・`Q6`・`Q2-b`・B4・I6 失敗の 6 記述子 + **7-5 の未送信バッジ**の画面接続。
  文面は `syncNotices.ts` が正で、画面側で再定義しない)
- **ルートの pytest は 1129 passed**。`test_doc_check_profile.py` の
  「`.claude/` に未コミット差分が無いこと」の assert は、**コミット後に green** になる
  (TSK-280 の台帳候補どおり**作業中だけ red** になる。実測として再確認した)
- **残る `[手動・外部]`**: 6.3 規則⑤の**敵対レビュー + 人間承認**、および
  **PR 作成者以外による逐行確認**(/pr で PR テンプレートへ記録する)

### 申し送り(追加)

- **6-3 の B5 行(`:748`)は「再ログインを促す + キューが保持されていることを併せて示す」**という
  利用者向け通知を定めているが、`U-*` が実装計画へ送っていないため本タスクの 6 記述子に含めなかった。
  **`Q7` と対になる通知**であり、**後続 δ で扱うか改訂ゲートで整理するかは未決** → **不明として記録**

### 実測: ステップ記法を入れ子の括弧に書くと現在地導出が「欠番」を出す

**ステップ 6・7・9・10 のコミット件名を `((B) 論点 19-a・ステップ 6/14)` の形で書いてしまい、
`feature_status.py` が 4 件ともトークン無しと判定して「実装状況: 不整合(完了ステップに欠番がある)」
になった。**

- ハーネス設計書 6.1 の厳密文法どおり、**トークンは `(ステップ <k>/<N>)` という完全な括弧の組**でなければならない。
  **より大きな括弧の中に `・` で並べると、`(` が直前に無いためトークンとして認識されない**
- `extract_step_tokens('feat: X ((B) 論点 19-a・ステップ 6/14)', 14)` → `([], False)` で再現した
- **是正**: `((B) 論点 19-a) (ステップ 6/14)` の形へ 4 件の件名を書き換え(push 前・`git filter-branch`)。
  導出は **14/14・実装完了・/pr 前**になった
- **台帳候補**: 人間もエージェントも踏みやすい形であり、**コミット時点で機械検査されない**
  (導出は後から気づく)。commit-msg フックか /implement 側での検査が候補

## 結果サマリ(/pr クローズ処理)

### 実装したもの

**正本 7-2 / 7-4 / 7-7 + `K5`(5-5)+ W4 のクライアント規律側(4-3-A)+ `NFR-019(d)` のクライアント側 4 項目。**
製品ファイル **11 新設**(+ 故障系アダプタ 1)、故障系資産 **4 件**、**frontend 371 tests / root 1129 tests** green。

**(B) 論点のうち正本が実装計画へ委譲した 3 件を本タスクで確定した**:
永続化 = **IndexedDB**(`Q1` の不可分性)/ 書き出し = **JSON + 注入 codec**(`X1` の原形保持)/
単一書き手 = **Web Locks**(`Q2-r1` の `steal`・`Q2-r2` の解放保証が同 API の仕様)/ 破棄トリガ = 起動時 + 明示要求 /
`U-5` の B4 通知文面。
...
| `scripts/design_relations/sync-protocol.json` | **反映なし**(読むだけ) |
| `.claude/core-areas.json` | **37 パス登録**(sync-protocol 25 / recording-rights 12・**削除 0**) |
| `tests/test_core_guard.py` | `EXPECTED_AREA_PATHS` を同一コミットで追随 |
| `docs/development/harness-evaluation.md` | **`## 候補` へ 2 件 + 既存候補 3 件へ実測**(下記) |
| `docs/README.md` | 台帳行の最終更新日を現行化 |

### 評価台帳への追記(**該当する**と判断)

- **候補①** ステップ記法を大きな括弧の中へ入れ子で書くとトークンとして認識されず「欠番」になる
- **候補②** 長い実装タスクで Codex の `--resume` セッションが肥大し実行が強制終了される
- **実測 3 件** PR 射程ガードの恒久化(**再発**)/ 重複帰属の範囲が再び論点(12 パス)/
  計画レビューの終端条件へ**対照例**(1 周で収束した理由)

**`H-*` の新規採番はしていない・版は上げない**(7.6-3 前段)。

### 残る `[手動・外部]`

- **6.3 規則⑤の敵対レビュー + 人間承認**(`.claude/core-areas.json` の paths 追加が対象)
- **逐行確認 — PR 作成者以外**。対象は新設 11 製品ファイル + アダプタ、および**逐語照合表 79 行**

## 実装の敵対レビュー(4 周・2026-09-05)

**P0 の推移: 5 → 4 → 3 → 0。** 反映は **10 コミット**。

| 周 | P0 | P1 | P2 | 指摘の性質 | 反映 |
| --- | --- | --- | --- | --- | --- |
| 1 | 5 | 5 | 2 | 実在の欠陥。**うち 1 件は計画書の読み自体の誤り**(`X2`) | 反映 1〜6 |
| 2 | 4 | 1 | 0 | **すべて 1 周目の是正が作った欠陥** | 反映 7・8 |
| 3 | 3 | 1 | 0 | 2 件は 2 周目の是正が作ったもの、1 件は偽装経路の移動 | 反映 9 |
| 4 | **0** | 3 | 2 | 境界の**強度**の問題 | 反映 10(1 件)+ **後続へ送付** |

### 収束した理由(**H-68 の実測**)

**2・3 周目の P0 はいずれも「前周の是正が作った欠陥」だった。**

- 1 周目の是正で **`eventKind` 引数**を足す → 2 周目「**誤った既知種別**で迂回できる」
- 2 周目の是正で **receipt / preparation 境界**を足す → 3 周目「**偽装経路が `slot.content.V5` へ移っただけ**」
  「**`instanceof` を偽装すれば捏造できる**」「**preparation の再利用で `append` が複数回成立する**」
- **3 周目は「境界を増やさず、既存の preparation / receipt の 2 種で閉じる」方針にした** →
  **4 周目で P0 が 0**

**新しい機構を足す方向の是正は収束せず、既存の境界へ集約する是正で収束した。**
これは台帳 **H-68**(存在しない対象への設計の先取り / 是正が指摘を量産する)と **H-78**(局所編集を重ねる是正は
収束しない — 規則は 1 箇所へ集約し、残存を検出する機械検査を別に置く)の**対応案の裏付け**にあたる。

### 打ち切りの判断

**4 周目で P0 が 0 になった時点で終端**とした(3 周目の時点で人間が「もう 1 周だけ構造変更、
その後は P0 が出ても打ち切り」と裁定していた)。

残った P1 3 件・P2 2 件のうち、**A5 preparation の owner 未照合だけ反映**した
(**owner を保存しているのに使っていない**という見落としで、閉じる価値が明確だったため)。
残りは [永続化証跡の残った経路を閉じる](https://app.notion.com/p/3d293b75e6878116b961df7ae8091bcf) へ送った。

### 天井として記録すること

**公開 static の書き換え(monkey-patch)への対策には天井がある。**
**同一オリジンのコードは IndexedDB を直接開ける**ため、証跡の偽造対策は
**悪意ある呼び出し側を防ぐものではなく、誤用を防ぐもの**である。
3 周目でレビュー自身が「プラットフォーム境界の限界」と判定しており、指摘に数えていない。

### 環境上の事実(**通算 6 回の強制終了**)

レビュー実行と実装委任が**ハーネスのメモリ監視によって 6 回強制終了**された。

- **セッション肥大時**(ステップ 12 で 97 万トークン・fix4 で 68 万トークン)
- **新規セッションの起動直後**(fix7・review4)— **肥大とは無関係に落ちる**
- **150KB 出力まで進んだ途中**(review4 の 2 回目)
- **メモリは前後とも 14GB 空き**。**同じコマンドの再実行で成功することが多い**

**当初「`--resume` のセッション肥大が原因」と見立てたが、新規セッションでも落ちるため誤りだった。**
**4 周目は射程を極小化(読むファイル 4 つ・問い 4 つ・出力 3 行以内)して完走させた。**

### 再承認(2026-09-05・山田正輝)

**`X2` の是正**(取り込み専用の重複判定を作らない — 同 D5・異内容は原形のままキューへ入り、
サーバー側で `B3b` になる)を反映した計画書を**再承認**。
あわせて **3 周目の設計方針**(**境界を増やさず既存の preparation / receipt の 2 種へ集約する構造変更**)も承認された。

**残る手続は人間の作業**: 6.3 規則⑤の承認と、**PR 作成者以外による逐行確認**
(PR #44 のチェック 2 項目。CI の core-guard が `- [x]` を機械検査する)。
