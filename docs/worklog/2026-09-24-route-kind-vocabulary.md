...
## P0-2 の決着 — 承知のうえでマージする判断(2026-09-26)

**逐語**:

> P0 を承知でマージする → その判断を worklog に残す

- **取得元**: 2026-09-26 の Claude Code セッションでの発話
- **決めたこと**: **凍結基準 `history[1]` の `approved_by` / `approved_at` が
  敵対レビューで 3 周連続 P0 と判定されている状態を承知のうえで、本 PR をマージする**

### なぜこうなったか — 推奨した是正手段が成立しなかった

当方は **4 度**「PR #77 に Approve レビューを付け、`user.login` と `submitted_at` を
機械可読な出所として逐語転記する」ことを推奨した。**実行したところ GitHub が拒否した**:
...
**PR の作成者は自分の PR を承認できない。** 本リポジトリは実質単独メンテナなので、
**「PR の Approve を承認記録の出所にする」という統制はこの体制では原理的に成立しない。**
当方は 4 度提案する間、**この制約を一度も確認していなかった**。

代替として **PR コメント**(自分の PR にも書ける・`user.login` と `created_at` が機械可読)を
提示したが、**人間はそれを採らず、現状のままマージする判断をした**。

### 残る状態(記録)

- **`approved_by: 山田正輝` / `approved_at: 2026-09-25` は推定時の値のまま**である
- `reason` に取得元(2026-09-25 のセッションでの発話・逐語「承認するから入れてくれない?」)を書いている。
  **ただしその逐語は氏名も日付も含まない**ので、2 つのフィールドは**取得元からの逐語転記になっていない**
- **`docs/worklog/2026-09-20-frozen-baseline-ledger.md:63` の規律**
  (承認者・承認日は取得元を明記して逐語転記する。取得不能なら停止する)**を満たしていない**
- **台帳は追記のみで書き換え不可**なので、**この記録は残り続ける**
- 敵対レビューは **3 周目・4 周目・5 周目のすべてで P0** と判定している

### 後から解消したい場合

**追記でしか直せない**。将来の受理(別 PR)で、**その時点の承認記録を規律どおりの形で残し、
本件が推定値であったことを `reason` に明記した訂正レコードを足す**のが唯一の経路になる。
**本 PR の `history[1]` 自体は書き換えられない。**
