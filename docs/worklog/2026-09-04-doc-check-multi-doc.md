...
### 人間の裁定(2026-09-04・/implement 途中)— 委任粒度の粗化

**設計書 6.1「1 委任 = 1 ステップ = 1 コミット」の委任側だけを緩める**(逸脱につき人間の了承を取得済み):

- **まとめる**: DSL 群のうち **12〜15**(`section-contains` / `exact-set` / `element-lookup` / `row-scoped-forbidden`)と **16〜19**(`any-of` / `required-exclusion` / `conditional-forbidden` / `well-formedness`)を各 1 委任。
  以降の **23〜28**(COV・資産・サンプル)と **29〜33**(新検査 5 本)も 2〜3 ステップ単位でまとめる
- **まとめない**: **20**(`cross-reference`)/ **21**(規則 6 の前提検証)/ **22**(旧分岐と shadow 基盤の撤去)/ **34〜38**(ランナー・配線・記録)。
