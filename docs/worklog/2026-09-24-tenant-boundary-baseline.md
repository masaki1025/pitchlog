...
## 再レビュー依頼時のクローズ処理(2026-09-26)

**PR #78 は OPEN のままなので、`/pr` の「修正・検証完了(再レビュー依頼)」経路で処理した。** `gh pr create` は行わない。

### 台帳への追記の判断 — **該当する**

**`## 候補` へ 1 件追記した。**

**PR がコンフリクトしていると CI が 1 つも走らず、「走っていない」と「通っている」が区別できない。**

**3 回の push にわたり run が 0 件**だった。**PR が base と衝突していると GitHub は `refs/pull/<N>/merge` を作れず、`pull_request` の workflow は `synchronize` が発火しても起動しない。** 衝突していたのは索引と台帳の 2 ファイルだけで、**実装は 1 バイトも衝突していなかった**。

**`gh pr checks` は `no checks reported` としか言わず、「まだ走っていない」と「走ったが報告が無い」を区別しない。** この状態で 3 コミット進み、**その間に検査器を 2 回大きく書き換えた**。**計画書 DoD の「実 PR 受理モードが実 CI で実走し green」の充足が 3 コミット前で止まっていた。**

### 追記しなかったもの(判断の記録)

| 知見 | 扱い | 理由 |
| --- | --- | --- |
| **「関数は正しいが本番経路がその結果を使っていない」型が 3 周連続で出た** | **TSK-440 セッションが候補②として書く。本タスクからは追記しない** | **先方が明言しており、本タスクの実例(1 周目 P0-1・P0-2、3 周目 P0-1)も受け渡し済み**。両方から書くと二重計上になる |
| **Codex のサービス障害(401)** | **追記しない**(worklog にのみ記録) | **外部サービスの障害であって、ハーネスの設計や運用の知見ではない**。復旧手順も「待つ」以外に無い |
| **`grep` がシェル関数として壊れる** | **追記しない**(worklog にのみ記録) | **Claude Code 同梱の `ugrep` への委譲が原因**で、本リポジトリのハーネスとは無関係 |
| **時点の事実を記録へ焼き込むと古くなる** | **TSK-440 セッションが候補⑦として書く** | **本タスクの実例(TSK-450 のカードが対象を「7 件」と固定していた件)は受け渡し済み** |

### 突合

- **未コミット**: なし
- **正本反映の双方向突合**: `check_plan_docs_sync.py` で機械突合
- **現在地導出**: `feature_status.py` の出力を PR 本文へ転記
- **品質**: 全ゲート green(ルート 1802 passed / backend 792 passed〔DB 必須分を含む・実機 PostgreSQL〕/ ruff・format・ty・検査器)

### 再レビューの観点(前回から変わった点)

**敵対レビュー 4 周を経て P0 = 0 に到達した。** 前回の依頼時から**実装が大きく変わっている**ので、次を重点的に見てほしい。

1. **不変量モードの範囲**(4 周目 P1 の是正)— `design.md` 4 節の範囲へ戻した結果、**ローカル実行では資産本体の変異を検出しない**。検出は PR 受理モードの担当で、7 資産すべてで red になることを確認済み
2. **予約 marker の判定**(4 周目 P2 の是正)— 完全一致・語境界・既知の暫定形式の 3 つへ限定した。**過剰検出と検出漏れの両方**を見てほしい
3. **`movement_evaluation` が実際に判定を決めているか**(3 周目 P0-1)— 束縛しただけで実質 `derive_aspects` と同値になっていないか
4. **本番経路への結線**(3 周連続で出た型)— 機構を壊す変異で本番経路テストが落ちるか
