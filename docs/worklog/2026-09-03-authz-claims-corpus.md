...
### ステップ 11 — oracle 6 資産の追随 + seal の 2 段目(codex 委任・差分敵対レビュー 4 周)

- **内容追随**: predicates 定義の新設(恒真を宣言できない形)/ F12(SECURITY DEFINER の owner 依存 ACL・caller schema USAGE の整合)/ F13(app_role の制御表 4 表 DML 剥奪 → SELECT のみ・攻撃木へ `ATTACK:APP-ROLE-DIRECT-CONTROL-DML` + cut set 4 件)/ execution class 追随(probe 33 / contract 165)/ atomic 追随(oracle_claims 198・mutants 231・cut_sets 24)/ 6 資産 + seal の `oracle_commit` = ステップ 10 SHA(e10f2b1)
- **差分敵対レビュー(seal の review_policy)4 周**: R1 P0×1(攻撃木が DML 再付与を表現せず)→ 是正(target_pairs 一般化・後方互換・DML 再付与 mutant 4 件)/ R2 P0×1(攻撃木検査の必須対象が旧 prefix のみ)→ 是正(target_pairs 由来の全 mutant へ一般化)/ R3 P0×2(宣言の自己参照・対応未固定)→ 是正(REQUIRED_TABLE_PRIVILEGE_TARGETS の checker 内定数化・(role,table,privilege)→goal/cut の機械導出照合)— いずれも負例 red→green つき
...
- **実装**: 全 12 ステップ + 計画改訂 3 回。検査器へ 6 意味論 + 従属 2 件(legacy origin・表権限 mutant 一般化)を負例 red→green つきで導入し、実資産(母集合 1063・派生 3 + oracle 6 + seal)を裁定リストどおり是正・再封印。checker 全 green・pytest 862 passed
- **正本へ反映**: 要件書(例外表 2 セル有効化 + 変更履歴・版 2.4)/ README(要件書行・台帳行)/ contracts/authz 全 15 資産 / 台帳(H-87 新設・H-53/H-81 実測 — 版上げなし)
- **台帳への追記あり**(クローズ処理判断): H-87(一次記録の scratchpad 終着による消失 — 本タスクの起点そのもの)+ H-53 成功実測 + H-81 実測。単発・典拠未確定の候補該当なし
- **残余リスク**: oracle 差分レビュー 4 周目の協調改変型 4 件(PO 範囲確定 2026-09-03 — 恣意的迂回・最終吸収先 TSK-250 実機検証)
- **後続**: F1 → TSK 起票済(ブロッカにしない)/ F9・F10・F11 → oracle 網羅拡張タスク(TSK-250 開始条件へ紐付け済)/ H-85 案②③ → 設計タスク起票済
- **人間の残作業**: 逐行確認(`verification-sheet.md` — 1,193 単位・A 群優先)→ PR チェック行の記入 → マージ
