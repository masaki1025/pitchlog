...
| 1.1 | 2026-09-27 | **実装追随(版は上げない — 7.6-3 前段)**: TSK-463 ステップ 3〜4 — ラッパー定数を本表へ更新し、Codex 版検査(0.157.0 未満は fail-closed)・`probe`・fast の 3 値検査・決定表(固定構文)との同期テストを実装(`.claude/scripts/codex_run.py`・`tests/test_codex_run.py`・`tests/test_agents_frontmatter.py`)。**実機検証(codex-cli 0.157.1・2026-09-27)**: `probe` で本表の全 5 組(`gpt-6-sol` × medium/max/xhigh/high・`gpt-6-luna` × xhigh)が `exec` 経路で受理(終了コード 0)、`review normal`(gpt-6-sol max)と `review adversarial`(gpt-6-sol xhigh)が本番様式の依頼文で判定行を返却・安全分類器の遮断なし。記録: `docs/worklog/2026-09-26-harness-model-refresh.md` | approved |
