...
| 1.1 | 2026-09-27 | **GPT-6 世代への対応表の再設計(起案)**(TSK-463・PO 判断 2026-09-27・徳光 尋弥): 見直しトリガー 3 条件(世代交代・料金改定・新 tier 登場)が同時に成立したため改訂。**決定** = 全枠を `gpt-6-sol`(機械的軽作業のみ `gpt-6-luna`)へ・effort は v1.0 の値を据え置き・`gpt-6-astra` は本版では載せない(昇格が必要になった実例で v1.2)・Fast / Ultra は使わない。**新設** = 決定表の固定構文(ラッパーとの同期テストが読む)・実測期間(移行後 3 回の確定ゲートまたは 4 週間)と巻き戻し条件・Codex CLI 0.157.x 以上の前提。**旧表(v1.0)の要約** = 通常実装/一次レビュー `gpt-5.6-terra` max・軽微 terra medium・コア領域/敵対レビュー `gpt-5.6-sol` xhigh・機械的軽作業 `gpt-5.6-luna` xhigh・Web 調査 terra high(深い技術検証は sol xhigh)。**射程宣言(7.3-7)**: 本改訂で確定する範囲 = ① 決定表 8 行のモデル ID と effort ② 決定表の固定構文と行キー → ラッパー定数の対応 ③ 実測期間・記録項目・巻き戻し条件・見直しトリガー ④ astra 不掲載と Fast / Ultra 不使用の方針。実装時(TSK-463 の後続ステップ・後続タスク)に確定する範囲 = ラッパー定数と同期テストの実装・`exec` 経路での受理の実機検証(追随行で記録)・実測期間後の再判定(v1.2 の要否)・Claude 側のモデル既定(設計書 8.1 / 8.5 — 本 ADR の対象外)。設計書 v1.18 と**単一の確定ゲート**(7.3)で一括検証する | in-review |

| 項目 | 内容 |
| --- | --- |
| 状態 | **in-review**(v1.1 起案 2026-09-27 — 設計書 v1.18 と単一の確定ゲート /finalize-doc で検証中。直前の approved = v1.0〔2026-08-07・ハーネス設計書の確定ゲートで一括〕) |
...
- **公式の位置づけ・ベンチマーク**: GPT-6 Sol は "improves substantially over GPT-5.6 Sol"(FrontierCode)・"match Claude Fable 5.1 xhigh at much lower cost"・"higher usage limits and lower cost"(発表ページ — Claude からは取得不可のため Codex の原文引用。**未照合**)。Astra vs 5.6-sol は Terminal-Bench 4.0 57.9% vs 37.3%・FrontierCode 1.1 Main 53.3 vs 47.5(同・未照合)。公式は「高い effort は利用枠をより多く使い得るが常に良い結果になるわけではない」と明記
...
「作業の重さ」で機械的に選ぶ。重さの判定はコア領域リスト(設計書 6.3)で決まり、人が都度迷わない。**下表は固定構文**(ヘッダ固定・8 行・「作業」セルは行キーと完全一致・「モデル」セルは code span 1 個・「effort」セルはトークン 1 個)で、ラッパーとの同期テスト(`tests/`)が構造的に読む:
...
- 行の意味: 通常実装 = CRUD・画面・帳票・テスト / 軽微な修正 = 小さな差分・微調整(fast path) / コア領域の実装 = 設計書 6.3 のリスト / 機械的軽作業 = リネーム・ボイラープレート / 一次コードレビュー = 通常 PR / 敵対レビュー = コア領域 PR と正本確定ゲート(7.3)/ Web 調査 = `/research`(`--deep` はコア領域に関わる深い技術検証)
...
6. **表と機構の乖離を機械検査する(v1.1 で追加)**: v1.0 では対応表とラッパー定数の一致を検査するテストが無く、乖離は CI で検出されなかった。決定表を固定構文にして同期テストが読む

## 帰結

- `.claude/scripts/codex_run.py` が本表どおり `-m <model> -c model_reasoning_effort=<effort>` を自動指定する(設計書 9.4)。**決定表 ↔ ラッパー定数の同期テスト**(固定構文の抽出・負例・各経路の引数)を `tests/` に置き、乖離を CI(harness ジョブ)で検出する
- **前提: Codex CLI 0.157.x 以上**(gpt-6-sol / gpt-6-luna の `minimal_client_version` 0.155.0・カタログ追加 0.157.0)。本機は 2026-09-27 に 0.157.1 へ更新し、カタログで `gpt-6-sol` の supported_reasoning_levels(low〜max)と `gpt-6-luna`(low〜max)を確認済み。**`exec` 経路(ラッパー)での受理は TSK-463 ステップ 4 で実機確認し、本表へ版を上げない追随行で記録する**
- **実測期間**: 移行後 **3 回の確定ゲート(または 4 週間)** を実測期間とし、各ゲートの周回数・P0/P1 検出数・安全分類器による遮断の有無を worklog と運用評価台帳の候補に記録する
- **巻き戻し条件(いずれかで v1.2 を起案し新規の確定ゲートで再改訂)**: (a) 本表の `(model, effort)` が受理されない (b) 安全分類器の遮断が 5.6-sol 時代(1 ゲートで flagged 2 回 — 台帳 2026-09-21 追記)より増える (c) 実測期間中に「敵対レビューが計画レビュー通過後の P0 を検出できなかった」実例が出る。品質下限の判定は周回数でなく**検出実例の有無**で行う(H-30: 周回数はモデル差だけの効果と切り分けられない)。v1.2 の候補 = astra 昇格(コア領域・敵対レビュー・`--deep`)
...
- 設計書 9.4(対応表の再掲)・8.5(調査サブエージェントの既定)・8.1(主セッションの既定)・6.3(コア領域)・7.3(確定ゲート): [dev-harness-design-2026-08-07.md](../development/dev-harness-design-2026-08-07.md)
