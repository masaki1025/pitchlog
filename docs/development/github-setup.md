...
| 1.2 | 2026-09-21 | **TSK-235 の実装追随(版は上げない — 設計書 7.6-3 前段)**: 2 章の常時実行を **5 ジョブ → 7 ジョブ**へ更新し、`consistency` / `mutation` を追加した。3 章 Ruleset の `required_status_checks` も **9 context → 11 context**へ同期した。実測ではリポジトリは public、Ruleset `protect-main-develop` (id `23694095`) は `active`、`bypass_actors` は空、strict policy は有効であり、1 章の public 化前の記述と食い違う。1 章および同じ前提に依存する記述の全面是正は `TSK-429`(`feature/post-public-doc-sync`)が既に用意しており、同タスクのマージで解消する。本追随はジョブ列挙と Ruleset JSON に限定する。**3 章 JSON は `TSK-429` が API 実測の全現値へ同期した版を基線とし、本タスクは context を 2 件足すだけにする**(同タスクが補完した `require_extra_approval_for_unattributed_changes` 等を落とさないため)。計画: `../features/domain-calc-dsl/plan.md` | **approved** |
...
> **現況注記(2026-09-21・本 PR 時点)**: リポジトリは public 化済みで Ruleset `protect-main-develop` (id `23694095`) が `active`・bypass なし・strict policy 有効である。**本 PR の時点では 1 章が public 化前の記述のままだが、その全面是正は `TSK-429`(`feature/post-public-doc-sync`)で既に用意されており、同タスクのマージで解消する。****本注記は同タスクのマージ後に不要となるので削除してよい。** 本節では現行 CI の必須ジョブ列挙だけを同期する。
