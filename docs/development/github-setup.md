...
- **`integration_id` は未指定**(9 context とも)。**未指定だと任意ソースの同名 status を受け入れる**。**これは設計書 10.2 のリスク受容記録が解除されない理由そのものであり、`TSK-433` は「指定可否の判断」ではなく**「発行元拘束の成立」**を完了条件とする(設計書 10.2)。**成立しない限り解除条件は満たされず、リスク受容記録は存続する。****
