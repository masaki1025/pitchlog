...
| 1.0 | 2026-08-22 | **Phase 4-5 の実装追随(版は上げない — 設計書 7.6-3 前段の実装追随の節更新)**: 必須チェックを **4 ジョブ → 5 ジョブ**へ更新(`nfr021-append-only` を追加 — NFR-021 受入証跡の append-only 統合時検査。設計書 10.1 のジョブ表が正)。2 章のマージ手続 2 項・4 項の列挙と 3 章 Ruleset の `required_status_checks` を同時更新した(本書 3 章の「必須ジョブの追加・削除・改名時は本書と Ruleset を 同時更新する」に従う)。あわせて **2 章のマージ手続へ 5 項目目を追加**(NFR-021 の予約 PR を同一ゲートキーで並行させない — 同検査は PR イベントの `base.sha` を基準に評価するため、同じ base の 2 PR がどちらも green のまま未閉塞予約を 2 件作れる。**敵対レビュー起点**。3 章の Ruleset 適用で解消する)。**保護の適用状況・その他の暫定運用の内容は変更していない**(縮退は継続中)。計画: `docs/features/nfr021-evidence-verifier/plan.md` | approved |
...
- マージの手続(すべて必須):
  ...
  5. **NFR-021 の予約 PR は同一ゲートキーで並行させない**(`nfr021-append-only` の残余リスク)。同検査は **PR イベントの `base.sha`(= PR 作成・更新時点の develop)** を基準に評価するため、**同じ base から作った 2 つの PR がそれぞれ `seq002` の予約を足すと、どちらも個別には green になる**。先の PR をマージしても後の PR の head SHA は変わらないので、古い green のままマージすると**同一ゲートキーに未閉塞の予約が 2 件並ぶ**(設計書 10.1「ゲートキー単位で同時に進行できる受入試行は 1 件だけ」に反する)。**当面は予約 PR を直列に扱い**、先行 PR のマージ後に後続 PR へ develop を取り込んで(= head SHA を更新して)CI を再実行してからマージする。**3 章の Ruleset を適用すれば `strict_required_status_checks_policy: true` が base 最新化を必須にするため、この手続きは不要になる**
