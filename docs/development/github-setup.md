...
> 2. **運用の実地検証: 未完。** 本節が下で要求する「docs-only PR で下流 skipped がマージ可能」「変更検知ジョブ失敗時にマージがブロック」の**両方の実測は未実施**である。→ 後続タスク **[F-2 ブランチ保護の実地検証](https://app.notion.com/p/3e193b75e6878100980dcacbd9c80456)**(担当: PO / 完了条件 = 両方の実測 PR URL の記録と本節の二段階記述の解消)。
>
