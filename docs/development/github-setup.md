...
| 1.2 | 2026-09-19 | **必須チェックの同時更新(版は上げない — 設計書 7.6-3 前段の実装追随の節更新)**: **`tenant-boundary-bypass` を追加**(U-T1 テナント境界の強制点が新設する迂回検査ジョブ。`nfr021-append-only` の先例に倣い **paths filter なしの常時実行**・`fetch-depth: 0`。フィルタ自体が誤ると検査が黙って通り fail-closed に反するため)。**2 章の手続 2 の常時実行ジョブの列挙(5 → 6)と 3 章 Ruleset の `required_status_checks` を同時更新**した(本書 3 章の「必須ジョブの追加・削除・改名時は本書と Ruleset を同時更新する」に従う)。**適用の順序に注意**: `pull_request` イベントでは PR ブランチ側の `ci.yml` が使われるため、**当該ジョブを持たないブランチでは context が永久に報告されず全 PR が pending で止まる**。したがって **Ruleset への実適用は本ジョブが `develop` の `ci.yml` に載った後**に行う(`strict_required_status_checks_policy: true` が base 最新化を強制するので運用上は整合する)。**保護の適用状況・暫定運用の内容は変更していない**。計画: `../features/tenant-boundary-enforcement/plan.md` | approved |
...
| 1.2 | 2026-09-21 | **TSK-235 の実装追随(版は上げない — 設計書 7.6-3 前段)**: 2 章の常時実行を **6 ジョブ → 8 ジョブ**へ更新し(TSK-429 の `tenant-boundary-bypass` 追加を取り込んだ後の数)、`consistency` / `mutation` を追加した。3 章 Ruleset の `required_status_checks` も **10 context → 12 context**へ同期した。実測ではリポジトリは public、Ruleset `protect-main-develop` (id `23694095`) は `active`、`bypass_actors` は空、strict policy は有効であり、1 章の public 化前の記述と食い違う。1 章および同じ前提に依存する記述の全面是正は `TSK-429`(`feature/post-public-doc-sync`)が既に用意しており、同タスクのマージで解消する。本追随はジョブ列挙と Ruleset JSON に限定する。**3 章 JSON は `TSK-429` が API 実測の全現値へ同期した版を基線とし、本タスクは context を 2 件足すだけにする**(同タスクが補完した `require_extra_approval_for_unattributed_changes` 等を落とさないため)。計画: `../features/domain-calc-dsl/plan.md` | **approved** |
| 1.2 | 2026-09-21 | **2 章 NFR-019 の記述の訂正(版は上げない — 7.6-3 前段)**(TSK-429・確定ゲート 初周 `P0`): **前行(2026-09-20)の「リモート強制の未達は解消した」は誤りだったので訂正する。** **必須 9 context とも `integration_id` が未指定**であり、**書き込み権限を持つ主体が同名の成功 status を立てれば必須チェックを満たせる**(CI が実際に失敗していても・一度も走っていなくてもマージできる)。したがって **Ruleset の適用により止まるのは(**設定上は**)「直 push・force push・ブランチ削除・PR を経ないマージ」までであり、「全グリーンでないとマージ不可」の強制は成立していない**。**設計書 10.2 のリスク受容記録はこの理由で解除されず、解除条件に「必須チェックの発行元が拘束されていること」が追加された**(→ `TSK-433`)。**「設定が存在すること」と「統制が成立していること」は別である。** | **approved** |
...
- **NFR-019 の現況(2026-09-20 更新)**: 要件書 [5 章 非機能要件](../requirements/requirements-pitchlog-2026-07-22.md) の NFR-019 は「PR ごとに CI で自動実行、**全グリーンでないとマージ不可**」を求める。**2026-09-19 の Ruleset 適用により、直 push・force push・ブランチ削除・PR を経ないマージは設定上は止まる**(**設定上は拒否される — API 実測で Ruleset は `active`。ただし拒否動作そのものの実測は未実施**。`gh ruleset check` は適用対象の列挙にすぎず拒否を実証しない → `TSK-433`)。 ただし**「全グリーンでないとマージ不可」の強制は成立していない** — 必須チェックの**発行元が拘束されていない**(`integration_id` 未指定・3 章)ため、書き込み権限を持つ主体が**同名の成功 status を立てれば満たせる**。**設計書 10.2 のリスク受容記録はこの理由で解除せず存続しており、解除条件に「必須チェックの発行元が拘束されていること」が追加されている**(→ `TSK-433`)。
...
>    - **削除の負例は使い捨てブランチでのみ行う**(復旧は再作成で足りることを確認してから)
>    - **force push・直 push の負例は、失敗しても失われるものが無い状態**(直前の SHA を記録し、再現可能)で行う
>    - **各試験の前に復旧手順を書き、実施後に復旧を確認した記録を残す**
>    - **【最終構成への拘束】試験は Ruleset の最終構成に対して行う。** 試験の実施後に**Ruleset へ何らかの変更を加えた場合は再試験する**(発行元拘束の指定を含む。**設定変更は種類を問わない**)。**試験時点の Ruleset を API で取得して証跡に含め、完了時点の実構成と一致することを照合する**
>    - **【隔離先の同一性】rule 集合の一致だけでは足りない。** **最終的な API 投影(全 rule の全パラメータ)・対象 ref の条件・実行主体(誰の権限で操作するか)**が本番と同等であることを立証する。立証できない項目は**その項目について試験が成立していない**と記録する。**③は「指定可否を判断した」だけでは満たされない**〔設計書 10.2 — **DoD に発行元拘束の成立を含むことが追跡先としての要件**〕。**成立しない限り本タスクは閉じられない**(解除条件が満たされないだけであり、fail-closed でよい)。)
>
> **したがって「適用済み」と「運用開始可」は別である。** 本節を根拠に運用開始を主張しないこと。

再開トリガー: 1 章の選択肢のいずれかが実施されたとき。適用は **Rulesets** を推奨(新設に適する — 積層・bypass 管理・閲覧性)。

**Ruleset を有効化しても防げる範囲は、設定上は「直接 push・force push・削除」と「PR を経ないマージ」まで**(**設定上は拒否される — API 実測で Ruleset は `active`。ただし拒否動作そのものの実測は未実施**。`gh ruleset check` は適用対象の列挙にすぎず拒否を実証しない → `TSK-433`)。 **「必須チェックの欠落・failure のままのマージ」は防げていない** — 必須チェックの**発行元が未拘束**(`integration_id` 未指定)のため、**同名の成功 status を立てれば失敗・未実行のままマージできる**(2 章と設計書 10.2 を参照)。**検査ロジック自体の改変(2 章の循環参照)も防げない** — したがって 2 章の手続 3・4(コア領域 ∪ guard_paths の人間逐行確認)は継続する。
  ...
  "rules": [
    ...
    { "type": "required_status_checks", "parameters": {
        ...
        "required_status_checks": [
          ...
          { "context": "frontend" },
          { "context": "backend" },
          { "context": "tenant-boundary-bypass" }
        ] } }
  ],
  "bypass_actors": []
}
...
- **`integration_id` は未指定**(**必須チェックは全 context とも**)。**未指定だと任意ソースの同名 status を受け入れる**。**これは設計書 10.2 のリスク受容記録が解除されない理由そのものであり、`TSK-433` は「指定可否の判断」ではなく**「発行元拘束の成立」**を完了条件とする(設計書 10.2)。**成立しない限り解除条件は満たされず、リスク受容記録は存続する。****
...
- **ローカル監査は git モードで行う**: `.gitleaksignore` に登録した fingerprint は**コミット SHA を含むリポジトリ履歴モードの形式**であり、`dir` サブコマンド(作業ツリーのみ)の fingerprint とは**形式が異なるため一致しない**。**`dir` では作業ツリー上の 2 件が検出される**(全履歴の 3 件との差は、`auth_catalog` のダイジェストが更新され 2 つのコミットで別々に検出されるため)。**これは想定どおりで、CI(履歴モード)では抑止される** — 抑止の実測は run `35513315849` の `secrets` success。
