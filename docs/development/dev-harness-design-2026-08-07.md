...
手順: /task-start（ブランチ+worktree は維持）→ `codex_run.py fast`（モデル・effort は ADR-001 の「軽微な修正」行をラッパーが固定。**適用時に計画書雛形の `重さ分類: 軽微` と `実行方式: fast` を一括更新する** — 人間の事前 OK の記録であり、ラッパーは当該ブランチの計画書で両値を検査してから実行する。v1.18）または Claude が直接修正 → /check → PR 本文に**短縮計画**（目的/変更/確認方法）→ CI + 反対側レビュー1本（Codex 実装なら Claude、Claude 直なら Codex）→ 人間マージ。
