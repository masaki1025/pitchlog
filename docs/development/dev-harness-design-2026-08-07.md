...
| 1.1 | 2026-08-10 | 6.4 に**ハーネス確定ベースラインマージ(1 回限り)の例外**を追記 — PO 判断(2026-08-10): 「ハーネス完成 = 13 章 Phase 3 完了」と定義し、初回リリースに先立つ `develop → main` マージを 1 回実施(リリース非該当・実施完了で失効)。**版繰り上げなしの節更新**(7.6-3。v1.0 の frontmatter 追加行と同じ扱い) | approved |
...
- **例外 — ハーネス確定ベースラインマージ(1 回限り・PO 判断 2026-08-10)**: プロダクト初回リリースに先立ち、ハーネス完成(= 13 章 Phase 3 完了)時点で `develop → main` のベースラインマージを 1 回実施する。**リリースには該当しない**ため、本節の `/release`・DoD 8 項目・`vX.Y.Z` タグは適用しない。実施手順: PR 経由(`gh pr create --base main --head develop`)→ 最新 HEAD の CI 全グリーン確認 → 人間がマージ(merge commit — squash しない。保護未適用中の管理手続 = github-setup.md 2 章に従う)。副次効果: ci.yml が既定ブランチ main に載り、gitleaks 全履歴スキャン(workflow_dispatch)が Actions から起動可能になる(10.1)。**本例外は実施完了をもって失効**し、以後の `develop → main` は本節のリリースフローのみとする
