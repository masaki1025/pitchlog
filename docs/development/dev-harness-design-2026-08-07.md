...
| 1.5 | 2026-08-15 | **現在地導出の規約を実装と一致させる(起案)**: 6.1 へ 3 点を追加 — ① **ステップ検出書式の厳密文法**(同種括弧の一致 / `k=0` は不正 / `/<N>` は表の総数と一致 / 数値直後に許す文字の列挙 / 括弧のない文言は不算入 / 1 件名に 2 トークン以上は不整合。**いずれも実装が既に行っている検査の明文化**であり、これまで一時ディレクトリの文書と実装にしか存在しなかった)② **`確定ゲート周回` の反映周コミット規定**(指摘反映を伴う 1 周ごとに 1 コミットを作り件名に `反映<r>周目` を含める。**実装ステップ表の番号は割り当てない**〔ステップ記法を付けると同一ステップ番号のコミットが複数本生まれ 6.1 の「1 ステップ = 1 コミット」と衝突するため〕。**突合は件数ではなく `<r>` の集合**で行い、**コミットの作成自体は強制しない**〔警告のみ〕。**適用は `plan.md` の初回コミットが本版の確定コミットの子孫である feature に限る** — 規則制定前の行為を違反扱いしないため。**判定は git の祖先関係**で行い日付では判定しない〔`created` は日付精度しかなく確定日当日の着手を一意に評価できない〕。**境界は「着手」ではなく「計画書の初回コミット」**〔着手時点は Git に痕跡を持たないため〕・**rename は追跡せず候補が 1 件のときだけ判定**し 0 件/複数/確定コミット未解決は縮退)③ **「計画系/文書系コミットのみ → known 0(実装前)」の例外**(ステップ表に N ≥ 1 があり有効トークンが 0 件のとき「ステップ記法のコミットなし(書式未一致の可能性)」を併記する。**判定自体は変えず注記のみ**。**承認直後は注記しない**)。あわせて **6.1 から `docs/features/feature-status/design.md` への参照を除去**(活動中のみ意味を持つ一時ディレクトリの文書を正本が「導出規則の詳細」として名指ししていた)。**確定ゲート 1 周目の反映**(P1 2 件): ② の書き方を 2 運用から**番号を割り当てない 1 通りへ一本化**し、適用境界を**確定日以降 → 確定コミットの子孫**へ改めた。**確定ゲート 2 周目の反映**(P1 1 件): その適用境界を**「着手」から「計画書の初回コミット」へ明示的に再定義**し、rename 非追跡・候補 1 件限定・3 つの縮退理由を規定した。**確定ゲート 3 周目の反映**(P1 3 件・P2 1 件): **突合の履歴範囲を `base` = `merge-base(origin/develop, HEAD)` で固定**し**マージ済み feature は `対象外(develop へマージ済み)`** と明示・**`確定ゲート周回` が `0` でも空集合と突合**し**不正値は `未取得(確定ゲート周回 不正値)`** で縮退・**確定コミットの定数を 3 条件で自己検証**し系統外は `未取得(v1.5 確定コミット系統外)` で縮退。**起案の契機**: 要件書 v2.0 改訂タスクで 9 ステップ完了済みの feature を「実装前(全 9 ステップ)」と**無言で**表示した事象(台帳 H-45)と、確定ゲート 5 周ぶんの反映コミットが 0 件でもカウンタだけ進む事象(同 H-41)。計画: `docs/features/feature-status-degradation/plan.md` | **in-review**(敵対レビュー・人間承認 未了) |
...
- **plan frontmatter の拡張キー**（feature 作業自身の進行事実のみを置く — 正本 status の複製は置かない〔7.1-1〕。**本表が契約の正** — v1.3）:
  ...
  **反映周コミットの規定**（v1.5 — 台帳 H-41。`確定ゲート周回` は +1 がコミットなしでも実行できるため、**カウンタだけが進んで証跡が残らない**事象が実際に起きた）:
  ...
  - **適用境界は「タスク着手」ではなく「計画書の初回コミット」と定義する**。規則制定前の行為を違反扱いしないための境界だが、**着手時点は Git に痕跡を持たない**（`/task-start` はブランチと**未コミットの** `plan.md` を作るだけで着手コミットを残さない）ため、機械的に評価できる点へ境界を置く（v1.5 — 確定ゲート 2 周目 P1）
  - **突合の履歴範囲を固定する**（v1.5 — 確定ゲート 3 周目 P1）: **`base` = `merge-base(origin/develop, HEAD)`**（現在地導出の既存定義と同一 — `scripts/feature_status.py`）。**候補探索と反映周コミット探索は同一範囲を使う**。**`base..HEAD` が空になる場合**（HEAD が `origin/develop` の祖先 = feature が既に develop へマージ済み）は**突合の対象外**とし **`対象外(develop へマージ済み)`** と表示する — マージ後の feature の完了は「PR merged + Notion 完了 + worktree 除去」の組で導出され現在地導出の対象ではない（7.6-4）。**無言では省かない**
  - **判定は git の祖先関係で行う**: **上記の範囲で `plan.md` を追加したコミットが、v1.5 を approved にした確定コミットの子孫であるか**。**日付では判定しない**（`created` は `YYYY-MM-DD` の精度しかなく、確定日当日に着手したタスクを一意に評価できない。実際、本規定を起案したタスク自身が確定日と同日の `created` を持つ — v1.5 確定ゲート 1 周目 P1）
  - **rename は追跡しない**（`--follow` を使わない）。Git は rename を履歴に保持せず類似度で推定するため、追跡の有無で「初出」が変わる。**現在のパスでの追加コミットだけ**を見る（同 2 周目 P1）
  - **候補がちょうど 1 件のときだけ判定する**。**0 件**（`base` より前から存在するなど）→ **`未取得(plan 初出コミット未特定)`**、**2 件以上**（削除・再追加、分岐上の複数追加）→ **`未取得(plan 初出コミット複数)`**。**確定コミットを解決できない場合**（squash/rebase で SHA が失われた場合を含む — 本ハーネスは no-ff マージを必須とするため通常は起きない）→ **`未取得(v1.5 確定コミット未解決)`**。いずれも**縮退表示**とし、突合を無言でスキップしない
  - **確定コミットの定数は自己検証する**（v1.5 — 確定ゲート 3 周目 P1）。定数 SHA が ① **解決できる** ② **`origin/develop` の祖先である** ③ **その時点の本書の frontmatter が `status: approved`** の **3 条件をすべて満たすときだけ**判定に使い、欠ける場合は **`未取得(v1.5 確定コミット系統外)`** で縮退する。②③ を課さないと、**旧 SHA が reflog や別 ref に残って「解決可能」なまま系統外になり、全 feature が縮退も警告もなく適用外**になる（no-ff マージ規則は**マージ前の feature ブランチの rebase/amend を防がない**。新規 clone では同じ SHA が解決不能となり、同一のリポジトリ内容でも出力が分岐する）
  - **この定義の帰結**（意図した挙動であり誤判定ではない）: ① v1.5 を取り込む前に plan を初コミットしたタスクは、確定後に着手していても**適用外**になる — 初コミット時点で**規則が作業ツリーに存在しなかった**ため妥当である ② v1.5 より前に着手しても、plan の初コミット時点で v1.5 を取り込んでいれば**適用対象**になる — その時点で規則を参照できるため妥当である
