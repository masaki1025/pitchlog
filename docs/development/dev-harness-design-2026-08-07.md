...
**何を凍結するかは定めない** — それは各資産の宣言と実装が決める(7.7-6)。
...
**7.7-6 委任の境界**: **各資産の宣言と実装へ委任するのは、次の 3 つに限る。**
...
これは **宣言が列挙する対象がすべて検査されたことを、検査自身が示せること**で塞ぐ
(**部分集合ではなく完全一致で示す** — 本書の既存の検査が用いる形と同じ)。

**7.7-7 本節が保証する範囲**(**何を保証しないかを含めて一意にする** — v1.5 の先例と同型):

**保証するのは、次の 1 つに限る。**

> **検査が実行され、かつ比較元が本節の外で改変されていないとき、
...
**保証するのは「上の 2 条件が成立している変更のうち、記録を伴わないものは受理されない」まで**である。
