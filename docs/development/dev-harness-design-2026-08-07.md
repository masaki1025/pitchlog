...
| 版 | 日付 | 変更内容 | 状態 |
| --- | --- | --- | --- |
| 1.15 | 2026-09-14 | **`7.7 凍結基準の更新経路` を新設(TSK-386)**: **ある資産集合の不変性を検査で強制するとき、その基準(コミット・版・digest)を機械可読な宣言として資産側へ置き、検査器のソースへ値を直書きしない**ことを条文化する。あわせて **① 更新経路の必置**(凍結を謳う検査は、基準を別の値へ遷移させる**受理条件**を宣言の中に持ち、満たさない遷移を受理しない。**受理条件は「7.7-1 の構造要求(基準が宣言側にあり検査器のソースに直書きされていないこと)」「7.7-3〜7.7-5 の要求」「7.7-6 が委任する凍結対象・識別値・粒度の指定」の 3 つで尽きる**)**② 追記のみ**(既存記録を書き換える変更・削除する変更を受理しない)**③ 更新の記録**(**新しい基準の識別値・直前の基準の識別値・動かした事実・理由・承認者・承認日**を追記し、**2 つの不変条件** — 追記の「直前」が追記前の履歴末尾の「新」と一致すること、および**検査器が現に用いる基準が履歴末尾の「新」と一致すること** — を満たさない状態を受理しない)**④ fail-closed**(基準の妥当性を確かめられないとき緑にしてはならない)**⑤ 本節が保証する範囲**(**保証は「検査が実行され、かつ比較元が本節の外で改変されていないとき、7.7-2〜7.7-5 を満たさない変更は受理されない」の 1 つに限る**。**保証しないもの 4 件**〔承認の真正性 / 比較元の内容 / 検査が実行されたこと / 複数変更の合成〕を**発火しない状態とともに列挙する** — v1.5 の「本例外が保証する範囲」・7.3-8 と同型)を置く。**委任するのは「何を凍結するか」と「同一性をどの粒度・どの識別値で比べるか」の 2 つに限り、7.7-2〜7.7-5 の判定は委任しない**(`data-model.md:244` と同型の委任 — 同書も**不変条件を先に固定したうえで**対象集合と述語を委任している)。**宣言と実装の食い違いは 2 種類あり拒否する条項が違う** — **基準の識別値の食い違いは 7.7-4 の 2 つ目の不変条件**、**凍結対象・識別値・粒度の食い違いは 7.7-6 自身(独立の要求)**が拒否する(**7.7-4 は識別値しか比べないので、識別値が一致したまま対象を狭める実装は 7.7-4 を通る**)。「宣言が優先する」とは書かない — 黙って宣言を採る実装は食い違いを隠す。**合否条件の新設なので 7.6-3 前段の実装追随ではなく後段(版繰り上げ + 7.3 の確定ゲート)を通す**(先例: v1.14・裁定 `Q-1`)。**背景**: 要件書 v2.8 改訂(TSK-379)で「入力ベースラインは永久に動かない」を前提にした検査が 5 箇所あることに突き当たり、うち 4 箇所は PR #60 で解消したが `scripts/check_authz_catalog.py:107` の `ORACLE_INPUT_BASELINE_COMMIT` は応急処置のまま残った。**正本に凍結の規律を定めた条文が 1 つも無く**、規律は資産と検査器の中だけの宣言だった(調査 — `../features/oracle-input-baseline/research.md`)。上位型は運用評価台帳 `H-85`。 **射程宣言(7.3-7)**: **本改訂で確定する範囲** = 7.7 の **7 項**(基準の外出し・更新経路の必置・追記のみ・更新の記録・fail-closed・委任の境界・**本節が保証する範囲**)。**7.7-7 は保証境界を定める規範であり、確定範囲に含む**(確定ゲート 1 周目 `P1`)。**実装時に確定する範囲** = 台帳 `contracts/authz/frozen-baselines.json` のスキーマと系列構成、各述語の実装、走査方式と allow-list、`ci.yml` への結線、digest 連鎖の短縮(`H-85` 対応案②)の具体。**本改訂は 7.3・7.6 を変更しない**(7.6-3 の決定表は不変)。計画: `../features/oracle-input-baseline/plan.md` | **in-review** |
...
3. **7.7-6 が委任する、当該資産の凍結対象と識別値・粒度の指定。** **宣言はこれを明示的に持つ**
...
**7.7-6 委任の境界**: **各資産の宣言と実装へ委任するのは、次の 2 つに限る。**
...
**宣言と実装が食い違う状態を検査は受理しない。** **食い違いは 2 種類あり、拒否する条項が違う**:

| 食い違い | 拒否する条項 |
| --- | --- |
| **検査器が現に用いる基準の識別値**が、履歴末尾の「新しい基準」と違う | **7.7-4 の 2 つ目の不変条件** |
| **実装が現に用いる凍結対象の集合・識別値・粒度**が、宣言の定めと違う | **本項(独立の要求)** |

**2 つ目を 7.7-4 に帰属させてはならない**(確定ゲート 3 周目 `P0`)—
**7.7-4 の不変条件が比べるのは基準の識別値だけ**であり、
**識別値が履歴末尾と一致したまま凍結対象を狭める実装は、7.7-4 を満たしたまま通る。**

(**「宣言が優先する」とは書かない** — 食い違いを検出したときに宣言の値を黙って採る実装は、
**食い違いが起きたこと自体を隠す**。**7.7-5 と同じ理由で赤にする**。
**7.3-1 の「条文がスキルに優先する」とは形が違う** — あちらは人が読む 2 文書の優先順位であり、
こちらは**機械が検出できる不一致**である。)
