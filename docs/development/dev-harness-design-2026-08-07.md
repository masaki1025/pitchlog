...
- **従う導線**: **6.1 の実装計画書ゲート**(承認済みの実装計画書 → ステップ単位の実装 → PR)。本章冒頭の「**原則 各 Phase = 1 PR、この順に develop へ**」は**適用しない**。**本章は製品実装の開始条件を定めない** — **着手可否は 6.1 の実装計画書ゲート(人間承認)で判断する**。各実装計画書は**着手時点の Phase 4 の受入状態(10.1 の受入証跡と 4-6 のマージ・マージ後検査の状況)を前提として記録**し、その上で承認を受ける
