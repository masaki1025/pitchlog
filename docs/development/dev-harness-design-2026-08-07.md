...
- **plan frontmatter の拡張キー**（feature 作業自身の進行事実のみを置く — 正本 status の複製は置かない〔7.1-1〕。**本表が契約の正** — v1.3）:
  ...
  | `Phase4スロット` | `対象外` \| `4-1`〜`4-6` \| `P4-後`（これ以外は不正） | **なし（必須）** | **v1.6 で新設**。Phase 4 の分割 PR を識別し、`/task-done` が `P4-後` の起票対象を判定するのに使う（13 章）。**任意キーにしてはならない**（v1.6 — 確定ゲート 10 周目 P1）— **欠落を「Phase 4 ではない」と解釈すると、4-6 の計画書で書き忘れたときに `P4-後` を起票しないまま worktree 除去と Notion 完了まで進む fail-open** になる。**欠落・空値・列挙外の値では `/task-done` を中断する**（`反映周コミット` と同じ理由 — 記入漏れを正常系に紛れさせない）。**ただし必須化だけでは 4-6 の書き忘れを塞げない**（v1.6 — 確定ゲート 11 周目 P1）— **テンプレが既定値を出力すると、書き換え忘れが「欠落」ではなく「正常値」として通る**。**11 周目に置いた「Notion タスク名から決めて突合する」方式は 12 周目 P1 で撤回した** — **`/task-start` と `/task-done` が同じタイトルを同じ規則で解釈するだけで独立照合になっておらず、実体が 4-6 でもタイトルにマーカーが無ければ両者とも `対象外` と判断する**（タスク名は規約化されていない自由入力である）。**代わりに人間の明示選択を入力とする**（v1.6 — 確定ゲート 12 周目 P1）: **テンプレは値を持たない空欄を出力し、`/task-start` が本表の列挙を提示して人間に選ばせて埋める**。**選択肢は 13 章が定める 7 個（`4-1`〜`4-6`・`P4-後`）と `対象外` で固定**であり、自動判定しない。**選択の誤りは起票側のマーカーで検出する**（v1.6 — 確定ゲート 13 周目 P1。**選択だけを唯一の入力にすると、誤って `対象外` を選んだ場合に「有効な列挙値」として通り、4-6 のマージ後検査と `P4-後` の起票がまるごと省略されて Phase 4 展開が終端不能になる**）: **`/task-done` が `P4-後` または「Phase 4-6 再試行」を起票するとき、Notion タスクの DoD へ機械可読マーカー行 `Phase4スロット: <値>` を書く**。**`/task-start` はこのマーカーを読み、人間の選択と食い違えば中断する**。**これは 11 周目に撤回した「タスク名の二重読み」とは異なる** — **マーカーは起票側（`/task-done`）が書き、選択は着手側（人間）が行うため、別々の入力である**。**マーカーが無いタスクは Phase 4 の分割スロットではない**ので、選択は自由（`対象外` を含む）。**欠落・空値・列挙外はすべて「既定なし」として `/task-done` を中断する**（本表の既定列が `なし（必須）` である理由 — 同 12 周目 P1。**既定を `対象外` と書くと「欠落は中断」と矛盾し、欠落を `対象外` と読む実装が成立してしまう**）。**旧計画書（`Phase4スロット` を持たない v1.6 以前のもの）も同じく中断し、人間が `対象外` を明示して再実行する**（`反映周コミット` の `規約制定前` と同じ明示移行。**「Phase 4 以外は書き換え不要」ではない** — 同 11 周目 P2） |
...
- **マージ後検査の合格は、Git からの実測と人間の判定記録の両方で認可する**（v1.6 — 確定ゲート 9 周目 P1 で記録先を一意化し、**10 周目 P1 で認可根拠を Git 実測へ移した**）。
  ...
  - **期待値は `E` の中に置き、3 項目とも完全一致で検査する**（v1.6 — 確定ゲート 13 周目 P1。**11 周目の「PR コメントの記録から取る」も 12 周目の `merge-base` による代替も、どちらも撤回した**）。**`M`（マージコミット）と `E`（4-6 の PR head）は GitHub API から取得**し、以下を Git オブジェクトで検査する: **① `M` の第 2 親 = `E`** / **② `M` のツリー = `E` のツリー** / **③ `M` の第 1 親 = `E` に収録された証跡の `phase4_base_sha`**（完全一致）。
    - **`phase4_base_sha` は証跡ファイルの必須キー**とする（10.1 — v1.6 で追加）。**`E` の Git ツリーに収録されるため不変**であり、**値を書き換えれば `E` 自体が変わって受入をやり直すことになる**ので、11 周目に問題となった「可変媒体を単独根拠にする」状態には戻らない
