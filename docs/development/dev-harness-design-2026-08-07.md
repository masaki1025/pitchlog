...
| 証跡の必須項目 | 要件書 NFR-021 の「証跡」箇条書きのとおり（項目を本書に複製せず参照する）。**加えて機械検証のためのキーを持たせる** — `gate_kind`（`phase4` \| `release`）/ `tested_commit_sha` / `onboarding_blob_sha`（**版番号では内容を一意に識別できない** — 実装追随の節更新は版繰り上げなしで可能なため）/ `result`（`passed` \| `failed`）/ `release_version`（`gate_kind: release` のときのみ必須） |
| 検証器の契約 | 入力 = **候補 SHA・要求ゲート種別・要求版**。合格条件 = ① 同一 `gate_kind` の証跡が存在 ② `result: passed` ③ **`tested_commit_sha` が候補 SHA の祖先**（`git merge-base --is-ancestor`）④ `gate_kind: release` なら `release_version` が要求版と一致 ⑤ 下記の失効判定に掛からない。**いずれか判定不能なら fail-closed** |
