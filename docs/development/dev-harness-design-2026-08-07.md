...
- **対象(1 変更集合に限る — 正本 4 本)**: **① 要件書 v2.3**(NFR-021 受入プロファイルの**アーキテクチャ両対応**〔x64 / arm64〕・1 回の受入は実施ホストのアーキテクチャ 1 つに一意化・固定版切替規則の判定単位・証跡のログ参照欄へアーキテクチャ識別の要求・**10 章 未決事項「NFR-021 の継続検証基盤」の決着記録**)+ **② 本書 v1.11**(本節の新設・順序と本数〔8 → 9〕の改訂・10.1 の併存記述と `win-setup` 不採用理由の現況化・実施責任者行の「暫定方針」の現況化)+ **③ `onboarding.md` v1.2**(受入プロファイルの x64 複製の解消・**実施アーキテクチャの採取とログ保存手順**の追加)+ **④ 受入証跡 README v1.3**(ブートストラップ手順を第 3 号へ対応)+ それらに伴う `docs/README.md`(索引)と評価台帳の追随。**プロダクト実装その他の変更を含めてはならない**
- **なぜ要件書を対象に含められるのか(第 2 号の除外理由との関係)**: 第 2 号は要件改訂を「**歯止め『1 変更集合に限る』の適用範囲を超える**」として明示的に外した。**その除外理由は本号では成立しない** — 第 2 号における要件改訂は「`onboarding.md` v1.1 等を確定させる変更集合」とは**別の変更集合**であり、抱き合わせれば 2 変更集合になった。本号では**受入プロファイルの改訂と 10 章の現況化それ自体が唯一の変更集合**であり、本書 v1.11・`onboarding.md` v1.2・受入証跡 README v1.3 は**その変更集合が必然的に伴う従属改訂**(x64 表現の複製解消・号の追加に伴う手順と本数の追随)である。したがって 1 変更集合に収まる
- **理由(なぜスロット外か)**: 受入プロファイルが実機と一致しない限り **`gate_kind: phase4` の受入そのものが実行できない**(10.1「環境」= プロファイル外の結果は合格の証跡にしない)。一方 4-6 のマージ条件は受入合格である。同一スロットに置くと**前提と結論が循環する**(第 1 号・第 2 号と同型)
