...
- **plan frontmatter の拡張キー**（feature 作業自身の進行事実のみを置く — 正本 status の複製は置かない〔7.1-1〕。**本表が契約の正** — v1.3）:
  ...
  **反映周コミットの規定**（v1.5 — 台帳 H-41。`確定ゲート周回` は +1 がコミットなしでも実行できるため、**カウンタだけが進んで証跡が残らない**事象が実際に起きた）:
  ...
  - **本規定が保証する範囲**（v1.5 — 確定ゲート 7 周目。**何を保証しないかを含めて一意にする**）: **`反映周コミット: 適用` の未マージ feature でスクリプトが実行された場合に**、周回集合の一致・1 件名 1 マーカー・各周 1 コミットを**警告として表示する**。**保証しないもの** — 強制（非ブロッキング）／実行そのもの（スキル本文の指示であり機構ではない）／追加 push 後の再検査／マージ時点の最新 HEAD での検査。**残余は台帳 `H-50` で追跡する**
