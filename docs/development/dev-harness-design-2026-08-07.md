...
- **順序**（v1.6 — 確定ゲート 1 周目 P1 で一意化・2 周目 P1 で監査 PR を明示）: **4-1 が先行** → **4-2 / 4-3 は相互任意** → **10.1 のブートストラップが定める予約レコード監査 PR**（`docs/ops/nfr021-acceptance/` の新設）→ **4-4**（同ディレクトリの正本を追加）→ **4-5**（検証器は 4-4 の失効パス正本と証跡テンプレートを本番構成として消費するため）→ **4-6** → **マージ後検査** → **4-7**。本章冒頭の「この順に develop へ」は **Phase 間の順序**を指し、**Phase 4 内はこの規則が正**とする
- **「1 PR」の例外**（本章冒頭）で定めた**監査 PR（予約レコード PR・失敗閉塞 PR）は、この 7 本に数えない**
