...
| **4-4** | **`docs/ops/` の新設**（`nfr021-acceptance/` の `README.md` とテンプレート）+ **失効対象パスの正本** `.claude/nfr021-invalidating-paths.json` | 同上 |
| **4-5** | **証跡の機械検証器** `scripts/verify_nfr021_evidence.py` + **append-only の CI 検査** + これらのテスト + **`/release` 手順 0 の置き換え** | 同上。**この PR 単体では NFR-021 判定を実行しない**（検証器を作る PR と、それを使って判定する PR を分ける） |
| **4-6** | **`onboarding.md` の完成と v1.0 approved 化**（実際の依存導入・DB 初期化・起動・疎通確認まで）+ **Phase 4 完了判定** | **`gate_kind: phase4` の判定に合格していること。** `/release` を経由せず本 PR 上で `candidate_sha` = 証跡コミット E として検証器を直接実行する（10.1 の実施経路）。**合格した場合にのみ develop へマージしてよい** |

- **4-1 〜 4-5 は「Phase 4 の部分成果」であり、単体では Phase 4 の完了を意味しない。** これらのマージは NFR-021 判定を前提としない
- **Phase 4 の完了は 4-6 のマージをもって成立する。** `onboarding.md` の v1.0 approved 化（7.3 の確定ゲート）と NFR-021 受入判定を**同じ PR で**行うのは、受入が「approved な `onboarding.md` で実際に完走した」ことを機械的に裏付ける建付け（10.1）だからである
- **順序の制約**: 4-6 は 4-1 〜 4-5 のすべてがマージされた後でなければ実行できない（依存導入・DB 初期化・起動・疎通確認の対象が揃わないため）。4-2 〜 4-5 の相互順序は個別判断でよい
- **「1 PR」の例外**（本章冒頭）で定めた**監査 PR（予約レコード PR・失敗閉塞 PR）は、この 6 本に数えない**
- **13 章の現況追随は 4-6 で行う**（Phase 4 が完了した時点で初めて「完了した」と書けるため）
