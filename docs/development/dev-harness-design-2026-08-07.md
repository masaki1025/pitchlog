...
| 1.13 | 2026-09-02 | **13 章へ「製品機能の実装」を新設(v1.13)**: **要件書の FR が定める製品機能の実装は本章の Phase の対象外である**ことを宣言した(Phase は本書が定める開発ハーネスの導入単位であり〔**Phase 0 = 本設計案の確定**〕、製品機能の作業単位ではない)。5 項目 — **従う導線** = **6.1 の実装計画書ゲート**(柱書「原則 各 Phase = 1 PR、この順に develop へ」は**適用しない**)/ **開始条件** = **① `passed` 判定の記録 ② 検証器の合格条件①〜⑩の合格 ③ 4-6 の `develop` へのマージ**の 3 条件(①②= 10.1「4-6 マージ前条件」)。**`P4-後` のクローズと Phase 5 の完了は待たない**が、**①〜③は「正式受入」ではなく**、**再受入が必要になった場合は成立まで新規着手を止める**/ **適合判定との関係** = 対象横断の正解ベクタの未完成は**適合と判定しない**ことを意味するのみで実装を禁じないが、**`NFR-018` の対象計算は ADR-003 の導入順序(検査基盤とベクタ schema の先行確定 → 対象ごとに同一変更で追加 → 全検査緑をマージ条件)に従う**(**検査基盤の未整備を理由に対象計算を先に実装することはできない**)/ **適用する規約** = **`AGENTS.md` が正**。8.6 が Phase 4 で導入と定める `.claude/rules/backend.md`・`frontend.md`・`testing.md` は未整備で、整備は別タスク(未整備は着手を止めない)/ **本章では順序・群・段階を定めない**(どの FR から着手するかは各実装計画書の判断)。**Phase は追加していない**(Phase 表の行数は不変・既存 Phase 0〜5 の各行と「Phase 4 の分割」節は改訂前とバイト一致)。**13 章「Phase 4 の分割」節末尾の「機械化しない」の箇条(論理スロットと Notion タスクの対応の機械化)と衝突しない**(段階計画・群・順序・進捗の機械判定を持ち込まないため)。背景 = approved 設計(`../design/sync-protocol.md` 11-3)が FR 10 件を決めている一方で製品コードは `GET /health` のみに留まっており、**Phase 表が Phase 5 で終わることにより製品実装の位置づけが規範上不明**で、それが着手を止めていた。**射程宣言(7.3-7)**: **本改訂で確定する範囲** = ① 製品機能の実装が Phase の対象外であり 6.1 の実装計画書ゲートに従うこと ② 開始条件(上記①〜③と再受入時の停止)③ 適合判定との関係(ADR-003 の導入順序への従属を含む)④ 適用する規約の正が `AGENTS.md` であること ⑤ 本章では順序・群・段階を定めないこと。**実装時(各実装単位のタスク)で確定する範囲** = 実装単位の数・名称・境界・PR 分割・着手順・見積もり、API / UI / データモデルの設計正本、`NFR-018` の検査基盤と正解ベクタの具体、`.claude/rules/` パス別規約の内容。**進捗とタスク対応の機械化は本改訂の射程外であり、実装時にも再検討しない**(13 章「機械化しない」の箇条で既に確定済み — 実装時確定範囲に含めない)。計画: `../features/product-impl-phase/plan.md` | **in-review** |
...
- **従う導線**: **6.1 の実装計画書ゲート**(承認済みの実装計画書 → ステップ単位の実装 → PR)。本章冒頭の「**原則 各 Phase = 1 PR、この順に develop へ**」は**適用しない**
- **開始条件**(次の 3 条件を**すべて**満たすこと): **① `gate_kind: phase4` の結果証跡に人手確認を経た `passed` 判定が記録済み ② 同証跡が 10.1 の検証器の合格条件①〜⑩に合格 ③ 4-6 が `develop` へマージ済み**(①②は 10.1「**4-6 マージ前条件**」と同一)。**`P4-後` のクローズと Phase 5 の完了は待たない** — 製品実装が Phase 4 に求めるのは骨格・CI・開発 DB が実機で動くことであり、①〜③がそれを証明している。**①〜③は 10.1 の「正式受入」ではない**(正式受入はマージ後検査の合格まで満たして初めて成立する)。**10.1 の再受入が必要になった場合**(証跡の失効・マージ後検査の不合格を含む)は、**再受入が成立するまで製品実装の新規着手を止める**(進行中の実装単位の扱いは当該実装計画書の判断)
...
- **適用する規約**: 製品実装コードに適用するコーディング規約の正は **`AGENTS.md`**。**8.6 が Phase 4 で導入すると定める `.claude/rules/backend.md`・`frontend.md`・`testing.md` は未整備**であり、その整備は**別タスク**で行う。**未整備は製品実装の着手を止めない**(規約自体は `AGENTS.md` が持つため)
