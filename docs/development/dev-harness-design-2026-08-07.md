...
手順: /task-start（ブランチ+worktree は維持）→ `codex_run.py fast`（モデル・effort は ADR-001 の「軽微な修正」行をラッパーが固定。**適用時に計画書雛形の `重さ分類: 軽微` と `実行方式: fast` を一括更新する** — 人間の事前 OK の記録であり、ラッパーは登録済み worktree のブランチ名から導出した正規位置 `docs/features/<slug>/plan.md` の計画書だけを受理して両値を検査してから実行する〔不在・読取不能・frontmatter 不正・`branch` 不一致・同一 branch の別位置計画書は停止〕。v1.18）または Claude が直接修正 → /check → PR 本文に**短縮計画**（目的/変更/確認方法）→ CI + 反対側レビュー1本（Codex 実装なら Claude、Claude 直なら Codex）→ 人間マージ。
