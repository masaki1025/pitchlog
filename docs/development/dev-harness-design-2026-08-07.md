...
| 版 | 日付 | 変更内容 | 状態 |
| --- | --- | --- | --- |
| 1.15 | 2026-09-14 | **`7.7 凍結基準の更新経路` を新設(TSK-386)**: **ある資産集合の不変性を検査で強制するとき、その基準(コミット・版・digest)を機械可読な宣言として資産側へ置き、検査器のソースへ値を直書きしない**ことを条文化する。あわせて **① 更新経路の必置**(凍結を謳う検査は、基準を別の値へ遷移させる**受理条件**を宣言の中に持ち、満たさない遷移を受理しない。**受理条件は「7.7-1 の構造要求(基準が宣言側にあり検査器のソースに直書きされていないこと)」「7.7-3〜7.7-5 の要求」「7.7-6 が委任する 3 指定を宣言が持ち実装がそれを導出していること」の 3 つで尽きる**)**② 追記のみ**(既存記録を書き換える変更・削除する変更を受理しない)**③ 更新の記録**(**新しい基準の識別値・直前の基準の識別値・動かした事実・理由・承認者・承認日**を追記し、**2 つの不変条件** — 追記の「直前」が追記前の履歴末尾の「新」と一致すること、および**検査器が現に用いる基準が履歴末尾の「新」と一致すること** — を満たさない状態を受理しない)**④ fail-closed**(基準の妥当性を確かめられないとき緑にしてはならない)**⑤ 本節が保証する範囲**(**保証は「① 検査が実行され、② 比較元が本節の外で改変されておらず、③ 検査器が本節の要求どおりに動作するとき、7.7-2〜7.7-5 を満たさない変更は受理されない」の 1 つに限る**(**③ は変更ではなく状態の条件** — 6 周目 `P0`。**当該変更が弱めていないことだけでは、先行変更で弱めて後続変更で遷移させる経路が残る**。**検査は自分自身の正しさを判定できないので、③ は検証する条件ではなく前提である**)。**保証しないもの 5 件**〔承認の真正性 / 比較元の内容 / 検査が実行されたこと / 複数変更の合成 / **検査器が本節の要求どおりに動作すること**〕を**発火しない状態とともに列挙する** — v1.5 の「本例外が保証する範囲」・7.3-8 と同型)を置く。**委任先は宣言に一本化し実装へは委任しない**(5 周目 `P1`)。**宣言へ委任するのは「何を凍結するか」「同一性をどの粒度・どの識別値で比べるか」「どの宣言レコード(系列)を基準とするか」の 3 つに限り、7.7-2〜7.7-5 の判定は委任しない**(`data-model.md:244` と同型の委任 — 同書も**不変条件を先に固定したうえで**対象集合と述語を委任している)。**委任した 3 つは宣言が持ち、実装は宣言から導出する。実装がこれらを自分で保持してはならない**(7.7-1 の構造要求を委任先へ及ぼす)。**「実装と宣言の一致を検査する」とは書かない** — **一致を検査する形は食い違いの種類を数え上げる形になり、数え落とす**(**範囲確定 — PO 裁定 2026-09-14・7.3-6**。本改訂の確定ゲートで 3 周書き直し、4 周目に 3 種類目〔結線の食い違い〕を数え落としていたことが判明した)。**導出だけでは残る「実装が宣言の一部を読み落とす」「宣言に由来しない値を混ぜる」は、検査が「自分が現に用いた 3 指定」を出力し宣言と完全一致することを示すことで塞ぐ**(**対象集合だけでは足りない** — 全対象を走査しつつ粒度・識別方式を固定値で保持する実装や、現在値が同じ別系列を読む実装が通る。5 周目 `P0`)。**合否条件の新設なので 7.6-3 前段の実装追随ではなく後段(版繰り上げ + 7.3 の確定ゲート)を通す**(先例: v1.14・裁定 `Q-1`)。**背景**: 要件書 v2.8 改訂(TSK-379)で「入力ベースラインは永久に動かない」を前提にした検査が 5 箇所あることに突き当たり、うち 4 箇所は PR #60 で解消したが `scripts/check_authz_catalog.py:107` の `ORACLE_INPUT_BASELINE_COMMIT` は応急処置のまま残った。**正本に凍結の規律を定めた条文が 1 つも無く**、規律は資産と検査器の中だけの宣言だった(調査 — `../features/oracle-input-baseline/research.md`)。上位型は運用評価台帳 `H-85`。 **射程宣言(7.3-7)**: **本改訂で確定する範囲** = 7.7 の **7 項**(基準の外出し・更新経路の必置・追記のみ・更新の記録・fail-closed・委任の境界・**本節が保証する範囲**)。**7.7-7 は保証境界を定める規範であり、確定範囲に含む**(確定ゲート 1 周目 `P1`)。 **範囲確定(7.3-6 の PO 裁定・2026-09-14・山田正輝)**: **4 周連続で `起因件数 × 2 > 母集団` が成立**したため裁定を起動した。**確定範囲の変更点** = **7.7-6 を「実装と宣言の一致を検査する」から「実装は委任先 3 つを宣言から導出する」へ組み替えた**(一致の検査は食い違いの種類の数え上げになり終わらない — 3 周書き直して 4 周目に 3 種類目が出た)。**あわせて委任先を 2 つから 3 つへ**(「どの宣言レコード〔系列〕を基準とするか」を追加 — **値が同じ 2 系列のうち誤った側へ結線する食い違い**が委任の外に落ちていた)。**実装時に確定する範囲は不変。** **裁定後の次周は全文レビュー**(7.3-4 の射程変更周)。**実装時に確定する範囲** = 台帳 `contracts/authz/frozen-baselines.json` のスキーマと系列構成、各述語の実装、走査方式と allow-list、`ci.yml` への結線、digest 連鎖の短縮(`H-85` 対応案②)の具体。**本改訂は 7.3・7.6 を変更しない**(7.6-3 の決定表は不変)。**確定ゲート通過(approved)**: 敵対レビュー **8 周**(sol xhigh — 1 周目 P0×1/P1×4・2 周目 P1×2/P2×1・3 周目 P0×1/P1×2・4 周目 P0×1/P1×2・5 周目〔全文〕P0×2/P1×1・6 周目 P0×1/P2×1・7 周目 P2×1・8 周目〔最終全文〕P2×1 を**全件採用・不採用 0 件**、**最終全文確認周の再実施で指摘ゼロ**により収束)→ **PO 承認(2026-09-14・山田正輝)**。**PO の関与は 2 回** — **4 周目に `範囲確定`**(7.3-6。`起因件数 × 2 > 母集団` が 4 周連続成立。**7.7-6 を「実装と宣言の一致を検査する」から「実装は宣言から導出する」へ組み替え**、**委任先へ「どの宣言レコード〔系列〕を基準とするか」を追加**)/ **6 周目に 6 周警告への `続行の明示指示`**。**`P0` 6 件はすべて「保証範囲の過大主張」か「塞ぐ範囲の限定」**であり、**その是正の到達点が 7.7-7(保証する範囲を 1 文に限り、保証しないもの 5 件を発火しない状態とともに列挙)である**。 計画: `../features/oracle-input-baseline/plan.md` | approved |
...
- **縮退注記(2026-08-16)**: リモート保護(ブランチ保護)は現在**未適用**(10.2 の実装状況・リスク受容記録)。保護未適用の間、前項の「未チェックのままマージしない」はリモートでは強制されず、`github-setup.md` 2 章の管理手続(マージ前の人間確認)で補償する
...
**本節は、検査が「この資産は変わらない」と主張するときの規律を定める。**
**何を凍結するかは定めない** — それは**各資産の宣言が定める**(実装は宣言から導出する — 7.7-6)。

**背景**: 凍結を謳う検査は、基準となるコミット・版・digest を持つ。**その基準を検査器の
ソースへ直書きすると、基準を動かす作業が「検査器を書き換える作業」と区別できなくなる。**
**正当な更新と、検査をすり抜けるための書き換えが、同じ形になる。**

**7.7-1 凍結基準の外出し**: ある資産集合の不変性を検査で強制するとき、
**その基準(コミット・版・digest)は機械可読な宣言として資産側へ置く。**
**検査器のソースへ基準の値を直書きしない。**

**7.7-2 更新経路の必置**: **凍結を謳う検査は、基準を現在の値から別の値へ遷移させる
「受理条件」を、宣言の中に持たなければならない。** **受理条件を満たさない遷移を検査は受理しない。**
**受理条件を持たない凍結は置けない。**

**受理条件は次の 3 つからなる**(**これで尽きる。第四の要素を足さない**):

1. **7.7-1 の構造要求** — **基準の値が宣言側にあり、検査器のソースに直書きされていないこと。**
   (**履歴を正しく追記したうえで、その末尾と同じ値を検査器へ書き込む変更**は、
   下の 2・3 と 7.7-4 の不変条件をすべて満たしながら **7.7-1 に違反する** — 確定ゲート 3 周目 `P1`。
   **7.7-1 は検査の構築時だけの規律ではなく、遷移のたびに満たされていなければならない。**)
2. **7.7-3〜7.7-5 が定める要求**(追記のみ・記録の必須項目と 2 つの不変条件・fail-closed)。
   **資産によらず同一であり、委任しない。** **宣言はこれを書き写さなくてよい** — 本節が定めている
3. **7.7-6 が委任する 3 つの指定を宣言が明示的に持ち、実装がそれを宣言から導出していること。**
   (**実装が指定を自分で保持していれば、この条件を満たさない** — 確定ゲート 4 周目 `P1`。
   **本条件は受理条件の一部であり、独立した第四の条件ではない**。**7.7-7 の保証が及ぶ**。)

(**「経路がちょうど 1 本」とは、受理条件を満たす遷移だけが通ることを指す** — 経路を名前で数えるのではない。
凍結は「永久に変えない」の意ではなく「決められた条件でしか変えない」の意である。
受理条件のない凍結は、いずれ必ず検査器の書き換えとして破られる。)

**7.7-3 追記のみ**: **基準の履歴は追記のみとする。**
**既存の記録を書き換える変更・削除する変更を検査は受理しない。**

**7.7-4 更新の記録**: 基準を動かすとき、次を新しい記録として追記する。

1. **新しい基準の識別値**(コミット・版・digest — 何で識別するかは 7.7-6 の委任先が定める)
2. **直前の基準の識別値**(履歴の先頭では、直前が無いことを表す値)
3. **動かした事実・理由・承認者・承認日**

**不変条件は 2 つ**:

- **追記された記録の「直前の基準」が、追記前の履歴の末尾の「新しい基準」と一致すること**
- **検査器が現に用いる基準が、履歴の末尾の「新しい基準」と一致すること**

**どちらかを満たさない状態を検査は受理しない。**
(2 つ目が無いと、**履歴に形式的な記録を足しつつ、検査器は別の値を見る**状態が通る。)

**7.7-5 fail-closed**: **基準の妥当性を確かめられないとき**(参照先が解決できない、
比較元を読めない、履歴の取得に失敗した等)、**検査は緑にしてはならない。**
(確かめられないことを「異常なし」と扱う検査は、確かめる能力を失ったことを隠す。)

**7.7-6 委任の境界**: **各資産の宣言へ委任するのは、次の 3 つに限る**(**実装へは委任しない** — 実装は宣言から導出する。確定ゲート 5 周目 `P1`)。

1. **何を凍結するか**(凍結対象の集合)
2. **凍結対象の同一性をどの粒度・どの識別値で比べるか**
3. **どの宣言レコード(系列)を基準とするか**

(`../design/data-model.md` の「**各項目の検査対象集合と合格述語は実装が確定する**」と同型の委任。
同書も**不変条件を先に固定したうえで**対象集合と述語を委任している。
**本節は委任先を宣言に一本化する点で同書より狭い** — 実装に独立の裁量を残さない。)

**7.7-2〜7.7-5 の判定そのものは委任しない。** **受理条件の有無・追記のみ・記録の必須項目と
2 つの不変条件・fail-closed は、資産によらず適用される。**

**委任した 3 つは、いずれも宣言が持ち、実装は宣言から導出する。**
**実装がこれらを自分で保持してはならない**(7.7-1 が基準の値について定めるのと同じ構造要求を、
委任先の 3 つへも及ぼす)。

**「実装と宣言の一致を検査する」とは書かない**(**範囲確定 — PO 裁定 2026-09-14・7.3-6**)。
**一致を検査する形は、食い違いの種類を数え上げる形になり、数え落とす。**

**数え落としの例**: **同じ値を持つ 2 つの系列のうち、実装が誤った側を参照している**とき、
**基準の値の一致も、凍結対象・粒度の一致も通る。** **一方の系列を更新すると、
他方の基準が暗黙に動く。** **参照先そのものが食い違いの一種**であり、
**種類を列挙する形ではこれを事前に尽くせない。**

**導出を要求すれば、食い違いは構造上生じない。**

**導出だけでは残るもの**: **実装が宣言の一部を読み落とすこと**、
および **宣言に由来しない値を混ぜること。**

これは **検査が「自分が現に用いた 3 指定」を出力し、その出力が宣言の内容と完全一致することを
示せること**で塞ぐ(**部分集合ではなく完全一致** — 本書の既存の検査が用いる形と同じ)。

**対象集合だけを完全一致で示すのでは足りない**(確定ゲート 5 周目 `P0`)—
**全対象を走査しつつ粒度・識別方式を固定値で保持する実装**や、
**現在値が同じ別の系列を読む実装**は、**対象集合の完全一致を通ってしまう。**
**3 指定のすべてについて示すこと。**

**7.7-7 本節が保証する範囲**(**何を保証しないかを含めて一意にする** — v1.5 の先例と同型):

**保証するのは、次の 1 つに限る。**

> **① 検査が実行され、② 比較元が本節の外で改変されておらず、
> ③ 検査器が本節の要求どおりに動作するとき、
> 7.7-2〜7.7-5 を満たさない変更は受理されない。**

**③ は変更ではなく状態の条件である**(確定ゲート 6 周目 `P0`)。
**当該変更が検査器を弱めていないことだけでは足りない** — **先行する変更で検査器を弱めておき、
後続の変更で不正な遷移を行えば、後続の変更では ①〜③ のうち ①② が成立し、
「当該変更は検査器を弱めていない」も成立する。**
**③ を状態の条件にすることで、この経路は「保証しないもの」の側に落ちる。**

**検査は自分自身の正しさを判定できない。** **③ は本節が検証する条件ではなく、前提である。**

**「信頼できる比較元」とは書かない**(確定ゲート 2 周目 `P1`)— **信頼性は本節が判定できる性質ではない。**
**条件は 3 つとも、成立していないことを本節が検出できない事実**として、次に列挙する。

**保証しないもの**(**発火しない状態を具体的に挙げる** — v1.5 の先例・7.3-8 と同じ精度で):

| # | 保証しないもの | 発火しない状態 |
| --- | --- | --- |
| 1 | **記録された承認者が実際に承認したこと** | 承認欄が**非空でありさえすれば**通る。**前の記録からの複写も通る** |
| 2 | **比較元そのものの内容** | **リモートのマージ保護が未達である間**(10.2)、**比較元を直接書き換えたのち検査を実行**すると、**書き換え後の比較元に対して検査は正常に緑を返す** |
| 3 | **検査が実行されたこと** | **検査の結線が外れている**、または**当該変更を含む経路で実行されない**。結線は本節の射程外(10.1・10.2) |
| 4 | **同一の比較元に対する複数の変更の合成** | **1 件ずつでは受理条件を満たす変更**が、**合わさると凍結を崩す**場合。本節は変更を 1 件ずつ判定する |
| 5 | **検査器が本節の要求どおりに動作すること**(条件 ③ の裏返し)| **検査器を弱める変更**。**同一の変更に基準遷移を含める**場合も、**先行する変更で弱めておき後続の変更で遷移させる**場合も、**どちらも塞がらない**。**検査は自分自身の正しさを判定できない**(既知の循環参照)。**③ は本節が検証する条件ではなく前提である** |

**したがって 7.7-3・7.7-4 は「記録を残さずに基準を動かせない」を無条件には保証しない。**
**保証するのは「上の 3 条件が成立している変更のうち、記録を伴わないものは受理されない」まで**である。
