...
**宣言と実装が食い違う場合は宣言が優先する**(実装は宣言の写像であり規範ではない — 7.3-1 と同型)。

**7.7-7 本節が保証する範囲**(**何を保証しないかを含めて一意にする** — v1.5 の先例と同型):

**保証するのは、次の 1 つに限る。**

> **信頼できる比較元に対して検査が実行されたとき、7.7-2〜7.7-5 を満たさない変更は受理されない。**

**保証しないもの**:

- **記録された承認者が実際に承認したこと。** 検査が見るのは項目の充足であって承認の真正性ではない
- **比較元そのものの正しさ。** **リモートのマージ保護が未達である間**(10.2)、
  **比較元を直接書き換える経路は本節では塞がらない** — 書き換えられた比較元に対して
  検査は正常に緑を返す
- **検査が実行されたこと。** 検査の結線は本節の射程外である(10.1・10.2)

**したがって 7.7-3・7.7-4 は「記録を残さずに基準を動かせない」を無条件には保証しない。**
**保証するのは「検査が実行された変更のうち、記録を伴わないものは受理されない」まで**である。
