...
**受理条件は次の 2 つからなる**(**これで尽きる。第三の要素を足さない**):

1. **7.7-3〜7.7-5 が定める要求**(追記のみ・記録の必須項目と 2 つの不変条件・fail-closed)。
   **資産によらず同一であり、委任しない。** **宣言はこれを書き写さなくてよい** — 本節が定めている
2. **7.7-6 が委任する、当該資産の凍結対象と識別値・粒度の指定。** **宣言はこれを明示的に持つ**
