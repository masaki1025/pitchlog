...
| 版 | 日付 | 変更内容 | 状態 |
| --- | --- | --- | --- |
| **1.16** | 2026-09-21 | **10.1 の実装追随(TSK-235、版は上げない — 7.6-3 前段)**: `consistency` を検査基盤の移行判定・要求①〜④・計画/履歴監査の実装へ現行化し、独立した `mutation` ジョブを追加した。PR #74 の `pull_request` run `35529110007` で両ジョブが実行され `success` となった証跡を、同節の構造化レコードへ固定した。前 run `35525816853` の `consistency` failure は PR merge commit の第一親を誤って監査した欠陥であり、PR head を起点に直して再実行した。リポジトリ public 化と active Ruleset の現況により 10.1・10.2 の保護未適用記述は古いが、その全面是正は別タスクとし、本追随はジョブ表と発効証跡に限定する。計画: `../features/domain-calc-dsl/plan.md` | **approved** |
...
| `consistency` | `NFR-018` (e) `BOOT-ACTIVATION` の機構・負例・正例(ステップ 17〜25)と、計画書・履歴監査(ステップ 51・52)を常時実行する。**移行状態における本ジョブの green の意味は下記「移行状態における CI 全ジョブ green の意味」が定める**(v1.15) | **TSK-235 で導入済み** |
| `mutation` | Python / TypeScript / SQL / 表示系の変異テストを `consistency` と分離して並列実行する。変異処理内部の 10 分 / 30 分制限とは別に、ジョブ自身の `timeout-minutes` を持つ | **TSK-235 で導入済み** |
...
> **現況注記(2026-09-21)**: リポジトリは public 化済みで Ruleset `protect-main-develop` (id `23694095`) が `active`・bypass なし・strict policy 有効である。下記および 10.2 の保護未適用記述は public 化前のもので現状と異なり、是正は別タスクが行う。本追随はジョブ表と発効証跡に限定する。

**`BOOT-ACTIVATION` 実行証跡(2026-09-21)**: 次のレコードは PR #74 の `pull_request` run を実測した結果である。ネットワークを使わない検査では、レコードの exact-set、各要求のテスト集合と `ci.yml` の実行対象との一致、およびレコード欠落時の failure を検査する。

<!-- BOOT-ACTIVATION-EVIDENCE -->
```json
{
  "schemaVersion": 1,
  ...
  "jobs": {
    "consistency": {
      ...
      "activationRequirements": {
        "mechanism": [
          "tests/domain/boot/test_stall.py",
          "tests/domain/boot/test_phase2.py",
          "tests/domain/boot/test_report.py",
          "tests/domain/boot/test_clauses.py",
          "tests/domain/boot/test_judge.py"
        ],
        "negativeCases": [
          "tests/domain/boot/test_negatives_seal.py",
          "tests/domain/boot/test_negatives_state.py"
        ],
        "exceptionalTransitions": [
          "tests/domain/boot/test_positives_a.py"
        ],
        "normalOperation": [
          "tests/domain/boot/test_positives_b.py"
        ]
      },
      "structuralAudits": [
