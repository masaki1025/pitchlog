...
| 版 | 日付 | 変更内容 | 状態 |
| --- | --- | --- | --- |
| 1.17 | 2026-09-21 | **10.2 のリスク受容記録・承認済み例外の解除と、解除手続の新設(起案)**(TSK-429): 2026-09-19 の public 化と Ruleset 適用により、10.2 が持つ 2 つの留保の前提条件が消滅した。本書はこれまで解除**条件**を書く一方で**誰がどう閉じるか**を定めておらず、閉じる手続が存在しなかった。 **射程宣言(7.3-7)** — **本改訂で確定する範囲**: ①**解除手続の新設**(提案者の立証義務 / 判定者は当該記録の承認者と同じ役割 / **追記による解除**(記録を削除しない) / **解除の範囲は当該記録が明示した対象に限る** / **追跡先が閉じている場合は解除前に実在する追跡先へ差し替える** の 5 項) ②**承認済み例外(本節の自動再現要件)を、後続タスク `TSK-432` を持つ新しい未達へ切り替える**(「保護が利用可能になるまで」という前提が消滅し例外が成立しなくなったため。**スクリプトは未実装で要件自体は依然として未達**) ③**リスク受容記録(2026-08-10・NFR-019 のリモート強制未達)を上記手続に従って解除**し、**追跡先を完了済みの `TSK-202` から `TSK-217`(PO 裁定 `C`・2026-09-12 の NFR-019 柱書 owner)へ差し替える**。 **本改訂で変更しない範囲**: **NFR-019 条文の (a) 一致性・(b) 越境・(c) E2E・(d) 同期故障系の未達状態**(解除は「リモート強制未達」に限る) / 10.1 の受入ゲートと残余リスクの規定 / `github-setup.md` 2 章 手続 3・4 の継続 / 補償統制の内容 / 6.4 の過去事実の記述。 **実装時に確定する範囲**: `setup_branch_protection.py` の実装形式と適用後検証の具体(`TSK-432`)。 | **in-review** |
...
- **三重の強制(目標状態)**: (1) hooks が **main/develop 上での `commit`・`merge`・`rebase`** と、**main/develop 宛ての `push`** をブロック（8.3。**push は宛先で判定する** — カレントブランチは宛先の代理にならないため。v1.5 で是正）、(2) GitHub ブランチ保護が直 push と CI 未達マージを拒否（10.2）、(3) CLAUDE.md / AGENTS.md に明記（両AIの行動規範）。**(2) は 2026-09-19 に適用し、三重の強制が揃った**（10.2 実装状況）: hooks は Claude Code 経由の操作のみ遮断し（人間の端末・別 clone・GitHub UI には効かない）、マージ阻止は github-setup.md 2 章の管理手続で補償する
...
- **リモート保護の適用(2026-09-19 — 2026-08-16 に置いた注記を更新)**: リモート保護(ブランチ保護)は**適用済み**(10.2 の実装状況)。前項の「未チェックのままマージしない」は**必須 9 context + `strict_required_status_checks_policy` によりリモートで強制される**。ただし `github-setup.md` 2 章の**手続 3・4(コア領域 ∪ guard_paths の逐行確認)は継続**する — 保護は検査ロジック自体の改変を防げないため(同 2 章の循環参照)
...
- **CI 全ジョブ green をマージ条件にする**（10.2）— 要件書 7.3 / NFR-019 の直接要求。**このマージ条件のリモート強制は 2026-09-19 に有効化した**（10.2 実装状況。必須 9 context + `strict_required_status_checks_policy`）
...
- **【2026-09-21 改訂・TSK-429】承認済み例外の切り替え**: **前提条件は 2026-09-19 に消滅した**(public 化により Rulesets が利用可能になり、`protect-main-develop`(id `23694095`)を適用済み)。**ただし `scripts/setup_branch_protection.py` は未実装であり、本節の自動再現要件は依然として未達である。** 前提条件が消滅した以上「保護が利用可能になるまで」の承認済み例外は成立しないため、**後続タスクを持つ新しい未達へ切り替える**:
  - **未達の内容**: 本節の「`scripts/setup_branch_protection.py`(gh api 使用)で再現可能にする」要件
  - **担当**: PO / **追跡先**: Notion `TSK-432`
  - **DoD**: 冪等(GET→POST/PUT)・重複検知・dry-run・適用後検証を備えること
  - **再確認条件**: 当該タスクの完了時、または Ruleset の構成が変わった時点
...
- **リスク受容記録・承認済み例外の解除手続(2026-09-21 新設・TSK-429)**: 本書はこれまで解除**条件**を書く一方で**誰がどう閉じるか**を定めていなかった。以下を定める。
  ...
  2. **判定者は、当該記録を承認した者と同じ役割**とする(本節の記録では PO)。承認者個人が不在の場合は現任者が判定する
  3. **解除は当該記録へ追記して行い、記録を削除しない**。追記には **解除日・判定者・充足の典拠・解除後に残る未達**を書く
  4. **解除の範囲は当該記録が明示した対象に限る。** 記録が対象としていない要件・条文が充足したとは主張しない
  5. **追跡先が閉じたタスクである場合は、解除の前に実在する追跡先へ差し替える**(閉じたタスクは残作業を追えないため)
- **【2026-09-21 解除・TSK-429】** 上記手続に従って解除した。
  - **解除日** = 2026-09-21 / **判定者** = PO
  - **充足の典拠** = ①プラン制約の解消: 2026-09-19 の public 化 ②3 章の保護適用: ruleset `protect-main-develop`(id `23694095`)を GitHub API で実測 — `enforcement: active` / 必須 9 context / `strict_required_status_checks_policy: true` / `bypass_actors: []`
  - **解除の範囲**(手続 4): 本記録が対象とした「**リモート強制未達**」に限る。**NFR-019 条文の (a) 一致性・(b) 越境・(c) E2E・(d) 同期故障系は依然として未達**であり、本解除はそれらの充足を主張しない。また **`required_approving_review_count: 0`** であるため、**強制されているのは必須チェックの green であって人間のレビュー承認ではない**
  - **解除後に残る未達**(手続 3): 上記 (a)〜(d) / 本節の自動再現要件(上の切り替え先)/ `github-setup.md` 3 章が運用開始の条件とする実地検証 2 件(→ `TSK-433`)
  - **追跡先の差し替え**(手続 5): 旧追跡先 **`TSK-202` は完了済み(2026-08-10)で残作業を追えない**。**`TSK-217`** へ差し替える — 同タスクは **PO 裁定 `C`(2026-09-12)**により NFR-019 柱書「**比較単位と正解ベクタの定義((a)(c)(d) を含む)**」と「CI 全ランナー配線」の owner と定められている。(d) の実装カードは **`TSK-324`**
