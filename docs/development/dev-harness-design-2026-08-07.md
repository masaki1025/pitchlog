...
| 1.13 | 2026-09-02 | **7.2 に「旧 SPA(React)参照資料の置き場」の注記を追加**(TSK-308 A-1 裁定の反映: リポへ追加せず外部アーカイブ固定コミット `dd03160` への典拠記録に留める — **規範条件は変えない節更新・7.6-3 前段。版は上げない**。裁定記録: `../worklog/2026-09-02-decision-sheet-ruling.md`) | approved(様式は不変) |
...
- **旧 SPA(React)参照資料の置き場**(PO 裁定 2026-09-02・TSK-308 A-1): `docs/legacy/` は版固定アーカイブで追加禁止(AGENTS.md 絶対規則 3)のため、**旧 SPA の参照資料はリポジトリへ追加せず、典拠の記録に留める** — 典拠は外部 private アーカイブ `masaki1025/Baseball_Scoring-archive` の固定コミット `dd03160044aa5932d3b5a025870c9a5a56d979ef`(annotated tag `pitchlog-req-v2.0-evidence`)への `旧:<path>:<line>` 形式で記す。**照合には当該アーカイブへのアクセスが要る**(保全・許諾・再検証の記録: `../worklog/2026-08-13-req-v2-legacy-parity.md`・`../worklog/2026-09-02-decision-sheet-ruling.md`)
