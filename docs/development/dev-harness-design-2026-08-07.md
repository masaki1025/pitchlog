...
- **plan frontmatter の拡張キー**（feature 作業自身の進行事実のみを置く — 正本 status の複製は置かない〔7.1-1〕。**本表が契約の正** — v1.3）:
  ...
  | `Phase4スロット` | `4-1`〜`4-6` \| `P4-後`（これ以外は不正） | **なし（任意）** | **v1.6 で新設**。Phase 4 の分割 PR を識別する。**キーが無い計画書は Phase 4 のスロットではない**（既存の計画書に影響しない）。`/task-done` が `P4-後` の起票対象を判定するのに使う（13 章） |
...
- **Phase 4 の完了は「4-6 のマージ**かつ**マージ後検査の合格」をもって成立し、その事実は `P4-後` で記録する**（v1.6 — 確定ゲート 1 周目 P1・2 周目 P0）。10.1 は**マージ後に M の両親・ツリーを検査し、不一致なら Phase 4 未完了として M を再受入する**と定めている。**マージだけで完了を確定させない**。**不合格だった場合**: 10.1 の定めどおり **M を新たな T として再受入**する。**再試行は新しいタスク・新しいブランチで行う**（v1.6 — 確定ゲート 9 周目 P1。`M` は既に `develop` にあり、元のブランチは閉じているため）。**元のタスクは `/task-done` を通さない** — **再試行タスクを起票して元タスクからリンクし、元タスクの Notion を `ブロック中` にする**。**`P4-後` の起票は、合格した再試行タスクの `/task-done` で行う**。**論理スロットの識別は計画書 frontmatter の `Phase4スロット` キー**（`4-1` 〜 `4-6` / `P4-後`。**任意キーで、無い計画書は Phase 4 のスロットではない**）。**その追加の合格証跡 PR は「4-6 の再試行」として数え、監査 PR には数えない**（本章冒頭の監査 PR の例外は**予約レコード PR・失敗閉塞 PR に限る** — 同 2 周目 P1）。**再試行が合格するまで `P4-後` は実行できない**
...
- **マージ後検査の合格は 4-6 の PR コメントへ記録する**（v1.6 — 確定ゲート 9 周目 P1）。**記録項目**: `M` の完全 OID / **期待した両親とツリー** / **実測した両親とツリー** / 実施日 / **合否** / 確認者。**`/task-done` はこの記録だけを確認する**。**13 章の現況追随（完了証跡）を条件にしてはならない** — それは `P4-後` で書くものであり、**起票の条件に使うと「記録が存在しない時点で参照する」循環になる**。**10.1 の排他区間の解除記録では代替できない** — 解除は中断時にも行うため合格を証明しない
