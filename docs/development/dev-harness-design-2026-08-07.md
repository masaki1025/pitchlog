...
手順: /task-start（ブランチ+worktree は維持）→ `codex_run.py fast`（モデル・effort は ADR-001 の「軽微な修正」行をラッパーが固定）または Claude が直接修正 → /check → PR 本文に**短縮計画**（目的/変更/確認方法）→ CI + 反対側レビュー1本（Codex 実装なら Claude、Claude 直なら Codex）→ 人間マージ。
...
- エイリアス（`gpt-6` 等 — 提供側が上位 tier へ振り向け得る）を使わず明示 ID で固定する（モデルドリフト防止 — 8.5 の Opus 5.5 固定と同じ規律）。**GPT-6 世代では全枠を `gpt-6-sol`（機械的軽作業のみ `gpt-6-luna`）に寄せ、`gpt-6-astra` は載せない**（利用上限内で長く働く目的 — 理由と巻き戻し条件は ADR-001 v1.1）
- 本表は `.claude/scripts/codex_run.py` が**機械適用**する（計画書の重さ分類 → フラグ変換。人が都度選ばず、スキルの自然言語にも依存しない — P0-2/P1-6）。**表の正は ADR-001 の決定表（固定構文）**で、`tests/` の同期テストが ADR-001 を読んでラッパー定数との一致を検査する（本節は値を持たない）
