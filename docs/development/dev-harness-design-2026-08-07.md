...
| `/task-done`（**Phase 4 の 4-6 を閉じる場合は `P4-後` の起票・相互リンクが先** — v1.6・13 章） | 完了の唯一の出口: PR マージ確認 → develop 最新化（`pull --ff-only`）→ worktree 除去（12.1）→ Notion 完了。worklog 締め・計画書 in-review 化は **/pr が PR 内で実施済み**（マージ後の develop を直接編集しない） |
...
- **マージ後検査の合格は、Git からの実測と人間の判定記録の両方で認可する**（v1.6 — 確定ゲート 9 周目 P1 で記録先を一意化し、**10 周目 P1 で認可根拠を Git 実測へ移した**）。
  - **① Git からの実測（改竄不能な根拠）**: `M`（マージコミット）と `E`（4-6 の PR head）を GitHub API から取得し、**`M` の第 2 親 = `E`** かつ **`M` のツリー = `E` のツリー**であることを Git オブジェクトで確認する。**ツリーの一致が本質**であり、これが成立すれば「検証後に `develop` へ入った変更が `M` に混入していない」ことが保証されるため、**第 1 親（判定時点の `origin/develop`）の照合は合格条件として要らない**（10.1 が第 1 親も挙げているのは同じ性質を別角度から述べたもの）。**Git オブジェクトを解決できないときは判定不能として fail-closed**
