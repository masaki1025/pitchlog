...
- **plan frontmatter の拡張キー**（feature 作業自身の進行事実のみを置く — 正本 status の複製は置かない〔7.1-1〕。**本表が契約の正** — v1.3）:
  ...
  **反映周コミットの規定**（v1.5 — 台帳 H-41。`確定ゲート周回` は +1 がコミットなしでも実行できるため、**カウンタだけが進んで証跡が残らない**事象が実際に起きた）:
  ...
  - **判定は git の祖先関係で行う**: **`base..HEAD` で `plan.md` を追加したコミットが、v1.5 を approved にした確定コミットの子孫であるか**。**日付では判定しない**（`created` は `YYYY-MM-DD` の精度しかなく、確定日当日に着手したタスクを一意に評価できない。実際、本規定を起案したタスク自身が確定日と同日の `created` を持つ — v1.5 確定ゲート 1 周目 P1）
