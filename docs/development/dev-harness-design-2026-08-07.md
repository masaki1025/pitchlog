...
- **plan frontmatter の拡張キー**（feature 作業自身の進行事実のみを置く — 正本 status の複製は置かない〔7.1-1〕。**本表が契約の正** — v1.3）:
  ...
  | `Phase4スロット` | `対象外` \| `4-1`〜`4-6` \| `P4-後`（これ以外は不正） | **なし（必須）** | **v1.6 で新設**。Phase 4 の分割 PR を識別し、`/task-done` が `P4-後` の起票対象を判定するのに使う（13 章）。**任意キーにしてはならない**（v1.6 — 確定ゲート 10 周目 P1）— **欠落を「Phase 4 ではない」と解釈すると、4-6 の計画書で書き忘れたときに `P4-後` を起票しないまま worktree 除去と Notion 完了まで進む fail-open** になる。**欠落・空値・列挙外の値では `/task-done` を中断する**（`反映周コミット` と同じ理由 — 記入漏れを正常系に紛れさせない）。**ただし必須化だけでは 4-6 の書き忘れを塞げない**（v1.6 — 確定ゲート 11 周目 P1）— **テンプレが既定値を出力すると、書き換え忘れが「欠落」ではなく「正常値」として通る**。**11 周目に置いた「Notion タスク名から決めて突合する」方式は 12 周目 P1 で撤回した** — **`/task-start` と `/task-done` が同じタイトルを同じ規則で解釈するだけで独立照合になっておらず、実体が 4-6 でもタイトルにマーカーが無ければ両者とも `対象外` と判断する**（タスク名は規約化されていない自由入力である）。**代わりに人間の明示選択を唯一の入力とする**（v1.6 — 確定ゲート 12 周目 P1）: **テンプレは値を持たない空欄を出力し、`/task-start` が本表の列挙を提示して人間に選ばせて埋める**。**選択肢は 13 章が定める 7 個（`4-1`〜`4-6`・`P4-後`）と `対象外` で固定**であり、自動判定しない。**欠落・空値・列挙外はすべて「既定なし」として `/task-done` を中断する**（本表の既定列が `なし（必須）` である理由 — 同 12 周目 P1。**既定を `対象外` と書くと「欠落は中断」と矛盾し、欠落を `対象外` と読む実装が成立してしまう**）。**旧計画書（`Phase4スロット` を持たない v1.6 以前のもの）も同じく中断し、人間が `対象外` を明示して再実行する**（`反映周コミット` の `規約制定前` と同じ明示移行。**「Phase 4 以外は書き換え不要」ではない** — 同 11 周目 P2） |
...
- **マージ後検査の合格は、Git からの実測と人間の判定記録の両方で認可する**（v1.6 — 確定ゲート 9 周目 P1 で記録先を一意化し、**10 周目 P1 で認可根拠を Git 実測へ移した**）。
  ...
  - **3 項目とも Git だけで検証する**（v1.6 — 確定ゲート 12 周目 P1。**11 周目に「第 1 親の期待値は記録から取る」と書いたのは撤回した** — **`phase4_base_sha` を PR コメントから取ると、base が動いた後にコメントの期待値を動いた先へ書き換えるだけで 3 条件がすべて成立し、「可変媒体を単独根拠にしない」に反する**）。**`M`（マージコミット）と `E`（4-6 の PR head）は GitHub API から取得**し、以下を Git オブジェクトで検査する: **① `M` の第 2 親 = `E`** / **② `M` のツリー = `E` のツリー** / **③ `git merge-base <E> <M の第 1 親>` == `M` の第 1 親**（= **`M` の第 1 親が `E` の祖先である**）。**③ が「base が動いていない」ことの記録非依存な検査になる** — `E` のブランチは `origin/develop` から切られているので、**分岐後に `develop` が進んでいれば `M` の第 1 親は `E` の祖先ではなくなる**。**10 周目の revert 反例（`B → D → C` で `C` のツリーが `B` に戻る）でも `merge-base(E, C) = B ≠ C` となり検出できる**。**`phase4_base_sha` を記録する必要はない**（10.1 の排他区間における base 固定は、マージ**前**の再照合のための運用値として従前どおり残す）。**Git オブジェクトを解決できないときは判定不能として fail-closed**
  - **② 人間の判定記録**: 4-6 の PR コメントへ **`M` の完全 OID / 期待した両親とツリー / 実測した両親とツリー / 実施日 / 合否 / 確認者**を記録する（10.1 が「人間の判定者による合格判定は必須」と定めるため）
