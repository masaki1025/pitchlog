...
| 1.6 | 2026-08-16 | **Phase 4 を複数 PR へ分割（起案）**: 13 章へ「**Phase 4 の分割**」節を新設し、**PR 列**（**初期起案時は 6 本** → 確定ゲート 2 周目で 7 本 → **4 周目で Phase 4 は 6 論理スロット + Phase 4 外の後処理 1 本へ** — **現行は本文の表が正**）と、**各 PR のマージ条件**、**最終統合と NFR-021 判定の担当**（**4-6 のみ `gate_kind: phase4` を実行**）を明記した。**NFR-021 の合格項目は変えない**（要件書 NFR-021 の測定方法が正で、本書では再列挙しない）— 変えたのは**成立時点の具体化**（Phase 4 の完了 = 4-6 のマージ後検査の合格）と**担当 PR の明示**であり、受入は Phase 4 全体に対して 1 回のまま。13 章の見出しを「各 Phase = 1 PR」から「**原則** 各 Phase = 1 PR。**Phase 4 のみ分割**」へ改め、Phase 4 行の完了条件にも分割を明記。**起案の契機**: Phase 4 は 8 系統を含み 1 PR では規模・レビュー負荷とも成立しない（PO 判断 2026-08-16）。**ゲート区分**: 当初は「納品の粒度であって統制ではない」として PR レビューで通す想定だったが、**13 章はマージ条件そのものを定めている**ため粒度の変更はマージ条件の変更にあたる、との指摘（計画レビュー 1 周目 P1-4）を受けて**確定ゲートへ切り替えた**。**確定ゲート 1 周目の反映**（P0×1 / P1×5 を全件採用）: **`/release` 手順 0 の置き換えを 4-5 から 4-6 へ移した**（4-5 で解除すると Phase 4 合格前にリリース経路が開く — `gate_kind: release` の契約に「phase4 が合格済み」条件が無いため。v1.4 で閉じた経路の再発）。**完了条件を「4-6 のマージかつマージ後検査の合格」へ**（10.1 はマージ後に M の両親・ツリーを検査し不一致なら再受入と定めている）。**`docs/ops/nfr021-acceptance/` の新設を 4-4 から外した**（10.1 のブートストラップが監査 PR で新設すると定めており作成主体が競合。README・テンプレートは確定ゲート対象で通常 PR では通せない）。**順序を一意化**（4-1 先行 → 4-2/4-3/4-4 相互任意 → 4-4→4-5 → 全件後に 4-6）。**`win-setup` のランナー選定を 4-6 へ割り当て**（10.1 の CI 表が「Phase 4 で選定」としているため）。**確定ゲート 2 周目の反映**（P0×1 / P1×3 / P2×2 を全件採用）: **`/release` 手順 0 の置き換えを 4-6 から新設の 4-7 へ移した** — **マージと事後検査は原子的に実行できない**ため、4-6 で置換するとマージ後検査が不合格だった場合に「Phase 4 未完了のまま `/release` が開いた」状態が develop に残る（1 周目 P0 と同型の再発）。**無条件中断は 4-6 のマージ後検査に合格するまで維持する**。**`docs/ops/nfr021-acceptance/` の `README.md` とテンプレートを 4-4 へ戻し、4-4 に確定ゲートを割り当てた**（元の Phase 4 の 8 系統のうちこれだけが未割当になっていた。同ディレクトリの正本は 10.1 の文書分類が確定ゲート対象と定めている）。**13 章の現況追随を 4-7 へ**（4-6 はマージ済みで事後変更できない）。**マージ後検査が不合格だった場合の扱いを明記**（M を新たな T として再受入し、その PR は 4-6 の再試行として数える。監査 PR の例外は予約・失敗閉塞に限る）。**`win-setup` の後送りを認めない**（10.1 が Phase 4 で選定と確定しており、13 章だけで片側から緩和できない）。**順序へ予約レコード監査 PR を組み込んだ**。**確定ゲート 3 周目の反映**（P1×4 を全件採用。**P0 なし**）: **「受入完了」（4-6 のマージ後検査の合格）と「クローズ」（4-7 のマージ）の二時点を区別**し、**4-7 に再受入・マージ後検査が不要な理由**を明記（4-7 の変更は `release/SKILL.md` など 10.1 の失効対象を含むため、**次回リリースの受入で捕捉される** — **8 周目 P2 で因果を訂正**: 失効判定は `T` 以後の差分のみを見るため、`P4-後` は **`T` に取り込んだ状態で新規受入を行う**）。**4-4 は「確定ゲートの完了」と「マージ」を分ける**（未確定形式の予約レコードを先に統合すると、形式変更時に**改変禁止のレコードを救済できない**）。**「4-1 〜 4-7」は論理スロットで物理 PR 数ではない**ことを明記（4-6 には 0 回以上の再試行 PR が属する）。**完了証跡の保存先を 13 章の現況追随と定めた**（`docs/ops/` へ置くと 10.1 の証跡種別が増え、検証器の列挙対象の定義が要るため）。**元の 8 系統との再突合で抜けなしを確認**。**確定ゲート 4 周目の反映**（P1×1 / P2×1。**P0 なし**）: **`4-7` を `P4-後`（Phase 4 完了後の後処理）へ再分類し、Phase 4 の範囲を 4-1 〜 4-6 に閉じた** — `P4-後` を Phase 4 に含めたまま完了を 4-6 時点と定めると、**Phase 4 の一部を 4-6 の証跡が覆えない**状態になり、「Phase 4 全体に対して受入を 1 回」という原則と衝突する（もう一方の案は**判定前に `/release` が開く**ため循環する）。**論理スロットは 7 → 6**。**`P4-後` の 13 章追随は 7.6-3 前段で通常 PR レビュー**（`M` の SHA・実施日・合格の事実のみで規範条件を変えないため）。**確定ゲート 5 周目の反映**（P1×2。**P0 は 3 周連続ゼロ**）: **改訂範囲を 13 章から 10.1・`.claude/skills/release/SKILL.md` へ拡張した** — 両者が「手順 0 の置換は **Phase 4 の PR でのみ**」と限定しており、`P4-後` を Phase 4 外にした結果**置換がどの Phase の成果物でもない状態**になっていた（安全側には閉じているが、正規手順でリリース経路を有効化できない）。**「Phase 4 完了後の後処理 PR（`P4-後`）でのみ」へ同期**し、Phase 4 行の内容欄も**「Phase 4 の成果物は検証器の実装まで」**へ整理した。**`P4-後` を必須の後続処理と定め**、**そのマージを Phase 5 着手・Phase 4 展開タスクのクローズの前提**とした（起票と完遂の責任は 4-6 の PR 作成者）— 定めないと `P4-後` を放置でき、`/release` の無条件中断が残り続ける。**確定ゲート 6 周目の反映**（P0×1 / P1×3 / P2×1）: **手順 0 の中断条件を「Phase 4 完了まで」という時点ベースから「手順 0 が実際に置換されるまで」という状態ベースへ改めた**（10.1・8.4・`release/SKILL.md` の 3 箇所）— **完了〔4-6 のマージ後検査の合格〕と置換〔`P4-後` のマージ〕は別の時点**であり、時点で書くと**その間に中断条件が偽になるのに置換もされていない中間状態**が生まれ、NFR-021 を飛ばしてリリースへ進める。**`P4-後` のマージをプロダクトリリース開始の前提にも加えた**。**改訂範囲へ 8.4 を追加**（`/release` スキル一覧が旧規定のままだった）。**`P4-後` と `/task-done` を接続**（4-6 の `/task-done` で `P4-後` を起票・リンクし、`P4-後` のタスクが「Phase 4 展開」の終端）。**`.claude/skills/release/SKILL.md` を `guard_paths` へ追加**し、**`P4-後` の DoD に手順 0 の配線検査**を課した（`P4-後` は唯一のリリース入口自身を書き換える PR であり、「次回リリース時の失効判定で捕捉」は入口が変更対象のため成立しない）。**確定ゲート 7 周目の反映**（P1×2 / P2×2。**P0 なし**）: **`/task-done` を同期対象へ追加**し、**4-6 を閉じる場合は `P4-後` の起票・相互リンクを先に行う**手順を実スキルへ書いた（設計書に書くだけでは実行経路が無く、正規スキルだけを読んだ実行者が `P4-後` を作らず 4-6 を閉じられた）。**`P4-後` の内容の捕捉を 2 系統に分けた** — ① 手順 0 の配線そのものは `P4-後` の DoD + `guard_paths` の逐行確認（**入口自体が変更対象なので次回リリースでは捕捉できない**）② 配線成立後のその他の失効対象は次回リリースの `gate_kind: release`。**8.4 の「それまで」を明確化**（検証器が実装されても `P4-後` の置換までは止まる）。**`release/SKILL.md` の報告文を状況で使い分け**（検証器実装後・`P4-後` 前に「未実装」と誤報しないため）。**確定ゲート 8 周目の反映**（P1×4 / P2×2。**P0 なし**）: **`/task-done` の `P4-後` 起票を worktree 除去の *前* へ移し**（除去後だと中断時の再実行が成立しない）、**対象条件を「論理スロット 4-6〔再試行を含む〕」かつ「当該 `M` のマージ後検査の合格が記録済み」へ固定**（`feature` slug では一意に判定できず、不合格の 4-6 や再試行で重複起票されうるため）。**相互リンクから `URL` プロパティを排除**（同欄は `/pr` の GitHub PR URL 専用で、使うと上書きし合う）し**コメントに限定**。**再実行契約（冪等）を明記**（既存なら照合して再利用・片側リンクは残りだけ補う・worktree 不在は完了済み扱い）。**`.claude/skills/task-done/SKILL.md` を `guard_paths` へ追加**（唯一の接続経路が保護されておらず、手順が欠落しても逐行確認が要求されないため）。**10.1・`release/SKILL.md` の「Phase 4 PR」を「13 章の 4-6〔再試行を含む〕」へ具体化**。**失効判定の因果を訂正**（`P4-後` は次回リリースの `T` より前にマージされるため失効差分には現れない。`T` に取り込んだ状態で新規受入を行う）。**確定ゲート 9 周目の反映**（P1×3 / P2×1。**P0 なし**）: **マージ後検査の合格記録を 13 章で一意化**（**4-6 の PR コメント**へ `M` の完全 OID・期待/実測の両親とツリー・実施日・合否・確認者を記録する。**規範を置いた節は 13 章**であり、10.1 は検査の実行を定める節である — v1.6・確定ゲート 10 周目 P2 で「10.1 で一意化」という記述を是正した）— **13 章の完了証跡を条件にすると「記録が存在しない時点で参照する」循環になる**（完了証跡は `P4-後` で書くため）。**再試行のタスク運用を固定**（`M` は既に `develop` にあり元ブランチは閉じているため**新しいタスク・新しいブランチ**で行い、**元タスクは `/task-done` を通さず `ブロック中` にして再試行タスクへリンク**する。`P4-後` の起票は合格した再試行タスクで行う）。**論理スロットの識別に計画書 frontmatter の任意キー `Phase4スロット` を新設**（無い計画書は Phase 4 のスロットではない — 既存計画書に影響しない）。**`/task-done` の先頭へ worktree の存在判定を移した**（不在なら手順 2・3 を完了済み扱い — 8 周目の冪等契約が手順 2 を覆えていなかった）。**確定ゲート 10 周目の反映**（P1×5 / P2×1。**P0 なし**。**是正が次の穴を生む事象は 4 度目**〔1→2・5→6・7→8・9→10〕であり、本周は**範囲を広げず単純化して閉じる**方針を採った）: **① `Phase4スロット` を必須キーへ**（`対象外` を含む列挙・テンプレは `対象外` を出力）— **任意キーは 4-6 で書き忘れたときに `P4-後` を起票しないまま完了まで進む fail-open** だった。**② 合格の認可根拠を PR コメントから Git 実測へ移した** — `M` の第 2 親 = `E` かつ **`M` のツリー = `E` のツリー**を Git オブジェクトで確認する（**ツリー一致が本質**であり第 1 親の照合は合格条件に要らない）。**PR コメントの人間判定記録と併用**し、**記録だけを根拠にしない**（可変媒体のため）。**`P4-後` へ実測値と記録項目を転記**して監査可能性を Git 管理下へ引き継ぐ。**③ 不合格時に元タスクを `/task-done` に通さない規定を撤回** — 元 PR はマージ済みで worktree も回収すべきであり、通さないと **worktree 回収と Notion 終端遷移の経路が消える**（`ブロック中` は「外部要因待ち」で終端遷移を持たない）。**代わりに「4-6 の再試行」タスクを起票して相互リンク**する。**④ worktree 不在時の扱いを手順単位へ**（不在なら手順 3・5 だけを完了済み扱いにし、**develop 最新化は必ず実施**する）— 9 周目は手順 5 の `git worktree remove` が無条件のままで再実行がそこで止まった。**⑤ 6.1（拡張キー表）を計画書の改訂範囲へ明示**。**確定ゲート 11 周目の反映**（P1×4 / P2×1。**P0 なし**）: **① 10 周目の「第 1 親の照合は要らない」を撤回した** — **第 1 親と第 2 親・ツリーは別の性質を検査している**。第 2 親・ツリーは統合結果の内容、**第 1 親は手続き**（排他区間中に base が動いていないか）であり、**`develop` に変更とその revert が入るとツリーは判定時点へ戻るため、ツリー一致だけでは base の移動を検出できない**。取得と照合を分担し、**第 2 親・ツリーは Git で独立に再検査、第 1 親は記録された `phase4_base_sha` と照合**する（判定時点の `origin/develop` は事後に痕跡を残さず Git から実測できないため）。**② `Phase4スロット` の必須化だけでは 4-6 の書き忘れを塞げていなかった** — **テンプレが `対象外` を出力するので書き換え忘れは「欠落」ではなく「正常値」として通る**。**`/task-start` が Notion タスク名からスロット値を決め、`/task-done` がタスク名と突合する独立照合**を足した。**③ 再試行タスクを後継責務として初期化する規定を新設**（名称・スロット伝播・担当者・開始ステータス・DoD・完遂責任者）— 名称とリンクだけでは**再試行が未着手のまま放置できる**（9 周目が `ブロック中` で塞ごうとした穴が形を変えて残っていた）。あわせて 4-6 の完了条件へ**不合格経路（再試行タスクの起票）**を追記した。**④ `P4-後` の完了証跡の項目が 13 章内で二重定義されていた**（「`M` の SHA・実施日・合格の事実のみ」と拡張仕様）— **拡張仕様へ一本化**した（最小仕様のままでは「PR コメントが失われても監査できる」という 10 周目の是正が成立しない）。**⑤ 旧計画書の移行規則**: `Phase4スロット` を持たない v1.6 以前の計画書は `/task-done` の再実行時に中断し、**人間が `対象外` を明示して再実行する**（`反映周コミット` の `規約制定前` と同じ明示移行）。**確定ゲート 12 周目の反映**（P1×4 / P2×1。**P0 なし**）: **① 11 周目の「第 1 親の期待値は記録から取る」を撤回し、3 項目とも Git だけで検証する形にした** — **記録から取ると、base が動いた後に記録の期待値を動いた先へ書き換えるだけで 3 条件がすべて成立する**（可変媒体の単独根拠化）。**`git merge-base <E> <M の第 1 親>` == `M` の第 1 親**（= 第 1 親が `E` の祖先）で置き換えた — `E` のブランチは `origin/develop` から切られているので、**分岐後に develop が進んでいれば第 1 親は祖先でなくなる**。**`phase4_base_sha` の記録は不要になった**（10.1 の base 固定はマージ前の再照合の運用値として存続）。**② 11 周目の「Notion タスク名から決めて突合する」を撤回した** — **`/task-start` と `/task-done` が同じタイトルを同じ規則で解釈するだけで独立照合になっておらず、タスク名は規約化されていない自由入力**である。**人間の明示選択を唯一の入力**とし、**テンプレは空欄を出力し `/task-start` が 8 択を提示して選ばせる**。あわせて **`.claude/skills/task-start/SKILL.md` を `guard_paths` へ加えた**（8 件目）。**③ 再試行タスクの初期化を Notion タスクの範囲へ限定**した — 計画書・ブランチ・worktree を作る唯一の経路は `/task-start` で、実行するとステータスが `進行中` になるため、**「計画書へスロットを書く」をクローズ条件にすると「呼ばなければ未完了」「呼べば `未着手` を保てない」の二択**になっていた。**スロットの記入は後続の `/task-start` の責務**とし、DoD へ明記する形にした。**④ 拡張キー表の既定列を `対象外` から `なし（必須）` へ是正** — **列見出しが「既定（キー欠落時）」なので、`対象外` と書くと「欠落は中断」と矛盾し、欠落を `対象外` と読む実装が成立してしまう**。**確定ゲート 13 周目の反映**（P1×2 / P2×1。**P0 なし**。**指摘は 4 → 2 件へ減り、レビューは「Phase 4 実行中へ先送りできるブロッカーはない」と判定**）: **① 12 周目の `merge-base` による第 1 親検査を撤回した** — **排他区間中に `develop` を `E` の祖先の一つへ fast-forward すると `merge-base(E, X) = X` で条件が成立**してしまい、未検証内容の混入こそ起きないが**「排他区間が守られた」という手続きの保証にならない**。**期待値を `E` の中に置く形へ改めた**: **`phase4_base_sha` を `gate_kind: phase4` の証跡の必須キー**とし（10.1）、**マージ後検査は `M` の第 1 親とこれを完全一致で照合する**。**証跡は `E` の Git ツリーに収録されるため不変**であり、書き換えれば `E` 自体が変わって受入をやり直すことになるので、**11 周目の「可変媒体を単独根拠にする」状態には戻らない**。PR コメントの人間記録は**「人間が判定した」事実の記録**であって期待値の供給源ではないと位置づけ、**証跡と食い違えば不合格**とした。**② 人間の 8 択選択だけでは誤選択を検出できない**（誤って `対象外` を選ぶと有効な列挙値として通り、**4-6 のマージ後検査と `P4-後` の起票がまるごと省略されて Phase 4 展開が終端不能になる**）ため、**`/task-done` が `P4-後`・「Phase 4-6 再試行」を起票するとき Notion の DoD へ機械可読マーカー行 `Phase4スロット: <値>` を書き、`/task-start` が人間の選択と突合して食い違えば中断する**形にした。**マーカーは起票側が書き選択は着手側が行うため、11 周目に撤回した「タスク名の二重読み」とは別の入力である**。**確定ゲート 14 周目の反映**（P1×2 / P2×1。**P0 なし**）: **① `phase4_base_sha` を証跡へ書く時点で base が確定していなかった** — 10.1 の実施経路は base の固定を判定後に置いており、**`E` が確定した後に値を入れると `E` 自体が変わって検証をやり直すことになる**。**base の「取得」だけを証跡作成の直前へ移した**（③-a 取得 → ③-b 証跡へ書いて `E` をコミット → ③-c 検証器）。**排他区間は従前どおり最終照合の直前から**でよく、**live な base との一致は検証器ではなく再照合とマージ後検査が担保する**。あわせて**証跡の必須項目・合格条件 ⑩（完全 OID と commit 解決・`release` での記載禁止）・実施経路の再照合**を明示し、**照合対象の証跡を `evidence_path` で名指しした 1 ファイルに特定**した（`E` には複数試行の証跡が収録されうる）。**② DoD マーカーが初回スロットを保証していなかった** — マーカーを書くのは `/task-done` が起票する `P4-後` と「Phase 4-6 再試行」だけで、**初回 4-6 で誤って `対象外` を選ぶとマージ後検査と `P4-後` の起票がまるごと省略できた**。**マーカーを必須条件へ格上げし、マーカーが無いタスクでは `対象外` しか選べない**形にした（**初回 4-1〜4-6 も着手前にマーカー付きで事前起票する**）。**マーカーの文法**（行全体の完全一致・DoD 欄にちょうど 1 行・重複と列挙外は中断）、**突合を worktree 作成より前へ**（不一致で中断したときに worktree が残らないため）、**子タスク URL との突合と同名タスクの直接起票の中断**を定めた。**残余**: **自由記述の DoD である以上、マーカーを有効な別の値へ改変されたことは検出できない**（運用規律に留まる範囲として明記）。**確定ゲート 15 周目の反映**（P1×3 / P2×2。**P0 なし**）: **① 14 周目の「`evidence_path` による一意特定」「初回スロットも守られる」は過大評価だった**（同周の記述を本周で是正）。**② `phase4_base_sha` が `E` の祖先である条件が無かった** — **試験後・証跡作成前に別 PR が `develop` へ入ると、合格条件 ⑩ も排他後の `base == B` も通るのに `M` が `B` を含んで `tree(M) ≠ tree(E)` が必ず成立し、事後検査だけが落ちる候補**を作れた。**⑩ へ `git merge-base --is-ancestor <phase4_base_sha> <E>` を加えた**（**候補作成時の前提条件**であり、12 周目に撤回した事後の第 1 親検査とは用途が異なる）。**③ `evidence_path` を永続化する場所が無かった** — **`E` には複数試行の証跡が収録されうるため、記録しないと別セッションの `/task-done` が「実際に検証した 1 件」を復元できない**。**PR コメントの記録項目と `P4-後` の完了証跡へ `evidence_path`（と `E` の完全 OID）を加えた**。**④ 初回 4-1〜4-6 の事前起票を検査できなかった** — `/task-start` は名前指定で新規タスクを作れるため、**初回 4-6 を直接起票して `対象外` を選ぶ経路が残っていた**。**「Phase 4 展開」親タスクへ 7 行の「期待スロット ↔ 子タスク URL」台帳を置き、Phase 4 のスロットは URL 指定の着手だけを許可**する形にした（名前起票は中断）。あわせて **DoD の取得失敗・欄の特定不能・ページング不能を「マーカー無し」と扱わず中断**する fail-closed 規定を足した。**⑤ 拡張キー表の構造破損を修復**（14 周目に**表セル内へ箇条書きを入れて表を壊していた** — F5 の確定ゲートで同型の破損を起こした C-5 の再発）。**規定の本体を 13 章へ移し、6.1 は値の列挙と欠落時の扱いに限定**した。**マーカー文法を trim 後の完全一致に統一**（設計書とスキルで表現が食い違っていた）。計画: `docs/features/frontend-skeleton/plan.md` | **in-review**（敵対レビュー・人間承認 未了） |
...
- **plan frontmatter の拡張キー**（feature 作業自身の進行事実のみを置く — 正本 status の複製は置かない〔7.1-1〕。**本表が契約の正** — v1.3）:
  ...
  | `Phase4スロット` | `対象外` \| `4-1`〜`4-6` \| `P4-後`（これ以外は不正） | **なし（必須）** | **v1.6 で新設**。Phase 4 の分割 PR を識別し、`/task-done` が `P4-後` の起票対象を判定するのに使う。**欠落・空値・列挙外はすべて「既定なし」として `/task-done` を中断する**（**既定を `対象外` と書くと「欠落は中断」と矛盾し、欠落を `対象外` と読む実装が成立してしまう** — 確定ゲート 12 周目 P1）。**旧計画書（本キーを持たない v1.6 以前のもの）も同じく中断し、人間が `対象外` を明示して再実行する**（`反映周コミット` の `規約制定前` と同じ明示移行）。**値の決定手順・DoD マーカーによる突合・初回スロットの事前起票は 13 章が正**（v1.6 — 確定ゲート 15 周目 P2 で**表セル内に箇条書きを入れて表構造を壊していたのを是正し、規定の本体を 13 章へ移した**） |
...
| `/task-start <タスク名 or NotionURL>` | 着手の唯一の入口: Notion タスク取得/起票（11.1）→ **`Phase4スロット` の決定（DoD マーカーとの突合 — v1.6・13 章。**worktree 作成より前**）** → `feature/*` ブランチ+worktree 作成（12.1）→ 計画書雛形（6.1）→ worklog 雛形 → ステータス着手。**Phase 4 の分割スロットは親台帳の URL でのみ着手**し、名前起票は中断する（同） |
...
| `/task-done`（**Phase 4 の 4-6 を閉じる場合は、合格なら `P4-後`・不合格なら「Phase 4-6 再試行」タスクの起票と相互リンクが先** — v1.6・13 章。確定ゲート 11 周目 P1 で不合格経路を追記） | 完了の唯一の出口: PR マージ確認 → develop 最新化（`pull --ff-only`）→ worktree 除去（12.1）→ Notion 完了。worklog 締め・計画書 in-review 化は **/pr が PR 内で実施済み**（マージ後の develop を直接編集しない） |
| `/finalize-doc <文書パス>` | 正本確定ゲート（7.3）: 敵対レビュー → 指摘反映ループ → 人間承認 → approved 化・索引更新 |
| `/release <vX.Y.Z>` | **手順 0 = NFR-021 受入証跡ゲート(fail-closed)**: **手順 0 が実際に検証器の実行へ置き換えられるまでは、検証器の有無にかかわらず無条件中断**（10.1。**置換は Phase 4 完了後の後処理 PR〔13 章の `P4-後`〕でのみ** — v1.6・確定ゲート 6 周目 P1）→ 要件書8章 DoD 8項目チェック → 人間のリリース判定 → develop→main PR → タグ。**検証器本体は Phase 4 の成果物**（**検証器が実装されても、`P4-後` で手順 0 が置換されるまでは止まる** — v1.6・確定ゲート 7 周目 P2。「それまで」が検証器の実装時か Phase 4 完了時か曖昧だったため明確化）。**`gate_kind: phase4` の受入判定は `/release` を経由しない**（10.1 の実施経路） |
...
| 実施順序（必須） | **① `onboarding.md` を確定ゲート（7.3）に通し approved 化**（Phase 4 初回は v1.0。**リリース候補時はその時点で現行の approved 版**であればよく、特定の版を要求しない）**→ ② その approved 版の手順を受入プロファイル上の新規環境で完走 → ③ 証跡を作成して Git にコミットし（`tested_commit_sha` = ② で試験したコミット T）、下記「実施経路」に従って候補 SHA を確定 → ④ 判定**。**`gate_kind: phase4` では ③ の直前に base を取得する**（v1.6 — 確定ゲート 14 周目 P1）: **③-a GitHub API から PR の base（その時点の `origin/develop`）を完全 OID `B` として取得 → ③-b `phase4_base_sha: B` を証跡へ書いて `E` をコミット → ③-c 検証器を実行**。**この順序でなければならない** — **`E` が確定した後に値を入れると `E` 自体が変わり、検証をやり直すことになる**。**排他区間を証跡作成前へ延ばす必要はない**（base の**取得**だけを前へ移し、排他区間は従前どおり最終照合の直前から開始する）。**live な base との一致は検証器ではなく、下記「実施経路」の再照合とマージ後検査が担保する**。③ が「作成 → コミット → 候補 SHA を確定」の順である必要がある（検証器は**候補 SHA のツリーに収録された証跡だけ**を見るため。証跡ディレクトリは失効の allowlist なので T→ 候補 SHA の差分で失効しない）。要件書 NFR-021 は「**approved な** onboarding の手順を完走」を要求しており、**draft 版で実行した結果は証跡にならない** |
| 証跡の必須項目 | 要件書 NFR-021 の「証跡」箇条書きのとおり（項目を本書に複製せず参照する）。**加えて機械検証のためのキーを持たせる** — `gate_kind`（`phase4` \| `release`）/ **`phase4_base_sha`（`gate_kind: phase4` のみ**必須。**証跡作成の直前に GitHub API から取得した base の完全 OID**。**`gate_kind: release` では記載自体を不合格とする** — v1.6・確定ゲート 13・14 周目 P1）/ `tested_commit_sha` / `onboarding_blob_sha`（= **`tested_commit_sha` における `docs/development/onboarding.md` の Git blob SHA**。**版番号では内容を一意に識別できない** — 実装追随の節更新は版繰り上げなしで可能なため）/ `result`（`passed` \| `failed`）/ `release_version`（`gate_kind: release` のとき**必須**・`phase4` のとき**記載禁止**）/ `attempt_seq`（**ゲートキー単位で 1 から始まる単調増加の試行番号**。ゲートキー = `phase4`、または `release + release_version`。**再実施のたびに +1**。`failed` の試行も番号を消費する。**採番は下記「試行の排他と採番」の予約レコードで確定**する）/ `attempt_id`（**予約時に発行する不変・一意の識別子**。結果証跡は**自分が閉じる予約の `attempt_id` を必須記録**する — これが無いと「予約 0 件の候補ツリーに、大きい `attempt_seq` の `passed` を手書きするだけで採番統制を迂回できる」） |
...
| **P4-後** | **Phase 4 完了後の後処理**（**Phase 4 の論理スロットに数えない** — v1.6・確定ゲート 4 周目 P1）: **完了証跡の記録** + **`/release` 手順 0 の置き換え** + **13 章の現況追随** | **4-6 のマージ後検査（M の両親・ツリー）に合格していること**（v1.6 — 確定ゲート 2 周目 P0・P1）。**マージと事後検査は原子的に実行できない**ため、4-6 のマージ時点では Phase 4 の完了は未確定である。**4-6 で `/release` を置換すると、検査が不合格だった場合に「Phase 4 未完了のまま `/release` が開いた」状態が develop に残る**（1 周目 P0 と同型の再発）。したがって**無条件中断は 4-6 のマージ後検査に合格するまで維持し、本 PR で初めて置換する**。**Phase 4 の成果物ではなく、完了後の有効化とクローズである**（同 4 周目 P1）。**本 PR の DoD に「手順 0 が実際に検証器を呼び出す配線になっていること」の検査を含める**（v1.6 — 確定ゲート 6 周目 P1）— **`P4-後` は唯一のリリース入口そのものを書き換える PR** であり、**呼び出しを欠落・弱体化させた場合、「次回リリース時の失効判定で捕捉」は成立しない**（失効判定を呼ぶ入口自体が変更対象のため）。あわせて **`.claude/skills/release/SKILL.md` を `guard_paths` に加えた**（同）。**完了証跡は 13 章の現況追随として記録する**（v1.6 — 確定ゲート 3 周目 P1）— **記録項目は下記「マージ後検査の合格」の②と同一**（`M` の完全 OID / `E` の完全 OID / `evidence_path` / 期待した両親とツリー / 実測した両親とツリー / 実施日 / 合否 / 確認者）**および①の実測値**（`M`・`E`・両者のツリー）とする（v1.6 — 確定ゲート 11 周目 P1 で **「M の SHA・実施日・合格判定の 3 項目」という最小仕様を廃し、拡張仕様へ一本化した** — **最小仕様のままだと「PR コメントが失われても監査できる」という 10 周目の是正が成立しない**）。**`docs/ops/nfr021-acceptance/` へは置かない** — 10.1 が定義する**予約レコード・結果証跡とは別種**になり、命名・必須項目・append-only の扱い・**検証器が列挙対象に含めるか**を新たに定義する必要が生じるため。13 章は正本であり、変更履歴表に残ることで永続性が担保される。4-6 はマージ済みで事後変更できないため本 PR で書く |

- **4-1 〜 4-5 は「Phase 4 の部分成果」であり、単体では Phase 4 の完了を意味しない。** これらのマージは NFR-021 判定を前提としない
- **Phase 4 の範囲は 4-1 〜 4-6 で閉じる**（v1.6 — 確定ゲート 4 周目 P1）。**`P4-後` は Phase 4 の成果物ではない**。**この再分類が必要な理由**: `P4-後` を Phase 4 に含めたまま完了を 4-6 時点と定めると、**「Phase 4 全体に対して受入を 1 回」という原則を、Phase 4 の一部（`P4-後` の内容）が覆えていない**状態になる（`P4-後` が変更する `.claude/skills/release/SKILL.md` は 10.1 の明示的な失効対象であり、4-6 の証跡では覆えない）。**要件書が求める「Phase 4 完了時」の判定を、次回リリース時の `gate_kind: release` で代替することはできない**。
  ...
  - **`P4-後` の内容の捕捉は 2 系統に分かれる**（v1.6 — 確定ゲート 7 周目 P2）: **① 手順 0 の配線そのもの**（検証器を実際に呼び出すか）は **`P4-後` の DoD による事前検査 + `guard_paths` による人間の逐行確認**で捕捉する — **失効判定を呼ぶ入口自体が変更対象なので、次回リリースでは捕捉できない**。**② 配線が成立した後の、その他の失効対象の変更**は**次回リリースの受入で捕捉される** — **正確には**（v1.6 — 確定ゲート 8 周目 P2）: 失効判定は **`T` → 最終候補の差分**だけを見るため、**`P4-後` は次回リリースの試験対象 `T` より前にマージされる以上、その変更自体は失効差分には現れない**。**`P4-後` の内容を取り込んだ状態で新規に受入を行い、`T` より後の変更だけを失効判定が捕捉する**
- **Phase 4 の完了は「4-6 のマージ**かつ**マージ後検査の合格」をもって成立し、その事実は `P4-後` で記録する**（v1.6 — 確定ゲート 1 周目 P1・2 周目 P0）。10.1 は**マージ後に M の両親・ツリーを検査し、不一致なら Phase 4 未完了として M を再受入する**と定めている。**マージだけで完了を確定させない**。**不合格だった場合**: 10.1 の定めどおり **M を新たな T として再受入**する。**再試行は新しいタスク・新しいブランチで行う**（v1.6 — 確定ゲート 9 周目 P1。`M` は既に `develop` にあり、元のブランチは閉じているため）。**元のタスクはそのまま `/task-done` で閉じる**（v1.6 — 確定ゲート 10 周目 P1。**9 周目の「元タスクを `/task-done` に通さず `ブロック中` にする」は撤回した** — **元 PR は既にマージ済みで worktree も回収すべき**であり、通さないと **worktree の検査・除去と Notion の終端遷移の経路が無くなる**。`.claude/notion-map.json` の `ブロック中` は「外部要因待ち・人間判断」であって、**再試行成功後に元タスクを終端化する遷移が存在しない**という別の穴も生む）。**閉じる際に `P4-後` は起票せず、代わりに「4-6 の再試行」タスクを起票して相互リンクする**。**再試行タスクは後継責務として初期化する**（v1.6 — 確定ゲート 11 周目 P1。名称とリンクだけでは**再試行が未着手のまま放置できる** — 9 周目が `ブロック中` で塞ごうとした穴が形を変えて残るため）: **初期化は Notion タスクの範囲だけで行う**（v1.6 — 確定ゲート 12 周目 P1。**計画書・ブランチ・worktree を作る唯一の経路は `/task-start` であり、それを実行するとステータスが `進行中` になる**ため、「計画書へ `Phase4スロット: 4-6` を書く」を元タスクのクローズ条件にすると **「呼ばなければ初期化未完了」「呼べば開始ステータスを保てない」の二択**になっていた）: **① タスク名に `Phase 4-6 再試行` を含める ② 担当者・優先度は元タスクを引き継ぐ ③ ステータスは `未着手` ④ DoD へ**機械可読マーカー行 `Phase4スロット: 4-6`**（`/task-start` が人間の選択と突合する — 同 13 周目 P1）と「`M` を新たな `T` として 10.1 の受入をやり直し、マージ後検査に合格すること」を書く ⑤ 完遂責任者は元タスクの担当者 ⑥ 元タスクと相互にリンクする**。**計画書の作成とスロットの記入は、後続の `/task-start` の責務**である。**元タスクを閉じる前にこの Notion 側の初期化を完了する**（起票だけして閉じない）。**`P4-後` の起票は、合格した再試行タスクの `/task-done` で行う**。**論理スロットの識別は計画書 frontmatter の `Phase4スロット` キー**（**必須キー** — 6.1 の拡張キー表）。**その追加の合格証跡 PR は「4-6 の再試行」として数え、監査 PR には数えない**（本章冒頭の監査 PR の例外は**予約レコード PR・失敗閉塞 PR に限る** — 同 2 周目 P1）。**再試行が合格するまで `P4-後` は実行できない**
- **順序**（v1.6 — 確定ゲート 1 周目 P1 で一意化・2 周目 P1 で監査 PR を明示）: **4-1 が先行** → **4-2 / 4-3 は相互任意** → **4-4 の確定ゲート完了**（`README.md` とテンプレートを approved にする）→ **10.1 のブートストラップが定める予約レコード監査 PR**（`docs/ops/nfr021-acceptance/` の新設。**4-4 で確定した blob に準拠した形式で書く**）→ **4-4 のマージ**（予約の統合点を取り込む）→ **4-5**（検証器は 4-4 の失効パス正本と証跡テンプレートを本番構成として消費するため）→ **4-6** → **マージ後検査（合格で Phase 4 完了）** → **`P4-後`**。本章冒頭の「この順に develop へ」は **Phase 間の順序**を指し、**Phase 4 内（4-1 〜 4-6）はこの規則が正**とする。**`P4-後` は Phase 4 外だが、この順序列の末尾に必ず位置する**（上記の終端条件）。
...
- **「4-1 〜 4-6」は論理スロットであり、物理 PR 数ではない**（v1.6 — 確定ゲート 3 周目 P1・4 周目 P1 で範囲を 6 へ）。**4-6 には 0 回以上の再試行 PR が属する**ため、実際の本数は**成功時 6 本・再試行が n 回なら 6+n 本**になる。**`P4-後` はこのスロットに数えない**（Phase 4 の成果物ではないため）
- **「1 PR」の例外**（本章冒頭）で定めた**監査 PR（予約レコード PR・失敗閉塞 PR）は、このスロットに数えない**
- **マージ後検査の合格は、Git からの実測と人間の判定記録の両方で認可する**（v1.6 — 確定ゲート 9 周目 P1 で記録先を一意化し、**10 周目 P1 で認可根拠を Git 実測へ移した**）。
  - **① 検査項目は 10.1 の定めどおり第 1 親・第 2 親・ツリーの 3 つ**（v1.6 — 確定ゲート 11 周目 P1。**10 周目に「ツリー一致が本質だから第 1 親の照合は要らない」と書いたのは誤りで、撤回した**）。**第 1 親と、第 2 親・ツリーは別の性質を検査している** — **第 2 親・ツリーは統合結果の内容**（検証後に `develop` へ入った変更が `M` に混入していないか）、**第 1 親は手続き**（判定時点の `phase4_base_sha` から base が動いていないか = 10.1 の人間管理の排他区間が守られたか）である。**反例**: `develop` に `D` とその revert `C` が入るとツリーは判定時点の `B` に戻るため、`M` の第 2 親 = `E` も `M` のツリー = `E` のツリーも成立するが `M` の第 1 親は `C`（≠ `B`）になり、**base が動いた事実を検出できない**
  - **期待値は `E` の中に置き、3 項目とも完全一致で検査する**（v1.6 — 確定ゲート 13 周目 P1。**11 周目の「PR コメントの記録から取る」も 12 周目の `merge-base` による代替も、どちらも撤回した**）。**`M`（マージコミット）と `E`（4-6 の PR head）は GitHub API から取得**し、以下を Git オブジェクトで検査する: **① `M` の第 2 親 = `E`** / **② `M` のツリー = `E` のツリー** / **③ `M` の第 1 親 = `E` に収録された証跡の `phase4_base_sha`**（完全一致）。
    - **照合対象の証跡は、検証器へ `evidence_path` として名指しした 1 ファイル**である（v1.6 — 確定ゲート 14 周目 P1。**`E` には複数試行の証跡が収録されうるため「`E` に収録された証跡」だけでは一意にならない**）。**`phase4_base_sha` はその証跡ファイルの必須キー**とする（10.1 — v1.6 で追加）。**`E` の Git ツリーに収録されるため不変**であり、**値を書き換えれば `E` 自体が変わって受入をやり直すことになる**ので、11 周目に問題となった「可変媒体を単独根拠にする」状態には戻らない
    - **12 周目に置いた `git merge-base <E> <M の第 1 親>` == `M` の第 1 親（第 1 親が `E` の祖先）では不十分**である — **排他区間中に `develop` を `B` から `X`（`E` の祖先の一つ）へ fast-forward すると、`merge-base(E, X) = X` となって条件が成立してしまう**。`X` は既に `E` に含まれるため未検証内容の混入は起きないが、**「排他区間が守られた」という手続きの保証にはならない**
    - **Git オブジェクトを解決できないとき・証跡に `phase4_base_sha` が無いときは判定不能として fail-closed**
  - **② 人間の判定記録**: 4-6 の PR コメントへ **`M` の完全 OID / `E` の完全 OID / **検証器へ渡した `evidence_path`** / 期待した両親とツリー / 実測した両親とツリー / 実施日 / 合否 / 確認者**を記録する（**`evidence_path` の永続化は v1.6・確定ゲート 15 周目 P1** — **`E` には複数試行の証跡が収録されうるため、記録しないと別セッションの `/task-done` が「実際に検証した 1 件」を復元できない**）（10.1 が「人間の判定者による合格判定は必須」と定めるため）。**この記録は「人間が判定した」という事実の記録であって、①の期待値の供給源ではない**（v1.6 — 確定ゲート 13 周目 P1。**期待値は①のとおり `E` に収録された証跡から取る** — 記録された「期待した両親」と証跡の `phase4_base_sha` が食い違えば**不合格**として扱う）
  - **`/task-done` は ① と ② の両方を要求する**。**② だけを根拠にしてはならない** — **PR コメントは編集・削除できる可変媒体**であり、単独では認可証跡にならない（v1.6 — 確定ゲート 10 周目 P1）。**① と ② が食い違うときは不合格**として扱う
  - **`P4-後` は ① の実測値（`M`・`E`・両者のツリー）と ② の全項目を Git 管理下の完了証跡へ転記する**（PR コメントが失われても合格判断を監査できるようにする — 同）
  - **13 章の現況追随（完了証跡）を起票の条件にしてはならない** — それは `P4-後` で書くものであり、**条件に使うと「記録が存在しない時点で参照する」循環になる**（同 9 周目 P1）。**10.1 の排他区間の解除記録でも代替できない** — 解除は中断時にも行うため合格を証明しない
- **`Phase4スロット` の決定と検査**（v1.6 — 確定ゲート 12〜15 周目。**6.1 の拡張キー表は値の列挙と欠落時の扱いを定め、手順は本節が正**）
  - **値は `/task-start` で人間が 8 択（`対象外` / `4-1`〜`4-6` / `P4-後`）から明示選択する**。**タスク名などからの自動判定はしない**（`/task-start` と `/task-done` が同じ情報を同じ規則で解釈するだけになり、独立照合にならないため — 12 周目 P1）
  - **Phase 4 の分割タスクは、着手前に Notion タスクの DoD へ機械可読マーカー行 `Phase4スロット: <値>` を持つ**。**マーカーが無いタスクでは `対象外` しか選べない**（14 周目 P1）— **13 周目の形〔マーカーが無ければ選択自由〕では初回の 4-1〜4-6 が守られなかった**（マーカーを書くのは `/task-done` が起票する `P4-後` と「Phase 4-6 再試行」だけで、**初回 4-6 で誤って `対象外` を選ぶとマージ後検査と `P4-後` の起票がまるごと省略できた**）
  - **マーカーの文法**: **行を trim した結果が `Phase4スロット: <値>` に完全一致**し（値は 6.1 の列挙。**trim 後の完全一致であって生文字列の一致ではない** — 15 周目 P2）、**DoD 欄にちょうど 1 行だけ存在する**こと。**重複・列挙外・書式不一致はすべて中断**（fail-closed）
  - **DoD を取得できない場合と、取得できてマーカーが 0 件の場合を区別する**（v1.6 — 確定ゲート 15 周目 P1）。**取得失敗・欄を特定できない・ページングを完了できないときは「マーカー無し」と扱わず中断する**（fail-closed）
  - **`/task-start` はマーカーと人間の選択を突合し、食い違えば中断する**。**突合はブランチ・worktree の作成より前に行う**（不一致で中断したときに作成済みの worktree が残らないようにするため — 14 周目 P1）
  - **スロット台帳を「Phase 4 展開」の親タスクに置く**（v1.6 — 確定ゲート 15 周目 P1）: **Phase 4 着手前に、親タスクへ 7 行（`4-1`〜`4-6`・`P4-後`）の「期待スロット ↔ 子タスク URL」を固定して記録する**。**`/task-start` は Phase 4 のスロットについては URL 指定の着手だけを許可**し、**名前指定による新規起票（親台帳に URL が無いタスク）は中断する**。**これがないと「判定者が事前起票する」という規定を `/task-start` が検査できず、初回 4-6 を直接起票して `対象外` を選ぶ経路が残る**
  - **これは 11 周目に撤回した「タスク名の二重読み」とは異なる** — **マーカーと親台帳は起票側（`/task-done` または実施責任者）が書き、選択は着手側（人間）が行うため、別々の入力である**
  - **残余**: **自由記述の DoD 欄である以上、マーカーを有効な別の値へ改変されたことは検出できない**（親台帳との突合が効くのは「どのタスクがどのスロットか」までで、DoD 欄の改変そのものは検出しない）。**機構ではなく運用規律に留まる範囲**として明記する
- **`P4-後` は必須の後続処理であり、放置してはならない**（v1.6 — 確定ゲート 5 周目 P1）。**`P4-後` のマージを、① Phase 5 への着手 ② Phase 4 展開タスクのクローズ ③ **プロダクトリリースの開始** の前提とする**（③ は v1.6・確定ゲート 6 周目 P0 — **Phase 4 の完了は `P4-後` の起票資格を開くだけ**であり、**置換が済むまでリリースへは進めない**）。**起票と完遂の責任は Phase 4 の完了を確認した者（= 4-6 の PR 作成者）が負う**。**実行経路まで接続する**（v1.6 — 確定ゲート 6 周目 P1）: **4-6 の `/task-done` では、合格なら `P4-後` を、不合格なら「Phase 4-6 再試行」タスクを Notion へ起票して 4-6 のタスクへリンクするまでを完了条件とし**（v1.6 — 確定ゲート 11 周目 P1 で不合格経路を追記。従前は合格経路しか書いておらず、10 周目で不合格でも元タスクを閉じるようにした結果と食い違っていた）、**`P4-後` のマージ確認をもって「Phase 4 展開」を閉じる**。**通常のタスク粒度（1 タスク = 1 成果物 ≒ 1 PR）では 4-6 と `P4-後` は別タスクになる**ため、**どちらのタスクを「Phase 4 展開」と呼ぶかを一意にする** — **`P4-後` のタスクが「Phase 4 展開」の終端**である。**これを定めないと、Phase 4 だけを完了扱いにして `P4-後` を放置でき、`/release` の無条件中断が残り続ける**
- **13 章の現況追随は `P4-後` で行う**（v1.6 — 確定ゲート 2 周目 P1）。**4-6 はマージ済みで事後変更できない**ため、4-6 で書くと「不合格時に虚偽の完了記録が develop に残る」か「事後追加できない」かのどちらかになる。**この追随は上記の記録項目（`M`・`E`・両者のツリー・期待/実測の両親・実施日・合否・確認者）を追記するものであり、規範条件は変えない**ため（v1.6 — 確定ゲート 11 周目 P1 で「SHA・実施日・合格の事実のみ」から拡張仕様へ統一。**項目が増えても事実の記録である性質は変わらない**） **7.6-3 前段（実装追随の節更新・変更履歴追記）に該当し、通常の PR レビューで足りる**（同 4 周目の確認結果。規範条件まで変える場合だけ 7.3 の確定ゲートへ戻る）
