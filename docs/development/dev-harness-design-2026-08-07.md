...
- **採用理由**: **SQLAlchemy 2.x** = 型付き API を `ty` の検査対象にできる唯一の系列であり、
  同一 dialect で同期・非同期の両方の engine を提供する。**psycopg 3** = SQLAlchemy 2.x が
  ネイティブな `asyncio` 対応として受ける DBAPI であり、同期経路と dialect を共有する。
  **Alembic** = SQLAlchemy 公式のマイグレーションツールで、単一 head の維持と差分検査
  (`alembic check`)を機構として持つ
- **却下案**:
  ...
  - **native enum を既定にしない** — autogenerate が enum 値の増減を信頼して扱わないため、
    **既定は参照テーブル + FK** とする。**native enum を一律に禁じるものではない** —
    値セットが非常に安定した場合の許容は 12-4 節が正であり、本節はその判断を上書きしない
