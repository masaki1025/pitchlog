...
| 版 | 日付 | 変更内容 | 状態 |
| --- | --- | --- | --- |
| **1.18** | 2026-09-27 | **確定ゲート通過(approved)**: 敵対レビュー **18 回**(gpt-5.6-sol xhigh — 全文 6 + 差分 12)・**反映 13 周**・**指摘 48 件(P0 0 / P1 39 / P2 9)を全件採用・不採用 0**・7.3-6 エスカレーション 2 回(3 周目・5 周目 — いずれも PO 裁定 = 続行指示)・6 周警告 1 回(PO 判断 = 続行)・適用版 = 7.3 v1.17(条文 SHA `ab72a58a`)→ **PO 承認(2026-09-27・徳光 尋弥)**。採否記録は `docs/worklog/2026-09-26-harness-model-refresh.md`(ADR-001 v1.1 と単一ゲート) | approved |
| 1.18 | 2026-09-27 | **モデル既定の世代更新(起案)**(TSK-463・PO 判断 2026-09-27・徳光 尋弥): **8.5** の調査サブエージェントの実行既定を `claude-opus-5` → **`claude-opus-5-5`**(Opus 5.5・2026-09-22 公開・Fable 5.1 相当の性能で cache read $0.20)へ更新し、frontmatter の **`effortLevel` 併記を廃止**(公式リファレンスの frontmatter キーは `effort` のみで、未知キーは無視される — 併記の根拠だった「公式キーの表記ゆれ」は公式仕様の確定で解消。定義例も同期)/ **8.1** に**主セッション(Claude Code 本体)の既定**を新設(プロジェクト共有 `.claude/settings.json` で `claude-opus-5-5`・effort high を機構化。Fable 5.1 は Max の週次上限 50% 枠・速い消費のため常用しない・`/model` ピッカーの `s` で現セッションだけ一時昇格 / Fast mode は共有 settings の `env` `CLAUDE_CODE_DISABLE_FAST_MODE=1` で無効化 / 前提 = Claude Code 2.1.280 以上 / 優先順位は公式どおり、起動時指定〔`--model` → `ANTHROPIC_MODEL`〕→ settings の `model`〔managed → local → 共有 → 利用者〕。managed の `availableModels` は全選択経路を制約)/ **9.4** の対応表の**再掲を廃止**し ADR-001 v1.1(GPT-6 世代: 全枠 `gpt-6-sol`・機械的軽作業 `gpt-6-luna`・effort 据え置き・astra 不掲載)への参照へ(7.1-1 の複製禁止)。決定表の固定構文と同期テスト・CLI 0.157.x 以上の前提とラッパーの版検査・`probe` 経路・カタログでの effort 候補確認を注記 / **6.1・8.4・9.2** の旧モデル記述(`terra medium`・`terra high`)を「ADR-001 の該当行をラッパーが固定」参照へ(版焼き込みの撤去 — H-20 対応案④と同型)。**並列度 3・出典必須・read-only は不変**。**射程宣言(7.3-7)**: 本改訂で確定する範囲 = ① 8.5 の実行既定(モデル ID・effort・`effortLevel` 併記廃止)② 8.1 の主セッション既定(モデル・effort・Fable/Fast の扱い・機構化の場所)③ 9.4 の参照化と注記 ④ 6.1/8.4/9.2 の参照化と 8.4 `/setup-dev` の版下限確認。実装時(TSK-463 の後続ステップ)に確定する範囲 = agents frontmatter・CLAUDE.md・スキル本文・`.claude/settings.json`・ラッパー定数とテストの実装・`exec` 経路の受理の実機検証。**コーディングの Claude 側への移管は本改訂の対象外**(別タスク — 利用枠が別財布・計画書ゲートの機械検証がラッパー側にあるため)。ADR-001 v1.1 と**単一の確定ゲート**(7.3)で一括検証する。計画: `docs/features/harness-model-refresh/plan.md`。**確定ゲート 反映1周目**(sol xhigh・P0 0 / P1 12 / P2 3 — 全件採用): 9.4 の再掲廃止(7.1-1)/ 8.5 の動的 effort 調整文を削除し high 固定・`effortLevel` を「未知キー」に統一 / 8.1 の優先順位を公式どおりに是正・CLAUDE.md 項目へ主セッション規則を追加・Fast を `env` で機構的に無効化・Claude Code 2.1.280 以上の前提 / 8.4 の `--deep` 行キー参照の是正と `/setup-dev` の版下限確認 / 9.4 にラッパーの版検査と `probe` を注記。**反映2周目**(P0 0 / P1 8 / P2 1 — 全件採用・全件起因): 9.4 の行キー一覧も再掲をやめ ADR 参照のみへ / 9.2 のコマンド一覧へ `probe`(read-only + cached)を追加。**反映3周目**(P0 0 / P1 4 / P2 1 — 全件採用。起因過半 2 周連続で 7.3-6 発火 → **PO 裁定 2026-09-27・徳光 = 続行指示**〔射程不変〕): 9.4 の注記から割当ての再掲を除去 / 6.1 の fast path に `重さ分類: 軽微` + `実行方式: fast` の一括更新とラッパーの両値検査を追記。**反映4周目**(P0 0 / P1 3 / P2 0 — 全件採用): 6.1 の fast path で受理する計画書を正規位置に限定。**最終全文確認周(7 回目)→ 反映6周目**(P0 0 / P1 2 — 全件採用): 8.1 の `/model` の意味を公式仕様(Enter は利用者設定へ保存・現セッション限定はピッカーの `s`)へ是正 / 計画書 DoD の実測期間を ADR 参照へ。**反映7周目**(最終全文確認周の再実施・P0 0 / P1 3 — 全件採用): 9.2 の注記で `probe` は stdin を読まない固定短文と明記 / `finalize-doc` スキルの割当て複製の同期を計画へ。**反映9周目**(最終全文確認周・12 回目・P0 0 / P1 1 — 採用): 6.1 の fast path で `status: active` を含む 3 値検査。**反映11周目**(最終全文確認周・14 回目・P0 0 / P1 1 / P2 1 — 全件採用): 9.4 の fail-closed 注記を 3 値へ同期 / カタログ追加版の記録を公式変更履歴(0.156.1 hotfix)と GitHub リリースノート(0.157.0)の両出典で記載。**反映13周目**(最終全文確認周・16 回目・P0 0 / P1 1 — 採用): 8.1 のモデル選択の優先順位を公式仕様(起動時指定 → settings の `model`、settings 間は managed → local → 共有 → 利用者)へ是正 | in-review |
| 1.17 | 2026-09-21 | **10.2 の承認済み例外の解除・解除の 2 原則の新設・10.1 の排他区間の是正(起案)**(TSK-429): 2026-09-19 の public 化と Ruleset 適用により、10.2 が持つ **2 つの留保のうち承認済み例外(「保護が利用可能になるまで」という**期限付き**の例外)の前提条件が消滅した**。**リスク受容記録(状態条件)の前提は消滅していない** — Ruleset は適用されたが**必須チェックの発行元が未拘束**(`integration_id` 未指定)のため**対象の統制そのものが成立していない**。**片方だけ閉じる根拠はこの非対称にある**。 **【射程変更 2 回 — 7.3-6 に基づき記録】** **《1 回目・初周後》《縮小》リスク受容記録の解除を射程から外した**(敵対レビュー初周 `P0`)。**《拡大》10.1 の「ブランチ保護が利用可能になった時点で排他区間を保護設定へ置き換える」を撤回**(同 `P1` — `strict_required_status_checks_policy` は topic を base へ追随させる規則であって base の更新を凍結しないため置換は成立しない)。 **《2 回目・7 周後》《縮小》解除手続の詳細を送り出した**(PO 裁定 2026-09-21)。**確定ゲート 7 周・指摘 33 件を全件採用しても収束せず、件数が 5→4→3→2→4→6 と反転して増加した**。原因は、**本改訂が実施する解除は 1 件(期限付き例外)だけで 2 原則だけで判定できる**一方、**送り出した詳細が必要になるのは条件の充足を判断する解除(= リスク受容記録の解除)であり、それを射程から外したため、その詳細手続には適用対象が存在しない**ことにある。**適用対象のない手続を机上で完成させると是正が次の穴を生む閉じた輪になる**(v1.7 の確定ゲート 12〜15 周目・ADR-003 の PO 裁定 2026-08-19 と同型)。**今回の期限付き例外の解除にも 2 原則は適用されており**(原則 2 で範囲を限定)、**送り出しによって現在の解除に穴は開かない**。**詳細は初回の実解除を行う `TSK-435` で実機とともに確定する。** **本改訂で確定する範囲**: ①**解除の 2 原則**のみ — **(1) 形式的な充足で解除しない**(設定が存在することと統制が成立していることは別)**(2) 解除の範囲は当該記録が明示した対象に限る** ②**承認済み例外(期限付き)の解除**と、後続タスク `TSK-432` を持つ**新しい未達への切り替え**(**自動再現要件の未達状態そのものは解除しない**) ③**10.1 の排他区間の存続の確定**。 **本改訂で変更しない範囲**: **NFR-019 (a)(b)(c)(d) の未達状態** / **リスク受容記録そのもの(解除しない・存続。解除条件へ「必須チェックの発行元が拘束されていること」を追加)** / 10.1 の受入ゲートと `release/SKILL.md` 手順 0 の無条件中断 / `github-setup.md` 2 章 手続 3・4 の継続 / 補償統制の内容 / 6.4 の過去事実。 **実装時に確定する範囲**: 解除手続の詳細(`TSK-435`)/ `setup_branch_protection.py` の実装形式(`TSK-432`)/ 発行元拘束の指定可否と 4 経路の負例試験(`TSK-433`)。 | **in-review** |
| **1.17** | 2026-09-21 | **確定ゲート通過(approved)**: 敵対レビュー **12 回**(sol xhigh — 全文 6 + 差分 6)・**反映 9 周**・**指摘 42 件を全件採用・不採用 0 件**(**P0 2 件**〔初周・7 周目〕/ P1 34 / P2 6)→ **PO 承認(2026-09-21・山田正輝)**。 **P0 の内容**: ①初周 — **`integration_id` 未指定で「全グリーンでないとマージ不可」の統制が成立していない**のに解除しようとしていた(**解除を撤回し、解除条件へ「必須チェックの発行元が拘束されていること」を追加して存続**)②7 周目 — **保護ブランチを実際に更新・削除しうる負例試験**を安全条件なしで指示していた(**`main`/`develop` を対象にしない等の安全条件を必須化**)。 **射程変更 2 回・PO 裁定 3 回**: 初周後の**縮小**(解除の取り下げ)+ **拡大**(10.1 の置換撤回)/ 3 周目の**エスカレーション**(2 周連続で起因が過半 → 裁定 `A` = 続行 + 機械的な全数掃き出し)/ 6 周目の**6 周警告**(→ 続行)と**自己承認の可否の裁定**(`A` = 同一人物を許し明記を義務化)/ 7 周目の**縮小**(**解除手続の詳細を `TSK-435` へ送り出し**)。 **7 周目の送り出しが転機**で、P1 件数は 6 → 2 → 0 と落ちた。**適用対象のない手続を机上で完成させようとしていたこと**が収束しなかった構造的原因であり、**v1.7 の確定ゲート 12〜15 周目・ADR-003 の PO 裁定(2026-08-19)と同型の失敗**だった。 計画: `docs/features/post-public-doc-sync/plan.md` | **approved** |
...
手順: /task-start（ブランチ+worktree は維持）→ `codex_run.py fast`（モデル・effort は ADR-001 の「軽微な修正」行をラッパーが固定。**適用時に計画書雛形の `重さ分類: 軽微` と `実行方式: fast` を一括更新する** — 人間の事前 OK の記録であり、ラッパーは登録済み worktree のブランチ名から導出した正規位置 `docs/features/<slug>/plan.md` の計画書だけを受理し、`status: active`・`重さ分類: 軽微`・`実行方式: fast` の 3 値を検査してから実行する〔`status: in-review`・不在・読取不能・frontmatter 不正・`branch` 不一致・同一 branch の別位置計画書・3 値の欠落/空値/不一致は停止 — 差し戻しの往復規約〔in-review → active〕を fast でも機械強制する〕。v1.18）または Claude が直接修正 → /check → PR 本文に**短縮計画**（目的/変更/確認方法）→ CI + 反対側レビュー1本（Codex 実装なら Claude、Claude 直なら Codex）→ 人間マージ。
...
- **三重の強制(目標状態)**: (1) hooks が **main/develop 上での `commit`・`merge`・`rebase`** と、**main/develop 宛ての `push`** をブロック（8.3。**push は宛先で判定する** — カレントブランチは宛先の代理にならないため。v1.5 で是正）、(2) GitHub ブランチ保護が直 push を拒否（10.2。**CI 未達マージの拒否は成立していない** — 下記)、(3) CLAUDE.md / AGENTS.md に明記（両AIの行動規範）。**(2) は 2026-09-19 に適用したが、三重の強制は揃っていない**（10.2 実装状況）: **直 push・force push・ブランチ削除・PR を経ないマージは設定上は止まる(**設定上は拒否される — API 実測で Ruleset は `active`。ただし拒否動作そのものの実測は未実施**。`gh ruleset check` は適用対象の列挙にすぎず拒否を実証しない → `TSK-433`)。ただし必須チェックの発行元が未拘束(`integration_id` 未指定)のため、「CI が実際に green であること」の強制は成立していない**(10.2 のリスク受容記録は解除せず存続)。 hooks は Claude Code 経由の操作のみ遮断し（人間の端末・別 clone・GitHub UI には効かない）、マージ阻止は github-setup.md 2 章の管理手続で補償する
...
- **リモート保護の適用(2026-09-19 — 2026-08-16 に置いた注記を更新)**: リモート保護(ブランチ保護)は**適用済み**(10.2 の実装状況)。前項の「未チェックのままマージしない」は**必須チェックと `strict_required_status_checks_policy` がリモートで要求する**。**ただし発行元(`integration_id`)が未拘束のため、必須チェックは同名の成功 status で満たせる — 「CI が実際に緑であること」の強制は成立していない**(10.2 のリスク受容記録を参照)。ただし `github-setup.md` 2 章の**手続 3・4(コア領域 ∪ guard_paths の逐行確認)は継続**する — 保護は検査ロジック自体の改変を防げないため(同 2 章の循環参照)
...
- ルートに配置。**`@AGENTS.md` をインポート**し（公式 @import 構文）、共通規約を単一ソース化した上で Claude 固有の導線のみを追記する:
  ...
  - 調査サブエージェントの運用既定（8.5: Opus 5.5 固定・high 固定・既定3並列、並列度のみ重さで調整）
  - 主セッションの共有既定（本節末尾: Opus 5.5・effort high・Fable 5.1 への一時昇格の条件・Fast mode 禁止）
  - ドキュメントマップへのポインタ（`docs/README.md`）
- 目安 100〜200 行以内（公式推奨）。詳細規約は `.claude/rules/` とリンク先文書に逃がす
- **主セッション（Claude Code 本体）の既定**（v1.18）: 共有プロジェクト設定 `.claude/settings.json`（8.2）に `"model": "claude-opus-5-5"`・`"modelSettings": {"claude-opus-5-5": {"effortLevel": "high"}}`・`"env": {"CLAUDE_CODE_DISABLE_FAST_MODE": "1"}` を置き、**新規セッションの既定を Opus 5.5・effort high にし、Fast mode を無効化する**。**前提: Claude Code v2.1.280 以上**（Opus 5.5 の最低版 — `/setup-dev` で版を確認）。**モデル選択の優先順位**（公式 model-config / settings）: 現セッション中は `/model` が直ちにモデルを切り替える。新規セッション開始時は、起動時の `--model` → `ANTHROPIC_MODEL` → settings の `model` の順であり、settings の `model` 同士は managed 設定 → `.claude/settings.local.json` → **共有プロジェクト設定（本既定）** → 利用者設定（`~/.claude/settings.json`）の順となる。managed の `availableModels` は、この優先順位とは別に全モデル選択経路を制約する。したがって本既定は利用者設定（例: `fable[1m]`）より優先するが、`--model`・`ANTHROPIC_MODEL`・managed 設定・`settings.local.json` がある場合はそれらが優先する。`/model`（Enter または `/model <name>`）は現セッションへ直ちに適用されると同時に利用者設定 `~/.claude/settings.json` の `model` に保存されるが、共有プロジェクト設定が `model` を持つ本リポジトリでは次回起動時に共有設定が優先して既定へ戻る。**現セッションだけの一時変更は `/model` ピッカーで対象行の `s` を選ぶ**（利用者設定を変えない — 公式 model-config）。恒久的なプロジェクト個人差分は gitignore 済みの `.claude/settings.local.json` へ置く。Opus 5.5 の既定 effort は medium で、利用者設定のトップレベル `effortLevel` は Opus 5.5 に適用されないため、`modelSettings` でモデル別に明示する（ここでの `effortLevel` は settings.json の正式なキーであり、8.5 で廃止した agents frontmatter の未知キーとは別物）。**Fable 5.1 は常用しない**: Max プランでは週次上限の 50% までしか使えず他モデルより速く消費するため、Opus 5.5 high で足りない長期自律作業・相談役に限り `/model` ピッカーで Fable 5.1 の行を選び `s` を押して現セッションだけ一時的に昇格する。**Fast mode は使わない**（プラン枠でなく usage credits を消費する。`/fast` の ON は次セッションへ持続するため、共有設定の `env` で `CLAUDE_CODE_DISABLE_FAST_MODE=1` を置いて機構的に無効化する — 公式 fast-mode）。根拠 = Opus 5.5 は公式に「Fable 5.1 相当の性能で Opus 5 より 40% 安い」（$4/$20・cache read $0.20）、ローカル実測では主セッションが Claude 側消費の約 8 割で 1 ターン平均 40 万トークンのキャッシュ読み取りが支配項（`docs/features/harness-model-refresh/research.md` A-3 / B-1）。**コーディングの Claude 側への移管は本既定の対象外**（別タスク — 3 章の分担は不変）
...
| スキル | 内容 |
| --- | --- |
| `/setup-dev` | 初回セットアップ: 開発者↔Notion ユーザーの紐づけ（11.2）→ Codex の trust 設定と WSL2 の確認（手順の正は `onboarding.md`（**v1.9: 同書の approved 化に伴い、参照を「配置先」から「規範」へ格上げした**）— 7.1-5 / NFR-021。`[windows] sandbox` の設定は案内しない — 12.1）→ 開発ツールの疎通確認と**版の下限の確認**（Claude Code 2.1.280 以上〔8.1 — Opus 5.5 の最低版〕・Codex CLI 0.157.0 以上〔9.4 — ラッパーも起動前に検査〕。未達なら `claude update` / onboarding 1-6 で更新してから進む） → permissions 構文検証 |
...
| `/research <テーマ>` | Web 調査の Codex 委任: read-only + `-c web_search="live"`・モデル・effort は ADR-001 の「Web 調査」行〔`--deep` は「Web 調査(深い技術検証 `--deep`)」行〕をラッパーが固定。結論と参照 URL を記録 |
...
- **実行設定の既定**: `model: claude-opus-5-5`（**Opus 5.5 に固定** — エイリアス `opus` でなく明示 ID で指定。v1.18 で前世代の Opus から更新。**前提: Claude Code v2.1.280 以上** — Opus 5.5 の最低版〔公式 model-config〕）・`effort: high` を frontmatter で指定（**`effort` が公式の frontmatter キー**で、Opus 5.5 の既定 effort は medium のため明示が必須。v1.17 まで誤って併記していた**未知キー `effortLevel`** は公式リファレンスに存在せず無視されるため **v1.18 で併記を廃止** — 併記の根拠だった「公式キーの表記ゆれ対策、P1-6」は公式仕様の確定で解消）。**effort は 3 本とも high 固定**（frontmatter の `effort` は当該 agent の実行時にセッション値を上書きする公式仕様のため、呼び出しごとの動的調整はしない。別の effort が要る用途は agent 定義を分けて追加する）。並列度は**既定 3 並列**とし、Claude が作業の重さで増減する（軽い単発の事実確認は 1 本、リリース前の広域監査・設計の全面突合などは増やす — **モデルと effort は固定のまま**）。この運用既定は CLAUDE.md に記載して Claude の既定動作にする（8.1）
...
python .claude/scripts/codex_run.py fast -                           # 軽微 fast path（worktree 内・ADR-001 の「軽微な修正」行）
...
python .claude/scripts/codex_run.py probe                             # ADR-001 決定表の全一意 (model, effort) 組の受理確認（read-only + cached search — v1.18）
# implement / fast / research / review のプロンプトは stdin（ヒアドキュメント）で渡す。probe はラッパー内蔵の固定短文を使用し、stdin を読まない。
...
### 9.4 モデル選定（ADR-001 — 現行版の正は同 ADR の変更履歴）

コーディング委任のモデル・effort は「作業の重さ」で決まる。**モデル・effort・行キー・分類主体・優先順位の唯一の正は [`ADR-001`](../adr/ADR-001-codex-model-selection.md) の「決定」節**であり、**本節では一覧・割当てを再掲しない**（7.1-1 の複製禁止 — v1.17 までの再掲は v1.18 で廃止）。

- エイリアス（`gpt-6` 等 — 提供側が上位 tier へ振り向け得る）を使わず明示 ID で固定する（モデルドリフト防止 — 8.5 の Opus 5.5 固定と同じ規律）。現行のモデル ID・割当て・不掲載方針・理由・巻き戻し条件は ADR-001 v1.1 を参照する
- 同 ADR の決定表は `.claude/scripts/codex_run.py` が**機械適用**する（計画書の重さ分類 → フラグ変換。人が都度選ばず、スキルの自然言語にも依存しない — P0-2/P1-6。通常経路の `重さ分類` の欠落・空値・不正値、および fast 経路の `status: active`・`重さ分類: 軽微`・`実行方式: fast` の 3 値の欠落・空値・不一致は fail-closed で停止）。`tests/` の同期テストが ADR-001 を読んでラッパー定数との一致を検査し、本節は値を持たない
- effort の **max は CLI から指定可能**なことを実機検証済み（v0.146.1・2026-08-07: `-c model_reasoning_effort=max` で実行成功。公式 config リファレンスの記載は xhigh までだが実装は受理する）。GPT-6 世代は **codex-cli 0.157.1 のカタログで `gpt-6-sol` の supported_reasoning_levels に low〜max（+ 対話専用 ultra）を確認**（2026-09-27。`exec` 経路での受理は TSK-463 ステップ 4 で確認し ADR-001 に追随行を残す）。対話ピッカー限定なのは Ultra のまま。**Codex CLI は 0.157.x 以上が前提**（`gpt-6-sol` / `gpt-6-luna` — 公式変更履歴上のカタログ追加は Codex CLI 0.156.1〔0.156.0 向け hotfix〕・GitHub リリースノートでは 0.157.0〔2026-09-25〕に記載・モデル定義の `minimal_client_version` は 0.155.0。**本ハーネスの運用下限は本機で確認済みの 0.157.0**。onboarding 1-6 のインストーラで更新）。**ラッパーは起動前に `codex --version` を解析し、0.157.0 未満・解析不能なら fail-closed で停止して更新手順を示す**（ADR-001 帰結 — 既存開発者が onboarding を再実行しない経路を閉じる）。受理確認は ADR-001 の決定表に現れる全 `(model, effort)` 組を read-only の **`probe` 経路**で行う（TSK-463 ステップ 4）
...
- **CI 全ジョブ green をマージ条件にする**（10.2）— 要件書 7.3 / NFR-019 の直接要求。**このマージ条件は 2026-09-19 に Ruleset へ設定した**（10.2 実装状況。必須チェック + `strict_required_status_checks_policy`）。**ただし「CI 全ジョブ green」の強制は成立していない** — 必須チェックの**発行元が未拘束**(`integration_id` 未指定)であり、書き込み権限を持つ主体が同名の成功 status を立てれば満たせるため。**10.2 のリスク受容記録は解除せず存続している**
...
- **【2026-09-21・TSK-429】承認済み例外の解除と、新しい未達への切り替え**
  - **解除の対象**: 本節の「自動再現要件は**保護が利用可能になるまで**未達(承認済み例外)」という**期限付きの例外そのもの**。**自動再現要件の未達状態は解除しない**(下記 2 原則の②)
  - **解除の理由**: 前提条件が 2026-09-19 に消滅した(public 化により Rulesets が利用可能になり、`protect-main-develop`(id `23694095`)を適用済み)。**期限付きの例外は、期限の到来により成立しなくなった**
  - **判定 — 成立時点は 1 つだけ**: **本行へ承認日・承認者が記入された時点(確定ゲート 手順 6)で成立する。** 手順 5 の人間承認は**成立の必要条件**であって成立時点ではない。**手順 5 と手順 6 の間は「未成立」** — その区間では**承認済み例外は従前のまま存続する**
    - **承認日: 2026-09-21 / 承認者: 山田正輝(PO)**。**本改訂の確定ゲート 手順 5 の承認者であり、解除の判定者と同一人物である(自己承認)**
  - **解除後に残る未達**: **本節の「`scripts/setup_branch_protection.py`(gh api 使用)で再現可能にする」要件は依然として未達。** スクリプトは未実装(2026-09-20 実測)
  - **切り替え先の未達**: **担当** = PO / **追跡先** = Notion **`TSK-432`** / **DoD** = 冪等(GET→POST/PUT)・重複検知・dry-run・適用後検証 / **再確認の時期** = ①`TSK-432` の完了時 ②Ruleset に変更を加えた時点(**変更の種類を問わない**)。**2 つは独立した契機であり、どちらを実施しても他方の義務は消えない**
  - **未確認中の制限**: **Ruleset の構成変更を手作業で行った場合は、その差分を `github-setup.md` 3 章の JSON へ同一 PR で反映する**(反映のない構成変更を行わない)
...
- **リスク受容記録・承認済み例外の解除 — 本改訂で確定する 2 原則**(2026-09-21 新設・TSK-429)。本書はこれまで解除**条件**を書く一方で**どう閉じるか**を定めていなかった。**本改訂で確定するのは次の 2 つだけである。**
  1. **形式的な充足で解除しない。** 解除条件が指す統制が**実際に機能していること**を確認する。**設定が存在することと統制が成立していることは別である**(実例: 必須チェックが設定されていても、**発行元が拘束されていなければ**同名の成功 status を立てるだけで満たせるため「全グリーンでないとマージ不可」は成立しない — 下記の検討)
  2. **解除の範囲は当該記録が明示した対象に限る。** 記録が対象としていない要件・条文が充足したとは主張しない
  - **【送り出し・PO 裁定 2026-09-21】上記 2 原則を超える手続の詳細は、初回の実解除を行うタスクで実機とともに確定する** → 後続タスク **`TSK-435`**。送り出す範囲: 提案者と判定者の関係(自己承認の可否と記録)/ 判定の成立時点 / 追記の必須項目 / 追跡先が満たすべき条件と**無断完了の検知契機** / 再確認の主体と契機。
    - **理由**: **本改訂が実施する解除は 1 件(上の期限付き例外)だけで、それは 2 原則だけで判定できる。** 期限付き例外の解除は「**期限が到来したか**」という客観的事実で決まり、**原則 2 で範囲を限定する**(=自動再現要件の未達状態は解除しない)以上の判断を要しない。**送り出した詳細**(判定者の同定・自己承認の扱い・追記の必須項目・追跡先の条件と無断完了の検知・再確認の契機)**が必要になるのは、条件の充足を判断する解除** — すなわち**リスク受容記録の解除**であり、**本改訂はそれを射程から外した**(下記のとおり存続させる)。したがって詳細を今決めると**適用対象のない設計**になる。**適用対象のない手続を机上で完成させようとすると、是正が次の穴を生む閉じた輪になる**(本改訂の確定ゲート 7 周で実測 — **設計書 v1.7 の確定ゲート 12〜15 周目**および **ADR-003 の PO 裁定 2026-08-19** と同型の失敗)
    - **今回の例外解除にも 2 原則は適用される**(原則 2 により、解除の範囲を「期限付き例外そのもの」に限定し、**自動再現要件の未達状態には及ばないと明記した**)。**送り出しによって現在の解除に穴は開かない**
    - **送り出しても原則は分岐しない** — 詳細はいずれも上記 2 原則の下で決まるため
    - **見直しトリガー**: 詳細の確定が上記 2 原則と両立しないと判明した時点で、本節を改訂ゲートへ戻す
- **【2026-09-21・TSK-429】本記録(リスク受容記録)は解除しない — 解除条件を追加して存続させる**(確定ゲート 敵対レビュー 初周 `P0` を採用した射程縮小。PO 裁定 2026-09-21)
  ...
  - **しかし実質的な強制が成立していない**(上記原則 1): **必須チェックは全 context とも `integration_id` が未指定**であり、**書き込み権限を持つ主体が同名の成功 status を立てれば必須チェックを満たせる**。CI が実際に失敗していても・一度も走っていなくてもマージできる。本記録が対象とする NFR-019「**全グリーンでないとマージ不可**」の統制は**立っていない**
  - **解除条件へ次を追加する**: **必須チェックの発行元が拘束されていること**(`integration_id` の指定、または同等の手段)。既存の 2 条件(プラン制約の解消・3 章の保護適用)は充足済みとして記録する
  - **追跡先の差し替え**: 旧追跡先 **`TSK-202` は完了済み(2026-08-10)で残作業を追えない**ため差し替える。目的別に分ける — **解除条件の追跡** = **`TSK-433`**(DoD に**発行元拘束の成立**を含む。**「指定可否を判断した」だけでは満たさない**)/ **解除後に残る未達の追跡** = **`TSK-217`**(PO 裁定 `C`・2026-09-12 の NFR-019 柱書 owner)。**(d) の実装カード `TSK-324` は担当者未設定のため追跡先に用いない**
  - **補償統制は継続する**: `github-setup.md` 2 章の管理手続(手続 1〜4)+ hooks のローカル遮断
  - **解除後に残る未達**(解除時に記録する項目の事前整理): NFR-019 条文の **(a) 一致性・(b) 越境・(c) E2E・(d) 同期故障系**は本記録の対象外であり、いずれも未達。また **`required_approving_review_count: 0`** であるため、仮に発行元が拘束されても**強制されるのは必須チェックの green であって人間のレビュー承認ではない**
