...
**③ が要る**(確定ゲート 5 周目 `P0`)— **検査器を同一の変更の中で弱めれば、
① と ② は成立したまま不正な遷移が通る。** **検査は自分自身の改変を判定できない。**

**「信頼できる比較元」とは書かない**(確定ゲート 2 周目 `P1`)— **信頼性は本節が判定できる性質ではない。**
**条件は 2 つとも、成立していないことを本節が検出できない事実**として、次に列挙する。
...
| 5 | **検査器自身が当該変更で弱められていないこと** | **検査器を弱める変更と、その弱めた検査を通る基準遷移を、同一の変更に含める**場合。**検査は自分自身の改変を判定できない**(既知の循環参照)|
