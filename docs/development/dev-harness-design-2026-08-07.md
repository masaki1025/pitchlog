...
- **`P4-後` は必須の後続処理であり、放置してはならない**（v1.6 — 確定ゲート 5 周目 P1）。**`P4-後` のマージを、① Phase 5 への着手 ② Phase 4 展開タスクのクローズ の前提とする**。**起票と完遂の責任は Phase 4 の完了を確認した者（= 4-6 の PR 作成者）が負う**。**これを定めないと、Phase 4 だけを完了扱いにして `P4-後` を放置でき、`/release` の無条件中断が残り続ける**
