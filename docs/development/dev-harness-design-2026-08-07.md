...
| 1.13 | 2026-09-02 | **13 章へ「製品機能の実装」を新設(v1.13)**: **要件書の FR が定める製品機能の実装は本章の Phase の対象外である**ことを宣言した(Phase は本書が定める開発ハーネスの導入単位であり〔**Phase 0 = 本設計案の確定**〕、製品機能の作業単位ではない)。4 項目 — **従う導線** = **6.1 の実装計画書ゲート**(柱書「原則 各 Phase = 1 PR、この順に develop へ」は**適用しない**)/ **開始条件** = **Phase 4 の受入判定が `passed` であること**(10.1。**`P4-後` のクローズと Phase 5 の完了は待たない** — 製品実装が Phase 4 に求めるのは骨格・CI・開発 DB が実機で動くことであり、10.1 の受入証跡がそれを証明している)/ **適合判定との関係** = 正解ベクタ・検査基盤の未整備は**適合と判定しない**ことを意味し、**実装を禁じるものではない**(ADR-003)/ **本章では順序・群・段階を定めない**(どの FR から着手するかは各実装計画書の判断)。**Phase は追加していない**(Phase 表の行数は不変・既存 Phase 0〜5 の各行と「Phase 4 の分割」節は改訂前とバイト一致)。**`:847`「論理スロットと Notion タスクの対応の機械化は行わない」と衝突しない**(段階計画・群・順序・進捗の機械判定を持ち込まないため)。背景 = approved 設計(`../design/sync-protocol.md` 11-3)が FR 10 件を決めている一方で製品コードは `GET /health` のみに留まっており、**Phase 表が Phase 5 で終わることにより製品実装の位置づけが規範上不明**で、それが着手を止めていた。**射程宣言(7.3-7)**: **本改訂で確定する範囲** = 13 章に「製品機能の実装は Phase の対象外であり 6.1 の実装計画書ゲートに従う」ことと、その開始条件・適合判定との関係・順序を定めないことの宣言。**実装時(各実装単位のタスク)で確定する範囲** = 実装単位の数・名称・境界・PR 分割・順序・見積もり、API / UI / データモデルの設計正本、`NFR-018` の検査基盤と正解ベクタ、進捗とタスク対応の機械化(**`:847` により行わない**)。計画: `../features/product-impl-phase/plan.md` | **in-review** |
...
- **開始条件**: **Phase 4 の受入判定が `passed` であること**(10.1)。**`P4-後` のクローズと Phase 5 の完了は待たない** — 製品実装が Phase 4 に求めるのは骨格・CI・開発 DB が実機で動くことであり、10.1 の受入証跡がそれを証明している
- **適合判定との関係**: 正解ベクタ・検査基盤の未整備は**適合と判定しない**ことを意味し、**実装を禁じるものではない**([ADR-003](../adr/ADR-003-domain-calc-method.md))
