...
**7.3-1 適用版の原則**: **すべての確定ゲートは、開始時(対象の in-review 化)の時点で approved の本節の版を適用版とし、当該ゲートは記録した適用版で最後まで運用する**(ゲート進行中に本節の新版が approved になっても途中で切り替えない)。**本節(7.3)自身の改訂ゲートは、対象の in-review 化の直前に approved であった版の規則で運用する**(ゲート対象の未確定条文を先取り適用しない — 循環の排除。恒久除外は作らない)。**適用版 = 初回敵対レビュー実行時点で approved の版**とする — in-review 化と同一コミットで worklog へ記録し(証跡 — 開始点の別コミットを要求しない)、**in-review 化から初回敵対レビュー実行までの間に本節の新版が approved になった場合は、適用版を新版へ更新してから開始する**(早期の in-review 化による旧版の先取り固定を許さない。固定〔途中で切り替えない〕が効くのは初回レビュー実行以後)。**複数正本の一括検証は単一のゲート**として扱い、適用版の記録は**ゲート単位で 1 件**(全対象の in-review 化が揃い、初回敵対レビューを実行した時点の版 — 対象ごとに分裂させない)。運用は**記録した適用版の条文**に従う: スキル(/finalize-doc)本文と適用版の条文が食い違う場合は**適用版の条文が優先**する(スキルは手順の写像であり規範ではない — 節番号の存否ではなく条文内容で判定する)。
...
**7.3-5 依頼文規約**: 依頼文の**冒頭に命令文を必須で置く** — 「実際にレビューを実施し、次周の依頼文を作らず、判定を返すこと」(メタ解釈の防止)。2 周目以降は前周指摘の全文並記をしない(差分と影響節の指定のみ)。ただし**終結済み指摘の要旨一覧(採否記録の抜粋)は依頼文に添付する**(全文並記の禁止の例外 — 再提示の抑止に必要)。**終結の定義(一意)**: ① 採用され反映済み ② **PO が不採用を支持したもの(1 回で終結 — 7.3-2 の全件不採用行 (i) と同一)** ③ (B) として送り先記録済み。**終結済みと同一内容の指摘が再提示された場合は、新規指摘として算入せず**(7.3-2 の遷移・7.3-6 の母集団から除外)、採否記録の該当項の参照で処理する。作成者判断のみの不採用(PO 未支持)は終結ではなく、再提示は通常どおり算入する(異議は 7.3-2 の全件不採用 2 周ルートで PO へ上がる)。レビュア応答の必須様式(欠落時は周として数えず同一周を再依頼): 各指摘に `P0|P1|P2` + `起因|非起因`(前周修正への帰属)+ `(A)|(B)`((A) 方式が分岐する / (B) 実装時に決めれば足りる — **(B) を付けられるのは P2 のみ**で、送り先〔実装タスク or 申し送り先〕を記録する)、最終行に**判定行**(`可決|否決` + P0/P1/P2 件数)。7.3-3 の重大度定義を依頼文プロンプトへ毎回組み込む。
