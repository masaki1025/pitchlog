...
**7.3-1 適用版の原則**: 確定ゲートには**その時点で approved の本節の規則**を適用する。**本節(7.3)自身の改訂ゲートは、当該改訂の開始時点(= 当該改訂の変更履歴行〔in-review〕を追加したコミットの時点)で approved であった版の規則で運用する**(ゲート対象の未確定条文を先取り適用しない — 循環の排除。恒久除外は作らない)。**適用版はゲート開始時に worklog へ記録する**(証跡)。スキル(/finalize-doc)の手順のうち、適用版に存在しない規則に由来するものは当該ゲートでは適用しない。
...
| --- | --- | --- | --- |
| 射程縮小 | 縮小範囲と理由を本文・変更履歴・worklog へ反映 | **エスカレーションを発火した周の反映コミットに同梱する**(当該周が採用反映を伴うならその 1 コミット・加算 1 回のみ。当該周に採用反映がなければ PO 編集自体を 1 反映周として 周回 +1・`反映<r>周目` コミット 1 件)。**1 周への加算は最大 1 — 二重加算しない**(6.1 と整合) | **全文レビュー**(7.3-4) |
