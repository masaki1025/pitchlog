...
- ルートに配置。**`@AGENTS.md` をインポート**し（公式 @import 構文）、共通規約を単一ソース化した上で Claude 固有の導線のみを追記する:
  ...
  - 調査サブエージェントの運用既定（8.5: Opus 5.5 固定・high・既定3並列、重さで調整）
  - ドキュメントマップへのポインタ（`docs/README.md`）
- 目安 100〜200 行以内（公式推奨）。詳細規約は `.claude/rules/` とリンク先文書に逃がす
- **主セッション（Claude Code 本体）の既定**（v1.18）: プロジェクト共有の `.claude/settings.json`（8.2）に `"model": "claude-opus-5-5"` と `"modelSettings": {"claude-opus-5-5": {"effortLevel": "high"}}` を置き、本リポジトリを開く全開発者の新規セッションを **Opus 5.5・effort high** にする（Opus 5.5 の既定 effort は medium で、利用者設定のトップレベル `effortLevel` は Opus 5.5 に適用されないため、モデル別に明示する。ここでの `effortLevel` は settings.json の正式なキーであり、8.5 で廃止した agents frontmatter の `effortLevel` とは別物）。**Fable 5.1 は常用しない**: Max プランでは週次上限の 50% までしか使えず他モデルより速く消費するため、Opus 5.5 high で足りない長期自律作業・相談役に限りセッション内の `/model` で昇格する（個人の恒久上書きは gitignore 済みの `.claude/settings.local.json`）。**Fast mode は使わない**（プラン枠でなく usage credits を消費する）。根拠 = Opus 5.5 は公式に「Fable 5.1 相当の性能で Opus 5 より 40% 安い」（$4/$20・cache read $0.20）、ローカル実測では主セッションが Claude 側消費の約 8 割で 1 ターン平均 40 万トークンのキャッシュ読み取りが支配項（`docs/features/harness-model-refresh/research.md` A-3 / B-1）。**コーディングの Claude 側への移管は本既定の対象外**（別タスク — 3 章の分担は不変）
...
| `/research <テーマ>` | Web 調査の Codex 委任: read-only + `-c web_search="live"`・モデル・effort は ADR-001 の「Web 調査」行〔`--deep` は「深い技術検証」行〕をラッパーが固定。結論と参照 URL を記録 |
...
- **実行設定の既定**: `model: claude-opus-5-5`（**Opus 5.5 に固定** — エイリアス `opus` でなく明示 ID で指定。v1.18 で前世代の Opus から更新）・`effort: high` を frontmatter で指定（**`effort` が公式の frontmatter キー**で、Opus 5.5 の既定 effort は medium のため明示が必須。v1.17 まで併記していた旧キー〔`effort` の別綴り — 廃止したキー名は変更履歴 v1.18 行〕は公式リファレンスに存在せず無視されるため **v1.18 で併記を廃止** — 併記の根拠だった「公式キーの表記ゆれ対策、P1-6」は公式仕様の確定で解消）。並列度は**既定 3 並列**とし、Claude が作業の重さで増減する（軽い単発の事実確認は 1 本、リリース前の広域監査・設計の全面突合などは増やす）。effort も同様に重さに応じて調整してよい（**モデルは固定のまま**）。この運用既定は CLAUDE.md に記載して Claude の既定動作にする（8.1）
...
| Web 調査（/research） | `gpt-6-sol` | high |
