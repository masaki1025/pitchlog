...
- **リスク受容記録・承認済み例外の解除手続(2026-09-21 新設・TSK-429)**: 本書はこれまで解除**条件**を書く一方で**誰がどう閉じるか**を定めていなかった。以下を定める。
  0. **提案者と判定者は同一人物であってよい**(PO 裁定 2026-09-21 — **現状 1 人開発であり職務分離を要求すると解除が永久に不能になる**。`required_approving_review_count: 0` を「現状 1 人開発のため」と正当化している 10.2 / `github-setup.md` 3 章の判断と整合する)。**ただし同一人物である場合は、手続 4 の追記へ「自己承認である」旨を明記する。** 明記のない解除は成立しない
  1. **提案者**は、解除条件の各項が充足したことを**実測の典拠つき**で示す(伝聞・推定で示さない)。**あわせて、追跡先の状態を追跡元(Notion)で再取得し、本節の記載と一致することを確認する**(不一致なら下記の「追跡先を失った状態」に当たる)
...
- **【2026-09-21・TSK-429】本記録は解除しない — 解除条件を追加して存続させる**(確定ゲート 敵対レビュー 初周 `P0` を採用した射程縮小。PO 裁定 2026-09-21)
  ...
  - **追跡先の差し替え**: 旧追跡先 **`TSK-202` は完了済み(2026-08-10)で残作業を追えない**ため差し替える。**手続 7 のうち「解除時に差し替える」という要件は、本記録が解除されないため発動していない。** ただし**条件①〜③(未完了・未達を担当範囲/DoD に含む・担当者が定まっている)は、追跡先が追跡先である限り継続して満たされなければならない**(条件を失った時点で追跡が切れるため)。**ただし死んだ参照を残すと解除時に突合できないため、いま差し替えておく。** 追跡する対象が 2 つあるので**目的別に分ける**:
    - **解除条件の追跡**(本改訂で追加した「必須チェックの発行元が拘束されていること」)→ **`TSK-433`**。未完了 / 担当者設定済み / **DoD に「発行元拘束の成立」を含む**
      ...
      - **確認の主体と記録先**: 当該タスクを**閉じる者**が、閉じる前に **①DoD が解除条件の成立を含むこと ②成立していない場合は差し替え先が定まっていること**を確認し、**確認結果を本節へ追記する**(手続 4 と同じ形式)
      - **無断完了を検知する契機**(本書は Notion 側の操作を阻止できないので、**検知する契機を定める**): ①**解除を提案する時点**(手続 1 — 提案者が追跡先の状態を再取得する)②**本節の再確認条件が発火した時点**(上の「再確認の主体・時期」)③**当該追跡先を参照する他の正本・計画書を改訂する時点**。**いずれかで不一致を見つけた者が本節へ追記する。**
      - **追記のない完了が起きた場合(fail-closed)**: 本書は Notion 側の操作を阻止できない。**追跡先が本節への追記なしに閉じられた場合、本節は「追跡先を失った状態」となり、その状態では解除条件の充足を主張できない**(手続 6 の「形式的な充足で解除しない」と同じ効力)。**回復するには、条件①〜③を満たす追跡先を定めて本節へ追記する**
      - **拘束が成立した場合の扱い**: 発行元拘束が成立したら、**当該タスクを閉じる前に本記録の解除(手続 1〜6)を実施するか、解除しないなら残る解除条件を担う追跡先を定める**。**「条件が成立したので追跡は不要」として閉じることは、条件①(未完了)を失わせるため認めない**
