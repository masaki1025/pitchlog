...
- **【2026-09-21・TSK-429】承認済み例外の解除と、新しい未達への切り替え**(下の「解除手続」に従う)
  ...
  - **判定**(手続 3): **本改訂の確定ゲート 手順 5 の人間承認をもって成立する。** 承認日・承認者は approved 化時に本行へ記入する(記入なきうちは解除は成立していない)
  ...
  - **切り替え先の未達**:
    ...
    - **再確認の主体・時期・記録先・未確認中の制限**(手続の実効化):
      - **主体** = PO / **時期** = ①`TSK-432` の完了時 ②Ruleset の構成が変わった時点(必須 context の増減・rule の追加削除・`bypass_actors` の変更)のいずれか早い方
...
- **【2026-09-21・TSK-429】本記録は解除しない — 解除条件を追加して存続させる**(確定ゲート 敵対レビュー 初周 `P0` を採用した射程縮小。PO 裁定 2026-09-21)
  ...
  - **追跡先を差し替える**(上記手続 7): 旧追跡先 **`TSK-202` は完了済み(2026-08-10)で残作業を追えない**。**`TSK-217`** へ差し替える — **未完了** / **PO 裁定 `C`(2026-09-12)により NFR-019 柱書「比較単位と正解ベクタの定義((a)(c)(d) を含む)」と CI 全ランナー配線の owner** / **担当者が設定済み**。発行元拘束の判断は **`TSK-433`**、(d) の実装は **`TSK-324`**(**同カードは担当者未設定** — 差し替え先には用いない)
