...
- **却下案**:
  ...
  - **native enum を既定にする案** — 却下する。autogenerate が enum 値の増減を信頼して
    扱わないため、**既定は参照テーブル + FK** とする。**却下したのは「既定にすること」だけ**であり、
    **native enum を一律に禁じるものではない** — 値セットが非常に安定した場合の許容は
    12-4 節が正であり、本節はその判断を上書きしない
