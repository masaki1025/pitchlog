...
コーディング委任のモデル・effort は「作業の重さ」で決まる。**モデル・effort・行キーの唯一の正は [`ADR-001`](../adr/ADR-001-codex-model-selection.md) の「決定」節の固定構文表**（8 行: 通常実装 / 軽微な修正 / コア領域の実装 / 機械的軽作業 / 一次コードレビュー / 敵対レビュー・コア領域 PR・正本確定ゲート / Web 調査 / Web 調査(深い技術検証 `--deep`)）であり、**本節では値を再掲しない**（7.1-1 の複製禁止 — v1.17 までの再掲は v1.18 で廃止）。分類主体（計画作成者）と優先順位（コア領域 > 軽微な修正 > 機械的軽作業 > 通常実装）も同 ADR が定める。
