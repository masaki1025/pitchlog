...
- **plan frontmatter の拡張キー**（feature 作業自身の進行事実のみを置く — 正本 status の複製は置かない〔7.1-1〕。**本表が契約の正** — v1.3）:
  ...
  | `Phase4スロット` | `対象外` \| `4-1`〜`4-6` \| `P4-後`（これ以外は不正） | **なし（必須）** | **v1.6 で新設**。Phase 4 の分割 PR を識別し、`/task-done` が `P4-後` の起票対象を判定するのに使う（13 章）。**任意キーにしてはならない**（v1.6 — 確定ゲート 10 周目 P1）— **欠落を「Phase 4 ではない」と解釈すると、4-6 の計画書で書き忘れたときに `P4-後` を起票しないまま worktree 除去と Notion 完了まで進む fail-open** になる。**欠落・空値・列挙外の値では `/task-done` を中断する**（`反映周コミット` と同じ理由 — 記入漏れを正常系に紛れさせない）。**ただし必須化だけでは 4-6 の書き忘れを塞げない**（v1.6 — 確定ゲート 11 周目 P1）— **テンプレが既定値を出力すると、書き換え忘れが「欠落」ではなく「正常値」として通る**。**11 周目に置いた「Notion タスク名から決めて突合する」方式は 12 周目 P1 で撤回した** — **`/task-start` と `/task-done` が同じタイトルを同じ規則で解釈するだけで独立照合になっておらず、実体が 4-6 でもタイトルにマーカーが無ければ両者とも `対象外` と判断する**（タスク名は規約化されていない自由入力である）。**代わりに人間の明示選択を入力とする**（v1.6 — 確定ゲート 12 周目 P1）: **テンプレは値を持たない空欄を出力し、`/task-start` が本表の列挙を提示して人間に選ばせて埋める**。**選択肢は 13 章が定める 7 個（`4-1`〜`4-6`・`P4-後`）と `対象外` で固定**であり、自動判定しない。**選択の誤りは起票側のマーカーで検出する**（v1.6 — 確定ゲート 13 周目 P1。**選択だけを唯一の入力にすると、誤って `対象外` を選んだ場合に「有効な列挙値」として通り、4-6 のマージ後検査と `P4-後` の起票がまるごと省略されて Phase 4 展開が終端不能になる**）: **Phase 4 の分割タスクは、着手前に Notion タスクの DoD へ機械可読マーカー行 `Phase4スロット: <値>` を持つ**（v1.6 — 確定ゲート 13 周目 P1 で導入・**14 周目 P1 で必須条件へ格上げした**）。
  - **マーカーが無いタスクでは `対象外` しか選べない**（v1.6 — 確定ゲート 14 周目 P1）。**`4-1`〜`4-6`・`P4-後` を選ぶにはマーカーが必須**である。**13 周目の形（マーカーが無ければ選択自由）では初回の 4-1〜4-6 が守られない** — マーカーを書くのは `/task-done` が起票する `P4-後` と「Phase 4-6 再試行」だけであり、**初回 4-6 で誤って `対象外` を選ぶとマージ後検査と `P4-後` の起票がまるごと省略できた**
  - **したがって初回の 4-1 〜 4-6 も、着手前にマーカー付きで事前起票する**（起票主体は Phase 4 の実施責任者 = 10.1 の判定者）
  - **マーカーの文法**: **行全体が `Phase4スロット: <値>` に完全一致**し（前後の空白は許容、値は本表の列挙）、**DoD 欄にちょうど 1 行だけ存在する**こと。**重複・列挙外・書式不一致はすべて中断**（fail-closed）
  - **`/task-start` はマーカーと人間の選択を突合し、食い違えば中断する**。**突合はブランチ・worktree の作成より前に行う**（不一致で中断したときに作成済みの worktree が残らないようにするため — 同 14 周目 P1）
  - **`P4-後`・再試行タスクは、元タスクが記録した子タスク URL と期待スロットにも突合する**。**同名タスクの直接起票（`/task-start` に名前を渡して新規作成する経路）は中断**し、既存タスクを使わせる
  - **これは 11 周目に撤回した「タスク名の二重読み」とは異なる** — **マーカーは起票側（`/task-done` または実施責任者）が書き、選択は着手側（人間）が行うため、別々の入力である**
  - **残余**: **自由記述の DoD 欄である以上、マーカーを有効な別の値へ改変されたことは検出できない**（元タスク側の記録との突合が効くのは `P4-後`・再試行のみ）。**機構ではなく運用規律に留まる範囲**として明記する**欠落・空値・列挙外はすべて「既定なし」として `/task-done` を中断する**（本表の既定列が `なし（必須）` である理由 — 同 12 周目 P1。**既定を `対象外` と書くと「欠落は中断」と矛盾し、欠落を `対象外` と読む実装が成立してしまう**）。**旧計画書（`Phase4スロット` を持たない v1.6 以前のもの）も同じく中断し、人間が `対象外` を明示して再実行する**（`反映周コミット` の `規約制定前` と同じ明示移行。**「Phase 4 以外は書き換え不要」ではない** — 同 11 周目 P2） |
...
- **マージ後検査の合格は、Git からの実測と人間の判定記録の両方で認可する**（v1.6 — 確定ゲート 9 周目 P1 で記録先を一意化し、**10 周目 P1 で認可根拠を Git 実測へ移した**）。
  ...
  - **② 人間の判定記録**: 4-6 の PR コメントへ **`M` の完全 OID / 期待した両親とツリー / 実測した両親とツリー / 実施日 / 合否 / 確認者**を記録する（10.1 が「人間の判定者による合格判定は必須」と定めるため）。**この記録は「人間が判定した」という事実の記録であって、①の期待値の供給源ではない**（v1.6 — 確定ゲート 13 周目 P1。**期待値は①のとおり `E` に収録された証跡から取る** — 記録された「期待した両親」と証跡の `phase4_base_sha` が食い違えば**不合格**として扱う）
