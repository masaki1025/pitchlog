...
- **「4-1 〜 4-6」は論理スロットであり、物理 PR 数ではない**（v1.7 — 確定ゲート 3 周目 P1・4 周目 P1 で範囲を 6 へ）。**4-6 には 0 回以上の再試行 PR が属する**ため、実際の本数は**成功時 6 本・再試行が n 回なら 6+n 本**になる。**`P4-後` はこのスロットに数えない**（Phase 4 の成果物ではないため）
