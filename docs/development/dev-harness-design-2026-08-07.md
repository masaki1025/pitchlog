...
| --- | --- | --- |
| 同期プロトコル | 断中記録・墓標/改訂の適用・再送・サーバー適用の原子性・複数タブの単一書き手競合(要件書 4.0-4・FR-012・NFR-007・NFR-019(d) の対象) | 同期セマンティクスを持たない単純な CRUD 通信 — 通常レビュー |
| 状況計算(クライアント・サーバー) | 要件書 4.0-4 の列挙(スコア・アウト・走者・打順・終了判定)+ **座標変換・捕球選手推定**(NFR-018・NFR-019(a) の対象 — 旧システムで 3 箇所コピー・「静かなデータ破壊」の事故前例〔改善台帳 I-2・I-17〕) | **成績集計の前処理・付録A 集計定義** — 付録A の固定ベクタ契約で防御する(NFR-018 実現方式 ADR〔台帳 H-55〕の管轄。契約検証を持つため逐行確認より機械検証が適する) |
| 記録権 | 記録権の付与・通常/緊急引き継ぎ・退避経路(FR-012・NFR-019(c) の対象) | 認可判定を伴わない権限の表示制御 — 通常レビュー |
| テナント分離 | **テナント境界の認可判定すべて** = 認可モデル(**FR-041/FR-042 共同分析グループを含む**)+ **全出力経路の越境**(管理者画面・エクスポート・CSV・PDF — 改善台帳 I-27 の迂回前例)+ NFR-010 の越境防止 | テナント内の権限細分(境界を跨がないロール制御)— 通常レビュー。出力エスケープ(NFR-023)は全経路で別途 P0 レビュー規則(AGENTS.md)が適用される |
| データ移行 | 88列⇄新スキーマ**変換層をモジュール単位で**(移行/CSV 出力の呼び出し経路を問わない)+ 移行検証(FR-038)・センチネル変換・名寄せ | 旧データと接しない新スキーマ内部の通常マイグレーション — 通常レビュー + テスト |
