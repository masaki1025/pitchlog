...
| 範囲確定 | 「本書で確定する範囲/実装時に確定する範囲」の宣言(7.3-7)を更新 | 同上 | **射程を拡大した場合(「実装時に確定」から「本書で確定」へ移す等)は全文レビュー**(7.3-4 の射程変更周)。縮小方向・現状維持なら差分再レビューを続行 |
