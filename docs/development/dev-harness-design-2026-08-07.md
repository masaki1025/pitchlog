...
**7.3-5 依頼文規約**: 依頼文の**冒頭に命令文を必須で置く** — 「実際にレビューを実施し、次周の依頼文を作らず、判定を返すこと」(メタ解釈の防止)。2 周目以降は前周指摘の全文並記をしない(差分と影響節の指定のみ)。ただし**終結済み指摘の要旨一覧(採否記録の抜粋)は依頼文に添付する**(全文並記の禁止の例外 — 再提示の抑止に必要)。**終結の定義(一意)**: ① **PO が不採用を支持したもの(1 回で終結 — 7.3-2 の全件不採用行 (i) と同一)** ② (B) として送り先記録済み(採用され反映済みの指摘は「終結」ではなく処理済み — 再提示は現成果物上の再発として通常どおり算入する)。**終結済みと同一根拠の単純な再提示は新規指摘として算入しない**(7.3-2 の遷移・7.3-6 の母集団から除外し、採否記録の該当項の参照で処理する)。ただし**現在の成果物上で同一の欠陥が再発・現存していることを示す指摘は封鎖の対象外**(新規指摘として算入する — 封鎖されるのは根拠の変わらない再提示のみ)。作成者判断のみの不採用(PO 未支持)は終結ではなく、再提示は通常どおり算入する(異議は 7.3-2 の全件不採用 2 周ルートで PO へ上がる)。**判定行と処理の整合**: 遷移の正は件数と採否処理(7.3-2)であり判定行は要約 — 判定行が件数と矛盾する応答(可決なのに P0/P1 あり・否決なのに全件 0 等)は**様式不備として周に算入せず、同一周を再依頼する**。レビュア応答の必須様式(欠落時は周として数えず同一周を再依頼): 各指摘に `P0|P1|P2` + `起因|非起因`(前周修正への帰属)+ `(A)|(B)`((A) 方式が分岐する / (B) 実装時に決めれば足りる — **(B) を付けられるのは P2 のみ**で、送り先〔実装タスク or 申し送り先〕を記録する)、最終行に**判定行**(`可決|否決` + P0/P1/P2 件数)。7.3-3 の重大度定義を依頼文プロンプトへ毎回組み込む。
