...
- **plan frontmatter の拡張キー**（feature 作業自身の進行事実のみを置く — 正本 status の複製は置かない〔7.1-1〕。**本表が契約の正** — v1.3）:
  ...
  | `Phase4スロット` | `対象外` \| `4-1`〜`4-6` \| `P4-後`（これ以外は不正） | **`対象外`（必須・明示）** | **v1.6 で新設**。Phase 4 の分割 PR を識別し、`/task-done` が `P4-後` の起票対象を判定するのに使う（13 章）。**任意キーにしてはならない**（v1.6 — 確定ゲート 10 周目 P1）— **欠落を「Phase 4 ではない」と解釈すると、4-6 の計画書で書き忘れたときに `P4-後` を起票しないまま worktree 除去と Notion 完了まで進む fail-open** になる。**欠落・空値・列挙外の値では `/task-done` を中断する**（`反映周コミット` と同じ理由 — 記入漏れを正常系に紛れさせない）。**ただし必須化だけでは 4-6 の書き忘れを塞げない**（v1.6 — 確定ゲート 11 周目 P1）— **テンプレが `対象外` を出力するため、書き換え忘れは「欠落」ではなく「正常値」として通る**。そこで **① `/task-start` が Notion タスク名を見てスロット値を決める**（タスク名に `Phase 4-<n>` / `Phase 4-6 再試行` / `P4-後` を含むならその値、含まないなら `対象外`）**② `/task-done` は Notion タスク名とスロット値を突合し、食い違えば中断する**（独立照合）。**旧計画書（`Phase4スロット` を持たない v1.6 以前のもの）は `/task-done` の再実行時に中断する** — **人間が `対象外` を明示して再実行する**（`反映周コミット` の `規約制定前` と同じ明示移行。**「Phase 4 以外は書き換え不要」ではない** — 同 11 周目 P2） |
...
- **マージ後検査の合格は、Git からの実測と人間の判定記録の両方で認可する**（v1.6 — 確定ゲート 9 周目 P1 で記録先を一意化し、**10 周目 P1 で認可根拠を Git 実測へ移した**）。
  ...
  - **取得と照合の分担**: **`M`（マージコミット）と `E`（4-6 の PR head）は GitHub API から取得し、第 2 親とツリーの一致は Git オブジェクトで独立に再検査する**（記録に依存しない改竄不能な根拠）。**第 1 親の期待値 `phase4_base_sha` は Git から実測できない**（判定時点の `origin/develop` は事後に痕跡を残さない）ため、**下記②の記録された期待値と照合する**。**Git オブジェクトを解決できないときは判定不能として fail-closed**
