...
| 状況計算(クライアント・サーバー) | 要件書 4.0-4 の列挙(スコア・アウト・走者・打順・終了判定 — 終了判定は**規則スナップショット〔付録F〕の参照**を含む)+ **付録E マトリクス(機械可読シード)とその解釈器・生成物**・**状態補正イベント(FR-040)**・**下流再計算/リプレイ**(FR-003〜FR-005)+ **座標変換・捕球選手推定**(NFR-018・NFR-019(a) の対象 — 旧システムで 3 箇所コピー・「静かなデータ破壊」の事故前例〔改善台帳 I-2・I-17〕)。**成績集計の前処理も、右欄の条件が成立するまでは含む** | **成績集計の前処理・付録A 集計定義は条件付き除外**: 付録A 契約(固定ベクタ + CI 強制)の**実装完了後に**除外を発効する(発効判断は NFR-018 実現方式 ADR〔台帳 H-55〕— 契約が存在しない間は左欄のとおり**含む**。固定ベクタは出力一致の検証でありコピー実装の検出は NFR-018 のレビュー責務のまま) |
