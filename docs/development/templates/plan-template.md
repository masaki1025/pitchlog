...
branch: feature/<slug>
created: YYYY-MM-DD
計画レビュー周回: 0        # /plan レビュー(指摘反映まで)1 周ごとに +1(/plan が更新。feature_status.py が表示)
確定ゲート周回: 0          # /finalize-doc 敵対レビュー 1 周ごとに +1(/finalize-doc が更新)
...
<!-- 1 ステップ = 1 委任 = 1 コミット(レビュー可能な粒度・1 論理変更)。/implement がこの表を上から実行する。
     ...
     ちょうど 1 個含める(全半角括弧可 — feature_status.py が進捗導出)。承認・起票コミットには付けない -->
