...
以下は **Windows の PowerShell**(管理者)で実行する。

```powershell
wsl --version                    # 表示されない場合は WSL 本体が古い
wsl --update                     # 新しい配布形式に対応させる
wsl --set-default-version 2      # 新規ディストリを WSL2 で作る
wsl --list --online              # 提供中の識別子を確認
wsl --install -d Ubuntu-26.04    # 番号付きイメージを指定する(`Ubuntu` ではない)
```

初回起動で **Linux のユーザー名とパスワードを対話的に作成**する。作成したアカウントがそのディストリの既定ユーザーになる。

```powershell
wsl -d Ubuntu-26.04              # 以降の作業はすべてこのシェルの中で行う
wsl --list --verbose             # 対象が VERSION=2 であることを確認
```

- **VERSION が 1 だったら** `wsl --shutdown` の後に `wsl --set-version Ubuntu-26.04 2` で変換する(Codex の Linux sandbox〔bubblewrap〕は **WSL1 非対応**)
...
> **この環境には役割の違う Python が 2 つある。**
> - **hooks**: `.claude/settings.json` が **`/usr/bin/python3` を絶対パスで**起動する。ディストリの python3(Ubuntu 26.04 では **3.14 系**)でよい。標準ライブラリのみで動く
> - **codex ラッパー**: PATH 上の `python` を使う。上記 symlink で満たされる
> - **backend**: `>=3.12,<3.13` を要求するが、**uv が `.python-version` を読んで自動で用意する**(7 章)。**システムに 3.12 を入れる必要はない**

> **罠(実例 2026-08-07)**: Windows 側の pyenv 等のシムが WSL の PATH に紛れ、`python` が「見つかるが実行できない」状態になることがある — この場合 **hooks 全体が fail-open する**。`which python` が `/mnt/c/...` を指すなら、`ln -s /usr/bin/python3 ~/.local/bin/python` 等で WSL 側解決を先行させ、`python --version` が通ることを必ず確認する。
...
導入は [Docker 公式の Ubuntu 手順](https://docs.docker.com/engine/install/ubuntu/)に従う。導入後:
...
- **daemon が動いていなければ起動する**。`/etc/wsl.conf` に `[boot]` の `systemd=true` を書いて PowerShell から `wsl --shutdown` した後なら `sudo systemctl start docker`、そうでなければ `sudo service docker start`
- **WSL を停止すると WSL 内の daemon も止まる**(Docker Desktop の常駐 daemon とは別物)。WSL を起動し直したら再確認する
...
**項目 1〜4 は WSL のシェルで実行する。項目 5・6 は Claude Code のセッション内で実行する**(hooks は Claude Code の PreToolUse として動くため、**通常のシェルで試しても再現しない**)。

| # | 確認 | 期待 |
| --- | --- | --- |
| 1 | Claude Code を起動 | SessionStart フックが「現在ブランチ…」を表示する |
| 2 | `/permissions` | `.claude/settings.json` のルールが有効に見える(Bash パターン構文が現行仕様か確認) |
| 3 | `uv run pytest tests/` | 終了コード 0(hooks・codex ラッパー・`scripts/` の検査に対する正負テスト。**件数は CI の harness ジョブの実行結果を正とする** — 固定件数は腐るため書かない〔[ハーネス設計書](dev-harness-design-2026-08-07.md) 8.3〕) |
| 4 | `uv run ruff check .` と `uv run ty check` | いずれも終了コード 0(CI の harness ジョブと同じ検査。**`ruff format` は未導入 — 走らせない**) |
| 5 | **Claude Code に** main ブランチ上で `git commit` を試させる | git_guard がブロックし、**コミットが作られない**(`git log` が進んでいないことで確認する) |
| 6 | **Claude Code に** 生の `codex exec` を試させる | codex_guard がブロックし、ラッパー経由の案内が出る |
...
> システムの python3 は 3.14 系だが、**uv が `.python-version` の版を自動で取得する**ので、3.12 を別途導入する必要はない。
