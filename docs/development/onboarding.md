...
**要件書 NFR-021 は、証跡の「標準出力またはログ成果物への参照」欄に 6 ラベルの実値を `; ` 区切りの 1 行として直接記載することを要求する**(実施したアーキテクチャの識別)。本節はその 1 行を作る手順である(**受入判定を実施する場合のみ必須**。通常のセットアップでは省略してよい)。
...
(
  ...
  printf '証跡セルへ貼る 1 行: host_raw=%s; host=%s; wsl_uname_raw=%s; wsl_uname=%s; wsl_dpkg_raw=%s; wsl_dpkg=%s\n' \
...
**出力された「証跡セルへ貼る 1 行」の `host_raw=…; wsl_dpkg=…` の部分を、結果証跡の「標準出力またはログ成果物への参照」欄へ 1 物理行のまま記載する。** **6 ラベルは `; ` 区切りの 1 行**にする — **当該欄は Markdown 表の 1 セル**であり、**改行を含めると検証器の表抽出がそこで打ち切られ、当該欄と後続の「判定者」欄が完全性検査(⑩)で欠落扱いになる**(確定ゲート 3 周目 P0 で実測)。**`<br>` も使わない** — プレースホルダ正規表現 `<...>` に一致して拒否される。別ファイルへの参照だけにもしない — **証跡本文は改変禁止・append-only の保護下にあるが、参照先のファイルは後続の試行で上書きされうる**ため(要件書 NFR-021)。
