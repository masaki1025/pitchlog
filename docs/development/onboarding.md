...
| 1.2 | 2026-08-27 | **受入プロファイルの複製を解消し、アーキテクチャの採取手順を新設**(前提 PR 第 3 号): 0 章の「番号付き **x64** WSL イメージ」を**要件書 NFR-021 への参照へ寄せた**(要件の複製であり、同要件の x64 / arm64 両対応化で approved 正本間の矛盾になるため)/ **2-2 節「アーキテクチャの採取」を新設** — `host` / `wsl_uname` / `wsl_dpkg` の 3 値の採取コマンド・正規化の対応表・生出力と正規化結果の両方をログへ残す規定・**欠落/表外の値/3 値の不一致を不適合とする判定**(要件書 v2.3 が証跡のログ参照欄にアーキテクチャの識別を要求したことへの追随) | **draft**(確定ゲート中) |
...
**受入プロファイルは x64 / arm64 の両方を保証対象とするため、要件書 NFR-021 の証跡項目「標準出力またはログ成果物への参照」は、実施したホストおよび WSL のアーキテクチャを識別できる出力を含むことを要求する。** 本節はその出力を採る手順である(**受入判定を実施する場合のみ必須**。通常のセットアップでは省略してよい)。

**3 値を採り、生出力と正規化結果の両方をログへ残す**(WSL 内で実行):

```bash
{
  echo "host_raw=$(cmd.exe /c 'echo %PROCESSOR_ARCHITECTURE%' 2>/dev/null | tr -d '\r')"
  echo "wsl_uname_raw=$(uname -m)"
  echo "wsl_dpkg_raw=$(dpkg --print-architecture)"
} | tee -a ~/pitchlog-acceptance-arch.log
```

**正規化の対応表**(この表に無い生出力が出た場合は**受入プロファイル外**として不適合にする):
...
**正規化結果も同じログへ、`host` / `wsl_uname` / `wsl_dpkg` のラベル付きで残す。**

**不適合の判定(いずれかに該当したら受入を続行しない)**:

1. 3 値のいずれかが**採れない**(空・コマンド失敗)
2. いずれかの生出力が**上表のどの行にも一致しない**
3. **`host` / `wsl_uname` / `wsl_dpkg` の正規化結果が 3 値そろって一致しない**(WSL2 はホストと同一アーキテクチャの VM で動くため、不一致は環境の異常を意味する)

**このログを証跡の「標準出力またはログ成果物への参照」欄から参照する。** どの値を代表値とするかを実施者の判断に委ねないため、**3 値すべてを記録する**。
