...
**受入プロファイルは x64 / arm64 の両方を保証対象とするため、要件書 NFR-021 の証跡項目「標準出力またはログ成果物への参照」は、参照先が候補コミットのツリーに収録され、かつ 6 ラベルの実値を含むことを要求する。** 本節はその成果物を作る手順である(**受入判定を実施する場合のみ必須**。通常のセットアップでは省略してよい)。

**次のブロックをそのまま実行する**(3 値の採取・正規化・不適合判定・ログ出力を 1 つのブロックで行う。**不適合なら非 0 で終了する** — その場合は受入を続行しない):

```bash
mkdir -p docs/worklog
{
  set +e
  host_raw=$(cmd.exe /C 'echo %PROCESSOR_ARCHITECTURE%' 2>/dev/null | tr -d '\r'); host_rc=$?
  ...
  {
    echo "host_raw=$host_raw (rc=$host_rc)"
    echo "host=$host"
    echo "wsl_uname_raw=$wsl_uname_raw (rc=$uname_rc)"
    echo "wsl_uname=$wsl_uname"
    echo "wsl_dpkg_raw=$wsl_dpkg_raw (rc=$dpkg_rc)"
    echo "wsl_dpkg=$wsl_dpkg"
  } | tee docs/worklog/acceptance-arch.log

  fail=0
  [ "$host_rc" -eq 0 ] && [ -n "$host_raw" ] || { echo "不適合: host を採取できない" >&2; fail=1; }
  [ "$uname_rc" -eq 0 ] && [ -n "$wsl_uname_raw" ] || { echo "不適合: wsl_uname を採取できない" >&2; fail=1; }
  [ "$dpkg_rc" -eq 0 ] && [ -n "$wsl_dpkg_raw" ] || { echo "不適合: wsl_dpkg を採取できない" >&2; fail=1; }
...
**出力先は `docs/worklog/acceptance-arch.log`(リポジトリ内)である。** 受入証跡と同じコミットに含め、**証跡の「標準出力またはログ成果物への参照」欄からこのパスを参照する**(`docs/worklog/**` は受入証跡の失効対象外なので、証跡を失効させない)。**どの値を代表値とするかを実施者の判断に委ねないため、6 ラベルすべてを記録する。**

**本ブロックは arm64 実機で実測済み**(2026-08-27): 正常系は `host_raw=ARM64` / `wsl_uname_raw=aarch64` / `wsl_dpkg_raw=arm64` → 3 値とも `arm64` に正規化され `適合: arch=arm64`・終了コード 0。不適合 3 系統(採取できない / 許容表にない値 / 3 値の不一致)はいずれも**終了コード 1** で、該当する理由が標準エラーへ出る。
