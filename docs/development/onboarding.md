...
**要件書 NFR-021 は、証跡の「標準出力またはログ成果物への参照」欄に 6 ラベルの実値を直接記載することを要求する**(実施したアーキテクチャの識別)。本節はその 6 行を作る手順である(**受入判定を実施する場合のみ必須**。通常のセットアップでは省略してよい)。
...
**出力された 6 行を、結果証跡の「標準出力またはログ成果物への参照」欄へそのまま記載する。** 別ファイルへの参照だけにしない — **証跡本文は改変禁止・append-only の保護下にあるが、参照先のファイルは後続の試行で上書きされうる**ため(要件書 NFR-021)。
