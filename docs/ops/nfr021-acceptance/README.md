...
**機械検査は「名指しされた 1 件」だけを対象とする。** 検証器が完全性を検査するのは
`evidence_path` で名指しされた結果証跡だけであり、**同じディレクトリにある過去の結果証跡へは
及ばない**(合格条件の正はハーネス設計書 10.1「NFR-021 受入ゲート」の「検証器の契約」⑩)。

**全件検査にしてはならない理由**: 証跡は append-only で訂正も削除もできず、同一 `attempt_id` の
訂正版を足すことも 1 対 1 の対応契約に反する。したがって過去の不完全な `failed` 記録が 1 件でも
あると、**そのゲートキーが恒久的に閉塞する**。
