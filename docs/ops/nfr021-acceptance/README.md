...
4. **孤児・多重対応の拒否** — 新規の結果証跡には対応する予約が統合後のツリーに**ちょうど 1 件**存在すること /
   同一の `attempt_id` に対する結果証跡が**2 件目にならない**こと

**3 と 4 が要る理由**: これを欠くと、本文と名前だけ正しい**孤児の結果証跡**や**同一予約への 2 件目**が
統合されてしまう。統合後は変更も削除もできないため、検証器の合格条件 ⑨(対応する予約がちょうど 1 件・
