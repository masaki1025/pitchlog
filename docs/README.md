...
| [データモデル設計](design/data-model.md) | **approved**(v0.2 — TSK-348: **approved 正本に実機で反証済みの `search_path` 契約が1 行残っていた**ことの是正〔`REJ-003` が `remove_temporary_schema_from_search_path` を `incorrect` と判定・正は `place_pg_temp_explicitly_last`〕。3-2 節の契約表 4 行(`pg_temp` の末尾明示 / `search_path` 設定の必須化 / スキーマ検査へ名前解決経路への注入可能性 / **「名前解決経路の非注入」を不変条件へ**)+ probe と製品の層の明記 + 12-7・12-8 節の受け取り先の是正。**確定ゲート レビュー実行 10 回・指摘 16 件のうち 15 件採用・不採用 1 件〔PO 支持で終結〕・`P0` 全周ゼロ・PO 裁定 4 回**〔範囲確定 / 構造的是正 / 派生値の撤去 / 打ち切り〕。**検査対象集合の厳密な列挙は TSK-317 へ**〔範囲確定〕。**申し送り**: 定義の言い換えによる残滓は TSK-352、その機械検査は TSK-353) | 0.2 | 2026-09-10 |
