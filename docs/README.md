...
| [開発ハーネス設計書](development/dev-harness-design-2026-08-07.md) | **in-review**(v1.13 起案 — **13 章へ「製品機能の実装」を新設**: 製品機能の実装は Phase の対象外であり 6.1 の実装計画書ゲートに従う。**本章は開始条件を定めず**着手可否は 6.1 の計画書ゲート(人間承認)で判断 / あわせて 3.1 分担表と 6.3 見出しの「提案」表記を現況化(規範不変) / `NFR-018` の対象計算は ADR-003 の導入順序に従う / 順序・群・段階は定めない。**Phase は追加していない**。確定ゲート 2 周目で**射程縮小**〔7.3-6 の PO 裁定〕。v1.12 = 7.3 収束規則の再設計〔適用版の原則・終端遷移表・重大度定義・差分再レビュー・依頼文規約・エスカレーションと 6 周警告・射程宣言 + 6.3 実施記録様式。敵対レビュー反映 12 周・54 件全採用 → PO 判断で 12 周クローズ・PO 承認〕) | 1.13 | 2026-09-02 |
| [ハーネス運用評価台帳](development/harness-evaluation.md) | **approved**(v1.0 — 敵対レビュー9周 → PO 承認。H-20・H-78・H-79・H-53・H-81・**H-22・H-42・H-61** の実測・**H-85 新設**〔PR #34〕/ **H-68 対応済み・H-75 部分対応**〔7.3 v1.12 の条文化〕+ H-42/H-39/H-12/H-8 補記・候補 1 件〔PR #35〕) | 1.0 | 2026-09-01 |
...
| [ADR-003: ドメイン計算の実現方式と契約化](adr/ADR-003-domain-calc-method.md) | **approved**(要件書 v2.2 と一括検証) | 0.1 | 2026-09-01 |
