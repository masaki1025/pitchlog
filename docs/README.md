...
| [データモデル設計](design/data-model.md) | **in-review**(v0.1 — TSK-342: 候補案 1〜12 節を素材に新設。エンティティ・関係・識別子のライフサイクル・一意性・削除の 4 系統・キャッシュ無効化の物理化・88 列の行き先。**行番号引用 296 件を安定 ID 参照へ貼り替え**〔対応表 `scripts/design_relations/citation-map-data-model.json`〕。RLS の設計判断・移管欠陥 ID 群の是正・`WAIT-01`〜`WAIT-08` の物理表現・射程宣言・原子境界の写像まで全 12 ステップを反映済み。**7.3 の確定ゲート中**) | 0.1 | 2026-09-08 |
