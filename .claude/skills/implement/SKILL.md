...
設計書 6.1 の fast path の**全 3 条件** — ① 非コア領域(`.claude/core-areas.json` に該当しない)② 小差分(目安 50 行以下)かつ正本への影響なし ③ typo・コメント・小さなテスト修正・設定微調整の類 — を満たす変更は、**計画書の承認・ステップ表なし**で `python .claude/scripts/codex_run.py fast -`(worktree 内で実行。モデル・effort は ADR-001「決定」節の「軽微な修正」行をラッパーが固定)。/task-start が作った計画書雛形は**メタデータとして保持**する(status: active のまま。/pr・/task-done が frontmatter の branch・notion を参照する)。**fast 適用時は、人間の事前 OK を得てから雛形 frontmatter の `重さ分類: 軽微` と `実行方式: fast` を一括更新し、その後に実行する**(ラッパーはブランチ名から導出した正規位置 `docs/features/<slug>/plan.md` の計画書で `status: active`・`重さ分類: 軽微`・`実行方式: fast` の 3 値を検査し、不一致・欠落・空値ではモデル実行前に停止する — ADR-001。現在地導出も `実行方式: fast` で「fast path 実装中」と識別する — feature_status.py)。PR は **/pr の fast path 分岐**を使い、本文に短縮計画(目的/変更/確認方法/人間の事前 OK への言及)を書く。
