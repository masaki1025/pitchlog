...
4. **ステップコミット**: **Codex はコミットしない** — Claude が `git -C <worktree>` でコミットする(Conventional Commits・日本語要約・**1 ステップ = 1 コミット**)。件名には完全トークン **`(ステップ <k>/<N>[ 付記])`** を**ちょうど 1 個**含める(全半角括弧可・k は計画書ステップ表の番号 — feature_status.py が進捗導出に使う。承認・起票などステップ外のコミットには付けない)
